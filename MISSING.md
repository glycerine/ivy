...
### [~] 7.5 mc: Propositional abstraction (`MkPropAbs`) — core ported, prev_expr detection is stub
**Python**: `ivy_mc.py:1287-1318`. **Go**: `PropAbs.MkPropAbs()` ported in propabs.go. Recursively abstracts non-propositional atoms: quantifiers (forall/exists), applications with non-finite-sort arguments, uninterpreted function applications are replaced with fresh boolean constants via `newProp()`. Finite symbols are tracked. `Apply()` orchestrates over transition formulas and defs. **Still missing**: `prevExpr()` detection for state-variable linkage across time steps (returns nil stub). This is needed for correct latch handling in AIGER encoding.
