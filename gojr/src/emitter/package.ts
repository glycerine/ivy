...
interface ExpressionEmitEnv {
  ...
  currentTypeParameterConstraints?: Record<string, string>;
  deferName?: string;
...
interface PackageEmitFacts {
  ...
  functionTypeParameterConstraints: Map<string, Record<string, string>>;
...
function emitSelectStatement(ctx: EmitterContext, statement: SelectStatement, env: ExpressionEmitEnv, indent: string): string[] | undefined {
  ...
  const selectedBodyLines: string[] = [];
  for (const [caseIndex, clause] of statement.clauses.entries()) {
    ...
    selectedBodyLines.push(
      `${indent}  case ${caseIndex}: {`,
      `${indent}    const ${selectedValue} = ${selectedResult}.result;`,
      ...bodyLines,
      `${indent}    break;`,
      `${indent}  }`
    );
...
function packageEmitFacts(ast: ProgramAst, artifact: GoJuniorPackageExportData): PackageEmitFacts {
  ...
  const functionTypeParameterConstraints = new Map<string, Record<string, string>>();
  ...
  for (const fn of ast.functions) {
    ...
    if (fn.typeParameterConstraints) functionTypeParameterConstraints.set(key, fn.typeParameterConstraints);
    if (!fn.receiver) {
      ...
      if (fn.typeParameterConstraints) functionTypeParameterConstraints.set(fn.name, fn.typeParameterConstraints);
      continue;
    }
...
function singleArgumentTupleTypeTexts(expression: Expression, env: ExpressionEmitEnv): Array<string | undefined> {
  ...
  if (annotatedTuple.length > 1) return annotatedTuple;
  ...
  if (callee.kind === "SelectorExpression") {
    ...
    if (methodKey) return env.facts.functionResultTypes.get(methodKey) ?? [];
  }
  return [];
}

function tupleResultTypeTextsFromAnnotation(typeText: string | undefined): Array<string | undefined> {
  const trimmed = typeText?.trim();
  if (!trimmed?.startsWith("(")) return [];
  const end = matchingParenIndex(trimmed, 0);
  if (end !== trimmed.length - 1) return [];
...
function typeTextPackagePathCore(typeText: string | undefined): string {
...
function fullImportPathTypePrefix(typeText: string, facts: PackageEmitFacts): string | undefined {
  const dot = typeText.lastIndexOf(".");
  if (dot <= 0) return undefined;
  const prefix = typeText.slice(0, dot);
  for (const importPath of facts.imports.values()) {
    if (prefix === importPath) return importPath;
  }
...
function renderTypeArgumentValue(typeArg: string | undefined, env: ExpressionEmitEnv): string {
...
function renderCallArgs(ctx: EmitterContext, args: Expression[], env: ExpressionEmitEnv, targetTypes: string[] = [], spreadLast = false): string[] | undefined {
  ...
  if (!spreadLast && onlyArg) {
    ...
    if (tupleTypes.length > 1) {
      ...
      const converted = tupleTypes.map((sourceType, index) =>
        valueForTargetType(`${valuesName}[${index}]`, callArgumentTargetType(targetTypes, index, false), sourceType, env)
      );
...
function renderCallArgTypes(args: Expression[], env: ExpressionEmitEnv): string[] {
  const onlyArg = args.length === 1 ? args[0] : undefined;
  if (onlyArg) {
    const tupleTypes = singleArgumentTupleTypeTexts(onlyArg, env);
    if (tupleTypes.length > 1) return tupleTypes.map((type) => JSON.stringify(type ?? ""));
  }
...
function instantiatedFunctionTypeArgObject(expression: Expression, functionName: string, env: ExpressionEmitEnv): string | undefined {
  ...
  const fields = parameterNames.map((name, index) =>
    `${JSON.stringify(name)}: ${renderTypeArgumentValue(typeArgs[index] ?? "any", env)}`
  );
  return `({ ${fields.join(", ")} })`;
}
...
function actualTypeForGenericInference(actualType: string | undefined, env: ExpressionEmitEnv): string | undefined {
  const trimmed = actualType?.trim();
  if (!trimmed || !env.typeParameters || !env.currentTypeParameterConstraints) return trimmed;
...
function inferGenericTypeArgumentsFromConstraints(
  ...
  for (let pass = 0; pass < parameterNames.length; pass += 1) {
    ...
    for (const name of parameterNames) {
      ...
      if (!sameStringMap(before, inferred)) changed = true;
    }
    if (!changed) return;
  }
}

function constraintInferencePattern(constraint: string | undefined, parameterSet: Set<string>): string | undefined {
  ...
  for (const option of splitTopLevelUnion(constraint)) {
    ...
    if (identifiers.some((identifier) => parameterSet.has(identifier))) return trimmed;
  }
  return undefined;
}
...
function isScalarTypeText(typeText: string): boolean {
...
function isKnownLocalNamedTypeText(typeText: string, env: ExpressionEmitEnv): boolean {
...
let cachedStage1SharedSupportSource: string | undefined;
let cachedStage1SharedSupportBindingNames: string[] | undefined;

export function stage1SharedSupportSource(): string {
  ensureStage1SharedSupport();
  if (!cachedStage1SharedSupportSource) throw new Error("could not extract GoJr Stage 1 shared support source");
  return cachedStage1SharedSupportSource;
}

export function stage1SharedSupportBindingNames(): string[] {
  ensureStage1SharedSupport();
...
function ensureStage1SharedSupport(): void {
  if (cachedStage1SharedSupportSource && cachedStage1SharedSupportBindingNames) return;
  stage1JavaScript(stage1IntrinsicSupportDummyArtifact(), false, [], [], emptyPackageEmitFacts);
}

function stripStage1SharedSupport(source: string): string {
...
function collectStage1SupportBindingNames(source: string): string[] {
  const names: string[] = [];
  const seen = new Set<string>();
...
function stage1SharedSupportLoaderSource(): string {
  return [
