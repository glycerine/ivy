...
async function loadPackageArtifactsFromBuildOnNode(
  ...
  for (const artifact of build.artifacts) {
    ...
    try {
      ...
      diagnostics.push(nodeArtifactDiagnostic(
        artifact.artifactPath,
        `could not initialize package ${importPath} from ${artifact.artifactPath}: ${error instanceof Error ? error.message : String(error)}`,
        error
      ));
      break;
    }
...
async function instantiatePackageArtifactJavaScript(
  source: string,
  ...
  try {
    ...
    diagnostics.push(nodeArtifactDiagnostic(
      artifactPath,
      `could not load package artifact ${artifactPath}: ${error instanceof Error ? error.message : String(error)}`,
      error
    ));
    return undefined;
  }
  try {
    ...
    diagnostics.push(nodeArtifactDiagnostic(
      artifactPath,
      `could not initialize package ${options.importPath} from ${artifactPath}: ${error instanceof Error ? error.message : String(error)}`,
      error
    ));
    return undefined;
  }
...
function packageSourceMapFromSpecs(specs: SourcePackageSpec[]): Record<string, SourceFile[]> | undefined {
  ...
  for (const spec of specs) {
    ...
    if (isIntrinsicPackageImport(spec.importPath)) continue;
