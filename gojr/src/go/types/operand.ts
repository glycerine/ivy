...
export class operand {
  ...
  public setConst(k: TokenKind, lit: string): void {
    let kind: BasicKind;
    switch (k) {
  ...
  public assignableTo(check: Checker | null, T: Type, cause: Cause | null): [boolean, unknown] {
    ...
    const errorf = (format: string, ...args: unknown[]) => {
    ...
    if (!hasName(V) && Tp !== null) {
      let ok = false;
      let code: unknown = "IncompatibleAssign";
      Tp.is((Tterm) => {
        if (Tterm === null) {
  ...
  public convertibleTo(check: Checker | null, T: Type, cause: Cause | null): boolean {
    return operandConvertibleTo(this, check, T, cause);
  }

  public clone(): operand {
...
export function compositeKind(typ: Type): string {
...
function implicitTypeAndValue(check: Checker | null, x: operand, target: Type): [Type | null, unknown, unknown] {
  const fn = check as unknown as { implicitTypeAndValue?: (x: operand, target: Type) => [Type | null, unknown, unknown] } | null;
