...
export class MethodSet {
  public constructor(public list: Selection[] = []) {}
  ...
  public At(i: number): Selection { return this.list[i]!; }

  // Lookup returns the method with matching package and name, or nil if not found.
  public Lookup(pkg: Package | null, name: string): Selection | null {
    if (this.Len() === 0) {
      return null;
    }
...
export function NewMethodSet(T: Type): MethodSet {
  ...
  while (current.length > 0) {
    ...
    const fset = new Map<string, boolean>(); // we only care about the field names
...
export class methodSet extends Map<string, Selection | null> {
  // Add adds all functions in list to the method set s.
  // If multiples is set, every function in list appears multiple times
  // and is treated as a collision.
  public add(list: Func[], index: number[] | null, indirect: boolean, multiples: boolean): methodSet {
    if (list.length === 0) {
      return this;
    }
  ...
  public addOne(f: Func, index: number[], indirect: boolean, multiples: boolean): methodSet {
    const key = f.Id();
