...
import { debug, type Checker } from "./check.js";
...
export class Union implements Type {
  public constructor(
    public terms: Term[] // list of syntactical terms (not a canonicalized termlist)
  ) {}

  public Len(): number { return this.terms.length; }
  public Term(i: number): Term { return this.terms[i]!; }
...
export function NewUnion(terms: Term[]): Union {
  if (terms.length === 0) {
...
export class Term extends term {
  public constructor(tilde: boolean, typ: Type) {
    super(tilde, typ);
  }

  public Tilde(): boolean { return this.tilde; }
  public Type(): Type { return this.typ!; }
  public String(): string { return super.String(); }
}

// NewTerm returns a new union term.
export function NewTerm(tilde: boolean, typ: Type): Term { return new Term(tilde, typ); }
...
export function parseUnion(check: Checker, uexpr: unknown): Type {
  const [blist, tlist] = flattenUnion([], uexpr);
  assert(blist.length === tlist.length - 1);

  const terms: Term[] = [];

  let u: Type = Typ[BasicKind.Invalid]!;
  for (let i = 0; i < tlist.length; i++) {
    ...
    if (i > 0) {
      (check as unknown as { recordTypeAndValue: (x: unknown, mode: unknown, typ: Type, val: unknown) => void }).recordTypeAndValue(blist[i - 1]!, typexpr, u, null);
    }
  }
  ...
  }).describef(uexpr as { Pos: () => Pos }, "check term validity %s", uexpr);
...
export function parseTilde(check: Checker, tx: unknown): Term {
  ...
  if (tilde) {
    (check as unknown as { recordTypeAndValue: (x: unknown, mode: unknown, typ: Type, val: unknown) => void }).recordTypeAndValue(tx, typexpr, new Union([trm]), null);
  }
  return trm;
}
...
export function overlappingTerm(terms: Term[], y: Term): number {
