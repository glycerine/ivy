...
type TypeHandle = { value: Type | null };

// A unifier maintains a list of type parameters and
// corresponding types inferred for each type parameter.
// A unifier is created by calling newUnifier.
export class unifier {
  public depth = 0; // recursion depth during unification
  ...
  public constructor(
    ...
    public handles: Map<TypeParam, TypeHandle>,
    public enableInterfaceInference: boolean // use shared methods for better inference
  ) {}

  // unify attempts to unify x and y and reports whether it succeeded.
  // As a side-effect, types may be inferred for type parameters.
  // The mode parameter controls how types are compared.
  public unify(x: Type, y: Type, mode: unifyMode): boolean {
    return this.nify(x, y, mode, null);
  }

  public tracef(format: string, ...args: unknown[]): void {
  ...
  public join(x: TypeParam, y: TypeParam): boolean {
    if (traceInference) {
      this.tracef("%s ⇄ %s", x, y);
    }
  ...
  public asBoundTypeParam(x: Type): TypeParam | null {
    const x0 = Unalias(x);
  ...
  public setHandle(x: TypeParam, h: TypeHandle): void {
    const hx = this.handles.get(x)!;
  ...
  public inferred(tparams: TypeParam[]): (Type | null)[] {
    const list = new Array<Type | null>(tparams.length);
    for (let i = 0; i < tparams.length; i++) {
      const x = tparams[i]!;
      list[i] = this.at(x);
    }
  ...
  public nify(x0: Type, y0: Type, mode: unifyMode, p: ifacePair | null): boolean {
    let x = x0;
    let y = y0;
    this.depth++;
    if (traceInference) {
    ...
    try {
      ...
      if (this.enableInterfaceInference && (mode & exact) === 0) {
        ...
        if (xi !== null && yi !== null) {
          ...
          const ymap = new Map<string, Func>();
...
export function newUnifier(check: Checker, tparams: TypeParam[], targs: (Type | null)[], enableInterfaceInference: boolean): unifier {
  assert(tparams.length >= targs.length);
  const handles = new Map<TypeParam, TypeHandle>();
...
// exactly.
export const assign: unifyMode = 1 << 0;
...
// interface.
export const exact: unifyMode = 1 << 1;

export function unifyModeString(m: unifyMode): string {
  switch (m) {
    case 0:
      return "inexact";
    case assign:
...
export class typeParamsById extends Array<TypeParam> {
  ...
  public Swap(i: number, j: number): void { [this[i], this[j]] = [this[j]!, this[i]!]; }
...
export function asInterface(x: Type): Interface | null {
