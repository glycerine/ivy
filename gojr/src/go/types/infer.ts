...
import { NewTypeName, type Var } from "./object.js";
import type { operand } from "./operand.js";
import { Pointer } from "./pointer.js";
import { Default, isTyped, isTypeParam, maxType } from "./predicates.js";
...
declare module "./check.js" {
  interface Checker {
    infer(posn: positioner, tparams: TypeParam[], targs: (Type | null)[], params: Tuple | null, args: operand[] | null, reverse: boolean, err: error_): Type[] | null;
...
export function coreTerm(tpar: TypeParam): [term | null, boolean] {
  let n = 0;
  let single: term | null = null; // valid if n == 1
  let tilde = false;
  tpar.is((t: term | null) => {
    if (t === null) {
      assert(n === 0);
...
export function killCycles(tparams: TypeParam[], inferred: (Type | null)[]): void {
  const w = new cycleFinder(tparams, inferred, new Map<Type, boolean>());
...
export class cycleFinder {
  public constructor(
    public tparams: TypeParam[],
    public inferred: (Type | null)[],
    public seen: Map<Type, boolean>
  ) {}

  public typ(typ0: Type): void {
...
Checker.prototype.infer = function infer(posn: positioner, tparams: TypeParam[], targs0: (Type | null)[], params0: Tuple | null, args0: operand[] | null, reverse: boolean, err: error_): Type[] | null {
  ...
  if (targs.length < n) {
    const targs2 = new Array<Type | null>(n).fill(null);
  ...
  if (params.Len() > 0) {
    const smap = makeSubstMap(tparams, targs as Type[]);
    params = this.subst(nopos, params, smap, null, this.context()) as Tuple;
  }
  ...
  const errorf = (tpar: Type, targ: Type, arg: operand) => {
    ...
    const smap = makeSubstMap(tparams, inferredArgs as Type[]);
  ...
  let maxUntyped: Map<TypeParam, Type> | null = null; // lazily allocated (we may not need it)
  for (const index of untyped) {
    const tpar = params.At(index).typ as TypeParam; // is type parameter (no alias) by construction of untyped
    if (u.at(tpar) === null) {
  ...
  while (dirty.length > 0) {
    ...
    // needed. Optimization.
    const smap = makeSubstMap(tparams, inferred as Type[]);
    let nextDirty: number[] = [];
...
Checker.prototype.renameTParams = function renameTParams(pos: number, tparams: TypeParam[], typ: Type): [TypeParam[], Type] {
  ...
  const tparams2 = new globalThis.Array<TypeParam>(tparams.length);
...
export function typeParamsString(list: TypeParam[]): string {
  // common cases
  const n = list.length;
  switch (n) {
    case 0:
...
export function isParameterized(tparams: TypeParam[], typ: Type | null): boolean {
  if (typ === null) {
    return false;
  }
  const w = new tpWalker(tparams, new Map<Type, boolean>());
...
export class tpWalker {
  ...
  public isParameterized(typ: Type): boolean {
    // detect cycles
    ...
    try {
      if (typ instanceof Basic) {
        ...
        return tset.is((t: term | null) => t !== null && this.isParameterized(t.typ!));
      } else if (typ instanceof MapType) {
  ...
  public varList(list: Var[]): boolean {
    for (const v of list) {
