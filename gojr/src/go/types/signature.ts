...
import { EndOf, PosOf, Unparen, type Expr, type Field, type FieldList, type FuncType, type Ident } from "../../front/ast.js";
import type { Scope } from "./scope.js";
import type { Type } from "./type.js";
import { TypeString } from "./typestring.js";
import { NewTuple, type Tuple } from "./tuple.js";
...
declare module "./check.js" {
  interface Checker {
    funcType(sig: Signature, recvPar: FieldList | undefined, ftyp: FuncType): void;
    collectRecv(rparam: Field, scopePos: number): [Var, TypeParamList | null];
    recordParenthesizedRecvTypes(expr: Expr, typ: Type): void;
    collectParams(kind: VarKind, list: FieldList | undefined): [Array<Ident | null>, Var[], boolean];
    declareParams(names: Array<Ident | null>, params: Var[], scopePos: number): void;
    validRecv(pos: positioner, recv: Var): void;
  }
}
...
registerCheckerMethod("collectRecv", function collectRecv(rparam: Field, scopePos: number): [Var, TypeParamList | null] {
  ...
  let recvType: Type = Typ[Invalid]!;
  let recvTParamsList: TypeParamList | null = null;
  if (rtparams === null) {
    ...
    let a = unpointer(recvType) instanceof Alias ? unpointer(recvType) as Alias : null; // recvType is not generic per above
    for (; a !== null;) {
    ...
    let baseType: Named | null = null; // nil if not valid
    const cause = { value: "" };
    ...
    const recvTParams = new Array<TypeParam>(rtparams.length);
    ...
    if (baseType !== null) {
      ...
      if (recvTParams.length === baseTParams.length) {
        ...
        for (let i = 0; i < recvTParams.length; i++) {
          ...
          (this.mono as { recordCanon?: (mpar: TypeParam, tpar: TypeParam) => void } | null)?.recordCanon?.(recvTPar, baseTPar);
      ...
      const targs = new Array<Type>(recvTParams.length);
      for (let i = 0; i < recvTParams.length; i++) {
...
function unpackRecv(rtyp: Expr, unpackParams: boolean): [boolean, Expr, Ident[] | null] {
  let e = Unparen(rtyp);
  let ptr = false;
  if (e.kind === "StarExpr") {
    ptr = true;
...
function isGenericType(t: Type): t is Type & { TypeParams(): TypeParamList | null } {
  return typeof (t as { TypeParams?: unknown }).TypeParams === "function";
}

function fileNameForPos(fset: unknown, pos: number): string {
  const file = (fset as { File?: (pos: number) => { Name?: () => string } | null } | null)?.File?.(pos);
  return file?.Name?.() ?? "";
}
