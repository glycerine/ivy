...
import { Basic, BasicKind, Byte, Rune, Uintptr, UnsafePointer } from "./basic.js";
import { Interface } from "./interface.js";
import { go1_17, go1_20 } from "./version.js";
import { type Cause } from "./lookup.js";
...
export function conversion(check: Checker, x: operand, T: Type): void {
  ...
  switch (true) {
    ...
    case constArg && isTypeParam(T):
      ...
      ok = (T as TypeParam).typeset((_t, u) => {
...
export function convertibleTo(x: operand, check: Checker | null, T: Type, cause: Cause | null): boolean {
...
export function isUintptr(typ: Type): boolean {
  const t = typ.Underlying();
  return t instanceof Basic && t.kind === Uintptr;
}

export function isUnsafePointer(typ: Type): boolean {
  const t = typ.Underlying();
...
export function isBytesOrRunes(typ: Type): boolean {
...
function isNonTypeParamInterfaceLocal(T: Type): boolean {
