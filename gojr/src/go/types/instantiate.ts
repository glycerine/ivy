...
export interface genericType extends Type {
  TypeParams(): import("./typelists.js").TypeParamList | null;
}

// Instantiate instantiates the type orig with the given type arguments targs.
export function Instantiate(ctxt: Context | null, orig: Type, targs: Type[], validate: boolean): [Type | null, Error | null] {
  if (ctxt === null) {
    ctxt = NewContext();
  }
...
function isGenericType(t: Type): t is genericType {
  return t !== null && typeof (t as genericType).TypeParams === "function";
}
...
function instance(check: Checker | null, pos: Pos, orig: genericType, targs: Type[], expanding: Named | null, ctxt: Context | null): Type {
  ...
  const ctxts: Context[] = [];
  const expandingInst = expanding?.inst as { ctxt?: Context | null } | null;
  ...
  const hashes = new Array<string>(ctxts.length);
  ...
  const updateContexts = (res0: Type): Type => {
    let res = res0;
...
export function validateTArgLen(check: Checker | null, pos: Pos, name: string, want: number, got: number): boolean {
  let qual: string;
  switch (true) {
    case got < want:
      qual = "not enough";
...
export function implements_(check: Checker | null, V: Type, T: Type, constraint: boolean, cause: Cause | null): boolean {
  ...
  const checkComparability = (): boolean => {
    if (!Ti.IsComparable()) {
      return true;
    }
  ...
  if (Ti.typeSet().is((t) => {
    if (t === null || !t.includes(V)) {
      ...
      if (t !== null && alt === null && !t.tilde && Identical(t.typ!, t.typ!.Underlying())) {
        const tt = Object.assign(Object.create(Object.getPrototypeOf(t)), t) as typeof t;
        tt.tilde = true;
        if (tt.includes(V)) {
          alt = t.typ;
        }
