...
declare module "./check.js" {
  interface Checker {
    isTerminating(s: Stmt, label: string): boolean;
    isTerminatingList(list: Stmt[], label: string): boolean;
    isTerminatingSwitch(body: CaseClause[], label: string): boolean;
  }
}
...
export function hasBreak(s: Stmt, label: string, implicit: boolean): boolean {
  switch (s.kind) {
    default:
      throw new Error("unreachable");
...
export function hasBreakList(list: Stmt[], label: string, implicit: boolean): boolean {
  for (const s of list) {
