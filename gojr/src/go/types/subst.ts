...
import { Union, Term, NewTerm } from "./union.js";
...
export class substMap extends Map<TypeParam, Type> {
  ...
  public lookup(tpar: TypeParam): Type {
...
export function makeSubstMap(tpars: TypeParam[], targs: Type[]): substMap {
...
export function makeRenameMap(from: TypeParam[], to: TypeParam[]): substMap {
...
declare module "./check.js" {
  interface Checker {
    subst(pos: Pos, typ: Type | null, smap: substMap, expanding: Named | null, ctxt: Context | null): Type | null;
  }
}
...
export function subst(check: Checker | null, pos: Pos, typ: Type | null, smap: substMap, expanding: Named | null, ctxt: Context | null): Type | null {
...
export class subster {
  public constructor(
    public pos: Pos,
    public smap: substMap,
    public check: Checker | null, // nil if called via Instantiate
    public expanding: Named | null, // if non-nil, the instance that is being expanded
    public ctxt: Context | null
  ) {}

  public typ(typ: Type | null): Type {
    if (typ === null) {
    ...
    if (typ instanceof Basic) {
      ...
      if (targs !== null) {
        const fn = this.check as unknown as { newAliasInstance?: (pos: Pos, orig: Alias, targs: Type[], expanding: Named | null, ctxt: Context | null) => Type };
      ...
      if (n > 0) {
        ...
        if (!instantiating) {
          ...
          for (let i = 0; i < tparams!.list().length; i++) {
            ...
            if (this.check !== null) {
              (this.check.mono as { recordCanon?: (ftp: TypeParam, tp: TypeParam) => void } | null)?.recordCanon?.(ftp, tp);
            }
          }
      ...
      if (targs !== null) {
        ...
        // help with debugging.
        const fn = this.check as unknown as { instance?: (pos: Pos, orig: Named, targs: Type[], expanding: Named | null, ctxt: Context | null) => Type };
  ...
  public typOrNil(typ: Type | null): Type {
    if (typ === null) {
  ...
  public tuple(t: Tuple | null): Tuple | null {
    if (t !== null) {
...
export function cloneVar(v: Var, typ: Type): Var {
  const copy = Object.assign(Object.create(Object.getPrototypeOf(v)), v) as Var;
  copy.typ = typ;
...
export function substList<T>(input: T[], fn: (t: T) => T): T[] | null {
  let out: T[] | null = null;
...
export function cloneFunc(f: Func, typ: Type): Func {
  const copy = Object.assign(Object.create(Object.getPrototypeOf(f)), f) as Func;
  copy.typ = typ;
...
export function replaceRecvType(input: Func[], old: Type, newType: Type): [Func[], boolean] {
  let out = input;
  let copied = false;
  for (let i = 0; i < input.length; i++) {
    ...
    if (sig.recv !== null && sig.recv.Type() === old) {
      ...
      const newsig = Object.assign(Object.create(Object.getPrototypeOf(sig)), sig) as Signature;
