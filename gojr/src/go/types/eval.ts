...
import type { Expr } from "../../front/ast.js";
import { ParseExprFrom } from "../../front/parser.js";
import { Info, TypeAndValue } from "./api.js";
import { Checker, NewChecker, bailout, debug, nopos } from "./check.js";
...
// type-checked.
export function Eval(fset: unknown, pkg: Package | null, pos: number, expr: string): [TypeAndValue, unknown] {
  // parse expressions
...
export function CheckExpr(fset: unknown, pkg: Package | null, pos: number, expr: Expr, info: Info | null): unknown {
  // determine scope
  let scope = null as import("./scope.js").Scope | null;
  let checkPkg = pkg;
  if (pkg === null) {
    scope = Universe;
    pos = nopos;
  ...
  try {
    ...
    (check as Checker & { recordUntyped: () => void }).recordUntyped();
  } catch (p) {
