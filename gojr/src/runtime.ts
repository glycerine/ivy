...
import { Codebase, type CodebaseTxn, type CodebaseUpdateTxn } from "./codebase.js";
import {
...
export class RuntimeGoString {
  ...
  public static fromBytes(bytes: Uint8Array | number[]): RuntimeGoString {
...
export interface GoJuniorFunction {
  ...
  preserveResultIdentity?: boolean;
  call(args: RuntimeValue[], context: EvaluationContext, typeArguments?: string[]): MaybePromise<RuntimeValue>;
}
...
interface StructTypeDef {
  ...
  declaringContext?: EvaluationContext;
}
...
interface MethodDef {
  ...
  intrinsic?: GoJuniorFunction | undefined;
}
...
interface TestingTState {
  ...
  skipped: boolean;
  logs: string[];
  cleanups: RuntimeValue[];
}

export interface EvaluationSharedState {
  output: string[];
  rootScope: Scope;
...
export class PackageRuntime {
  public package: RuntimeObject | undefined;
  private builtinsInstalled = false;

  public constructor(
    public readonly importPath: string,
    public readonly packageName: string,
    public readonly shared: EvaluationSharedState = createEvaluationSharedState()
  ) {}

  public createContext(options: EvaluationOptions = {}): EvaluationContext {
  ...
  public ensureBuiltins(context: EvaluationContext): void {
    if (this.builtinsInstalled) return;
...
export class EvaluationContext {
  ...
  public pointerToBinding(name: string, typeName: string): RuntimePointer {
    ...
    return new RuntimePointer(
      typeName,
      () => scope.lookup(name),
      (next) => scope.assign(name, next, this.assignmentChecker()),
      `binding:${objectIdentityId(bindingScope)}:${name}`
    );
  }
  ...
  public packageRuntimes(): Record<string, PackageRuntime> {
  ...
  public typeDef(name: string): StructTypeDef | undefined {
    ...
    if (foreign && qualified) return qualifyRuntimeStructTypeDef(foreign, qualified.importPath);
  ...
  public interfaceDef(name: string): InterfaceTypeDef | undefined {
    ...
    if (foreign && qualified) return qualifyRuntimeInterfaceDef(foreign, qualified.importPath);
  ...
  public aliasType(name: string): string | undefined {
    ...
    if (foreign && qualified) return qualifyLocalRuntimeTypeName(foreign, qualified.importPath);
  ...
  public registerMethod(declaration: FunctionDecl, intrinsic?: GoJuniorFunction): void {
  ...
  public importPackageMethods(importPath: string, source: PackageRuntime): void {
  ...
  public importPackageDefinitions(importPath: string, source: PackageRuntime): void {
  ...
  public registerImportBinding(localName: string, importPath: string, filename?: string): void {
  ...
  public importPathForLocalName(localName: string, filename?: string): string | undefined {
    if (filename) {
  ...
  public importedPackageValue(localName: string, filename?: string): RuntimeObject | undefined {
    const importPath = this.importPathForLocalName(localName, filename);
    if (!importPath) return undefined;
    return availablePackages(this)[importPath];
  }

  public resolveImportedTypeText(typeText: string, filename?: string): string {
...
class Scope {
  ...
  private readonly shortVarSites = new WeakSet<ShortVarStatement>();
  ...
  public bindingScope(name: string): Scope | undefined {
  ...
  public lookupOptional(name: string): RuntimeValue | undefined {
    const binding = this.bindings.get(name);
    if (binding) return binding.value;
...
export class RuntimePointer {
  public constructor(
    ...
    private readonly stableIdentity?: string,
    private readonly sequence?: RuntimePointerSequence
  ) {}
  ...
  public sequenceInfo(): RuntimePointerSequence | undefined {
    return this.sequence;
  }
}

interface RuntimePointerSequence {
  values: RuntimeValue[];
  index: number;
}

interface RuntimeArrayView {
  source: RuntimeValue[];
  offset: number;
  toView?: (value: RuntimeValue) => RuntimeValue;
  toSource?: (value: RuntimeValue) => RuntimeValue;
}
...
export class RuntimeInterfaceValue {
  public constructor(
    ...
    public readonly value: RuntimeValue,
    public readonly methodContext?: EvaluationContext
  ) {}
...
const arrayViews = new WeakMap<RuntimeValue[], RuntimeArrayView>();
const sparseArrayDefaultElementTypes = new WeakMap<RuntimeValue[], string>();
const externalizedPackageValueCache = new WeakMap<object, Map<string, RuntimeValue>>();
const internalizedPackageValueCache = new WeakMap<object, Map<string, RuntimeValue>>();
const packageRuntimeWrapperInfo = new WeakMap<object, {
  packageName: string;
  direction: "externalized" | "internalized";
  source: RuntimeValue;
}>();
...
const runtimeUintptrPointers = new Map<bigint, RuntimePointer>();
const runtimeFunctionPCNames = new Map<bigint, string>();
...
class RuntimeExternalizedMap extends RuntimeMap {
  public constructor(
    private readonly source: RuntimeMap,
    private readonly packageName: string
  ) {
    super(
      qualifyLocalRuntimeTypeName(source.keyType, packageName),
      qualifyLocalRuntimeTypeName(source.valueType, packageName)
    );
  }

  public override get(key: RuntimeValue): RuntimeValue {
    return externalizePackageRuntimeValue(
      this.source.get(internalizePackageRuntimeValue(key, this.packageName)),
      this.packageName
    );
  }

  public override getWithPresence(key: RuntimeValue): [RuntimeValue, boolean] {
  ...
  public override set(key: RuntimeValue, value: RuntimeValue): void {
    this.source.set(
  ...
  public override delete(key: RuntimeValue): void {
  ...
  public override orderedEntries(): Array<[RuntimeValue, RuntimeValue]> {
...
class RuntimeInternalizedMap extends RuntimeMap {
  public constructor(
    ...
    super(
      unqualifyLocalRuntimeTypeName(source.keyType, packageName),
      unqualifyLocalRuntimeTypeName(source.valueType, packageName)
    );
  }

  public override get(key: RuntimeValue): RuntimeValue {
    return internalizePackageRuntimeValue(
      this.source.get(externalizePackageRuntimeValue(key, this.packageName)),
      this.packageName
    );
  }
...
export async function evaluatePackageSourceFiles(files: SourceFile[], options: PackageEvaluationOptions = {}): Promise<PackageEvaluationResult> {
  ...
  if (checked.diagnostics.some((diagnostic) => diagnostic.severity === "error")) {
    if (!codebaseTxn.IsClosed()) codebaseTxn.Rollback();
    return {
...
class SourcePackageGraphEvaluator {
  ...
  private readonly packageRuntimes: Record<string, PackageRuntime>;
  ...
  public constructor(
    ...
    for (const spec of specs) {
      ...
      if (isIntrinsicPackageImport(spec.importPath)) {
        ...
        if (pkg) this.packageInfos[spec.importPath] = pkg;
        continue;
      }
  ...
  private progress(event: EvaluationProgressEvent): void {
  ...
  private refreshCodebaseRecords(): void {
...
function codebaseForOptions(options: EvaluationOptions): Codebase {
  ...
  for (const [importPath, pkg] of Object.entries(options.packageInfos ?? {})) {
    if (!codebase.PackageInfo(txn, importPath)) codebase.SetPackageInfo(txn, importPath, pkg);
  }
  for (const [importPath, runtime] of Object.entries(options.packageRuntimes ?? {})) {
    if (!codebase.PackageRuntime(txn, importPath)) codebase.SetPackageRuntime(txn, importPath, runtime);
  }
  txn.Commit();
  return codebase;
}

function codebasePackageInfo(codebase: Codebase, importPath: string): GoTypesPackage | undefined {
...
function codebasePackageInfosRecord(codebase: Codebase, txn?: CodebaseTxn): Record<string, GoTypesPackage> {
  if (txn) return codebase.PackageInfosRecord(txn);
...
function codebasePackageRuntimesRecord(codebase: Codebase, txn?: CodebaseTxn): Record<string, PackageRuntime> {
  if (txn) return codebase.PackageRuntimesRecord(txn);
  const viewTxn = codebase.NewViewTxn();
  try {
...
function syncLegacyCodebaseRecords(options: EvaluationOptions, codebase: Codebase): void {
  if (options.packageInfos) replaceRecord(options.packageInfos, codebasePackageInfosRecord(codebase));
  if (options.packageRuntimes) replaceRecord(options.packageRuntimes, codebasePackageRuntimesRecord(codebase));
}

function replaceRecord<T>(target: Record<string, T>, source: Record<string, T>): void {
  for (const key of Object.keys(target)) delete target[key];
  for (const [key, value] of Object.entries(source)) target[key] = value as T;
}
...
function installMainProgramArgs(
  ...
  if (osRuntime) osRuntime.createContext({ ...options, packages: graph.packages, packageInfos: graph.packageInfos, packageRuntimes: graph.packageRuntimes }).declareOrAssignRoot("Args", args, true, "[]string");
...
async function testProgram(
  ...
  let packageRuntime: PackageRuntime | undefined;
...
function testNameFilter(pattern: string | undefined, diagnostics: Diagnostic[], ast: ProgramAst): (name: string) => boolean {
...
async function runOneTest(
  ...
  let runtimeFailureStack: string | undefined;
  try {
...
function testDiagnostic(declaration: FunctionDecl, message: string, stack?: string): Diagnostic {
  return {
...
function makeTestingT(name: string): { value: RuntimePointer; state: TestingTState } {
  ...
  const object: RuntimeObject = {
    ...
    Run: hostCallable("testing.(*T).Run", async (args, context) => {
      ...
      if (subtest.state.failed) state.failed = true;
...
async function runTestingCleanups(state: TestingTState, context: EvaluationContext): Promise<void> {
...
export class GoJuniorSession {
  ...
  public async evaluate(source: string): Promise<EvaluationResult> {
    ...
    try {
      ...
      if (!typeInfoAccepted && !checkedTransaction.transaction.IsClosed()) checkedTransaction.transaction.Rollback();
      const message = error instanceof Error ? error.message : String(error);
      return withObservedDeps({
  ...
  private acceptTypeInfo(checked: { transaction: CodebaseUpdateTxn }): void {
    if (!checked.transaction.IsClosed()) checked.transaction.Commit();
  }
...
function importBindingFileKey(filename: string, localName: string): string {
...
function packageFunctionIntrinsic(declaration: FunctionDecl, importPath?: string): GoJuniorFunction | undefined {
  ...
  if (importPath === "reflect" && declaration.name === "New") {
    intrinsic = intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
      reflectNewValue(args[0] ?? null, context));
  }
  if (importPath === "reflect" && declaration.name === "MakeSlice") {
    intrinsic = intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
      reflectMakeSlice(args[0] ?? null, args[1] ?? 0n, args[2] ?? 0n, context));
  }
  if (importPath === "reflect" && declaration.name === "Append") {
    intrinsic = intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
      reflectAppend(args, context));
  }
  ...
  if (importPath === "internal/abi" && declaration.name === "TypeFor") {
    intrinsic = intrinsicGoJuniorFunction(declaration.name, declaration.signature, (_args, context, typeArguments) =>
      internalAbiTypeDescriptorAsTypePointer(typeArguments?.[0] ?? "interface{}", internalAbiRuntimeTypeNames(context), context));
  }
  ...
  if (importPath === "time" && declaration.name === "LoadLocation") {
    intrinsic = intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
      timeLoadLocation(args[0] ?? RuntimeGoString.fromUtf8Text(""), context));
  }
  return intrinsic
    ? {
      ...intrinsic,
...
async function timeLoadLocation(nameValue: RuntimeValue, context: EvaluationContext): Promise<RuntimeValue[]> {
...
function runtimePackageExport(context: EvaluationContext, importPath: string, name: string): RuntimeValue | undefined {
  const pkg = context.packages()[importPath];
  if (pkg && Object.prototype.hasOwnProperty.call(pkg, name)) return pkg[name] ?? null;
  ...
  if (context.importPath() === importPath && context.hasBinding(name)) return context.lookup(name);
  return undefined;
}

function timeLocationNameIsInvalid(name: string): boolean {
...
function packageMethodIntrinsic(declaration: FunctionDecl, importPath?: string): GoJuniorFunction | undefined {
  ...
  if (importPath === "reflect" && (receiver.baseType === "Value" || receiver.baseType === "reflect.Value")) {
    switch (declaration.name) {
      ...
      case "Index":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
          reflectValueIndex(args[0] ?? null, args[1] ?? 0n, context));
      case "Field":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
          reflectValueField(args[0] ?? null, args[1] ?? 0n, context));
      case "Addr":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
          reflectValueAddr(args[0] ?? null, context));
      case "Bool":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueBool(args[0] ?? null));
      case "Int":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueInt(args[0] ?? null));
      case "Uint":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueUint(args[0] ?? null));
      case "Float":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueFloat(args[0] ?? null));
      case "Complex":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueComplex(args[0] ?? null));
      case "String":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueString(args[0] ?? null));
      case "CanAddr":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
      ...
      case "SetBool":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, toBool(args[1] ?? false), "SetBool"));
      case "SetInt":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, toBigInt(args[1] ?? 0n), "SetInt"));
      case "SetUint":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, BigInt.asUintN(64, toBigInt(args[1] ?? 0n)), "SetUint"));
      case "SetFloat":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, toFloat(args[1] ?? 0), "SetFloat"));
      case "SetComplex":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, toComplex(args[1] ?? complexValue(0, 0)), "SetComplex"));
      case "SetString":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, RuntimeGoString.fromUtf8Text(toStringValue(args[1] ?? "")), "SetString"));
      case "SetBytes":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          reflectValueSetScalar(args[0] ?? null, args[1] ?? [], "SetBytes"));
      default:
        return undefined;
    }
  }
  if (importPath === "reflect" && (receiver.baseType === "rtype" || receiver.baseType === "reflect.rtype")) {
    switch (declaration.name) {
      case "Kind":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
          reflectRtypeKind(args[0] ?? null, context));
      case "Name":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          RuntimeGoString.fromUtf8Text(reflectRtypeDescriptorText(args[0] ?? null, "GoJrName")));
      case "String":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          RuntimeGoString.fromUtf8Text(reflectRtypeDescriptorText(args[0] ?? null, "GoJrString")));
      case "PkgPath":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          RuntimeGoString.fromUtf8Text(reflectRtypeDescriptorText(args[0] ?? null, "GoJrPkgPath")));
      case "Implements":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
          reflectRtypeImplements(args[0] ?? null, args[1] ?? null, context));
      case "Elem":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) =>
  ...
  if (receiver.baseType === "Type" || receiver.baseType === "internal/abi.Type") {
    switch (declaration.name) {
      case "HasName":
        return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
          (internalAbiTypeFlag(args[0] ?? null) & internalAbiTFlagNamed) !== 0n);
      default:
        return undefined;
    }
  }
  if (receiver.baseType !== "Name" && receiver.baseType !== "internal/abi.Name") return undefined;
  switch (declaration.name) {
    case "Name":
      return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
        RuntimeGoString.fromUtf8Text(internalAbiNameText(args[0] ?? null)));
    case "Tag":
      return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
        RuntimeGoString.fromUtf8Text(internalAbiNameTag(args[0] ?? null)));
    case "IsExported":
      return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
        internalAbiNameBool(args[0] ?? null, "Exported"));
    case "HasTag":
      return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
        internalAbiNameTag(args[0] ?? null) !== "");
    case "IsEmbedded":
      return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
        internalAbiNameBool(args[0] ?? null, "Embedded"));
    case "IsBlank":
      return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args) =>
        internalAbiNameText(args[0] ?? null) === "_");
    default:
      return undefined;
  }
}
...
function intrinsicGoJuniorFunction(
  ...
  options: Pick<GoJuniorFunction, "preserveResultIdentity"> = {}
): GoJuniorFunction {
  return {
    kind: "GoJuniorFunction",
    name,
  ...
  return {
    ...
    async call(args) {
      ...
      if (typeof value === "object") {
        ...
        if (isGoJuniorFunction(value) || isRuntimeCallable(value)) runtimeFunctionPCNames.set(pc, value.name);
        return pc;
      }
...
function bodylessSyscallIntrinsic(
  importPath: string | undefined,
  name: string,
  signature: FunctionDecl["signature"]
): GoJuniorFunction | undefined {
  if (importPath !== "syscall" && importPath !== "internal/runtime/syscall/linux") return undefined;
  if (
    name === "Syscall" ||
    name === "Syscall6" ||
    name === "RawSyscall" ||
...
function syscallOperands(args: RuntimeValue[]): RuntimeValue[] {
...
function syscallTrapKind(trap: bigint): "read" | "write" | undefined {
  const pcName = runtimeFunctionPCNames.get(trap);
  if (pcName) {
    if (/(^|[.])libc_read_trampoline$/.test(pcName)) return "read";
    if (/(^|[.])libc_write_trampoline$/.test(pcName)) return "write";
  }
  const key = trap.toString();
  if (syscallReadTrapNumbers.has(key)) return "read";
  if (syscallWriteTrapNumbers.has(key)) return "write";
  return undefined;
}

function syscallReadSlice(fdValue: RuntimeValue, bufferValue: RuntimeValue): RuntimeValue[] {
  const buffer = unwrapNamed(bufferValue);
  if (!Array.isArray(buffer)) throwTypeError(bufferValue, "[]byte", "syscall.read buffer");
...
function syscallReadErrnoSlice(fdValue: RuntimeValue, bufferValue: RuntimeValue): RuntimeValue[] {
...
function syscallReadPointer(fdValue: RuntimeValue, pointerValue: RuntimeValue, lengthValue: RuntimeValue): bigint {
...
function copyBytesToRuntimePointer(pointer: RuntimePointer, bytes: Uint8Array, count: number): void {
  ...
  if (count > 0) pointer.set(BigInt(bytes[0] ?? 0));
}

function bytesFromRuntimePointer(pointer: RuntimePointer, length: number): Uint8Array {
  const bytes = new Uint8Array(length);
  const sequence = pointer.sequenceInfo();
  if (sequence) {
  ...
  if (length > 0) bytes[0] = bytealgByte(pointer.get());
  return bytes;
}
...
function hostReadSync(fd: number, buffer: Uint8Array, offset: number, length: number, position: number | null): number {
  ...
  if (typeof host.__gojrReadSync !== "function") return 0;
  return normalizeHostByteCount(host.__gojrReadSync(fd, buffer, offset, length, position), length, "read");
}

function hostWriteSync(fd: number, buffer: Uint8Array, offset: number, length: number, position: number | null): number {
  const host = globalThis as typeof globalThis & { __gojrWriteSync?: GoJrHostWriteSync };
  if (typeof host.__gojrWriteSync !== "function") return length;
  return normalizeHostByteCount(host.__gojrWriteSync(fd, buffer, offset, length, position), length, "write");
}

function normalizeHostByteCount(value: unknown, limit: number, operation: "read" | "write"): number {
...
function isInternalAbiTypeName(typeText: string, context?: EvaluationContext): boolean {
  const type = normalizeTypeText(typeText);
  return type === "internal/abi.Type" || (type === "Type" && context?.importPath() === "internal/abi");
}

function runtimePointerHoldsTypeDescriptor(value: RuntimePointer): boolean {
...
function internalAbiCacheTypeText(typeText: string, context: EvaluationContext): string {
...
function internalAbiDescriptorStruct(typeText: string, names: InternalAbiRuntimeTypeNames, context: EvaluationContext): RuntimeStruct {
  ...
  if (underlyingType.startsWith("*")) return internalAbiPtrTypeStruct(typeText, underlyingType.slice(1), names, context);
  const structType = context.typeDef(typeText) ?? parseAnonymousStructTypeText(typeText);
  if (structType) return internalAbiStructTypeStruct(typeText, structType, names, context);
...
function internalAbiPtrTypeStruct(typeText: string, elementType: string, names: InternalAbiRuntimeTypeNames, context: EvaluationContext): RuntimeStruct {
...
function internalAbiStructTypeStruct(
  typeText: string,
...
function internalAbiSetEmbeddedTypeFields(struct: RuntimeStruct, base: RuntimeStruct): void {
...
function internalAbiStructFieldStruct(
...
function internalAbiNameStruct(
  name: string,
  tag: string,
  exported: boolean,
  embedded: boolean,
...
interface InternalAbiTypeDisplayInfo {
  typeText: string;
  name: string;
  string: string;
  pkgPath: string;
...
function internalAbiTypeDisplayInfo(typeText: string, context: EvaluationContext): InternalAbiTypeDisplayInfo {
  const type = normalizeTypeText(typeText);
...
function runtimeDisplayTypeString(typeText: string, context: EvaluationContext): string {
  const type = normalizeTypeText(typeText);
  const canonical = canonicalRuntimeTypeName(type);
  if (canonical === "any" || canonical === "interface{}") return "interface{}";
  if (isPredeclaredType(canonical)) return canonical;

  if (type.startsWith("*")) return `*${runtimeDisplayTypeString(type.slice(1), context)}`;
  if (type.startsWith("[]")) return `[]${runtimeDisplayTypeString(type.slice(2), context)}`;
  const arrayType = parseArrayOrSliceTypeText(type, context);
...
function runtimeDisplayLocalTypeName(typeText: string, context: EvaluationContext): string {
  const type = normalizeTypeText(typeText);
  const generic = genericTypeArguments(type);
  if (generic) {
...
function runtimePackageDisplayName(importPath: string, context: EvaluationContext): string {
...
function reflectRtypeDescriptorText(value: RuntimeValue, field: "GoJrName" | "GoJrString" | "GoJrPkgPath"): string {
  const descriptor = reflectRtypeDescriptor(value);
  if (!descriptor) return "";
  return runtimeDescriptorText(descriptor, field);
}

function reflectRtypeDescriptor(value: RuntimeValue): RuntimeStruct | undefined {
...
function reflectRtypeRuntimeTypeText(value: RuntimeValue): string {
  const descriptor = reflectRtypeDescriptor(value);
  return descriptor ? runtimeDescriptorText(descriptor, "GoJrTypeText") : "";
}

function reflectRtypeKind(value: RuntimeValue, context: EvaluationContext): bigint {
  const typeText = reflectRtypeRuntimeTypeText(value);
  return typeText ? internalAbiKindForType(typeText, context) : 0n;
}

function reflectRtypeElem(value: RuntimeValue, context: EvaluationContext): RuntimeValue {
...
function reflectRtypeElementTypeText(typeText: string, context: EvaluationContext): string | undefined {
...
function reflectRtypeImplements(source: RuntimeValue, target: RuntimeValue, context: EvaluationContext): boolean {
...
function reflectNewValue(typeValue: RuntimeValue, context: EvaluationContext): RuntimeStruct {
  const typeText = reflectRtypeRuntimeTypeText(typeValue);
  if (!typeText) throw new GoJuniorPanic("panic: reflect: New(nil)");
...
function reflectMakeSlice(typeValue: RuntimeValue, lengthValue: RuntimeValue, capacityValue: RuntimeValue, context: EvaluationContext): RuntimeStruct {
  const typeText = reflectRtypeRuntimeTypeText(typeValue);
  ...
  if (length < 0) throw new GoJuniorPanic("panic: reflect.MakeSlice: negative len");
  if (capacity < 0) throw new GoJuniorPanic("panic: reflect.MakeSlice: negative cap");
  if (length > capacity) throw new GoJuniorPanic("panic: reflect.MakeSlice: len > cap");
...
function reflectAppend(args: RuntimeValue[], context: EvaluationContext): RuntimeStruct {
  const target = args[0] ?? null;
  const info = requiredReflectValueInfo(target, "Append");
  const valueContext = info.context ?? context;
  if (info.kind !== 23n) throw new GoJuniorPanic(`panic: &ValueError{Method:reflect.Append Kind:${info.kind}}`);
  ...
  const values = args.slice(1).map((value, index) =>
    prepareAssignableToType(reflectValuePayload(value), sliceType.elementType, `reflect.Append argument ${index + 1}`, valueContext));
...
function reflectValueStoredPayload(ptr: RuntimeValue, typeText: string, kind: bigint): RuntimeValue {
...
function reflectValueStoresIndirectPayload(typeText: string, kind: bigint): boolean {
  if (kind === 22n || kind === 26n) return false;
...
function reflectValueElem(value: RuntimeValue, context: EvaluationContext): RuntimeStruct {
  ...
  if (info.kind === 22n) {
    ...
    if (actual instanceof RuntimePointer) {
      ...
      return reflectValueStruct(actual.get(), valueContext, elementType, (next) =>
        actual.set(prepareAssignableToType(next, elementType, `*${elementType}`, valueContext)));
    }
  }
  if (info.kind === 20n) {
...
function reflectValueIndex(value: RuntimeValue, indexValue: RuntimeValue, context: EvaluationContext): RuntimeStruct {
  const info = reflectValueInfo(value);
  if (!info || info.kind === 0n) {
  ...
  if ((info.kind === 17n || info.kind === 23n) && Array.isArray(actual) && arrayType) {
    ...
    const setter = info.kind === 23n || info.set
      ? (next: RuntimeValue) => setArrayElement(actual, index, prepareAssignableToType(next, elementType, "reflect.Value.Index", valueContext))
      : undefined;
...
function reflectValueField(value: RuntimeValue, indexValue: RuntimeValue, context: EvaluationContext): RuntimeStruct {
  const info = reflectValueInfo(value);
  if (!info || info.kind === 0n) {
  ...
  if (!struct) throw new GoJuniorPanic(`panic: &ValueError{Method:reflect.Value.Field Kind:${info.kind}}`);
  ...
  if (!field) throw new GoJuniorPanic("panic: reflect: Field index out of range");
  ...
  const setter = info.set
    ? (next: RuntimeValue) => accessor.set(prepareAssignableToType(next, accessor.type.type.text, `reflect.Value.Field(${index})`, valueContext))
    : undefined;
  return reflectValueStruct(fieldValue, valueContext, accessor.type.type.text, setter);
}

function reflectValueAddr(value: RuntimeValue, context: EvaluationContext): RuntimeStruct {
  const info = reflectValueInfo(value);
  if (!info || info.kind === 0n) {
    throw new GoJuniorPanic("panic: reflect: call of reflect.Value.Addr on zero Value");
  }
  const valueContext = info.context ?? context;
  if (!info.set) throw new GoJuniorPanic("panic: reflect.Value.Addr of unaddressable value");
...
function runtimeStructFieldsFromValue(struct: RuntimeStruct): StructFieldDecl[] {
...
function directRuntimeStructFieldAccessor(struct: RuntimeStruct, field: StructFieldDecl): StructFieldAccessor {
  return {
    type: field,
...
function reflectValueBool(value: RuntimeValue): boolean {
  const info = requiredReflectValueInfo(value, "Bool");
  if (info.kind !== 1n) throw new GoJuniorPanic(`panic: &ValueError{Method:reflect.Value.Bool Kind:${info.kind}}`);
  return toBool(info.value);
}

function reflectValueInt(value: RuntimeValue): bigint {
...
function reflectValueUint(value: RuntimeValue): bigint {
...
function reflectValueFloat(value: RuntimeValue): number {
...
function reflectValueComplex(value: RuntimeValue): ComplexLiteralValue {
...
function reflectValueString(value: RuntimeValue): RuntimeGoString {
...
function requiredReflectValueInfo(value: RuntimeValue, method: string): ReflectValueRuntimeInfo {
  const info = reflectValueInfo(value);
  if (!info || info.kind === 0n) {
...
function reflectValueSetScalar(target: RuntimeValue, source: RuntimeValue, method: string): RuntimeValue {
  const info = reflectValueInfo(target);
  if (!info || !info.set) throw new GoJuniorPanic(`panic: reflect: reflect.Value.${method} using unaddressable value`);
  info.set(source);
  return null;
}
...
function runtimeDescriptorText(descriptor: RuntimeStruct, field: string): string {
  const value = descriptor.get(field);
  return value !== undefined && isRuntimeString(value) ? goStringText(value) : "";
}

function internalAbiUnderlyingTypeText(typeText: string, context: EvaluationContext, seen = new Set<string>()): string {
  const type = normalizeTypeText(typeText);
  if (seen.has(type)) return type;
  seen.add(type);
  if (context.typeDef(type) || context.interfaceDef(type) || parseAnonymousStructTypeText(type) || parseAnonymousInterfaceTypeText(type)) return type;
  const alias = context.aliasType(type);
  if (alias && alias !== type) return internalAbiUnderlyingTypeText(alias, context, seen);
  return type;
}

function internalAbiNamedAliasTarget(typeText: string, context: EvaluationContext): string | undefined {
  const type = normalizeTypeText(typeText);
  if (context.typeDef(type) || context.interfaceDef(type)) return undefined;
  const alias = context.aliasType(type);
  if (!alias || alias === type) return undefined;
  if (context.trueAliasType(type)) return undefined;
  return alias;
}
...
function internalAbiSizeForType(typeText: string, context?: EvaluationContext, seen = new Set<string>()): bigint {
  const type = context ? internalAbiUnderlyingTypeText(typeText, context) : normalizeTypeText(typeText);
  if (seen.has(type)) return 8n;
  seen.add(type);
  ...
  const array = parseArrayOrSliceTypeText(type, context);
  if (array?.length !== undefined && !array.inferLength) return BigInt(array.length) * internalAbiSizeForType(array.elementType, context, new Set(seen));
  ...
  if (Array.isArray(value)) {
    return Uint8Array.from(
      Array.from({ length: value.length }, (_item, index) =>
        Number(BigInt.asUintN(8, toBigInt(getArrayElement(value, index)))))
    );
  }
...
function resolvedBindingTypeText(type: TypeNode | string | undefined, context: EvaluationContext): string | undefined {
...
function resolvedDeclaredTypeText(typeText: string, context: EvaluationContext, filename?: string): string {
...
function availablePackages(context: EvaluationContext): Record<string, RuntimeObject> {
  ...
  const runtimePackages = Object.fromEntries(
    Object.entries(context.packageRuntimes()).flatMap(([importPath, runtime]) =>
      runtime.package ? [[importPath, runtime.package] as [string, RuntimeObject]] : []
    )
  );
...
function unsafePackage(): RuntimeObject {
  return {
    ...
    Slice: hostCallable("unsafe.Slice", (args) => {
      ...
      if (!(pointer instanceof RuntimePointer)) throw new GoJuniorRuntimeError(`${formatValue(args[0] ?? null)} is not a pointer`);
      const sequence = pointer.sequenceInfo();
      ...
      if (sequence) markArrayView(values, sequence.values, sequence.index);
    ...
    String: hostCallable("unsafe.String", (args) => {
      ...
      const pointer = unsafePointerPayload(args[0] ?? null);
      if (isNilUnsafePointerPayload(pointer)) {
        if (length === 0) return RuntimeGoString.fromUtf8Text("");
    ...
    StringData: hostCallable("unsafe.StringData", (args) => {
      const value = unwrapNamed(args[0] ?? null);
      if (!isRuntimeString(value)) throwTypeError(value, "string", "unsafe.StringData argument");
...
function unsafePointerPayload(value: RuntimeValue): RuntimeValue {
  return unwrapNamed(value);
}

function isNilUnsafePointerPayload(value: RuntimeValue): boolean {
  return value === null ||
    (value instanceof RuntimeTypedNilValue && (value.typeName.startsWith("*") || isUnsafePointerType(value.typeName)));
}

function unsafePointerBytes(pointer: RuntimePointer, length: number): Uint8Array {
  if (length === 0) return new Uint8Array();
  const sequence = pointer.sequenceInfo();
  if (!sequence) {
    if (length !== 1) throw new GoJuniorRuntimeError("unsafe.String: pointer does not reference a byte sequence");
...
function reflectliteType(typeText: string, context: EvaluationContext): ReflectliteTypeObject {
  ...
  object.Name = hostCallable("internal/reflectlite.Type.Name", () =>
    RuntimeGoString.fromUtf8Text(reflectliteTypeName(type)), { signature: hostSignature([], ["string"]) });
  object.PkgPath = hostCallable("internal/reflectlite.Type.PkgPath", () =>
    RuntimeGoString.fromUtf8Text(reflectliteTypePkgPath(type)), { signature: hostSignature([], ["string"]) });
  object.Size = hostCallable("internal/reflectlite.Type.Size", () =>
    internalAbiSizeForType(type, context), { signature: hostSignature([], ["uintptr"]) });
...
function hostSignature(parameterTypes: string[], resultTypes: string[]): FunctionDecl["signature"] {
  return {
...
function reflectliteTypeName(typeText: string): string {
  const type = normalizeTypeText(typeText);
  if (type.startsWith("*")) return "";
  const qualified = packageQualifiedRuntimeTypeNameParts(type);
  if (qualified) return qualified.localName;
  if (/^[A-Za-z_]\w*$/.test(type)) return type;
  return "";
}

function reflectliteTypePkgPath(typeText: string): string {
...
function runtimeOsFile(fd: number, name: string): RuntimePointer {
  ...
  return new RuntimePointer("os.File", () => file, (next) => {
    ...
    if (value instanceof RuntimeStruct) {
      file.fields.clear();
      for (const [field, item] of value.orderedFields()) file.set(field, item);
    }
  }, `os.File:${fd}:${name}`);
}

function runtimeOsFileStruct(receiver: RuntimeValue): RuntimeStruct {
  const value = unwrapNamed(dereferenceIfPointer(receiver));
  if (value instanceof RuntimeStruct) return value;
  throw new GoJuniorRuntimeError(`${formatValue(receiver)} is not an os.File`);
}

function runtimeOsFileFd(receiver: RuntimeValue): number {
  const fd = runtimeOsFileStruct(receiver).get("fd") ?? 0n;
  return toNumber(fd);
}

function runtimeOsFileName(receiver: RuntimeValue): string {
  const name = runtimeOsFileStruct(receiver).get("name") ?? "";
  return toStringValue(name);
}

function runtimeOsFileRead(receiver: RuntimeValue, bufferValue: RuntimeValue): RuntimeValue[] {
  const buffer = unwrapNamed(bufferValue);
  if (!Array.isArray(buffer)) throwTypeError(bufferValue, "[]byte", "os.File.Read buffer");
  const bytes = new Uint8Array(buffer.length);
...
function markRuntimeByteSlice(values: RuntimeValue[]): RuntimeValue[] {
  markArrayType(values, "[]byte");
  return values;
}
...
function syscallJSFSObject(): Record<string, unknown> {
  ...
  const fail = (args: unknown[], error: unknown): void => {
    callback(args)?.(error);
  };
  return {
    constants,
...
function syscallJSRead(args: unknown[]): number {
...
function syscallJSWrite(args: unknown[]): number {
...
function syscallJSNumberArg(value: unknown, role: string): number {
  const number = typeof value === "bigint" ? Number(value) : typeof value === "number" ? value : Number(value);
  if (!Number.isInteger(number)) throw new GoJuniorRuntimeError(`${role} must be an integer`);
  return number;
}

function syscallJSPositionArg(value: unknown): number | null {
  if (value === null || value === undefined) return null;
  return syscallJSNumberArg(value, "fs position");
}

function syscallJSByteBufferArg(value: unknown, role: string): Uint8Array {
...
function hostCallable(
  name: string,
  call: (args: RuntimeValue[], context: EvaluationContext, typeArguments?: string[]) => MaybePromise<RuntimeValue>,
  options: { tupleResult?: boolean; signature?: FunctionDecl["signature"] } = {}
): RuntimeCallable {
  return {
    kind: "HostCallable",
    name,
...
function predeclarePackageVariables(declarations: Statement[], pkg: GoTypesPackage, context: EvaluationContext): void {
  const varsByName = new Map<string, GoTypesVar>();
  for (const object of packageScopeObjects(pkg)) {
    if (object instanceof GoTypesVar) varsByName.set(object.Name(), object);
  }
  for (const statement of declarations) {
  ...
  return {
    ...
    async call(args, parentContext, typeArguments) {
      ...
      const invoke = () => context.functionCallAsync(() => context.childScopeAsync(async () => {
        const activeTypeArgumentBindings = bindFunctionTypeParameters(
          declaration,
          signature,
          callArgs,
          context,
          typeArguments,
          mergeRuntimeTypeArgumentBindings(typeArgumentBindings, boundaryTypeArgumentBindings)
        );
...
function packageBoundaryArgumentValue(
  value: RuntimeValue,
  typeText: string | undefined,
...
function qualifyBareGenericRuntimeTypeArguments(typeText: string | undefined, packageName: string): string {
  const type = normalizeTypeText(typeText ?? "");
  if (!type) return type;
  if (type.startsWith("*")) return `*${qualifyBareGenericRuntimeTypeArguments(type.slice(1), packageName)}`;
  if (type.startsWith("[]")) return `[]${qualifyBareGenericRuntimeTypeArguments(type.slice(2), packageName)}`;
  const generic = genericTypeArguments(type);
  if (!generic) return type;
  return `${generic.base}[${generic.args.map((arg) => {
    const normalized = normalizeTypeText(arg);
    if (!normalized || runtimeTypeOwnerPackagePath(normalized) || isPredeclaredType(normalized)) return normalized;
...
function runtimeValueOwnerPackagePath(value: RuntimeValue): string | undefined {
  ...
  if (value instanceof RuntimeTypedNilValue) return runtimeTypeOwnerPackagePath(value.typeName);
  if (value instanceof RuntimePointer) return runtimeTypeOwnerPackagePath(value.typeName) ?? runtimeValueOwnerPackagePath(value.get());
  if (value instanceof RuntimeStruct) return runtimeTypeOwnerPackagePath(value.typeName);
  if (value instanceof RuntimeMap) return runtimeTypeOwnerPackagePath(`map[${value.keyType}]${value.valueType}`);
  if (value instanceof RuntimeChannel) return runtimeTypeOwnerPackagePath(`chan ${value.elementType}`);
  return undefined;
}

function runtimeValueLocalOwnerFallback(value: RuntimeValue, callerPackageName: string | undefined): string | undefined {
  if (!callerPackageName) return undefined;
  const type = runtimeValueConcreteTypeText(value);
  return type && runtimeTypeNeedsPackageOwner(type) ? callerPackageName : undefined;
}

function runtimeValueConcreteTypeText(value: RuntimeValue): string | undefined {
  if (value instanceof RuntimeInterfaceValue) return runtimeValueConcreteTypeText(value.value);
...
function runtimeTypeNeedsPackageOwner(typeText: string): boolean {
  const type = normalizeTypeText(typeText);
  if (!type || runtimeTypeOwnerPackagePath(type)) return false;
  if (isPredeclaredType(type) || isUnsafePointerType(type)) return false;
  if (type.startsWith("*")) return runtimeTypeNeedsPackageOwner(type.slice(1));
  if (type.startsWith("[]")) return runtimeTypeNeedsPackageOwner(type.slice(2));
  const arrayType = parseArrayOrSliceTypeText(type);
  if (arrayType) return runtimeTypeNeedsPackageOwner(arrayType.elementType);
  const mapType = parseMapTypeText(type);
  if (mapType) return runtimeTypeNeedsPackageOwner(mapType.keyType) || runtimeTypeNeedsPackageOwner(mapType.valueType);
  const chanType = parseChanTypeText(type);
  if (chanType) return runtimeTypeNeedsPackageOwner(chanType.elementType);
  const generic = genericTypeArguments(type);
  if (generic) return runtimeTypeNeedsPackageOwner(generic.base) || generic.args.some(runtimeTypeNeedsPackageOwner);
...
function mergeRuntimeTypeArgumentBindings(
...
function inferFunctionTypeArguments(
  ...
  for (const [name, type] of Object.entries(seedBindings ?? {})) {
    ...
    if (typeNames.has(localName)) inferred.set(localName, normalizeTypeText(type));
  }
  for (const [index, name] of names.entries()) {
    const explicit = explicitTypeArguments?.[index];
    if (explicit) inferred.set(name, normalizeInferredRuntimeTypeArgument(explicit, context));
  }
...
function normalizeInferredRuntimeTypeArgument(typeText: string, context?: EvaluationContext): string {
...
function qualifyRuntimeLocalTypeOwners(typeText: string, importPath: string, context?: EvaluationContext): string {
  const type = normalizeTypeText(typeText);
  if (!type || isPredeclaredType(type) || isUnsafePointerType(type) || runtimeTypeOwnerPackagePath(type)) return type;
  if (context?.scopedAliasType(type) !== undefined) return type;
  if (type.startsWith("*")) return `*${qualifyRuntimeLocalTypeOwners(type.slice(1), importPath, context)}`;
  if (type.startsWith("[]")) return `[]${qualifyRuntimeLocalTypeOwners(type.slice(2), importPath, context)}`;
  const arrayType = parseArrayOrSliceTypeText(type, context);
  if (arrayType) {
  ...
  if (type.startsWith("interface{") || type.startsWith("struct{")) return type;
  return /^[A-Za-z_]\w*$/.test(type) && runtimeBareTypeBelongsToContext(type, context) ? `${importPath}.${type}` : type;
}

function runtimeBareTypeBelongsToContext(typeText: string, context?: EvaluationContext): boolean {
  if (!context) return false;
  const type = normalizeTypeText(typeText);
  if (!type || !/^[A-Za-z_]\w*$/.test(type)) return false;
...
function intrinsicMethodDef(typeName: string, methodName: string): MethodDef | undefined {
  ...
  if (type === "reflect.Value" || type === "Value") {
    switch (methodName) {
      ...
      case "SetBool":
        return intrinsicMethod(type, methodName, ["bool"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, toBool(args[1] ?? false), methodName));
      case "SetInt":
        return intrinsicMethod(type, methodName, ["int64"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, toBigInt(args[1] ?? 0n), methodName));
      case "SetUint":
        return intrinsicMethod(type, methodName, ["uint64"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, BigInt.asUintN(64, toBigInt(args[1] ?? 0n)), methodName));
      case "SetFloat":
        return intrinsicMethod(type, methodName, ["float64"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, toFloat(args[1] ?? 0), methodName));
      case "SetComplex":
        return intrinsicMethod(type, methodName, ["complex128"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, toComplex(args[1] ?? complexValue(0, 0)), methodName));
      case "SetString":
        return intrinsicMethod(type, methodName, ["string"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, RuntimeGoString.fromUtf8Text(toStringValue(args[1] ?? "")), methodName));
      case "SetBytes":
        return intrinsicMethod(type, methodName, ["[]byte"], [], (args) =>
          reflectValueSetScalar(args[0] ?? null, args[1] ?? [], methodName));
      default:
        return undefined;
    }
  }
    ...
    switch (methodName) {
      ...
      case "PkgPath":
        return intrinsicMethod(type, methodName, [], ["string"], (args) =>
          RuntimeGoString.fromUtf8Text(reflectRtypeDescriptorText(args[0] ?? null, "GoJrPkgPath")));
      case "Implements":
        return intrinsicMethod(type, methodName, ["reflect.Type"], ["bool"], (args, context) =>
          reflectRtypeImplements(args[0] ?? null, args[1] ?? null, context));
      case "Elem":
        return intrinsicMethod(type, methodName, [], ["reflect.Type"], (args, context) =>
          reflectRtypeElem(args[0] ?? null, context));
      default:
        return undefined;
    }
  }
  if (type !== "time.Time") return undefined;
  const int64Result = (): MethodDef => intrinsicMethod("time.Time", methodName, [], ["int64"], (args) =>
    timeMethodInt64(methodName, args));
  switch (methodName) {
    case "Unix":
    case "UnixNano":
    case "UnixMilli":
...
function timeMethodInt64(methodName: string, args: RuntimeValue[]): RuntimeValue {
  switch (methodName) {
    case "Unix":
...
function timeUnixNano(value: RuntimeValue): bigint {
  return BigInt.asIntN(64, timeUnixSeconds(value) * 1000000000n + timeNanosecond(value));
}

function timeUnixSeconds(value: RuntimeValue): bigint {
  return timeSeconds(value) + timeInternalToUnix;
}

function timeSeconds(value: RuntimeValue): bigint {
  const struct = timeStruct(value);
  if (!struct) return -timeInternalToUnix;
...
function timeNanosecond(value: RuntimeValue): bigint {
  const struct = timeStruct(value);
  if (!struct) return 0n;
  return BigInt.asIntN(32, BigInt.asUintN(64, timeStructFieldBigInt(struct, "wall")) & timeNsecMask);
}

function timeStruct(value: RuntimeValue): RuntimeStruct | undefined {
  const actual = unwrapNamed(dereferenceIfPointer(value));
  if (!(actual instanceof RuntimeStruct)) return undefined;
  return runtimeTypeAssignableMatch(actual.typeName, "time.Time") ? actual : undefined;
}

function timeStructFieldBigInt(struct: RuntimeStruct, name: string): bigint {
  return toBigInt(struct.get(name) ?? 0n);
}

function boundMethodValue(
  method: MethodDef,
  receiver: RuntimeValue,
  callerContext?: EvaluationContext,
  declaredReceiverType?: string
): GoJuniorFunction {
...
function boundDynamicInterfaceMethodValue(
  receiver: RuntimeValue,
  receiverTypeText: string,
  methodName: string,
  methodSignature: FunctionDecl["signature"]
  ...
  return {
    ...
    async call(args, context, typeArguments) {
      ...
      if (!method) {
        ...
        if (fmtError !== undefined) return fmtError;
...
function interfaceMethodDef(receiverTypeText: string, method: InterfaceMethodDecl): MethodDef {
  ...
  return {
    ...
    intrinsic: {
      ...
      async call(args, context, typeArguments) {
        ...
        if (!lookup) throw new GoJuniorPanic(`runtime error: nil embedded interface method ${method.name}`);
...
function pointerToReceiver(receiver: RuntimeValue, typeName: string): RuntimeValue {
  ...
  if (receiver instanceof RuntimeTypedNilValue && typedNilPointerReceiverMatches(receiver.typeName, typeName)) return receiver;
...
function typedNilPointerReceiverMatches(actualPointerType: string, receiverType: string): boolean {
  if (runtimeTypeAssignableMatch(actualPointerType, `*${receiverType}`)) return true;
...
function runtimeLocalTypeBaseName(typeText: string): string {
...
async function executeStatementInner(statement: Statement, context: EvaluationContext): Promise<Completion> {
...
function typeSwitchBindingType(clause: SwitchStatement["clauses"][number]): TypeNode | undefined {
  if (clause.default || (clause.typeValues?.length ?? 0) !== 1) return undefined;
      ...
      const completion = await context.childScopeAsync(async () => {
        if (statement.range?.keyName) {
          if (statement.range.keyName !== "_") {
            if (statement.range.define) context.declare(statement.range.keyName, index, true, rangeTypes.key ?? inferredTypeText(index));
        ...
        if (statement.range?.valueName) {
          if (statement.range.valueName !== "_") {
            if (statement.range.define) context.declare(statement.range.valueName, value, true, rangeTypes.value ?? inferredTypeText(value));
...
async function executeAssign(statement: AssignStatement, context: EvaluationContext): Promise<void> {
  ...
  await context.withScopeAsync(assignmentScope, async () => {
    ...
    const expectedRoles = statement.values.length === statement.targets.length
      ...
      : [];
    const targets: AssignmentTargetHandle[] = [];
    for (const target of statement.targets) targets.push(await prepareAssignmentTarget(target, context));
...
function declareOrAssignShortVars(
  names: string[],
  values: RuntimeValue[],
  context: EvaluationContext,
  sources: Expression[] = [],
  ...
  if (values.length !== names.length) {
    throw new GoJuniorRuntimeError(
      `short declaration count mismatch: ${names.length} names (${names.join(", ")}) but ${values.length} values${shortValueShapeSuffix(values)}`,
      statement?.span
    );
  }
...
function shortValueShapeSuffix(values: RuntimeValue[]): string {
  if (values.length !== 1) return "";
  const value = values[0];
  if (Array.isArray(value)) return `; value is Array(len=${value.length}, tuple=${isTupleValues(value)})`;
  if (value === null) return "; value is <nil>";
  if (value instanceof RuntimeNamedValue) return `; value is RuntimeNamedValue(${value.typeName})`;
  if (value instanceof RuntimeInterfaceValue) return `; value is RuntimeInterfaceValue(${value.interfaceType})`;
  if (value instanceof RuntimePointer) return `; value is RuntimePointer(${value.typeName})`;
  if (value instanceof RuntimeGoString) return "; value is string";
  if (typeof value === "object") return `; value is ${value.constructor?.name ?? "object"}`;
...
async function evaluateAssignmentValues(
  expressions: Expression[],
  targetCount: number,
  context: EvaluationContext,
  expectedTypes: Array<string | undefined> = [],
  expectedRoles: Array<string | undefined> = []
  ...
  if (targetCount > 1 && values.length === 1) {
    ...
    if (
      ...
      return (first as RuntimeValue[]).slice(0, targetCount);
    }
  }
  return values;
}

function expressionStaticResultCount(expression: Expression | undefined, context: EvaluationContext): number {
  if (!expression) return 0;
  if (expression.kind === "CallExpression") {
    let count = 0;
    while (callExpressionResultTypeText(expression, context, count) !== undefined) count += 1;
    return count;
  }
  if (expression.kind === "IndexExpression" || expression.kind === "TypeAssertionExpression") return 2;
  if (expression.kind === "UnaryExpression" && expression.operator === "<-") return 2;
  return 1;
}

async function evaluateExpressionListWithExpectedTypes(
  expressions: Expression[],
  context: EvaluationContext,
  expectedTypes: Array<string | undefined>,
  expectedRoles: Array<string | undefined>
...
async function evaluateExpressionWithExpectedType(
...
function assignmentTargetExpectedTypeText(target: Expression, context: EvaluationContext): string | undefined {
  if (target.kind === "Identifier") return context.lookupTypeText(target.name);
  return undefined;
}

function assignmentTargetRole(target: Expression): string | undefined {
  if (target.kind === "Identifier") return `variable ${target.name}`;
  if (target.kind === "IndexExpression") return "index assignment";
  return undefined;
}
...
function prepareCompoundAssignmentOperands(
  operator: NonNullable<AssignStatement["operator"]>,
  left: RuntimeValue,
  right: RuntimeValue,
...
interface AssignmentTargetHandle {
  readonly expression: Expression;
  get(): Promise<RuntimeValue>;
  set(value: RuntimeValue, source?: Expression): Promise<void>;
}

async function prepareAssignmentTarget(target: Expression, context: EvaluationContext): Promise<AssignmentTargetHandle> {
  if (target.kind === "Identifier") {
    return {
      expression: target,
      async get() {
        if (target.name === "_") return null;
...
async function assignExpressionTarget(
  target: Expression,
  value: RuntimeValue,
  context: EvaluationContext,
  source?: Expression
...
function qualifyRuntimeTypeArgumentBindings(
  bindings: Record<string, string> | undefined,
  packageName: string
): Record<string, string> | undefined {
  if (!bindings) return undefined;
  const qualified: Record<string, string> = { ...bindings };
  for (const [name, type] of Object.entries(bindings)) {
    if (!name.includes(".")) qualified[`${packageName}.${name}`] = type;
  }
  return qualified;
}
...
function inferRuntimeCallTypeArgumentsFromCaller(
  callee: GoJuniorFunction,
  args: RuntimeValue[],
  expressions: Expression[],
  names: string[],
  ...
  const typeNames = new Set(names);
  if (importPath) {
    for (const name of names) typeNames.add(`${importPath}.${name}`);
  }
  for (const [index, parameter] of callee.signature?.parameters.entries() ?? []) {
    ...
    if (parameter.variadic) {
      for (let argIndex = index; argIndex < args.length; argIndex += 1) {
        ...
        if (actualType) inferTypeArgumentsFromTypeText(parameterType, actualType, typeNames, inferred, callerContext);
      }
      break;
    }
...
async function evaluateArrayLiteralEntries(
  ...
  for (const element of elements) {
    ...
    values[index] = prepareAssignableToType(
      await evaluateExpressionWithExpectedType(element.value, context, elementType, elementRole),
      elementType,
      elementRole,
      context
    );
...
async function evaluateStructLiteral(expression: StructLiteralExpression, context: EvaluationContext): Promise<RuntimeValue> {
  ...
  for (const [index, field] of expression.fields.entries()) {
    ...
    struct.set(
      declared.name,
      prepareValueForTargetType(value, declared.type.text, field.value, context, `field ${field.name ?? declared.name}`)
    );
  }
...
async function evaluateMapLiteral(expression: MapLiteralExpression, context: EvaluationContext): Promise<RuntimeMap> {
  ...
  for (const entry of expression.entries) {
    map.set(
      await evaluateExpressionWithExpectedType(entry.key, context, keyType, "map key"),
      await evaluateExpressionWithExpectedType(entry.value, context, valueType, "map value")
    );
  }
  return map;
}
...
function identifierRuntimeValue(expression: IdentifierExpression, context: EvaluationContext): RuntimeValue {
  if (context.hasBinding(expression.name)) return context.lookup(expression.name);
...
function prepareBinaryOperatorOperands(
...
function copyValueForNew(value: RuntimeValue, typeText: string, context: EvaluationContext): RuntimeValue {
  ...
  if (arrayType?.length !== undefined && !arrayType.inferLength && Array.isArray(value)) {
    const copy = Array.from({ length: value.length }, (_item, index) =>
      copyValueForNew(getArrayElement(value, index), arrayType.elementType, context)
    );
...
function makeTypeArgumentText(expression: Expression, context: EvaluationContext): string | undefined {
  if (expression.kind === "TypeExpression") return expression.type.text;
  const typeText = typeArgumentText(expression, true);
  return typeText && context.isKnownType(typeText) ? typeText : undefined;
}

function makeUnderlyingTypeText(typeText: string, context: EvaluationContext): string {
...
function appendTargetAcceptsStringSpread(
  target: RuntimeValue,
  context: EvaluationContext,
  staticTypeText?: string
): boolean {
  const candidates: Array<string | undefined> = [
    staticTypeText,
    inferredConcreteDynamicType(target)
  ];
  if (target instanceof RuntimeNamedValue) candidates.push(target.typeName);
  if (target instanceof RuntimeTypedNilValue) candidates.push(target.typeName);

  const actual = unwrapNamed(target);
  if (actual instanceof RuntimeTypedNilValue) candidates.push(actual.typeName);
  if (Array.isArray(actual)) candidates.push(arrayTypeTexts.get(actual));
  ...
  for (const typeText of candidates) {
    ...
    if (canonicalRuntimeTypeName(sliceType?.elementType ?? "") === "uint8") return true;
  }
  return false;
}
...
function copyValues(target: RuntimeValue, source: RuntimeValue): number {
  ...
  if (target instanceof RuntimeTypedNilValue && parseArrayOrSliceTypeText(target.typeName)) return 0;
  if (!Array.isArray(target)) throw new GoJuniorRuntimeError("copy destination must be a slice");
  if (source instanceof RuntimeTypedNilValue && parseArrayOrSliceTypeText(source.typeName)) return 0;
...
async function callRuntime(callee: RuntimeValue, args: RuntimeValue[], context: EvaluationContext, typeArguments?: string[]): Promise<RuntimeValue> {
  ...
  if (isRuntimeCallable(actual) || isGoJuniorFunction(actual)) {
    ...
    if (Array.isArray(result) && (
      (isRuntimeCallable(actual) && actual.tupleResult === true) ||
...
async function staticMethodSelectorValue(
  ...
  if (!method.pointerReceiver) return boundMethodValue(method, object, context, staticTypeText);
...
function promotedMethodAvailableForType(
  ...
  for (const field of typeDef.fields.filter((candidate) => candidate.embedded)) {
    ...
    if (interfaceMethodForType(embedded.baseType, methodName, context)) return true;
...
function getEvaluatedSelector(
  ...
  if (struct) {
    ...
    if (field) return field.get() ?? null;
  }
  const runtimeObjectForSelector = unwrapNamed(dereferenceIfPointer(object));
  if (isRuntimeObject(runtimeObjectForSelector)) {
    const value = runtimeObjectForSelector[expression.field];
...
function embeddedStructFromFieldValue(struct: RuntimeStruct, field: StructFieldDecl, context: EvaluationContext): RuntimeStruct | undefined {
...
function methodDefForRuntimeReceiver(
  context: EvaluationContext,
  receiverType: string,
  methodName: string
): MethodDef | undefined {
...
function shouldUseSparseZeroArray(type: ParsedArrayOrSliceType): boolean {
...
function isSparseZeroArrayElementType(typeText: string): boolean {
  const type = normalizeTypeText(typeText);
  return isIntegerType(type) || isFloatType(type) || type === "string" || type === "bool";
}

function makeSparseZeroArray(type: ParsedArrayOrSliceType): RuntimeValue[] {
  const values: RuntimeValue[] = [];
...
function runtimeTypeOwnerPackagePath(typeText: string | undefined, context?: EvaluationContext): string | undefined {
  const type = normalizeTypeText(typeText ?? "");
  if (!type) return undefined;
  if (type.startsWith("*")) return runtimeTypeOwnerPackagePath(type.slice(1), context);
  if (type.startsWith("[]")) return runtimeTypeOwnerPackagePath(type.slice(2), context);
  const arrayType = parseArrayOrSliceTypeText(type);
  if (arrayType) return runtimeTypeOwnerPackagePath(arrayType.elementType, context);
  const chanType = parseChanTypeText(type);
  if (chanType) return runtimeTypeOwnerPackagePath(chanType.elementType, context);
  const mapType = parseMapTypeText(type);
  if (mapType) return runtimeTypeOwnerPackagePath(mapType.valueType, context) ?? runtimeTypeOwnerPackagePath(mapType.keyType, context);
  const signature = parseFunctionTypeText(type);
  if (signature) {
  ...
  if (type.startsWith("interface{") || type.startsWith("struct{")) return undefined;
  const owner = runtimeQualifiedTypePackagePath(type);
  if (owner) return owner;
  const generic = genericTypeArguments(type);
  if (generic) {
    const baseOwner = runtimeTypeOwnerPackagePath(generic.base, context);
    if (baseOwner) return baseOwner;
    const importPath = context?.importPath();
    if (importPath && runtimeBareTypeBelongsToContext(generic.base, context)) return importPath;
    if (/^[A-Za-z_]\w*$/.test(generic.base)) return undefined;
    return generic.args.map((arg) => runtimeTypeOwnerPackagePath(arg, context)).find((candidate): candidate is string => Boolean(candidate));
  }
  return undefined;
}
...
function defaultIntrinsicNamedValue(type: string, context?: EvaluationContext): RuntimeValue | undefined {
  ...
  if (intrinsicStruct) return defaultStructValueForTypeDef(intrinsicStruct, context, type);
...
function intrinsicStructTypeDef(typeText: string): StructTypeDef | undefined {
  const type = normalizeTypeText(typeText);
  if (type === "time.Time") {
    return {
      name: "time.Time",
...
function zeroValueForMapValue(typeText: string, context?: EvaluationContext): RuntimeValue {
  ...
  if (arrayType) {
    if (arrayType.length === undefined && !arrayType.inferLength) return new RuntimeTypedNilValue(arrayType.typeText);
    if (shouldUseSparseZeroArray(arrayType)) return makeSparseZeroArray(arrayType);
...
function convertValueToType(
  ...
  if (type === "string") {
    ...
    if (actual === null && nilIntegerSliceConvertsToString(sourceType, context)) return RuntimeGoString.fromUtf8Text("");
  ...
  if (mapType) {
    if (actual === null) return new RuntimeTypedNilValue(type);
    if (!(actual instanceof RuntimeMap)) throwTypeError(actual, type, "conversion");
    if (
      !runtimeTypeAssignableMatchInContext(actual.keyType, mapType.keyType, context) ||
  ...
  if (type.startsWith("func(")) {
    if (!isRuntimeCallable(actual) && !isGoJuniorFunction(actual)) throwTypeError(actual, type, "conversion");
...
function reinterpretUnsafePointer(pointer: RuntimePointer, targetType: string, context: EvaluationContext): RuntimePointer {
  ...
  if (reflectStructView) return reflectStructView;
...
function reinterpretRuntimeTypeDescriptorOverlay(
  ...
  if (!isRuntimeTypeDescriptorOverlayType(targetType, context)) return undefined;
  let overlayView: RuntimeStruct | undefined;
  const currentValue = (): RuntimeValue => {
    const descriptor = runtimeTypeDescriptorFromUnsafePointer(pointer);
    if (!descriptor) return defaultValueForTypeText(targetType, context);
    if (!overlayView) {
  ...
  return new RuntimePointer(targetType, currentValue, (next) => {
    ...
    if (!nextStruct) throwTypeError(next, targetType, `*${targetType}`);
    const descriptor = runtimeTypeDescriptorFromUnsafePointer(pointer);
    if (descriptor) {
      for (const [field, value] of nextStruct.orderedFields()) descriptor.set(field, value);
    }
    overlayView = nextStruct;
  }, `descriptor.overlay:${targetType}:${runtimePointerIdentityKey(pointer)}`);
}

function runtimeTypeDescriptorFromUnsafePointer(pointer: RuntimePointer): RuntimeStruct | undefined {
  const value = pointer.get();
  const struct = structFromValue(value);
  if (!struct) return undefined;
  ...
  if (rtypeDescriptor !== undefined) {
    ...
    if (descriptor && isRuntimeTypeDescriptorStruct(descriptor)) return descriptor;
  }
  return isRuntimeTypeDescriptorStruct(struct) ? struct : undefined;
}

function runtimeTypeDescriptorOverlayStruct(
  descriptor: RuntimeStruct,
  targetType: string,
  context: EvaluationContext
): RuntimeStruct {
...
function refreshRuntimeTypeDescriptorOverlayStruct(
  overlay: RuntimeStruct,
  descriptor: RuntimeStruct,
  targetType: string,
  context: EvaluationContext
): void {
...
function runtimeTypeDescriptorEmbeddedFields(
  targetType: string,
  context: EvaluationContext,
  seen = new Set<string>()
): Array<{ name: string; type: TypeNode; context: EvaluationContext }> {
  ...
  const fields: Array<{ name: string; type: TypeNode; context: EvaluationContext }> = [];
  for (const field of typeDef.fields.filter((candidate) => candidate.embedded)) {
...
function isRuntimeTypeDescriptorStruct(struct: RuntimeStruct): boolean {
  return struct.get("Kind_") !== undefined &&
    struct.get("GoJrTypeText") !== undefined &&
    struct.get("GoJrString") !== undefined;
}

function isRuntimeTypeDescriptorOverlayType(
  typeText: string,
  context: EvaluationContext,
  seen = new Set<string>()
): boolean {
  ...
  seen.add(key);
  if (isInternalAbiDescriptorOverlayName(canonical, context)) return true;
  if (isReflectDescriptorOverlayName(canonical, context)) return true;
  const alias = context.aliasType(canonical);
  if (alias && alias !== canonical && isRuntimeTypeDescriptorOverlayType(alias, context, seen)) return true;
  const typeDef = context.typeDef(canonical);
  const fieldContext = typeDef?.declaringContext ?? context;
  return Boolean(typeDef?.fields.some((field) =>
    field.embedded && isRuntimeTypeDescriptorOverlayType(field.type.text, fieldContext, seen)));
}

function isInternalAbiDescriptorOverlayName(typeText: string, context: EvaluationContext): boolean {
...
function isReflectDescriptorOverlayName(typeText: string, context: EvaluationContext): boolean {
...
function runtimeLocalTypeNameForPackage(typeText: string, importPath: string, context: EvaluationContext): string | undefined {
  const type = normalizeTypeText(typeText);
  const qualified = packageQualifiedRuntimeTypeNameParts(type);
  if (qualified) return qualified.importPath === importPath ? qualified.localName : undefined;
  return context.importPath() === importPath ? type : undefined;
}

function reinterpretReflectRtypeAsReflectStructType(
  pointer: RuntimePointer,
  targetType: string,
  context: EvaluationContext
): RuntimePointer | undefined {
  if (!isReflectRtypeName(pointer.typeName, context) || !isReflectStructTypeName(targetType, context)) return undefined;
  let structTypeView: RuntimeStruct | undefined;
  const currentValue = (): RuntimeValue => {
    const rtype = structFromValue(pointer.get());
    const descriptor = rtype?.get("t");
    if (!(descriptor instanceof RuntimeStruct)) return defaultValueForTypeText(targetType, context);
    if (!structTypeView) structTypeView = new RuntimeStruct(targetType);
...
function isReflectStructTypeName(typeText: string, context: EvaluationContext): boolean {
  const type = normalizeTypeText(typeText);
...
function nilIntegerSliceConvertsToString(typeText: string | undefined, context?: EvaluationContext): boolean {
...
function prepareValueForParameterType(
  ...
  if (interfaceType) {
    ...
    try {
      ...
      if (sourceContext === context) throw error;
      const sourceInterfaceType = interfaceTarget(targetType, sourceContext);
      if (!sourceInterfaceType) throw error;
...
function goJuniorFunctionResultTypeText(
  callee: GoJuniorFunction,
  resultIndex: number,
  context: EvaluationContext,
  explicitTypeArguments?: string[]
...
function methodCallResultTypeText(expression: CallExpression, context: EvaluationContext, resultIndex: number): string | undefined {
...
function staticCalleeRuntimeValue(expression: Expression, context: EvaluationContext): RuntimeValue | undefined {
  try {
    if (expression.kind === "Identifier") return identifierRuntimeValue(expression, context);
...
function prepareAssignableToType(value: RuntimeValue, typeText: string, role: string, context?: EvaluationContext): RuntimeValue {
  ...
  const scopedAlias = context?.scopedAliasType(type);
  if (scopedAlias && scopedAlias !== type) return prepareAssignableToType(value, scopedAlias, role, context);
  const interfaceType = interfaceTarget(type, context);
  ...
  if (type.startsWith("func(")) {
    const actual = unwrapNamed(value);
    if (!isRuntimeCallable(actual) && !isGoJuniorFunction(actual)) throwTypeError(value, type, role);
    return value;
  }
  return value;
}
...
function interfaceTarget(type: string, context?: EvaluationContext): InterfaceTypeDef | undefined {
  ...
  if (normalized === "any" || normalized === "interface{}") return { name: normalized, methods: [], embeds: [] };
  if (normalized === "error") return errorInterfaceType;
  const alias = context?.trueAliasType(normalized);
  if (alias && alias !== normalized) return interfaceTarget(alias, context);
...
function prepareInterfaceAssignment(
  ...
  if (value instanceof RuntimeInterfaceValue) {
    ...
    if (methodContext && !valueImplementsInterface(dynamicValue, interfaceType, methodContext)) throwTypeError(value, type, role);
...
function valueImplementsInterface(value: RuntimeValue, interfaceType: InterfaceTypeDef, context: EvaluationContext): boolean {
  ...
  if (isRuntimeObject(value) && runtimeObjectImplementsInterface(value, interfaceType, context)) return true;
...
function runtimeObjectImplementsInterface(value: RuntimeObject, interfaceType: InterfaceTypeDef, context: EvaluationContext): boolean {
  for (const method of interfaceType.methods) {
    const candidate = value[method.name];
    if (candidate === undefined) return false;
    if (!isRuntimeCallable(candidate) && !isGoJuniorFunction(candidate)) return false;
    const signature = candidate.signature;
    if (signature && !signaturesCompatible(signature, method.signature, context)) return false;
  }
  return true;
}

function runtimeTypeTextImplementsInterface(typeText: string, interfaceType: InterfaceTypeDef, context: EvaluationContext): boolean {
  const sourceInterface = interfaceTarget(typeText, context);
  if (sourceInterface) return interfaceDefinitionImplementsInterface(sourceInterface, interfaceType, context);
...
function signatureTypeCompatible(actual: string, expected: string, context?: EvaluationContext): boolean {
  return runtimeTypeAssignableMatchInContext(
...
function resolveRuntimeCompositeAliases(typeText: string, context?: EvaluationContext, seen = new Set<string>()): string {
  const raw = context?.resolveImportedTypeText(typeText) ?? typeText;
  if (/^\s*(?:struct|interface)\s*\{/.test(raw) || /^\s*func\s*\(/.test(raw)) return raw.trim();
  const type = normalizeTypeText(raw);
  if (seen.has(type)) return type;
  ...
  if (alias && alias !== type) return resolveRuntimeCompositeAliases(alias, context, seen);
  if (type.startsWith("*")) return `*${resolveRuntimeCompositeAliases(type.slice(1), context, new Set(seen))}`;
  if (type.startsWith("[]")) return `[]${resolveRuntimeCompositeAliases(type.slice(2), context, new Set(seen))}`;
  const arrayType = parseArrayOrSliceTypeText(type, context);
  if (arrayType?.length !== undefined && !arrayType.inferLength) {
...
function runtimeValueMatchesType(value: RuntimeValue, typeText: string, context?: EvaluationContext): boolean {
  ...
  if (value instanceof RuntimeTypedNilValue) {
    ...
    if (type.startsWith("*")) return runtimeTypeIdentityMatchInContext(value.typeName, type, context);
  }
  ...
  if (type.startsWith("*")) return value instanceof RuntimePointer && runtimeTypeIdentityMatchInContext(value.typeName, type.slice(1), context);
  if (value instanceof RuntimeStruct) return runtimeTypeIdentityMatchInContext(value.typeName, type, context);
  if (value instanceof RuntimeMap) {
    const mapType = parseMapTypeText(type);
...
function canonicalRuntimeTypeName(type: string): string {
  const normalized = normalizeTypeText(type);
  if (normalized.startsWith("*")) return `*${canonicalRuntimeTypeName(normalized.slice(1))}`;
  ...
  if (mapType) return `map[${canonicalRuntimeTypeName(mapType.keyType)}]${canonicalRuntimeTypeName(mapType.valueType)}`;
...
function runtimePointerPointeeConversionMatchInContext(actual: string, expected: string, context: EvaluationContext): boolean {
  return runtimeTypeAssignableMatch(
    resolveRuntimeAliasTypeText(actual, context),
    resolveRuntimeAliasTypeText(expected, context)
  );
}

function runtimeTypeIdentityMatchInContext(actual: string, expected: string, context?: EvaluationContext): boolean {
...
function retagRuntimePointer(pointer: RuntimePointer, typeName: string, context?: EvaluationContext): RuntimePointer {
...
function valueLength(value: RuntimeValue): number {
  ...
  if (headerLength !== undefined) return headerLength;
...
function valueCapacity(value: RuntimeValue): number {
  ...
  if (headerCapacity !== undefined) return headerCapacity;
...
function runtimeHeaderLength(value: RuntimeValue): number | undefined {
  const struct = structFromValue(value);
  if (!struct || !isRuntimeSliceOrStringHeaderStruct(struct)) return undefined;
  return runtimeHeaderIntegerField(struct, "Len");
}

function runtimeHeaderCapacity(value: RuntimeValue): number | undefined {
  const struct = structFromValue(value);
  if (!struct || !isRuntimeSliceHeaderStruct(struct)) return undefined;
  return runtimeHeaderIntegerField(struct, "Cap");
}

function isRuntimeSliceOrStringHeaderStruct(struct: RuntimeStruct): boolean {
  return isRuntimeSliceHeaderStruct(struct) || isRuntimeStringHeaderStruct(struct);
}

function isRuntimeSliceHeaderStruct(struct: RuntimeStruct): boolean {
  const type = normalizeTypeText(struct.typeName);
  return type === "internal/unsafeheader.Slice" || type === "reflect.SliceHeader";
}

function isRuntimeStringHeaderStruct(struct: RuntimeStruct): boolean {
  const type = normalizeTypeText(struct.typeName);
  return type === "internal/unsafeheader.String" || type === "reflect.StringHeader";
}

function runtimeHeaderIntegerField(struct: RuntimeStruct, fieldName: "Len" | "Cap"): number | undefined {
  const value = struct.get(fieldName);
  if (typeof value === "bigint" || typeof value === "number") return toNumber(value);
  return undefined;
}
...
function markArrayView(
  value: RuntimeValue[],
  source: RuntimeValue[],
  offset: number,
  transforms: Pick<RuntimeArrayView, "toView" | "toSource"> = {}
): void {
  arrayViews.set(value, { source, offset, ...transforms });
}

function copyArrayRuntimeMetadata(
  source: RuntimeValue[],
  target: RuntimeValue[],
  typeTransform: (typeText: string) => string = (typeText) => typeText
): RuntimeValue[] {
  const typeText = arrayTypeTexts.get(source);
  if (typeText) markArrayType(target, typeTransform(typeText));
  const capacity = arrayCapacities.get(source);
  if (capacity !== undefined) arrayCapacities.set(target, capacity);
  const sparseDefault = sparseArrayDefaultElementTypes.get(source);
  if (sparseDefault) sparseArrayDefaultElementTypes.set(target, typeTransform(sparseDefault));
  return target;
}

function arrayPointerSequence(value: RuntimeValue[], index: number): RuntimePointerSequence {
...
function arrayElementTypeText(value: RuntimeValue[]): string | undefined {
  return parseArrayOrSliceTypeText(arrayTypeTexts.get(value) ?? "")?.elementType;
}

function getArrayElement(value: RuntimeValue[], index: number, seen = new Set<RuntimeValue[]>()): RuntimeValue {
  const view = arrayViews.get(value);
  if (view) {
  ...
  if (Object.prototype.hasOwnProperty.call(value, index)) return value[index] ?? null;
  const defaultType = sparseArrayDefaultElementTypes.get(value);
  if (defaultType) return zeroValueForMapValue(defaultType);
  return null;
}

function setArrayElement(value: RuntimeValue[], index: number, item: RuntimeValue, seen = new Set<RuntimeValue[]>()): void {
  const view = arrayViews.get(value);
  if (view) {
    if (!seen.has(value) && view.source !== value) {
      seen.add(value);
...
function parseArrayOrSliceTypeText(typeText: string, context?: EvaluationContext, filename?: string): ParsedArrayOrSliceType | undefined {
  const type = normalizeTypeText(typeText);
...
function resolveArrayLengthText(lengthText: string, context?: EvaluationContext, filename?: string): number | undefined {
...
function evaluateArrayLengthConstant(text: string, context?: EvaluationContext, filename?: string): bigint | undefined {
...
function externalizeRuntimeValue(value: RuntimeValue): RuntimeValue {
  ...
  if (value instanceof RuntimeInterfaceValue) return new RuntimeInterfaceValue(value.interfaceType, value.value === null ? null : externalizeRuntimeValue(value.value), value.methodContext);
  if (Array.isArray(value)) {
    const mapped = Array.from({ length: value.length }, (_item, index) =>
      externalizeRuntimeValue(getArrayElement(value, index))
    );
    if (isTupleValues(value)) markTupleValues(mapped);
...
function externalizePackageRuntimeValue(value: RuntimeValue, packageName: string): RuntimeValue {
  ...
  if (isGoJuniorFunction(value)) {
    ...
    const signature = value.signature
      ...
      : undefined;
    const wrapped: GoJuniorFunction = {
      ...value,
      ...(signature ? { signature } : {}),
      async call(args, context, typeArguments) {
        const result = await value.call(args, context, typeArguments);
        if (value.preserveResultIdentity) return result;
  ...
  if (value instanceof RuntimeNamedValue) {
    ...
    rememberPackageRuntimeValue(externalizedPackageValueCache, value, packageName, wrapped);
    markPackageRuntimeWrapper(wrapped, packageName, "externalized", value);
    (wrapped as { value: RuntimeValue }).value = externalizePackageRuntimeValue(value.value, packageName);
    return wrapped;
  }
  if (value instanceof RuntimeInterfaceValue) {
    const cached = cachedPackageRuntimeValue(externalizedPackageValueCache, value, packageName);
    ...
    rememberPackageRuntimeValue(externalizedPackageValueCache, value, packageName, wrapped);
    markPackageRuntimeWrapper(wrapped, packageName, "externalized", value);
    (wrapped as { value: RuntimeValue }).value = value.value === null ? null : externalizePackageRuntimeValue(value.value, packageName);
    return wrapped;
  }
  if (value instanceof RuntimeTypedNilValue) {
  ...
  if (value instanceof RuntimePointer) {
    const cached = cachedPackageRuntimeValue(externalizedPackageValueCache, value, packageName);
    if (cached !== undefined) return cached;
    const wrapped = new RuntimePointer(
      ...
      runtimePointerIdentityKey(value),
      value.sequenceInfo()
    );
    rememberPackageRuntimeValue(externalizedPackageValueCache, value, packageName, wrapped);
    markPackageRuntimeWrapper(wrapped, packageName, "externalized", value);
  ...
  if (Array.isArray(value)) {
    const cached = cachedPackageRuntimeValue(externalizedPackageValueCache, value, packageName);
    if (cached !== undefined) return cached;
    const mapped = Array.from({ length: value.length }) as RuntimeValue[];
...
function internalizePackageRuntimeValue(value: RuntimeValue, packageName: string): RuntimeValue {
  ...
  if (value instanceof RuntimeNamedValue) {
    ...
    (wrapped as { value: RuntimeValue }).value = internalizePackageRuntimeValue(value.value, packageName);
    return wrapped;
  }
  if (value instanceof RuntimeInterfaceValue) {
    const cached = cachedPackageRuntimeValue(internalizedPackageValueCache, value, packageName);
    ...
    rememberPackageRuntimeValue(internalizedPackageValueCache, value, packageName, wrapped);
    markPackageRuntimeWrapper(wrapped, packageName, "internalized", value);
    (wrapped as { value: RuntimeValue }).value = value.value === null ? null : internalizePackageRuntimeValue(value.value, packageName);
    return wrapped;
  }
  if (value instanceof RuntimeTypedNilValue) {
...
function cachedPackageRuntimeValue(
  cache: WeakMap<object, Map<string, RuntimeValue>>,
  value: RuntimeValue,
  packageName: string
): RuntimeValue | undefined {
  if (value === null || (typeof value !== "object" && typeof value !== "function")) return undefined;
  return cache.get(value as object)?.get(packageName);
}

function rememberPackageRuntimeValue(
  cache: WeakMap<object, Map<string, RuntimeValue>>,
  value: RuntimeValue,
  packageName: string,
  wrapped: RuntimeValue
): void {
  if (value === null || (typeof value !== "object" && typeof value !== "function")) return;
  let byPackage = cache.get(value as object);
  if (!byPackage) {
    byPackage = new Map();
    cache.set(value as object, byPackage);
  }
  byPackage.set(packageName, wrapped);
}

function markPackageRuntimeWrapper(
  wrapped: RuntimeValue,
  packageName: string,
  direction: "externalized" | "internalized",
  source: RuntimeValue
): void {
  if (wrapped === null || (typeof wrapped !== "object" && typeof wrapped !== "function")) return;
  packageRuntimeWrapperInfo.set(wrapped as object, { packageName, direction, source });
}

function packageRuntimeWrapperPassthrough(
  value: RuntimeValue,
  packageName: string,
  direction: "externalized" | "internalized"
): RuntimeValue | undefined {
  if (value === null || (typeof value !== "object" && typeof value !== "function")) return undefined;
  const info = packageRuntimeWrapperInfo.get(value as object);
  if (!info || info.packageName !== packageName) return undefined;
...
function qualifyLocalRuntimeTypeName(
  typeName: string,
  packageName: string,
  localTypeParameters = new Set<string>()
): string {
  const type = normalizeTypeText(typeName);
  if (localTypeParameters.has(type)) return type;
  const generic = genericTypeArguments(type);
  if (!type || !packageName || isPredeclaredType(type) || (!generic && isQualifiedRuntimeTypeName(type))) return type;
  if (type.startsWith("*")) return `*${qualifyLocalRuntimeTypeName(type.slice(1), packageName, localTypeParameters)}`;
  if (type.startsWith("[]")) return `[]${qualifyLocalRuntimeTypeName(type.slice(2), packageName, localTypeParameters)}`;
  const array = /^\[([0-9.]*)\]([\s\S]+)$/.exec(type);
  if (array) return `[${array[1] ?? ""}]${qualifyLocalRuntimeTypeName(array[2] ?? "", packageName, localTypeParameters)}`;
  const mapType = parseMapTypeText(type);
  if (mapType) {
...
function isQualifiedRuntimeTypeName(type: string): boolean {
...
function qualifyRuntimeSignature(
  signature: FunctionDecl["signature"],
  packageName: string,
  typeParameters?: Iterable<string>
): FunctionDecl["signature"] {
...
function functionDeclarationRuntimeTypeParameters(declaration: FunctionDecl | undefined): Set<string> {
...
const goTypesStructTypeDefCache = new WeakMap<GoTypesPackage, Map<string, StructTypeDef | null>>();
const goTypesInterfaceDefCache = new WeakMap<GoTypesPackage, Map<string, InterfaceTypeDef | null>>();
const goTypesMethodDefCache = new WeakMap<GoTypesPackage, Map<string, MethodDef | null>>();
const goTypesAliasTypeCache = new WeakMap<GoTypesPackage, Map<string, { target: string; trueAlias: boolean } | null>>();

function goTypesStructTypeDef(importPath: string, localName: string, requestedTypeText: string, pkg: GoTypesPackage): StructTypeDef | undefined {
  ...
  if (packageCache.has(baseName)) return packageCache.get(baseName) ?? undefined;
  ...
  for (let index = 0; index < struct.NumFields(); index += 1) {
    ...
    const fieldDecl: StructFieldDecl = {
      name: field.Name(),
...
function goTypesInterfaceDef(importPath: string, localName: string, pkg: GoTypesPackage): InterfaceTypeDef | undefined {
  ...
  if (packageCache.has(name)) return packageCache.get(name) ?? undefined;
  ...
  const def: InterfaceTypeDef = { name, methods, embeds: [] };
  packageCache.set(name, def);
  return def;
}

function goTypesMethodDef(importPath: string, localName: string, methodName: string, pkg: GoTypesPackage): MethodDef | undefined {
  ...
  if (packageCache.has(cacheKey)) return packageCache.get(cacheKey) ?? undefined;
...
function goTypesAliasType(importPath: string, localName: string, pkg: GoTypesPackage): { target: string; trueAlias: boolean } | undefined {
...
function goTypesStructLike(type: GoTypesType | null): {
  NumFields(): number;
  Field(index: number): GoTypesVar;
...
function goTypesAliasLike(type: GoTypesType | null): {
  Rhs(): GoTypesType | null;
} | undefined {
  const candidate = type as {
    constructor?: { name?: string };
    Rhs?: () => GoTypesType | null;
  ...
  return candidate?.constructor?.name === "Alias" && typeof candidate.Rhs === "function"
    ? candidate as { Rhs(): GoTypesType | null }
    : undefined;
}

function goTypesNamedLike(type: GoTypesType | null): {
  NumMethods(): number;
  Method(index: number): {
    Name(): string;
    Type(): GoTypesType | null;
    Signature?: () => {
      Recv?: () => GoTypesVar | null;
    };
    hasPtrRecv?: () => boolean;
  };
} | undefined {
...
function goTypesMethodReceiverTypeText(method: {
  Signature?: () => {
    Recv?: () => GoTypesVar | null;
  };
  hasPtrRecv?: () => boolean;
}, pkg: GoTypesPackage): string | undefined {
...
function goTypesInterfaceLike(type: GoTypesType | null): {
  NumMethods(): number;
  Method(index: number): {
    Name(): string;
    Type(): GoTypesType | null;
...
function goTypesTypeParameterNames(type: GoTypesType | null): string[] {
  const params = (type as {
    TypeParams?: () => {
      Len?: () => number;
      At?: (index: number) => { Obj?: () => { Name?: () => string } };
    } | null;
  } | null)?.TypeParams?.();
...
function goTypesRuntimeTypeText(type: GoTypesType | null, importPath: string, pkg: GoTypesPackage, typeParameters: string[] = []): string {
  if (!type) return "";
  return qualifyLocalRuntimeTypeName(
    GoTypesTypeString(type, GoTypesRelativeTo(pkg)),
    importPath,
...
function runtimeMapKeyId(key: RuntimeValue, options: { freshNonReflexive?: boolean } = {}): string {
  ...
  if (actual instanceof RuntimePointer) return `ptr:${runtimePointerIdentityKey(actual)}`;
...
function runtimePointerIdentityKey(pointer: RuntimePointer): string {
...
async function formatFmtValue(verb: string, goSyntax: boolean, value: RuntimeValue, context: EvaluationContext): Promise<string> {
  if (!goSyntax) {
    ...
    if (errorer !== undefined && (verb === "%v" || verb === "%s")) return errorer;
...
async function fmtErrorerValue(value: RuntimeValue, context: EvaluationContext): Promise<string | undefined> {
...
async function toStringValueAsync(value: RuntimeValue, context: EvaluationContext): Promise<string> {
  if (isRuntimeString(value)) return goStringText(value);
  const errorer = await fmtErrorerValue(value, context);
  if (errorer !== undefined) return errorer;
  return await fmtStringerValue(value, context) ?? formatValue(value);
}

interface ValueFormatState {
  active: Set<object>;
}

function newValueFormatState(): ValueFormatState {
  return { active: new Set() };
}

function withValueFormatCycleGuard(state: ValueFormatState, value: object, body: () => string): string {
...
function formatValueInner(value: RuntimeValue, state: ValueFormatState): string {
  if (value instanceof RuntimeNamedValue) return withValueFormatCycleGuard(state, value, () => formatValueInner(value.value, state));
  if (value instanceof RuntimeInterfaceValue) {
  ...
  if (value instanceof RuntimeChannel) return `chan ${value.elementType}`;
  if (value instanceof RuntimeMap) return formatRuntimeMap(value, state);
  if (value instanceof RuntimeStruct) return formatRuntimeStruct(value, state);
  if (value instanceof RuntimePointer) return withValueFormatCycleGuard(state, value, () => `&${formatValueInner(value.get(), state)}`);
  ...
  return withValueFormatCycleGuard(state, value, () =>
    `{${Object.entries(value).map(([key, item]) => `${key}:${formatValueInner(item, state)}`).join(" ")}}`
  );
}
...
function formatReplValueInner(value: RuntimeValue, state: ValueFormatState): string {
  if (value instanceof RuntimeNamedValue) return withValueFormatCycleGuard(state, value, () => formatReplValueInner(value.value, state));
  if (value instanceof RuntimeInterfaceValue) {
  ...
  if (value instanceof RuntimeChannel) return `chan ${value.elementType}`;
  if (value instanceof RuntimeMap) return formatReplMap(value, state);
  if (value instanceof RuntimeStruct) return formatReplStruct(value, state);
  if (value instanceof RuntimePointer) return withValueFormatCycleGuard(state, value, () => `&${formatReplValueInner(value.get(), state)}`);
  if (isGoJuniorFunction(value)) return formatFunctionValue(value);
  if (isRuntimeCallable(value)) return `<func ${value.name}>`;
  if (value instanceof SheetBinding) return `<sheet ${value.name}>`;
  const packageText = formatRuntimePackageObject(value);
  if (packageText !== undefined) return packageText;
  return withValueFormatCycleGuard(state, value, () =>
    `{${Object.entries(value).map(([key, item]) => `${key}:${formatReplValueInner(item, state)}`).join(" ")}}`
  );
}
...
function formatGoSyntaxValueInner(value: RuntimeValue, state: ValueFormatState): string {
  if (value instanceof RuntimeNamedValue) {
    return withValueFormatCycleGuard(state, value, () =>
      value.value instanceof RuntimeTypedNilValue
        ? `${value.typeName}(nil)`
  ...
  if (value instanceof RuntimeChannel) return `chan ${value.elementType}`;
  if (value instanceof RuntimeMap) return formatGoSyntaxMap(value, state);
  if (value instanceof RuntimeStruct) return formatGoSyntaxStruct(value, state);
  if (value instanceof RuntimePointer) return withValueFormatCycleGuard(state, value, () => `&${formatGoSyntaxValueInner(value.get(), state)}`);
  if (isGoJuniorFunction(value)) return formatFunctionValue(value);
  if (isRuntimeCallable(value)) return `<func ${value.name}>`;
  if (value instanceof SheetBinding) return `<sheet ${value.name}>`;
  return withValueFormatCycleGuard(state, value, () =>
    `map[string]interface{}{${Object.entries(value)
      .map(([key, item]) => `${JSON.stringify(key)}: ${formatGoSyntaxValueInner(item, state)}`)
      .join(", ")}}`
  );
}

function formatRuntimeMap(value: RuntimeMap, state: ValueFormatState): string {
  return withValueFormatCycleGuard(state, value, () => {
...
function formatRuntimeStruct(value: RuntimeStruct, state: ValueFormatState): string {
  return withValueFormatCycleGuard(state, value, () => {
...
function formatReplMap(value: RuntimeMap, state: ValueFormatState): string {
  return withValueFormatCycleGuard(state, value, () => {
  const entries = value.orderedEntries()
...
function formatReplStruct(value: RuntimeStruct, state: ValueFormatState): string {
  return withValueFormatCycleGuard(state, value, () => {
  const fields = value.orderedFields()
...
function formatGoSyntaxMap(value: RuntimeMap, state: ValueFormatState): string {
  return withValueFormatCycleGuard(state, value, () => {
  const entries = value.orderedEntries()
...
function formatGoSyntaxStruct(value: RuntimeStruct, state: ValueFormatState): string {
  return withValueFormatCycleGuard(state, value, () => {
  const fields = value.orderedFields()
