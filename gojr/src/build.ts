    ...
    if (!this.hasErrors()) {
      ...
      if (!this.hasErrors()) {
        ...
        const node: PackageBuildNode = {
          ...
          exports,
          artifactPath,
          artifactSource: generatedArtifactSource(pkgdef, files)
        };
...
function generatedArtifactSource(pkgdef: GoJuniorPackageExportData, files: SourceFile[]): string {
  const javascript = generatedArtifactJavaScript(pkgdef, files);
...
function generatedArtifactJavaScript(artifact: GoJuniorPackageExportData, files: SourceFile[]): string {
  ...
  return [
    ...
    "  }",
    "  return await runtime.evaluatePackageSourceFiles(gojrPackageSources, {",
    "    ...options,",
