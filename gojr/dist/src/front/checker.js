...
class FrontChecker {
    ...
    checkBasicLit(expr) {
        if (expr.token === TokenKind.IntLiteral)
            return { mode: "constant", type: this.universe.basic.untypedInt, value: BigInt(expr.value) };
        if (expr.token === TokenKind.FloatLiteral)
            return { mode: "constant", type: this.universe.basic.untypedFloat, value: Number(expr.value) };
    ...
    resolveStructType(expr, scope) {
        const fields = expr.fields.fields.flatMap((field) => {
            ...
            if (field.names.length === 0)
                return [{ name: embeddedFieldName(field.type), type, embedded: true }];
    ...
    error(message, span, code = "GJTYPE001") {
