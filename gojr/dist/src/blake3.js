...
export function blake3HashString(text) {
    return blake3HashBytes(encoder.encode(text));
}
export function blake3HashBytes(bytes) {
    return `blake3.33B-${base64Url(blake3RawBytes(bytes, 33))}`;
}
export function blake3RawBytes(bytes, length = 64) {
...
function rootParentNode(cvs, start, end) {
    const count = end - start;
    if (count < 2)
        throw new Error("internal blake3 error: root parent needs at least two children");
...
function subtreeCV(cvs, start, end) {
    const count = end - start;
    if (count === 1)
        return cvs[start].slice();
...
function compressChunk(chunk, key, counter, flags) {
    let node = {
        cv: [...key],
...
function parentNode(left, right, key, flags) {
    return {
        cv: [...key],
