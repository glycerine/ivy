...
const INDENT = "\t";
export function Source(source, filename = "gojr-format.go") {
...
export function Node(node) {
    switch (node.kind) {
        ...
        default:
            if (isStmt(node))
                return formatStmt(node, 0);
            if (isExpr(node))
                return formatExpr(node);
            return "";
    }
}
function formatFile(file, statements) {
    const parts = [];
    if (file.name)
        parts.push(`package ${file.name.name}`);
    for (const declaration of file.declarations)
        parts.push(formatDecl(declaration, 0));
    if (Array.isArray(statements)) {
        for (const statement of statements)
            parts.push(formatStmt(statement, 0));
    }
    return parts.filter(Boolean).join("\n\n");
}
function formatDecl(declaration, depth) {
...
function formatFuncDecl(declaration, depth) {
...
function formatGenDecl(declaration, depth) {
...
function formatImportSpec(spec) {
...
function formatBlock(block, depth) {
...
function formatStmt(statement, depth) {
...
function formatIfStmt(statement, depth) {
...
function formatForStmt(statement, depth) {
    if (!statement.init && !statement.condition && !statement.post)
        return `for ${formatBlock(statement.body, depth)}`;
    if (!statement.init && !statement.post)
        return `for ${formatExpr(statement.condition)} ${formatBlock(statement.body, depth)}`;
    const init = statement.init ? formatSimpleStmt(statement.init) : "";
...
function formatRangeStmt(statement, depth) {
    const lhs = [statement.key, statement.value].filter((expr) => Boolean(expr)).map((expr) => formatExpr(expr)).join(", ");
...
function formatSwitchStmt(statement, depth) {
    const init = statement.init ? `${formatSimpleStmt(statement.init)}; ` : "";
...
function formatCompositeLit(expr) {
    const type = expr.type ? formatExpr(expr.type) : "";
    return `${type}{${expr.elements.map((element) => formatExpr(element)).join(", ")}}`;
}
function formatCallExpr(expr) {
    return `${formatExpr(expr.fun, 8)}(${expr.args.map((arg, index) => `${formatExpr(arg)}${expr.ellipsis && index === expr.args.length - 1 ? "..." : ""}`).join(", ")})`;
}
function formatBinaryExpr(expr, parentPrecedence) {
...
function formatFieldListContent(list) {
    return list.fields.map(formatField).join(", ");
}
function formatInterfaceFields(list) {
...
function branchKeyword(statement) {
...
function assignmentOperator(token) {
    switch (token) {
