...
import { Func, Var } from "./object.js";
...
import { WriteSignature } from "./typestring.js";
...
// T must not be nil.
export function LookupSelection(T, addressable, pkg, name) {
...
export function LookupFieldOrMethod(T, addressable, pkg, name) {
    if (T === null) {
...
export function lookupFieldOrMethod(T, addressable, pkg, name, foldCase) {
...
export function lookupFieldOrMethodImpl(T, addressable, pkg, name, foldCase) {
...
// consolidated list.
export function consolidateMultiples(list) {
    if (list.length <= 1) {
        return list; // at most one entry - nothing to do
    }
    let n = 0; // number of entries w/ unique type
    const prev = new Map(); // index at which type was previously seen
    for (const e of list) {
...
export function lookupType(m, typ) {
...
export function MissingMethod(V, T, static_) {
    return missingMethod(null, V, T, static_, Identical, null);
}
registerCheckerMethod("missingMethod", function missingMethodMethod(V, T, static_, equivalent, cause) {
    return missingMethod(this, V, T, static_, equivalent, cause);
});
export function missingMethod(check, V, T, static_, equivalent, cause) {
...
registerCheckerMethod("hasAllMethods", function hasAllMethodsMethod(V, T, static_, equivalent, cause) {
    return hasAllMethods(this, V, T, static_, equivalent, cause);
});
export function hasAllMethods(check, V, T, static_, equivalent, cause) {
    if (!isValid(V)) {
...
export function hasInvalidEmbeddedFields(T, seen) {
...
export function isInterfacePtr(T) {
    const p = T.Underlying();
    return p instanceof Pointer && IsInterface(p.base);
}
registerCheckerMethod("interfacePtrError", function interfacePtrErrorMethod(T) {
    return interfacePtrError(this, T);
});
// check may be nil.
export function interfacePtrError(check, T) {
...
registerCheckerMethod("funcString", function funcStringMethod(f, pkgInfo) {
    return funcString(this, f, pkgInfo);
});
// funcString returns a string of the form name + signature for f.
// check may be nil.
export function funcString(check, f, pkgInfo) {
    const buf = [f.name];
    let qf = null;
    if (check !== null && !pkgInfo) {
        qf = (pkg) => check.qualifier(pkg);
    }
    WriteSignature(buf, f.typ, qf);
    return buf.join("");
}
registerCheckerMethod("assertableTo", function assertableToMethod(V, T, cause) {
    return assertableTo(this, V, T, cause);
});
// assertableTo reports whether a value of type V can be asserted to have type T.
export function assertableTo(check, V, T, cause) {
...
registerCheckerMethod("newAssertableTo", function newAssertableToMethod(V, T, cause) {
    return newAssertableTo(this, V, T, cause);
});
// newAssertableTo reports whether a value of type V can be asserted to have type T.
export function newAssertableTo(check, V, T, cause) {
    // no static check is required if T is an interface
    // spec: "If T is an interface type, x.(T) asserts that the
    //        dynamic type of x implements the interface T."
    if (IsInterface(T)) {
...
function implementsForLookup(V, T, constraint, cause) {
    return Checker.prototype.implements.call(null, V, T, constraint, cause);
}
// deref dereferences typ if it is a *Pointer (but not a *Named type
// with an underlying pointer type!) and returns its base and true.
// Otherwise it returns (typ, false).
export function deref(typ) {
    const p = Unalias(typ);
    if (p instanceof Pointer) {
...
export function derefStructPtr(typ) {
...
// See Object.sameId for the meaning of foldCase.
export function methodIndex(methods, pkg, name, foldCase) {
    if (name !== "_") {
...
export function fieldPath(typ0, index) {
    const names = [];
    let typ = typ0;
