...
import { AllErrors, ParseComments, ParseFile, SkipObjectResolution, Trace } from "../../front/parser.js";
...
export let parserMode = 0;
export function initParserMode() {
...
export function parseFiles(dir, filenames) {
    let files = new Array(filenames.length);
    const errors = new Array(filenames.length);
...
export function getPkgFiles(args) {
    if (args.length === 0) {
        // stdin
...
export function checkPkgFiles(files) {
    class bailout {
    }
...
export function printStats(d) {
    ...
    console.log(`${durationString(d)} (${fileCount} files, ${lineCount} lines, ${Math.trunc(lineCount / (d / 1000))} lines/s)`);
}
