...
function emitFunction(ctx, fn) {
    ...
    if (statements.length === 1 && statements[0]?.kind === "ReturnStatement") {
        ...
        if (returned)
            return [`  pkg[${JSON.stringify(fn.name)}] = async () => ${returned};`];
    }
    ctx.emitError(`unsupported Stage 1 function body for ${fn.name}`);
    return [];
}
function returnLiteralExpression(ctx, statement) {
...
function expressionToJs(ctx, expression) {
    if (!expression)
        return undefined;
...
function arrayLiteralToJs(ctx, expression) {
...
function mapLiteralToJs(ctx, expression) {
