...
const emptyPackageEmitFacts = {
    ...
    functionTypeParameterConstraints: new Map(),
...
function intrinsicPackageOverrideLines(importPath) {
    ...
    if (importPath === "time") {
        lines.push("  pkg[\"FixedZone\"] = async (name, offset) => __gojrTimeLocation(String(name ?? \"\"), offset);", "  pkg[\"LoadLocation\"] = async (name) => __gojrTuple([String(name ?? \"\") === \"UTC\" && pkg[\"UTC\"] ? pkg[\"UTC\"] : __gojrTimeLocation(String(name ?? \"\")), null]);", "  pkg[\"LoadLocationFromTZData\"] = async (name) => __gojrTuple([__gojrTimeLocation(String(name ?? \"\")), null]);");
    }
    return lines;
}
...
function emitGotoStateMachineStatements(ctx, statements, env, indent, defaultLines) {
...
function hoistedGotoLocals(statements) {
    ...
    for (const statement of statements) {
        ...
        if (statement.kind === "LabeledStatement" && (statement.statement?.kind === "ConstDecl" || statement.statement?.kind === "VarDecl")) {
            for (const declaration of statement.statement.declarations) {
                if (declaration.name !== "_")
                    out.set(declaration.name, declarationTypeText(declaration));
            }
        }
...
function cloneExpressionEnv(env) {
    return {
        ...
        ...(env.currentTypeParameterConstraints ? { currentTypeParameterConstraints: { ...env.currentTypeParameterConstraints } } : {}),
...
function emitLocalTypeDeclarationStatement(statement, env) {
    for (const declaration of statement.declarations) {
...
function emitLocalDeclarationStatement(ctx, statement, env, indent) {
    ...
    for (const group of declarationGroups(statement.declarations)) {
        ...
        for (const declaration of group) {
            if (declaration.name === "_")
                continue;
            const existing = env.declaredLocals.has(declaration.name) ? env.locals.get(declaration.name) : undefined;
            const name = existing ?? freshLocalName(declaration.name, localIndex++, env);
...
function emitRangeStatement(ctx, statement, env, indent) {
    ...
    const bindRangePart = (name, target, part, index, targetType) => {
...
function incDecOneLiteral(target, env) {
...
function emitSelectStatement(ctx, statement, env, indent) {
    const cases = [];
    const selectedResult = ctx.symbol("select");
    const selectedValue = ctx.symbol("selected");
    const selectedBodyLines = [];
    for (const [caseIndex, clause] of statement.clauses.entries()) {
        const clauseEnv = cloneExpressionEnv(env);
        const bodyLines = [];
        const prepared = emitSelectComm(ctx, clause.comm, clause.default, clauseEnv, `${indent}      `, selectedValue);
        if (!prepared)
            return undefined;
        ...
        selectedBodyLines.push(`${indent}  case ${caseIndex}: {`, `${indent}    const ${selectedValue} = ${selectedResult}.result;`, ...bodyLines, `${indent}    break;`, `${indent}  }`);
    }
    return [
        `${indent}const ${selectedResult} = await __gojrSelect([`,
        cases.join(",\n"),
        `${indent}]);`,
        `${indent}switch (${selectedResult}.index) {`,
        ...selectedBodyLines,
        `${indent}}`
    ];
}
...
function compoundAssignmentExpression(ctx, operator, left, right, resultType) {
    if (operator === "=")
        return right;
...
function functionParameterBindings(fn, facts) {
    ...
    return {
        ...
        env: {
            ...
            ...(fn.typeParameterConstraints ? { currentTypeParameterConstraints: fn.typeParameterConstraints } : {})
        }
    };
}
function functionDeclarationParameterCoercionLines(fn, env, indent) {
    const lines = [];
    if (fn.receiver) {
        const receiverName = fn.receiver.name && fn.receiver.name !== "_" ? fn.receiver.name : "__gojrRecv";
        const receiverJsName = safeLocalName(receiverName, -1);
...
function packageEmitFacts(ast, artifact) {
    ...
    const functionTypeParameterConstraints = new Map();
    ...
    for (const fn of ast.functions) {
        ...
        if (fn.typeParameterConstraints)
            functionTypeParameterConstraints.set(key, fn.typeParameterConstraints);
        if (!fn.receiver) {
            ...
            if (fn.typeParameterConstraints)
                functionTypeParameterConstraints.set(fn.name, fn.typeParameterConstraints);
            continue;
        }
    ...
    return {
        ...
        functionTypeParameterConstraints,
        methodPointerReceivers
    };
}
function collectPackageConstValues(ast) {
        ...
        for (const group of declarationGroups(statement.declarations)) {
            ...
            for (const declaration of group) {
                if (declaration.name === "_")
                    continue;
                const effectiveValue = declaration.value ?? inheritedValues[declaration.valueIndex ?? 0];
                const value = constantIntegerExpressionValue(effectiveValue, values, BigInt(declaration.iotaIndex ?? 0));
                if (value !== undefined)
...
function constantIntegerExpressionValue(expression, constants, iotaValue) {
    if (!expression)
        return undefined;
    switch (expression.kind) {
        case "Literal":
            if (expression.literalKind !== "int" && expression.literalKind !== "rune")
                return undefined;
            if (typeof expression.value === "bigint")
                return expression.value;
            if (typeof expression.value === "number" && Number.isSafeInteger(expression.value))
...
function applyIntegerConstantOperator(operator, left, right) {
    switch (operator) {
        case "+":
            return left + right;
        case "-":
...
function parseIntegerLiteralText(text) {
    const raw = text.replace(/_/g, "");
    try {
        if (/^0[xX][0-9a-fA-F]+$/.test(raw))
            return BigInt(raw);
        if (/^0[bB][01]+$/.test(raw))
            return BigInt(`0b${raw.slice(2)}`);
        if (/^0[oO][0-7]+$/.test(raw))
            return BigInt(`0o${raw.slice(2)}`);
...
function typeTextKind(typeText) {
    ...
    if (trimmed.startsWith("func("))
        return "func";
    if (trimmed === "any" || trimmed === "error" || trimmed === "interface{}")
        return "interface";
...
function compositeTypeDescriptorFields(typeText, facts) {
    const trimmed = typeText.trim();
...
function parseArrayLengthTypeText(typeText, facts = emptyPackageEmitFacts) {
    ...
    if (constant === undefined || constant < 0n || constant > BigInt(Number.MAX_SAFE_INTEGER))
        return undefined;
    const length = Number(constant);
    return Number.isSafeInteger(length) ? length : undefined;
}
function normalizeFixedArrayLengthTypeText(typeText, facts) {
    const trimmed = typeText.trim();
...
function evaluateArrayLengthConstantText(text, facts) {
...
function directSelectorTypePackagePath(typeText, facts) {
    const text = typeTextPackagePathCore(typeText);
    const fullPath = fullImportPathTypePrefix(text, facts);
    if (fullPath)
        return fullPath;
...
function valueForTargetType(value, targetType, sourceType, env) {
    ...
    if (staticTargetType && resolvedTargetType && isNilAssignableConcreteTypeText(resolvedTargetType)) {
        return `__gojrConvertNilable(${JSON.stringify(staticTargetType)}, ${value})`;
    }
    if (staticTargetType &&
        resolvedTargetType &&
        staticTargetType !== resolvedTargetType &&
        isScalarTypeText(resolvedTargetType) &&
        isKnownLocalNamedTypeText(staticTargetType, env)) {
        return `__gojrConvertDynamicType(${JSON.stringify(staticTargetType)}, ${value})`;
    }
...
function functionLiteralParameterCoercionLines(expression, env, indent) {
    const lines = [];
    for (const [index, parameter] of expression.signature.parameters.entries()) {
        const goName = parameter.name && parameter.name !== "_" ? parameter.name : `__gojrArg${index}`;
...
function binaryOperationToJs(operator, left, right, resultType) {
...
function selectorFieldTypeText(expression, env) {
    const objectType = expressionTypeText(expression.object, env);
    return structFieldTypeText(objectType, expression.field, env.facts);
}
function structFieldTypeText(typeText, fieldName, facts, seen = new Set()) {
    if (!typeText)
        return undefined;
    ...
    const promoted = fields
        ...
        .filter((fieldType) => fieldType !== undefined);
...
function packagePathForTypeText(typeText, env) {
    const base = typeTextPackagePathCore(typeText);
    if (!base)
        return undefined;
    const fullPath = fullImportPathTypePrefix(base, env.facts);
    if (fullPath)
        return fullPath;
...
function typeTextPackagePathCore(typeText) {
    ...
    while (/^\[[^\]]+\]/.test(text) && !text.startsWith("[]"))
        text = text.replace(/^\[[^\]]+\]/, "").trim();
    return genericBaseTypeText(text);
}
function fullImportPathTypePrefix(typeText, facts) {
    const dot = typeText.lastIndexOf(".");
    if (dot <= 0)
        return undefined;
    const prefix = typeText.slice(0, dot);
    for (const importPath of facts.imports.values()) {
        if (prefix === importPath)
            return importPath;
    }
    return prefix.includes("/") ? prefix : undefined;
}
...
function callExpressionToJs(ctx, expression, env) {
    ...
    if (expression.callee.kind === "SelectorExpression") {
        ...
        if (functionFieldCall)
            return functionFieldCall;
        const methodKey = methodPackageKeyForSelector(expression.callee, env);
        if (methodKey)
...
function functionFieldCallToJs(ctx, expression, env) {
    if (expression.callee.kind !== "SelectorExpression")
        return undefined;
    if (isImportedPackageSelector(expression.callee, env))
        return undefined;
...
function renderTypeArgumentValues(typeArgText, env) {
    return splitTopLevelTypes(typeArgText).map((typeArg) => renderTypeArgumentValue(typeArg, env));
}
function renderTypeArgumentValue(typeArg, env) {
    const trimmed = (typeArg ?? "any").trim() || "any";
    return runtimeTypeTextExpression(trimmed, env);
}
function conversionCalleeTypeText(callee, env, callTypeText) {
...
function selectorConversionMatchesCallType(candidate, importPath, callTypeText) {
    ...
    switch (expression.kind) {
        ...
        case "SelectorExpression":
            if (expression.object.kind !== "Identifier" || !env.facts.imports.has(expression.object.name))
                return undefined;
            if (expression.typeText?.trim().startsWith("func("))
                return undefined;
...
function isKnownConversionTypeText(typeText, env) {
    ...
    if (env.localTypeUnderlyings.has(base))
        return true;
...
function callArgumentTargetType(targetTypes, index, isSpread) {
    ...
    if (variadicIndex >= 0 && index >= variadicIndex)
        return targetTypes[variadicIndex]?.slice(3);
    return undefined;
}
...
function instantiatedFunctionTypeArgObject(expression, functionName, env) {
    ...
    const fields = parameterNames.map((name, index) => `${JSON.stringify(name)}: ${renderTypeArgumentValue(typeArgs[index] ?? "any", env)}`);
    return `({ ${fields.join(", ")} })`;
}
function inferredFunctionTypeArgObject(functionName, args, env) {
    ...
    for (const [index, parameterType] of parameterTypes.entries()) {
        const actualType = actualTypeForGenericInference(expressionTypeText(args[index], env), env);
        if (!actualType)
            continue;
        inferGenericTypeArguments(parameterType.startsWith("...") ? parameterType.slice(3) : parameterType, actualType, parameterSet, inferred);
    }
    inferGenericTypeArgumentsFromConstraints(parameterNames, env.facts.functionTypeParameterConstraints.get(functionName), parameterSet, inferred);
    const fields = parameterNames.map((name) => `${JSON.stringify(name)}: ${renderTypeArgumentValue(inferred.get(name) ?? "any", env)}`);
    return `({ ${fields.join(", ")} })`;
}
function actualTypeForGenericInference(actualType, env) {
    const trimmed = actualType?.trim();
    if (!trimmed || !env.typeParameters || !env.currentTypeParameterConstraints)
        return trimmed;
    if (env.typeParameters.has(trimmed)) {
        return constraintInferencePattern(env.currentTypeParameterConstraints[trimmed], env.typeParameters) ?? trimmed;
    }
    return trimmed;
}
function inferGenericTypeArgumentsFromConstraints(parameterNames, constraints, parameterSet, inferred) {
    if (!constraints)
        return;
    for (let pass = 0; pass < parameterNames.length; pass += 1) {
        let changed = false;
        for (const name of parameterNames) {
            const actual = inferred.get(name);
            if (!actual)
                continue;
            const pattern = constraintInferencePattern(constraints[name], parameterSet);
            if (!pattern)
                continue;
            const before = new Map(inferred);
            inferGenericTypeArguments(pattern, actual, parameterSet, inferred);
            if (!sameStringMap(before, inferred))
                changed = true;
        }
        if (!changed)
            return;
...
function constraintInferencePattern(constraint, parameterSet) {
    if (!constraint)
        return undefined;
    for (const option of splitTopLevelUnion(constraint)) {
        const trimmed = option.trim().replace(/^~/, "").trim();
        if (!trimmed)
            continue;
        const identifiers = trimmed.match(/[_$\p{L}][_$\p{L}\p{N}]*/gu) ?? [];
        if (identifiers.some((identifier) => parameterSet.has(identifier)))
            return trimmed;
    }
    return undefined;
}
...
function unsafePackageCallToJs(ctx, expression, env) {
    if (expression.callee.kind !== "SelectorExpression")
        return undefined;
    if (expression.callee.object.kind !== "Identifier")
        return undefined;
    if (env.facts.imports.get(expression.callee.object.name) !== "unsafe")
        return undefined;
...
function unsafeSizeofExpressionToJs(ctx, expression, env) {
    const typeText = expressionTypeText(expression, env)?.trim();
    if (typeText) {
        if (env.typeParameters?.has(typeText))
            return `__gojrSizeofType(__gojrTypeArg(__gojrTypeArgs, ${JSON.stringify(typeText)}))`;
...
function unsafeAlignofExpressionToJs(ctx, expression, env) {
    const typeText = expressionTypeText(expression, env)?.trim();
    if (typeText) {
        if (env.typeParameters?.has(typeText))
            return `__gojrAlignofType(__gojrTypeArg(__gojrTypeArgs, ${JSON.stringify(typeText)}))`;
...
function unsafeOffsetofExpressionToJs(ctx, expression, env) {
    if (!expression)
        return undefined;
...
function conversionCallToJs(ctx, typeText, args, env) {
    ...
    if (typeText.trim() !== resolvedType && isScalarTypeText(resolvedType) && isKnownLocalNamedTypeText(typeText, env)) {
        return `__gojrConvertDynamicType(${JSON.stringify(typeText)}, ${value})`;
    }
    ...
    if (typeText.trim() === "unsafe.Pointer" || resolvedType.trim() === "unsafe.Pointer")
        return `__gojrConvertPointer(${JSON.stringify(typeText)}, ${value}${pointerPkgPath ? `, ${JSON.stringify(pointerPkgPath)}` : ""})`;
    if (resolvedType === "uintptr" && isPointerLikeTypeText(sourceType))
        return `__gojrPointerToUintptr(${value})`;
    if (isIntegerType(resolvedType))
        return `__gojrIntegerConvert(${JSON.stringify(resolvedType)}, ${value})`;
    ...
    if (importedTypePkgPath)
        return `__gojrConvertDynamicType(${JSON.stringify(typeText)}, ${value}, __gojrActiveImportsByPath, ${JSON.stringify(importedTypePkgPath)})`;
    if (env.facts.typeUnderlyings.has(typeText))
        return value;
    ctx.emitError(`unsupported Stage 3 conversion to ${typeText}`);
    return undefined;
}
function pointerConversionPackagePath(typeText, env, currentImportPath) {
    const trimmed = typeText.trim();
    if (!trimmed.startsWith("*") && trimmed !== "unsafe.Pointer")
        return undefined;
...
function zeroValueForType(typeText, facts = emptyPackageEmitFacts) {
    ...
    if (arrayType && !valueType.startsWith("[]")) {
        ...
        if (length !== undefined) {
            if (arrayType.elementType === "byte" || arrayType.elementType === "uint8")
                return `__gojrSetCap(new Uint8Array(${length}), ${length})`;
...
function atomicIntrinsicTypeText(typeText, facts) {
    const trimmed = typeText.trim();
...
function weakIntrinsicTypeText(typeText, facts) {
    const trimmed = typeText.trim();
    if (/^weak\.Pointer(?:\[[\s\S]*\])?$/.test(trimmed))
        return trimmed;
    const dot = trimmed.indexOf(".");
    if (dot <= 0)
        return undefined;
...
function fsFileModeTypeText(typeText, facts) {
    const trimmed = typeText.trim();
    if (trimmed === "io/fs.FileMode")
        return trimmed;
    const dot = trimmed.indexOf(".");
    if (dot <= 0)
        return undefined;
...
function syncIntrinsicTypeText(typeText, facts) {
    const trimmed = typeText.trim();
    if (/^sync\.(?:Mutex|RWMutex|Once|Pool|WaitGroup|Cond|Map)$/.test(trimmed))
        return trimmed;
    if (/^internal\/sync\.HashTrieMap(?:\[[\s\S]*\])?$/.test(trimmed))
        return trimmed;
    const dot = trimmed.indexOf(".");
    if (dot <= 0)
        return undefined;
...
function isNilAssignableConcreteTypeText(typeText) {
    return typeText === "unsafe.Pointer" ||
        typeText.startsWith("*") ||
        typeText.startsWith("[]") ||
...
function isScalarTypeText(typeText) {
    return isIntegerType(typeText) ||
        isFloatType(typeText) ||
        isComplexType(typeText) ||
        typeText === "bool" ||
        typeText === "string";
}
function isKnownLocalNamedTypeText(typeText, env) {
    const base = genericBaseTypeText(typeText.trim());
    return env.localTypeUnderlyings.has(base) || env.facts.typeUnderlyings.has(base);
}
...
function expressionPrefersNumberArithmetic(expression, env) {
    if (!expression)
        return false;
    const typeText = expressionTypeText(expression, env);
    if (typeText && isFloatType(resolveUnderlyingTypeTextInEnv(typeText, env)))
        return true;
    switch (expression.kind) {
        case "Literal":
        ...
        case "CallExpression":
            if (expression.callee.kind === "TypeExpression")
                return isFloatType(resolveUnderlyingTypeTextInEnv(expression.callee.type.text, env));
...
function freshLocalName(name, index, env) {
...
function stage1JavaScript(artifact, usesWasm, bodyLines, typeDescriptorLines, facts) {
    ...
    const source = [
        ...
        "  const __gojrDefaultTextSinkFromRuntime = runtime.defaultTextSink || runtime.__gojrDefaultTextSink || options.defaultTextSink || options.__gojrDefaultTextSink;",
        "  const __gojrStdout = options.stdout || runtime.stdout || (typeof __gojrDefaultTextSinkFromRuntime === \"function\" ? __gojrDefaultTextSinkFromRuntime(\"stdout\") : undefined) || (() => {});",
        "  const __gojrStderr = options.stderr || runtime.stderr || (typeof __gojrDefaultTextSinkFromRuntime === \"function\" ? __gojrDefaultTextSinkFromRuntime(\"stderr\") : undefined) || (() => {});",
        "  __gojrActiveRuntimeOptions = { ...runtime, ...options, stdout: __gojrStdout, stderr: __gojrStderr };",
        "  const __gojrStage1Helpers = __gojrLoadStage1Helpers(runtime, options, {",
        "    gojrPackageArtifact,",
        ...
        "  });",
        STAGE1_SHARED_SUPPORT_BINDINGS_PLACEHOLDER,
        ...
        "}",
        "function __gojrStringFromRune(value) {",
        "  value = __gojrUnwrapNamed(__gojrUnwrapInterface(value));",
        "  let codepoint;",
        "  if (typeof value === \"bigint\") codepoint = value;",
        "  else if (typeof value === \"number\") codepoint = BigInt(Math.trunc(value));",
        "  else codepoint = BigInt(value);",
        "  if (codepoint < 0n || codepoint > 0x10ffffn) return \"\\ufffd\";",
        ...
        "  value = __gojrUnwrapNamed(__gojrUnwrapInterface(value));",
        "  if (typeof value === \"bigint\" || typeof value === \"number\") return __gojrStringFromRune(value);",
        ...
        "  return value;",
        "}",
        "function __gojrUnwrapNamed(value) {",
        "  while (value && value.__gojrNamed === true) value = value.value;",
        "  return value;",
        "}",
        "function __gojrEqual(left, right) {",
        "  const leftWasInterface = Boolean(left && left.__gojrInterface === true);",
        "  const rightWasInterface = Boolean(right && right.__gojrInterface === true);",
        ...
        "  if (left && left.__gojrNamed === true || right && right.__gojrNamed === true) {",
        "    if ((leftWasInterface || rightWasInterface) && left && right && left.__gojrNamed === true && right.__gojrNamed === true && (left.__gojrType !== right.__gojrType || (left.__gojrPkgPath || \"\") !== (right.__gojrPkgPath || \"\"))) return false;",
        "    left = __gojrUnwrapNamed(left);",
        "    right = __gojrUnwrapNamed(right);",
        "  }",
        ...
        "  if (actual && actual.__gojrNamed === true) return `n:${actual.__gojrPkgPath || \"\"}:${actual.__gojrType}:${__gojrMapKeyId(actual.value, options)}`;",
        ...
        "  return value;",
        "}",
        "function __gojrMakeArraySlice(elemType, length = 0, capacity = length) {",
        "  const len = Math.max(0, Number(length));",
        "  const cap = Math.max(len, Number(capacity));",
        "  const backing = Array.from({ length: cap }, () => __gojrZero(elemType || \"any\"));",
        "  return __gojrArraySliceView(backing, 0, len, cap, elemType || \"any\");",
        "}",
        "function __gojrArraySliceView(backing, offset, length, capacity, elemType = \"any\") {",
        "  const base = Array.isArray(backing) ? backing : [];",
        "  const start = Math.max(0, Number(offset));",
        "  const len = Math.max(0, Number(length));",
        "  const cap = Math.max(len, Number(capacity));",
        "  while (base.length < start + cap) base.push(__gojrZero(elemType || \"any\"));",
        "  const view = new Array(len);",
        "  Object.defineProperty(view, \"__gojrSliceBacking\", { value: base, configurable: true });",
        "  Object.defineProperty(view, \"__gojrSliceOffset\", { value: start, configurable: true });",
        "  Object.defineProperty(view, \"__gojrSliceElemType\", { value: elemType || \"any\", configurable: true });",
        "  Object.defineProperty(view, \"__gojrCap\", { value: cap, configurable: true });",
        "  for (let index = 0; index < len; index += 1) {",
        "    Object.defineProperty(view, index, {",
        "      get() { return base[start + index]; },",
        "      set(next) { base[start + index] = next; },",
        "      enumerable: true,",
        "      configurable: true",
        "    });",
        "  }",
        ...
        "  return String(typeName || \"\").trim().startsWith(\"[]\") ? __gojrArraySliceView(out, 0, length, length, elemType) : __gojrSetCap(out, length);",
        "}",
        ...
        "  let raw;",
        "  try { raw = __gojrIntegerFrom(value); }",
        "  catch (error) {",
        "    const detail = error instanceof Error ? error.message : String(error);",
        "    throw new TypeError(`${detail} for target ${String(typeName || \"\").trim() || \"integer\"}`);",
        "  }",
        ...
        "  }",
        "}",
        "function __gojrScalarTypeName(typeName) {",
        "  const text = String(typeName || \"\").trim();",
        "  return __gojrIntegerTypeName(text) || text === \"float32\" || text === \"float64\" || text === \"complex64\" || text === \"complex128\" || text === \"bool\" || text === \"string\";",
        "}",
        "function __gojrConvertScalarRuntimeType(typeName, value) {",
        "  const text = String(typeName || \"\").trim();",
        "  if (__gojrIntegerTypeName(text)) return __gojrIntegerConvert(text, value);",
        "  if (text === \"float32\") return Math.fround(Number(__gojrUnwrapNamed(__gojrUnwrapInterface(value))));",
        "  if (text === \"float64\") return Number(__gojrUnwrapNamed(__gojrUnwrapInterface(value)));",
        "  if (text === \"complex64\" || text === \"complex128\") return __gojrToComplex(value);",
        "  if (text === \"bool\") return Boolean(__gojrUnwrapNamed(__gojrUnwrapInterface(value)));",
        "  if (text === \"string\") return __gojrStringFrom(__gojrUnwrapNamed(__gojrUnwrapInterface(value)));",
        "  return value;",
        "}",
        "function __gojrNamedValue(typeName, value, pkgPath = undefined) {",
        "  const box = { __gojrNamed: true, __gojrType: typeName, __gojrPkgPath: pkgPath || gojrPackageArtifact.importPath, value };",
        "  Object.defineProperty(box, \"valueOf\", { value() { return this.value; } });",
        "  Object.defineProperty(box, \"toString\", { value() { return String(this.value); } });",
        "  return box;",
        "}",
        "function __gojrDescriptorNeedsNamedScalarIdentity(descriptor) {",
        "  return Boolean(descriptor && Array.isArray(descriptor.methods) && descriptor.methods.length > 0);",
        "}",
        ...
        "  const resolved = type ? __gojrUnderlyingRuntimeTypeName(type) : type;",
        "  const descriptor = type ? __gojrDescriptorForTypeName(type) : undefined;",
        "  if (type && type !== resolved && __gojrScalarTypeName(resolved) && __gojrDescriptorNeedsNamedScalarIdentity(descriptor)) return __gojrConvertDynamicType(type, value);",
        ...
        "  left = __gojrUnwrapNamed(__gojrUnwrapInterface(left)); right = __gojrUnwrapNamed(__gojrUnwrapInterface(right));",
        ...
        "  const descriptor = __gojrDescriptorForTypeName(text, importsByPath, pkgPath);",
        "  const descriptorPkgPath = descriptor && descriptor.pkgPath ? descriptor.pkgPath : pkgPath;",
        "  if (value && value.__gojrNamed === true && value.__gojrType === (descriptor && descriptor.type || text) && (value.__gojrPkgPath || \"\") === (descriptorPkgPath || gojrPackageArtifact.importPath || \"\")) return value;",
        "  const resolved = __gojrUnderlyingRuntimeTypeName(text, importsByPath, pkgPath);",
        "  if (descriptor && descriptor.underlying && descriptor.underlying !== text && __gojrScalarTypeName(resolved) && __gojrDescriptorNeedsNamedScalarIdentity(descriptor)) return __gojrNamedValue(descriptor.type || text, __gojrConvertScalarRuntimeType(resolved, value), descriptorPkgPath);",
        "  if (__gojrIsByteSliceTypeName(resolved)) return __gojrBytesFrom(value);",
        "  if (__gojrScalarTypeName(resolved)) return __gojrConvertScalarRuntimeType(resolved, value);",
        ...
        "    sliced = target.slice(low, high);",
        "  } else if (ArrayBuffer.isView(target) && target instanceof Uint8Array) {",
        "    const byteLength = Math.max(0, high - low);",
        "    const byteOffset = target.byteOffset + low;",
        "    if (high > capHigh || byteOffset + byteLength > target.buffer.byteLength) throw new RangeError(\"slice bounds out of range\");",
        "    sliced = new Uint8Array(target.buffer, byteOffset, byteLength);",
        "  } else if (ArrayBuffer.isView(target) && typeof target.subarray === \"function\") {",
        "    sliced = target.subarray(low, high);",
        "  } else if (Array.isArray(target)) {",
        "    const backing = target.__gojrSliceBacking || target;",
        "    const baseOffset = Number(target.__gojrSliceOffset || 0);",
        "    const elemType = target.__gojrSliceElemType || \"any\";",
        "    if (high > capHigh || capHigh > __gojrCap(target)) throw new RangeError(\"slice bounds out of range\");",
        "    sliced = __gojrArraySliceView(backing, baseOffset + low, high - low, capHigh - low, elemType);",
        "  } else {",
        "    sliced = typeof target.slice === \"function\" ? target.slice(low, high) : [];",
        "  }",
        ...
        "  return value;",
        "}",
        "function __gojrMakeByteSlice(length = 0, capacity = length) {",
        "  const len = Math.max(0, Number(length));",
        "  const cap = Math.max(len, Number(capacity));",
        "  const backing = new Uint8Array(cap);",
        "  return __gojrSetCap(new Uint8Array(backing.buffer, 0, len), cap);",
        "}",
        ...
        "  if (elem !== undefined) return elem === \"byte\" || elem === \"uint8\" ? __gojrMakeByteSlice(length, capacity) : __gojrMakeArraySlice(elem, length, capacity);",
        "  const descriptor = __gojrDescriptorForTypeName(text);",
        "  if (descriptor && descriptor.kind === \"slice\") return descriptor.elem === \"byte\" || descriptor.elem === \"uint8\" ? __gojrMakeByteSlice(length, capacity) : __gojrMakeArraySlice(descriptor.elem || \"any\", length, capacity);",
        ...
        "  if (slice && slice.__gojrTypedNil === true) slice = [];",
        "  if (slice instanceof Uint8Array) {",
        "    const oldLen = slice.length;",
        "    const newLen = oldLen + values.length;",
        "    let out;",
        "    let outCap;",
        "    if (newLen <= __gojrCap(slice) && slice.byteOffset + newLen <= slice.buffer.byteLength) {",
        "      out = new Uint8Array(slice.buffer, slice.byteOffset, newLen);",
        "      outCap = __gojrCap(slice);",
        "    } else {",
        "      const newCap = Math.max(newLen, oldLen * 2, 1);",
        "      const backing = new Uint8Array(newCap);",
        "      backing.set(slice, 0);",
        "      out = new Uint8Array(backing.buffer, 0, newLen);",
        "      outCap = newCap;",
        "    }",
        "    for (let index = 0; index < values.length; index += 1) out[oldLen + index] = Number(values[index]) & 255;",
        "    return __gojrSetCap(out, outCap);",
        "  }",
        "  if (ArrayBuffer.isView(slice)) {",
        "    return Array.from(slice).concat(values);",
        "  }",
        "  if (Array.isArray(slice)) {",
        "    const oldLen = __gojrLen(slice);",
        "    const newLen = oldLen + values.length;",
        "    const oldCap = __gojrCap(slice);",
        "    const elemType = slice.__gojrSliceElemType || \"any\";",
        "    const baseOffset = Number(slice.__gojrSliceOffset || 0);",
        "    let backing = slice.__gojrSliceBacking;",
        "    let outCap = oldCap;",
        "    if (!backing || newLen > oldCap) {",
        "      outCap = Math.max(newLen, oldLen * 2, 1);",
        "      backing = Array.from({ length: outCap }, (_item, index) => index < oldLen ? slice[index] : __gojrZero(elemType));",
        "      for (let index = 0; index < values.length; index += 1) backing[oldLen + index] = values[index];",
        "      return __gojrArraySliceView(backing, 0, newLen, outCap, elemType);",
        "    }",
        "    for (let index = 0; index < values.length; index += 1) backing[baseOffset + oldLen + index] = values[index];",
        "    return __gojrArraySliceView(backing, baseOffset, newLen, outCap, elemType);",
        "  }",
        "  return values.length === 0 ? [] : __gojrArraySliceView(values.slice(), 0, values.length, values.length, \"any\");",
        "}",
        ...
        "  constructor(value) {",
        "    super(`panic: ${__gojrPanicString(value)}`);",
        ...
        "}",
        "function __gojrPanicString(value) {",
        "  if (value instanceof Error) return value.message;",
        "  return __gojrDisplayValueString(value);",
        "}",
        "function __gojrDisplayValueString(value, seen = new Set()) {",
        "  if (value && value.__gojrInterface === true) return __gojrDisplayValueString(value.value, seen);",
        "  if (value && value.__gojrPointer === true) return `&${__gojrDisplayValueString(__gojrDerefIfPointer(value), seen)}`;",
        "  if (value && value.__gojrTypedNil === true) return `nil:${value.__gojrType || \"\"}`;",
        "  if (value === null || value === undefined) return \"nil\";",
        "  if (__gojrIsString(value)) return typeof value === \"string\" ? value : new TextDecoder().decode(__gojrStringBytes(value));",
        "  if (typeof value === \"bigint\") return `${value}n`;",
        "  if (typeof value === \"number\" || typeof value === \"boolean\") return JSON.stringify(value);",
        "  if (value instanceof Uint8Array) return `bytes:${Array.from(value).join(\",\")}`;",
        "  if (Array.isArray(value)) return `[${value.map((item) => __gojrDisplayValueString(item, seen)).join(\",\")}]`;",
        "  if (value instanceof Map) return `map:${Array.from(value.entries()).map(([key, item]) => `${__gojrDisplayValueString(key, seen)}:${__gojrDisplayValueString(item, seen)}`).join(\",\")}`;",
        "  if (typeof value === \"object\") {",
        "    if (seen.has(value)) return \"<cycle>\";",
        "    seen.add(value);",
        "    const keys = Object.keys(value).filter((key) => !key.startsWith(\"__gojr\")).sort();",
        "    return `{${keys.map((key) => `${key}:${__gojrDisplayValueString(value[key], seen)}`).join(\",\")}}`;",
        "  }",
        "  return String(value);",
        "}",
        ...
        "  const value = object[numericIndex];",
        "  if (integerResult || object instanceof Uint8Array || object instanceof Uint8ClampedArray || object instanceof Int8Array || object instanceof Uint16Array || object instanceof Int16Array || object instanceof Uint32Array || object instanceof Int32Array || object instanceof BigInt64Array || object instanceof BigUint64Array) {",
        "    if (value === undefined) throw new Error(`GOJR_RUNTIME001: index ${numericIndex} out of range [0:${__gojrLen(object)}]`);",
        "    return BigInt(value);",
        "  }",
        "  return value;",
        "}",
        ...
        "  };",
        "}",
        "function __gojrReflectValueOf(value, importsByPath = {}, typeName = undefined, pkgPath = undefined) {",
        "  const actual = value && value.__gojrInterface === true ? value.value : value;",
        "  const out = actual === null || actual === undefined",
        "    ? { __gojrReflectValue: true, __gojrValid: false, __gojrValue: undefined, __gojrImportsByPath: importsByPath, __gojrMethods: __gojrReflectValueMethods }",
        "    : { __gojrReflectValue: true, __gojrValid: true, __gojrValue: actual, __gojrImportsByPath: importsByPath, __gojrMethods: __gojrReflectValueMethods };",
        "  if (typeName !== undefined) Object.defineProperty(out, \"__gojrReflectTypeName\", { value: typeName });",
        "  if (pkgPath !== undefined) Object.defineProperty(out, \"__gojrReflectPkgPath\", { value: pkgPath });",
        "  return out;",
        "}",
        "function __gojrReflectValueType(self) {",
        "  if (!self.__gojrValid) return null;",
        "  const typeName = self.__gojrReflectTypeName || __gojrRuntimeTypeName(self.__gojrValue);",
        "  const pkgPath = self.__gojrReflectPkgPath !== undefined ? self.__gojrReflectPkgPath : __gojrRuntimeTypePackagePath(self.__gojrValue);",
        "  return __gojrReflectTypeForTypeName(typeName, self.__gojrImportsByPath, pkgPath);",
        "}",
        "function __gojrReflectZeroValue(importsByPath = {}) {",
        "  return { __gojrReflectValue: true, __gojrValid: false, __gojrValue: undefined, __gojrImportsByPath: importsByPath, __gojrMethods: __gojrReflectValueMethods };",
        "}",
        "const __gojrReflectValueMethods = {",
        "  IsValid: async (self) => Boolean(self.__gojrValid),",
        "  CanInterface: async (self) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.CanInterface on zero Value\");",
        "    return true;",
        "  },",
        "  CanAddr: async (self) => Boolean(self && self.__gojrReflectSet),",
        "  CanSet: async (self) => Boolean(self && self.__gojrReflectSet),",
        "  IsNil: async (self) => {",
        ...
        "    if (!field) throw new RangeError(\"reflect: Field index out of range\");",
        "    return __gojrReflectValueOf(__gojrGetField(self.__gojrValue, field.name), self.__gojrImportsByPath, field.type, field.pkgPath);",
        "  },",
        "  NumField: async (self) => {",
        "    const typ = __gojrReflectValueType(self);",
        "    const descriptor = typ && typ.__gojrDescriptor;",
        "    if (!descriptor || descriptor.kind !== \"struct\") throw new TypeError(`reflect: call of Value.NumField on ${descriptor && descriptor.kind || \"invalid\"} Value`);",
        "    return BigInt((descriptor.fields || []).length);",
        "  },",
        "  Elem: async (self) => {",
        "    const typ = __gojrReflectValueType(self);",
        "    const descriptor = typ && typ.__gojrDescriptor;",
        ...
        "    const value = self.__gojrValue;",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.Elem on zero Value\");",
        "    if (kind === \"ptr\") {",
        "      if (value && value.__gojrTypedNil === true) return __gojrReflectZeroValue(self.__gojrImportsByPath);",
        "      if (!__gojrPointerLike(value) || !value.__gojrPointer) throw new TypeError(\"reflect: Elem of invalid pointer Value\");",
        "      const elemType = descriptor.elem || value.__gojrElemType || \"any\";",
        "      return __gojrReflectValueOf(__gojrDeref(value), self.__gojrImportsByPath, elemType, descriptor.elemPkgPath || value.__gojrPkgPath);",
        "    }",
        "    if (kind === \"interface\") {",
        "      const dynamic = value && value.__gojrInterface === true ? value.value : value;",
        "      return dynamic === null || dynamic === undefined ? __gojrReflectZeroValue(self.__gojrImportsByPath) : __gojrReflectValueOf(dynamic, self.__gojrImportsByPath);",
        "    }",
        "    throw new TypeError(`reflect: call of Value.Elem on ${kind || \"invalid\"} Value`);",
        "  },",
        ...
        "  },",
        "  Uint: async (self) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.Uint on zero Value\");",
        "    return BigInt.asUintN(64, BigInt(self.__gojrValue));",
        "  },",
        "  Float: async (self) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.Float on zero Value\");",
        "    return Number(self.__gojrValue);",
        "  },",
        "  Complex: async (self) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.Complex on zero Value\");",
        "    return __gojrToComplex(self.__gojrValue);",
        "  },",
        "  Bool: async (self) => {",
        ...
        "  },",
        "  Len: async (self) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.Len on zero Value\");",
        "    return BigInt(__gojrLen(self.__gojrValue));",
        "  },",
        "  Index: async (self, index) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.Index on zero Value\");",
        "    const typ = __gojrReflectValueType(self);",
        "    const descriptor = typ && typ.__gojrDescriptor;",
        "    const kind = descriptor && descriptor.kind || \"\";",
        "    const elemType = descriptor && descriptor.elem || __gojrSliceElementRuntimeType(self.__gojrValue);",
        "    const item = __gojrIndex(self.__gojrValue, Number(index), __gojrIntegerTypeName(elemType));",
        "    return __gojrReflectValueOf(item, self.__gojrImportsByPath, kind === \"string\" ? \"uint8\" : elemType, descriptor && descriptor.elemPkgPath);",
        "  },",
        "  Pointer: async (self) => {",
        ...
        "  },",
        "  UnsafePointer: async (self) => {",
        "    if (!self.__gojrValid) throw new TypeError(\"reflect: call of Value.UnsafePointer on zero Value\");",
        "    const value = self.__gojrValue;",
        "    const typ = __gojrReflectValueType(self);",
        "    const kind = typ && typ.__gojrDescriptor && typ.__gojrDescriptor.kind || \"\";",
        "    if (__gojrPointerLike(value)) return value;",
        "    if (kind === \"chan\" || kind === \"func\" || kind === \"map\" || kind === \"ptr\" || kind === \"slice\" || kind === \"string\" || kind === \"unsafe.Pointer\") return __gojrRuntimeUintptrForValue(value);",
        "    throw new TypeError(`reflect: call of Value.UnsafePointer on ${kind || \"invalid\"} Value`);",
        "  }",
        "};",
        ...
        "  return undefined;",
        "}",
        "function __gojrCanonicalTypeText(typeName) {",
        ...
        "      if (ch === \"(\") depth += 1;",
        "      else if (ch === \")\") depth -= 1;",
        "      if (depth < 0 || (depth === 0 && index < text.length - 1)) { wraps = false; break; }",
        "    }",
        ...
        "  let text = __gojrCanonicalTypeText(typeName);",
        ...
        "  let name = __gojrCanonicalTypeText(typeName);",
        ...
        "  const indexed = cases.map((item, index) => ({ ...item, index }));",
        "  const defaults = indexed.filter((item) => item.op === \"default\");",
        "  const ready = indexed.filter((item) =>",
        ...
        "  if (!selected) return { index: -1, result: __gojrTuple([null, true]) };",
        "  if (selected.op === \"recv\") return { index: selected.index, result: await __gojrChanRecv(selected.channel) };",
        "  if (selected.op === \"send\") { await __gojrChanSend(selected.channel, selected.value); return { index: selected.index, result: __gojrTuple([null, true]) }; }",
        "  return { index: selected.index, result: __gojrTuple([null, true]) };",
        "}",
        ...
        "  if (value && value.__gojrInterface === true) value = value.value;",
        "  const text = __gojrCanonicalTypeText(typeText);",
        ...
        "  const convertedArgs = args.map((arg, index) => __gojrConvertRuntimeCallArg(arg, __gojrRuntimeCallArgumentTargetType(parameterTypes, index, false), importPath, typeArgs));",
        ...
        "}",
        "function __gojrConvertRuntimeCallArg(value, targetType, declaringImportPath = undefined, typeArgs = undefined) {",
        "  if (!targetType) return value;",
        "  const resolvedTargetType = typeArgs ? __gojrSubstituteType(targetType, typeArgs) : targetType;",
        "  const targetPkgPath = __gojrPackagePathForTypeName(resolvedTargetType, declaringImportPath);",
        "  if (__gojrRuntimeInterfaceTypeName(resolvedTargetType, targetPkgPath)) return __gojrToInterface(value, resolvedTargetType, __gojrRuntimeTypeName(value));",
        "  return __gojrConvertDynamicType(resolvedTargetType, value, __gojrActiveImportsByPath, targetPkgPath);",
        "}",
        ...
        "  const fullPath = __gojrFullImportPathForTypeName(typeName);",
        ...
        "}",
        "function __gojrFullImportPathForTypeName(typeName) {",
        "  let text = String(typeName || \"\").trim();",
        ...
        "  if (open > 0 && text.endsWith(\"]\") && !text.startsWith(\"map[\") && !text.startsWith(\"[\")) text = text.slice(0, open).trim();",
        "  const dot = text.lastIndexOf(\".\");",
        "  if (dot <= 0) return undefined;",
        "  const prefix = text.slice(0, dot);",
        "  for (const importPath of Object.values(__gojrImportPathsByQualifier || {})) if (prefix === importPath) return importPath;",
        "  return prefix.includes(\"/\") ? prefix : undefined;",
        "}",
        ...
        "  __gojrInferTypeArgumentsFromConstraints(names, __gojrFunctionTypeParameterConstraints(typeText), parameters, inferred);",
        ...
        "  return -1;",
        "}",
        "function __gojrFunctionTypeParameterConstraints(typeText) {",
        ...
        "  let pending = [];",
        "  for (const raw of __gojrSplitTopLevelTypes(text.slice(5, close))) {",
        "    const part = raw.trim();",
        "    if (!part) continue;",
        "    const boundary = __gojrFirstTopLevelWhitespace(part);",
        "    if (boundary < 0) { pending.push(part); continue; }",
        "    const head = part.slice(0, boundary).trim();",
        "    const constraint = part.slice(boundary).trim() || \"any\";",
        "    const names = [...pending, ...head.split(\",\").map((item) => item.trim()).filter(Boolean)];",
        "    pending = [];",
        "    for (const name of names) out[name] = constraint;",
        "  }",
        "  for (const name of pending) out[name] = \"any\";",
        "  return out;",
        "}",
        "function __gojrInferTypeArgumentsFromConstraints(names, constraints, parameters, inferred) {",
        "  if (!constraints) return;",
        "  for (let pass = 0; pass < names.length; pass += 1) {",
        "    let changed = false;",
        "    for (const name of names) {",
        "      const actual = inferred[name];",
        "      if (!actual) continue;",
        "      const pattern = __gojrConstraintInferencePattern(constraints[name], parameters);",
        "      if (!pattern) continue;",
        "      const before = JSON.stringify(Object.entries(inferred).sort());",
        "      __gojrInferTypeArgument(pattern, actual, parameters, inferred);",
        "      const actualUnderlying = __gojrUnderlyingRuntimeTypeName(actual);",
        "      if (actualUnderlying && actualUnderlying !== actual) __gojrInferTypeArgument(pattern, actualUnderlying, parameters, inferred);",
        "      if (before !== JSON.stringify(Object.entries(inferred).sort())) changed = true;",
        "    }",
        "    if (!changed) return;",
        "  }",
        "}",
        "function __gojrConstraintInferencePattern(constraint, parameters) {",
        "  if (!constraint) return undefined;",
        "  for (const option of __gojrSplitTopLevelUnion(String(constraint))) {",
        "    const trimmed = option.trim().replace(/^~/, \"\").trim();",
        "    if (!trimmed) continue;",
        "    const identifiers = trimmed.match(/[_$\\p{L}][_$\\p{L}\\p{N}]*/gu) || [];",
        "    if (identifiers.some((identifier) => parameters.has(identifier))) return trimmed;",
        "  }",
        "  return undefined;",
        "}",
        "function __gojrSplitTopLevelUnion(text) {",
        "  const parts = [];",
        "  let start = 0;",
        "  let paren = 0, bracket = 0, brace = 0;",
        "  for (let index = 0; index < text.length; index += 1) {",
        ...
        "  return out;",
        "}",
        STAGE1_SHARED_SUPPORT_END,
        "export default { artifact: gojrPackageArtifact, instantiateGoJrPackage };",
        ""
    ].filter((line) => line !== "").join("\n");
    return stripStage1SharedSupport(source);
}
let cachedStage1SharedSupportSource;
let cachedStage1SharedSupportBindingNames;
export function stage1SharedSupportSource() {
    ensureStage1SharedSupport();
    if (!cachedStage1SharedSupportSource)
        throw new Error("could not extract GoJr Stage 1 shared support source");
    return cachedStage1SharedSupportSource;
}
export function stage1SharedSupportBindingNames() {
    ensureStage1SharedSupport();
    return cachedStage1SharedSupportBindingNames ? [...cachedStage1SharedSupportBindingNames] : [];
}
export function stage1IntrinsicSupportSource() {
    return stage1SharedSupportSource();
}
function ensureStage1SharedSupport() {
    if (cachedStage1SharedSupportSource && cachedStage1SharedSupportBindingNames)
        return;
    stage1JavaScript(stage1IntrinsicSupportDummyArtifact(), false, [], [], emptyPackageEmitFacts);
}
function stripStage1SharedSupport(source) {
    const start = source.indexOf(STAGE1_SHARED_SUPPORT_START);
    const end = source.indexOf(STAGE1_SHARED_SUPPORT_END);
    if (start < 0 || end < 0 || end <= start)
        return source;
    const supportStart = start + STAGE1_SHARED_SUPPORT_START.length;
    const support = source.slice(supportStart, end).trim();
    const bindingNames = collectStage1SupportBindingNames(support);
    cachedStage1SharedSupportSource = support;
    cachedStage1SharedSupportBindingNames = bindingNames;
    const helperBindingLine = `  const { ${bindingNames.join(", ")} } = __gojrStage1Helpers;`;
    const before = source.slice(0, start).replace(STAGE1_SHARED_SUPPORT_BINDINGS_PLACEHOLDER, helperBindingLine).trimEnd();
    const after = source.slice(end + STAGE1_SHARED_SUPPORT_END.length).trimStart();
    return [
        before,
        stage1SharedSupportLoaderSource(),
        after
    ].join("\n");
}
function collectStage1SupportBindingNames(source) {
    const names = [];
    const seen = new Set();
    const pattern = /^(?:(?:async\s+)?function|const|let|var)\s+(__gojr[A-Za-z0-9_$]*)\b/gm;
...
function stage1SharedSupportLoaderSource() {
    return [
        "function __gojrLoadStage1Helpers(runtime, options, state) {",
        "  const factory = runtime.stage1Helpers || runtime.__gojrStage1Helpers || options.stage1Helpers || options.__gojrStage1Helpers || globalThis.__gojrStage1Helpers;",
        "  if (typeof factory !== \"function\") throw new Error(\"gojr Stage 1 package requires runtime.stage1Helpers from createStage1RuntimeCore\");",
