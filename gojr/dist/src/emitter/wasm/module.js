export function parseWasmModule(input) {
    ...
    while (offset < bytes.length) {
        ...
        if (payloadEnd > bytes.length)
            throw new Error(`truncated WebAssembly section ${id}`);
        const section = {
            id,
            name: sectionName(id),
            payloadStart,
    ...
    if (offset !== bytes.length)
        throw new Error("trailing bytes after WebAssembly module");
...
export function extractWasmFunction(input, exportName, options = {}) {
    const module = parseWasmModule(input);
    const exported = module.exports.find((item) => item.name === exportName && item.kind === "func");
    if (!exported)
        throw new Error(`WebAssembly function export ${exportName} not found`);
    ...
    if (!body)
        throw new Error(`WebAssembly function export ${exportName} has no code body`);
    const typeIndex = module.functionTypeIndices[codeIndex];
    if (typeIndex === undefined)
        throw new Error(`WebAssembly function export ${exportName} has no type index`);
    const signature = module.types[typeIndex];
    if (!signature)
        throw new Error(`WebAssembly function export ${exportName} has invalid type index ${typeIndex}`);
    ...
    if (importedMemory)
        out.importedMemory = importedMemory;
    return out;
}
function parseTypeSection(payload, absoluteStart) {
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        if (marker !== 0x60)
            throw new Error(`unsupported WebAssembly type form 0x${marker.toString(16)}`);
        out.push({
...
function parseImportSection(payload, absoluteStart) {
    const cursor = new Cursor(payload, absoluteStart);
    const count = cursor.u32();
    const out = [];
...
function parseFunctionSection(payload, absoluteStart) {
    const cursor = new Cursor(payload, absoluteStart);
    const count = cursor.u32();
    const out = [];
    for (let index = 0; index < count; index += 1)
...
function parseExportSection(payload, absoluteStart) {
    const cursor = new Cursor(payload, absoluteStart);
    const count = cursor.u32();
    const out = [];
    for (let index = 0; index < count; index += 1) {
...
function parseRawCodeSection(payload, absoluteStart) {
    const cursor = new Cursor(payload, absoluteStart);
    const count = cursor.u32();
    const out = [];
    for (let index = 0; index < count; index += 1) {
...
function parseCodeBody(bodyBytes, functionIndex, codeIndex) {
...
function parseValueTypeVector(cursor) {
    const count = cursor.u32();
    const out = [];
    for (let index = 0; index < count; index += 1)
        out.push(valueType(cursor.byte()));
    return out;
}
function parseLimits(cursor) {
...
function skipLimits(cursor) {
    parseLimits(cursor);
}
function importExportKind(kind) {
    switch (kind) {
        case 0: return "func";
        case 1: return "table";
...
function readU32(bytes, offset) {
    ...
    for (let index = 0; index < 5; index += 1) {
        ...
        if ((byte & 0x80) === 0)
            return { value: result >>> 0, next: offset + index + 1 };
        shift += 7;
    }
...
function readName(bytes, offset, limit) {
    ...
    if (end > limit)
        throw new Error(`truncated WebAssembly name at ${offset}`);
    return {
        value: new TextDecoder("utf-8").decode(bytes.subarray(start, end)),
        next: end
    };
}
function requireByte(bytes, offset) {
    const byte = bytes[offset];
    if (byte === undefined)
        throw new Error(`unexpected end of WebAssembly bytes at ${offset}`);
    return byte;
}
class Cursor {
    bytes;
    absoluteStart;
    offset = 0;
    constructor(bytes, absoluteStart) {
        this.bytes = bytes;
