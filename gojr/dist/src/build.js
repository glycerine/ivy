...
class PackageGraphBuilder {
    ...
    buildFromFreshArtifacts() {
        if (!this.store?.read || !this.request.importPath)
            return undefined;
        const nodes = new Map();
    ...
    freshArtifactNode(importPath, files, nodes, visiting) {
        const existing = nodes.get(importPath);
        if (existing)
            return existing;
        if (visiting.has(importPath))
            return undefined;
        ...
        if (!stringArraysEqual(sortedDependencyCacheKeys, [...archive.pkgdef.dependencyCacheKeys].sort()))
            return undefined;
        const node = {
            importPath,
        ...
        if (archive.pkgdef.standardLibrary)
            this.standardLibraryPackages.add(importPath);
        return node;
    }
    artifactMetadataMatchesRequest(pkgdef, importPath) {
    ...
    registerIntrinsicPackage(importPath) {
...
function packageRuntimePlan(importPath, packageName, pkg, initOrder) {
...
function artifactJSONString(value) {
    return JSON.stringify(value, (_key, item) => {
        if (typeof item === "bigint")
            return { __gojrBigInt: item.toString() };
        return item;
    }, 2);
}
...
function reviveArtifactValue(value) {
    if (Array.isArray(value))
        return value.map((item) => reviveArtifactValue(item));
    if (value && typeof value === "object") {
        const record = value;
        if (typeof record.__gojrBigInt === "string")
            return BigInt(record.__gojrBigInt);
        const out = {};
        for (const [key, item] of Object.entries(record))
            out[key] = reviveArtifactValue(item);
        return out;
    }
    return value;
}
