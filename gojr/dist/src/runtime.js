...
class EvaluationRootTransaction {
    ...
    rollback_;
    closed = false;
    constructor(commit_, rollback_) {
        this.commit_ = commit_;
        this.rollback_ = rollback_;
    }
    Commit() {
        this.ensureOpen();
        this.commit_();
    ...
    ensureOpen() {
        if (this.closed)
            throw new GoJuniorRuntimeError("runtime transaction already closed");
    }
}
export class EvaluationContext {
    ...
    replaceLocal(name, value, mutable = true, type) {
        if (name === "_")
            return;
        const typeText = resolvedBindingTypeText(type, this);
        const stored = typeText ? prepareAssignableToType(value, typeText, `variable ${name}`, this) : value;
    ...
    beginRootTransaction() {
        const snapshot = this.snapshotRootState();
        return new EvaluationRootTransaction(() => { }, () => this.restoreRootState(snapshot));
    }
    hasBinding(name) {
...
class Scope {
    ...
    replaceLocal(name, value, mutable, typeText) {
        if (!this.bindings.has(name)) {
...
function typePackageForImport(importPath, context) {
...
function packageMethodIntrinsic(declaration, importPath) {
    ...
    if (importPath === "reflect" && (receiver.baseType === "rtype" || receiver.baseType === "reflect.rtype")) {
        switch (declaration.name) {
            ...
            case "NumField":
                return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) => reflectRtypeNumField(args[0] ?? null, context));
            case "Field":
                return intrinsicGoJuniorFunction(declaration.name, declaration.signature, (args, context) => reflectRtypeField(args[0] ?? null, args[1] ?? 0n, context));
            default:
                return undefined;
        }
    }
...
function internalAbiDescriptorHasUnresolvedPackageType(typeText, context) {
    const type = normalizeTypeText(typeText);
    if (!type || isPredeclaredType(type) || isUnsafePointerType(type))
        return false;
    if (type.startsWith("*"))
        return internalAbiDescriptorHasUnresolvedPackageType(type.slice(1), context);
    if (type.startsWith("[]"))
        return internalAbiDescriptorHasUnresolvedPackageType(type.slice(2), context);
    const arrayType = parseArrayOrSliceTypeText(type, context);
    if (arrayType)
        return internalAbiDescriptorHasUnresolvedPackageType(arrayType.elementType, context);
    ...
    if (chanType)
        return internalAbiDescriptorHasUnresolvedPackageType(chanType.elementType, context);
    ...
    if (!qualified)
        return false;
    if (context.typeDef(type) || context.interfaceDef(type) || context.aliasType(type))
        return false;
...
function reflectRtypeStructFields(value, context) {
    const typeText = reflectRtypeRuntimeTypeText(value);
    if (!typeText)
        return undefined;
...
function reflectRtypeNumField(value, context) {
...
function reflectRtypeField(value, indexValue, context) {
    ...
    if (!field)
        throw new GoJuniorPanic("panic: reflect: Field index out of bounds");
