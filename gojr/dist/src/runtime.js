...
export class RuntimePointer {
    ...
    setValue;
    stableIdentity;
    constructor(typeName, getValue, setValue, stableIdentity) {
        this.typeName = typeName;
            ...
            const invoke = () => context.functionCallAsync(() => context.childScopeAsync(async () => {
                ...
                if (completion.kind === "return") {
                    ...
                    if (closureContext && closureContext !== parentContext && closureContext.importPath()) {
                        const resultValues = values.map((value, index) => packageBoundaryResultValue(value ?? null, activeSignature.results[index]?.type.text, closureContext.importPath(), parentContext.importPath()));
...
function externalizePackageRuntimeValue(value, packageName) {
    ...
    if (value instanceof RuntimePointer) {
        return new RuntimePointer(qualifyLocalRuntimeTypeName(value.typeName, packageName), () => externalizePackageRuntimeValue(value.get(), packageName), (next) => value.set(internalizePackageRuntimeValue(next, packageName)), value.identityKey());
    }
    if (Array.isArray(value)) {
...
function internalizePackageRuntimeValue(value, packageName) {
    ...
    if (value instanceof RuntimePointer) {
        return new RuntimePointer(unqualifyLocalRuntimeTypeName(value.typeName, packageName), () => internalizePackageRuntimeValue(value.get(), packageName), (next) => value.set(externalizePackageRuntimeValue(next, packageName)), value.identityKey());
    }
    if (Array.isArray(value)) {
    ...
    if (value instanceof RuntimeMap) {
        const map = new RuntimeMap(unqualifyLocalRuntimeTypeName(value.keyType, packageName), unqualifyLocalRuntimeTypeName(value.valueType, packageName));
        for (const [key, item] of value.orderedEntries()) {
            map.set(internalizePackageRuntimeValue(key, packageName), internalizePackageRuntimeValue(item, packageName));
        }
        return map;
    }
    if (value instanceof RuntimeStruct)
...
function runtimeMapKeyId(key, options = {}) {
    ...
    if (actual instanceof RuntimePointer)
        return `ptr:${actual.identityKey() ?? objectIdentityId(actual)}`;
    if (Array.isArray(actual))
