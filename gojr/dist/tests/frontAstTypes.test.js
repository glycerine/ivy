...
import { childNodes, ident, parseCellAddress, walk } from "../src/front/ast.js";
import { assignableTo, FuncObject, InterfaceType, MapType, NamedType, newUniverse, ObjectKind, PackageInfo, PointerType, Scope, SignatureType, SliceType, StructType, tuple, TypeNameObject, varOf } from "../src/front/types.js";
...
describe("Go-junior Go-style types", () => {
    ...
    test("formats signatures and compound types", () => {
        const universe = newUniverse();
        const signature = new SignatureType(undefined, tuple(varOf("format", universe.basic.string), varOf("args", new SliceType(universe.basic.any))), tuple(varOf("", universe.basic.int64), varOf("", universe.basic.error)), true);
    ...
    test("checks assignability for untyped constants, nil, named types, and method-set interfaces", () => {
        ...
        const lenSig = new SignatureType(varOf("p", point), tuple(), tuple(varOf("", universe.basic.float64)), false);
        const scaleSig = new SignatureType(varOf("p", new PointerType(point)), tuple(varOf("k", universe.basic.float64)), tuple(), false);
        point.addMethod(new FuncObject("Len2", lenSig));
        point.addMethod(new FuncObject("Scale", scaleSig));
        const hasLen = new InterfaceType([new FuncObject("Len2", new SignatureType(undefined, tuple(), tuple(varOf("", universe.basic.float64)), false))]).complete();
        const hasScale = new InterfaceType([new FuncObject("Scale", new SignatureType(undefined, tuple(varOf("k", universe.basic.float64)), tuple(), false))]).complete();
