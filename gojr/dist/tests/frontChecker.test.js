...
import { FuncObject, NamedType, PackageInfo, PointerType, SignatureType, SliceType, TypeKind, VarObject, methodSet, newUniverse, tuple, varOf } from "../src/front/types.js";
describe("Go-junior TypeScript front checker", () => {
    test("builds package scopes, imports, methods, local scopes, and range variable types", () => {
        ...
        fmt.scope.insert(new FuncObject("Printf", new SignatureType(undefined, tuple(varOf("format", universe.basic.string), varOf("args", new SliceType(universe.basic.any))), tuple(varOf("", universe.basic.int64)), true), fmt.scope, fmt));
...
`, {
        ...
        expect(methodSet(new PointerType(point)).map((method) => method.name)).toEqual(["Scale"]);
