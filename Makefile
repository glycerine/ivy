.PHONY: all build test test-conform clean z3ivy z3-build tr trt
...
golden: # parse with python ivy_check and goivy_check_xtrace. full xtrace output is written now to the files: out.go.xtrace; out.py.xtrace
	...
	cd lalr_full && go test -v -run TestOrdLive &> ../log.red
...
ordo: # not xtraced
	python3 -O /Users/jaten/pyivy/venv/bin/ivy_check isolate=cf_live /Users/jaten/go/src/github.com/glycerine/goivy/ivy-lang-examples/doc/examples/apple/ord_live.ivy
