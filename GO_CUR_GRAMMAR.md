# Plan: Extract Complete Go Grammar to WIP_GO_GRAMMAR_CURRENT.md

**Created:** 2026-03-23 20:15

## Context

Our Go LALR grammar port missed the `SYMBOL : SYMBOL LB SYMsubscr RB` rule.
To systematically identify all gaps, we extract the current Go grammar to a
reference document, which will be compared against `IVY_PYTHON_GRAMMAR.md`.

## Source Files

- `~/goivy/lalr_full/grammar_v17.y` (4,296 lines) — goyacc grammar, 138 nonterminals
- `~/goivy/lexer/token.go` — Token type definitions
- `~/goivy/lexer/lexer.go` — Lexer implementation, reserved words, version gating
- `~/goivy/lalr_full/lalr_parser.go` — Lex() adapter mapping lexer→goyacc tokens

## Execution

Single file write to `~/goivy/lalr_full/WIP_GO_GRAMMAR_CURRENT.md`.
...
After creating the file, do a quick sanity check:
```bash
grep -c '^[a-z]' ~/goivy/lalr_full/WIP_GO_GRAMMAR_CURRENT.md  # rough production count
wc -l ~/goivy/lalr_full/WIP_GO_GRAMMAR_CURRENT.md
```
