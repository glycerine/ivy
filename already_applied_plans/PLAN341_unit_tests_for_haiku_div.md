# TDD Conformance Tests for DIV-2,4,5,7,8,9,10,11,12,14

Created: 2026-04-28 ~UTC

## Context

Code review found 10 divergences where Go behavior differs from the Python spec. We need unit tests that:
- Currently FAIL — confirming the bug exists
- Will PASS once Go is fixed to match Python

Each test runs Go directly AND invokes a Python driver to confirm the expected behavior, following the pattern in `logic/sexp_cross_test.go`.
