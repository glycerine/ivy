...
#### DIV-1: `is_numeral_name` — Python has a bug that Go "fixed" (divergence either way)

**Python** `ivy_logic.py:128`:
```python
s[0] == '-' and len(s) > 1 and s[1].isdigit   # s[1].isdigit is a method ref (always truthy!)
```
Python accepts any `"-X"` string (len > 1) as a numeral name, regardless of what X is.

**Go** `ivylogic/ivylogic.go:282-294`:
```go
if s[0] == '-' && len(s) > 1 && s[1] >= '0' && s[1] <= '9' {
```
Go is more restrictive — requires second char to be a digit.

**Action**: Keep the Go behavior (it is more correct), but document the divergence. If Python relies on accepting `"-x"` as a numeral name anywhere, that would need a separate fix.
...
| Info | DIV-1 | is_numeral_name: Go is more correct than Python (Python has a bug) |
