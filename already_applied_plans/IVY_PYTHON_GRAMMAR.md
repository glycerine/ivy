# Complete Python Ivy Grammar Reference

**Source:** `~/pyivy/ivy/ivy/ivy_parser.py`, `ivy_logic_parser.py`, `ivy_lexer.py`
**Date extracted:** 2026-03-23
**Purpose:** Bit-accurate reference for the Go LALR parser port in `~/goivy/lalr_full/grammar_v17.y`

The main Ivy parser combines rules from `ivy_parser.py` (start='top', 439 p_ functions)
and `ivy_logic_parser.py` (112 p_ functions, imported via `from .ivy_logic_parser import *`).
Rules are version-conditional; version annotations show which era each rule belongs to.
Our target is **v1.7+**.
