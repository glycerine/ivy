# Plan: Production-by-Production Grammar Comparison → GRAMMAR_VARIANCES.md

**Created:** 2026-03-23 21:00

## Context

We extracted the Python grammar (IVY_PYTHON_GRAMMAR.md) and Go grammar
(WIP_GO_GRAMMAR_CURRENT.md). Now we compare them production-by-production
to identify all gaps in the Go port. Only v1.7+ Python rules are relevant.

## Execution

Single file write to `~/goivy/GRAMMAR_VARIANCES.md`.
...
```bash
grep -c '^\| V-' ~/goivy/GRAMMAR_VARIANCES.md   # count variance entries
grep 'CRITICAL' ~/goivy/GRAMMAR_VARIANCES.md     # verify critical items listed
```
