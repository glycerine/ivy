...
#            handler = MatchHandler(mclauses,model,vocab) if opt_trace.get() else ivy_trace.Trace(mclauses,model,vocab)
        ...
        if __debug__: xtracer.trace("check.checkFcsNormalPath history path postFmlas=%d postDefs=%d axiomFmlas=%d axiomDefs=%d checkers=%d" % (len(history.post.fmlas), len(history.post.defs), len(axioms.fmlas), len(axioms.defs), len(filter_fcs(fcs))))
...
def check_isolate(trace_hook = None):
    if __debug__: xtracer.trace("check.CheckIsolate ENTER")
    mod = im.module
    ...
    #    print 'check_lineno: {}'.format(check_lineno)
        ...
        if mod.initializers:
            ...
            for actname,action in sorted(mod.initializers, key=lambda x: x[0]):
                lineno = "{}".format(pretty_lineno(action))
                if lineno == "None":
                    lineno="(internal)"
                print("initializer:        {} {}".format(lineno, actname))
    ...
    #                    post = ag.execute(action, pre, None, actname)
        ...
        for actname,action in mod.actions.items():
            ...
            if __debug__: xtracer.trace("check.guarantee_phase HASH actname=%s canon=%s" % (actname, action.canon()))
            ...
            for _sub in _all_subs:
                ...
                if __debug__: xtracer.trace("check.guarantee_phase sub HASH actname=%s kind=%s isAssert=%s isRanking=%s canon=%s" % (actname, type(_sub).__name__, "True" if _is_assert else "False", "True" if _is_ranking else "False", _sub.canon()))
            ...
            if check_lineno is not None:
                guarantees = [sub for sub in guarantees if sub.lineno == check_lineno]
                if __debug__: xtracer.trace("check.guarantee_phase post check_lineno_filter actname=%s guarantees=%d" % (actname, len(guarantees)))
            _filtered = []
            for _x in guarantees:
                _pass = is_guarantee_mod_unprovable(_x)
                if __debug__: xtracer.trace("check.guarantee_phase unprov_filter HASH actname=%s pass=%s canon=%s" % (actname, "True" if _pass else "False", _x.canon()))
                if _pass:
                    _filtered.append(_x)
            guarantees = _filtered
            if __debug__: xtracer.trace("check.guarantee_phase post unprov_filter actname=%s guarantees=%d" % (actname, len(guarantees)))
        ...
        if not unprovable:
            check_temporals()
    if __debug__: xtracer.trace("check.CheckIsolate EXIT")
...
def check_subgoals(goals,method=None):
    ...
    for goal in goals:
        ...
        if isinstance(conc,ivy_ast.TemporalModels):
            ...
            mod.public_actions = {x: True for x in model.calls}
...
def check_module():
    ...
    if __debug__: im.module.canon_snapshot("top-of-CheckModule")
    ...
    if __debug__: xtracer.trace("check.CheckModule EXIT")
...
def start():
    ...
    if ivy_tactics.used_sorry:
        ...
        if __debug__: xtracer.trace("check.start EXIT")
        print("OK")
