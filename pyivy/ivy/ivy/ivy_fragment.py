...
def check_feu(assumes,asserts,macros):
    ...
    if __debug__:
        ...
        ## next line is not right, as it gives the error:
        ...
        for i, a in enumerate(arcs):
            ...
            xtracer.trace("  arc[%d]: from_id=%d(root=%d) to_id=%d(root=%d) fmla=%sargIdx=%d" % ( # lineno=%d 
...
def get_assumes_and_asserts(preconds_only):
    if __debug__: xtracer.trace("fragment: GetAssumesAndAsserts ENTER precondsOnly = %s" % preconds_only)
