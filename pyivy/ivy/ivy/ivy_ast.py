...
class LabeledFormula(AST):
    _in_clone = False  # class-level flag to suppress __init__ trace during clone
    def __init__(self,*args):
        global lf_counter
        self.args = args
        self.id = lf_counter
        ...
        lf_counter += 1
        if __debug__ and not LabeledFormula._in_clone:
            xtracer.trace("ast.LF.__init__ id=%d counter=%d" % (self.id, lf_counter))
    @property
    def label(self):
        return self.args[0]
    @property
    ...
    def clone(self,args):
        global lf_counter
        LabeledFormula._in_clone = True
        res = AST.clone(self,args)
        LabeledFormula._in_clone = False
        global always_clone_with_fresh_id
...
class TacticWithMatch(AST):
    ...
    def vocab(self,names):
        for m in self.match():
            names.update(symbols_ivy_ast(m.args[1]))
...
class IfTactic(Tactic):
    ...
    def vocab(self,names):
        names.update(symbols_ivy_ast(self.args[0]))
...
class TacticTactic(Tactic):
    ...
    def vocab(self,names):
        names.update(symbols_ivy_ast(self.args[1]))
        if not isinstance(self.args[2],NoneAST):
            self.args[2].vocab(names)
...
def ast_rewrite(x,rewrite):
    if (not isinstance(x,tuple)) and (not isinstance(x,list)):
        if __debug__: xtracer.trace("AstRewrite canon=%s" % (x.canon() if hasattr(x,'canon') else str(x) ))
