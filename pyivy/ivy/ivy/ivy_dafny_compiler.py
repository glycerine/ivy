...
def block(stmts):
    with ScopeContext():
        ...
        if scope_context.new_locals:
            actions = ip.LocalAction(*([a.rep for a in scope_context.new_locals] + [actions]),caller="dafny.compile_block")
    return actions
...
def methoddecl(d):
    ...
    mb = ip.LocalAction(*(ls+[ip.Sequence(*mb)]),caller="dafny.compile_method")
