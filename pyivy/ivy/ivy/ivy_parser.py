...
def inst_mod(ivy,module,pref,subst,vsubst,modname=None,lineno=None):
    ...
    def spaa(decl,subst,pref):
        if __debug__: xtracer.trace("parser.spaa ENTER HASH decl.canon=%s pref=%s" % (decl.canon(), (pref.canon() if pref else "nil")))
...
parser = yacc.yacc(start='top',tabmodule=ivy_ply_patch.runtime_tabmodule('ivy_parsetab'),errorlog=yacc.NullLogger(),outputdir=tabdir,debug=None,write_tables=False)
