...
class State(object):
    ...
    def update(self):
        if self.cached_update == None and self.expr != None and is_action_app(self.expr):
            if __debug__: xtracer.trace("interp.State.Update calling GetUpdate type=%s" % type(eval_action(self.expr.rep)).__name__)
...
def apply_action(ast,action_name,action,state):
    if __debug__: xtracer.trace("interp.ApplyAction calling GetUpdate actionName=%s type=%s" % (action_name, type(action).__name__))
...
#        print "post core: {} ".format(unsat_core(and_clauses(post[1],bg),true_clauses()))
        if __debug__:
            for act in acts:
                xtracer.trace("interp.Decompose calling IntUpdate type=%s" % type(act).__name__)
