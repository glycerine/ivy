...
def source_file(fn,f,**kwargs):
    if __debug__: xtracer.trace("init.SourceFile ENTER file=%s" % xtracer.normalize_filename(fn))
    try:
        ...
        print(str(e))
        sys.exit(1)
    if __debug__: xtracer.trace("init.SourceFile EXIT file=%s" % xtracer.normalize_filename(fn))
