...
##        print "update clauses: %s" % clauses
    ...
    def update(self,domain,in_scope):
        if __debug__: xtracer.trace("actions.GetUpdate ENTER type=%s" % type(self).__name__)
        ...
        for a in self.args:
            if not isinstance(a,Action):
                if __debug__ and isinstance(self, IfAction) and a is self.args[0]:
                    if isinstance(a, ivy_ast.Some):
                        kind = "some_min" if isinstance(a, ivy_ast.SomeMin) else "some_max" if isinstance(a, ivy_ast.SomeMax) else "some"
    ...
    def get_references(self,refs):
        if __debug__: xtracer.trace("actions.referencesRec ENTER type=%s n_before=%d" % (self.name(), len(refs)))
        self.references(refs)
        for a in self.args:
            if isinstance(a,Action):
                a.get_references(refs)
        if __debug__: xtracer.trace("actions.referencesRec EXIT type=%s n_after=%d" % (self.name(), len(refs)))
...
class AssertAction(Action):
    ...
    def formula(self):
        ...
        return fmla.formula if isinstance(fmla,ivy_ast.LabeledFormula) else fmla
    def action_update(self,domain,pvars):
        if __debug__: xtracer.trace("actions.AssertAction.action_update ENTER")
        fmla = self.args[0]
        unprovable = False
        if isinstance(fmla,ivy_ast.LabeledFormula):
        ...
        if check_unprovable.get() != unprovable:
            if __debug__: xtracer.trace("actions.AssertAction.action_update EXIT")
            return ([],true_clauses(annot = EmptyAnnotation()),false_clauses(annot = EmptyAnnotation()))
...
def assign_refs(self,refs):
    def recur(n):
        if n.rep.name in ivy_module.module.destructor_sorts:
            if __debug__ and n.rep not in refs:
                xtracer.trace("actions.assignRefs.add_destructor %s" % str(n.rep))
            refs.add(n.rep)
            recur(n.args[0])
            for a in n.args[1:]:
                for sym in symbols_ilu_ast(a):
...
class AssignAction(Action):
    ...
    def erase_unrefed(self,refs,ref_names):
        ...
        return self
    def action_update(self,domain,pvars):
        if __debug__: xtracer.trace("actions.AssignAction.action_update ENTER")
        lhs,rhs = self.args
        n = lhs.rep

        # Handle the hierarchical case
        if n in domain.hierarchy:
            if __debug__: xtracer.trace("actions.AssignAction.action_update branch=hierarchy")
            ...
            if __debug__: xtracer.trace("actions.AssignAction.action_update EXIT")
            return res
...
#            print "rhs: %s: %s" % (rhs,type(rhs))
        ...
        if n.name in ivy_module.module.destructor_sorts:
            if __debug__: xtracer.trace("actions.AssignAction.action_update branch=destructor")
            fmlas = []
        ...
        if domain.is_variant(lhs.sort,rhs.sort):
            if __debug__: xtracer.trace("actions.AssignAction.action_update branch=variant")
            new_clauses = mk_variant_assign_clauses(lhs,rhs)
        else:
            if __debug__: xtracer.trace("actions.AssignAction.action_update branch=standard")
...
class SetAction(Action):
    ...
    def name(self):
        return 'set'
    def action_update(self,domain,pvars):
        if __debug__: xtracer.trace("actions.SetAction.action_update ENTER")
        lit = self.args[0]
        n = lit.atom.relname
        new_n = new(n)
        args = lit.atom.args
        ...
        if __debug__: xtracer.trace("actions.SetAction.action_update EXIT")
...
class HavocAction(Action):
    ...
    def erase_unrefed(self,refs,ref_names):
        ...
        return self
    def action_update(self,domain,pvars):
        if __debug__: xtracer.trace("actions.HavocAction.action_update ENTER")
        lhs = self.args[0]
        n = lhs.rep
        new_n = new(n)
        args = lhs.args
        ...
        clauses = Clauses(clauses.fmlas,clauses.defs,EmptyAnnotation())
        if __debug__: xtracer.trace("actions.HavocAction.action_update EXIT")
...
class IfAction(Action):
    ...
    def subactions(self) :
        if isinstance(self.args[0],ivy_ast.Some):
            ...
            if_part = LocalAction(*(ps+[Sequence(AssumeAction(fmla),self.args[1])]),caller="actions.IfAction.action_update")
...
#                AssumeAction(Not(self.args[0]))]))
        if decreases is not None:
            res = LocalAction(aux,res,caller="actions.WhileAction.action_update")
        return res
...
#        print "local res: {}".format(res)
        ...
        return res
    def clone(self,args):
        from .ivy_ast import lineno_add_ref
        res = LocalAction(*args,caller="ast.LocalAction.clone")
        if hasattr(self,'lineno'):
...
class CrashAction(Action):
    ...
    def modifies(self,domain=None):
        ...
        n = lhs.rep
        if __debug__:
            xtracer.trace("actions.CrashAction.modifies ENTER lhs_type=%s n_type=%s n_val=%s" % (type(lhs).__name__, type(n).__name__, str(n)))
        dfnd = [ldf.formula.defines().name for ldf in domain.definitions]
        def recur(n,res):
            in_hier = n in domain.hierarchy
            if __debug__:
                xtracer.trace("actions.CrashAction.modifies.recur n_type=%s n_val=%s in_hierarchy=%s" % (type(n).__name__, str(n), in_hier))
            if in_hier:
                for child in domain.hierarchy[n]:
                    cname = iu.compose_names(n,child)
                    skip_spec = child == "spec" or iu.compose_names(cname,"spec") in domain.attributes
                    if __debug__:
                        xtracer.trace("actions.CrashAction.modifies.recur child=%s cname=%s skip=%s" % (child, cname, skip_spec))
                    if child != "spec" and iu.compose_names(cname,"spec") not in domain.attributes:
                        recur(cname,res)
            else:
                in_sig = n in domain.sig.symbols
                in_dfnd = n in dfnd
                if __debug__:
                    xtracer.trace("actions.CrashAction.modifies.recur LEAF n=%s in_sig=%s in_dfnd=%s" % (str(n), in_sig, in_dfnd))
                if in_sig and n not in dfnd:
                    for sym in domain.sig.all_symbols_named(n):
                        is_poly = symbol_is_polymorphic(sym)
                        is_interp = is_interpreted_symbol(sym)
                        if __debug__:
                            xtracer.trace("actions.CrashAction.modifies.recur SYM name=%s poly=%s interp=%s" % (str(sym), is_poly, is_interp))
                        if not is_poly and not is_interp:
                            res.append(sym)
        syms = []
        recur(n,syms)
        if __debug__:
            xtracer.trace("actions.CrashAction.modifies EXIT n_syms=%d syms=%s" % (len(syms), ",".join(str(s) for s in syms)))
        return syms
    def action_update(self,domain,pvars):
        if __debug__: xtracer.trace("actions.CrashAction.action_update ENTER")
        ...
        seq = Sequence(*havocs)
        if __debug__: xtracer.trace("actions.CrashAction.action_update EXIT")
...
class CallAction(Action):
    ...
    def __init__(self,*args):
        ...
        call_action_ctr += 1
        if __debug__:
            xtracer.trace("CallAction.__init__ uniqueID=%d counter=%d" % (self.unique_id, call_action_ctr))
    def name(self):
        return 'call'
    def __str__(self):
...
#            print 'no lineno in prefix_calls: {}'.format(self)
    ...
    def split_returns(self):
        ...
        res = LocalAction(*(new_returns+[asgn]),caller="actions.CallAction.action_update").sln(self.lineno)
        return res
...
def apply_mixin(decl,action1,action2):
    if __debug__: xtracer.trace("actions.apply_mixin ENTER")
    ...
    fp1, fp2 = action1.formal_params, action2.formal_params
    fr1, fr2 = action1.formal_returns, action2.formal_returns
    if __debug__: xtracer.trace("actions.apply_mixin fp1=%d fp2=%d fr1=%d fr2=%d isAfter=%s" %
        (len(fp1), len(fp2), len(fr1), len(fr2), isinstance(decl, MixinAfterDef)))
    if len(fp1) != len(fp2):
    ...
    if __debug__: xtracer.trace("actions.apply_mixin subs=%d" % len(subst))
...
#    print "action1_renamed: {}".format(action1_renamed)
    ...
    if __debug__: xtracer.trace("actions.apply_mixin EXIT")
    return res
