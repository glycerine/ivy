...
def add_mixins(mod,actname,action2,assert_to_assume=lambda m:[],use_mixin=lambda:True,mod_mixin=lambda name,m:m):
    ...
    for mixin in mod.mixins[actname]:
        ...
        if __debug__: xtracer.trace("isolate.add_mixins_ext actname=%s mixer=%s" % (actname, mixin_name))
...
def check_interference(mod,new_actions,summarized_actions,impl_mixins,check_term,interf_syms,
    ...
    for actname in sorted(summarized_actions):
        ...
        if __debug__: xtracer.trace("isolate.GetLocMods actname=%s locmods=%s" %
            (actname, ','.join(sorted(str(x) for x in locmods[actname]))))
...
def follow_definitions(ldfs,all_syms,label=""):
    ...
    if label:
        if __debug__: xtracer.trace("isolate.FollowDefinitions.%s before=%d after=%d" % (label, before, len(all_syms)))
    else:
...
def isolate_component(mod,isolate_name,extra_with=[],extra_strip=None,after_inits=None):
    ...
    for actname,ms in mod.mixins.items():
        ...
        for m in implements:
            ...
            if startswith_eq_some(m.mixer(),present,mod):
                ...
                if __debug__: xtracer.trace("isolate.impl_mixin mixer=%s mixee=%s" % (m.mixer(), m.mixee()))
    ...
    for actname,action in mod.actions.items():
        ...
        if pre: 
            ...
            if __debug__: xtracer.trace("isolate.summarizedActions_add actname=%s" % actname)
    ...
    def make_before_export(actname):
            ...
            for mixin in mod.mixins[actname]:
                ...
                if use_mixin(mixin_name) and before_mixins(mixin):
                    if __debug__: xtracer.trace("isolate.make_before_export actname=%s mixer=%s" % (actname, mixin_name))
...
#    implementation_map = save_implementation_map
    ...
    if after_inits:
        after_inits = sorted(after_inits)
        for actname in after_inits:
            if not startswith_eq_some(actname,present,mod):
                extname='ext:'+actname
                if __debug__: xtracer.trace("isolate.afterInits_delete actname=%s extname=%s" % (actname, extname))
    ...
    for name in new_actions:
        if __debug__: xtracer.trace("isolate.pre_cone actname=%s" % name)
    for name in sorted(exported):
        if __debug__: xtracer.trace("isolate.cone_root actname=%s" % name)
    cone = get_mod_cone(mod,actions=new_actions,roots=exported,after_inits=after_inits)
    def _cone_filter(x, y, cone):
        if x in cone:
            if __debug__: xtracer.trace("isolate.cone_survived actname=%s type=%s" % (x, type(y).__name__))
            return True
        if __debug__: xtracer.trace("isolate.cone_filtered actname=%s" % x)
        return False
    new_actions = dict((x,y) for x,y in new_actions.items() if _cone_filter(x, y, cone))
    ...
    all_names = set()
    if __debug__: xtracer.trace("isolate.proofs n=%d" % len(mod.proofs),1)
    for i, x in enumerate(mod.proofs):
        before = len(all_names)
        if __debug__: xtracer.trace("isolate.proof type=%s" % type(x[1]).__name__, 1)
        x[1].vocab(all_names)
...
#                print ('adding: {}'.format(x.formula.defines()))
    ...
    follow_definitions(mod.definitions,all_syms,label="allSyms")
...
#        print iu.pretty(ia.action_def_to_str(x,y))
    ...
    if do_check_interference.get():
        ...
        for actname, mxns in impl_mixins.items():
            if __debug__: xtracer.trace("isolate.implMixins actname=%s mixers=%s" % (actname, ','.join(sorted(m.mixer() for m in mxns))))
...
def hide_action_params(action):
    ...
    res = ia.LocalAction(*(params + [action]),caller="isolate.hide_action_params")
    return res
...
def create_isolate(iso,mod = None,**kwargs):
        ...
        if __debug__: xtracer.trace("check.CreateIsolate ENTER name=%s" % str(iso))
        ...
        for exp in mod.exports:
            ...
            if __debug__: xtracer.trace("check.CreateIsolate.export check name=%s found=%s" % (expname, expname in mod.actions))
            if expname not in mod.actions:
                for k in mod.actions:
                    if __debug__: xtracer.trace("check.CreateIsolate.export available_action=%s" % k)
...
#                    extra_with.append(ivy_ast.Atom(extname))
        ...
        if show_compiled.get():
            ivy_printer.print_module(mod)
        if __debug__: xtracer.trace("check.CreateIsolate EXIT name=%s" % str(iso))
