...
def expr_to_z3_no_type_cnst(expr,prefix=''):
    fmla = '(assert ' + slv.formula_to_z3_int(expr, "expr_to_z3_no_type_cnst").sexpr().replace('|!1','!1|').replace('\\|','').replace('\n',' "\n"') + ')'
    return 'z3::expr(g.ctx,Z3_parse_smtlib2_string({}ctx, "{}", {}sort_names.size(), &{}sort_names[0], &{}sorts[0], {}decl_names.size(), &{}decl_names[0], &{}decls[0]))'.format(prefix,fmla,prefix,prefix,prefix,prefix,prefix,prefix)
    ...
    if target.get() in ["gen","test"]:
        ...
        if is_primitive_sort(expr.sort) or False and isinstance(expr,HavocSymbol) or skip_z3:
            ...
            if not lu.free_variables(expr) and all(is_numeric_or_enumerated_constant(s) for s in ilu.used_symbols_ast(expr)):
                ...
                res_idx = len(vsyms) - 1 if vsyms else 0
                rsym = il.Symbol(name+'_res_{}'.format(res_idx),expr.sort)
...
#                code_line(impl,'std::cout << "check 3" << std::endl')
                ...
                code_line(impl,'dst.push_back(v);')
                code_line(impl,'g.ctx.check_error()')
...
def get_support_include_dir():
    ivy_py_dir = os.path.dirname(os.path.abspath(__file__))
...
#    header.append('#include <vector>\n')
    ...
    header.append('#include "ivy_hash.hpp"\n')
...
""".replace('CLASSNAME',classname))
    ...
    impl.append('#include "ivy_value.hpp"\n')
    if target.get() in ["repl","test"]:
        impl.append('#include "ivy_repl.hpp"\n')

    if target.get() in ["gen","test"]:
        impl.append('#include "ivy_z3_helpers.hpp"\n')
...
typedef ivy_z3_gen<classname, MODEL_LOG> gen;
""".replace('classname',classname).replace('MODEL_LOG','false' if target.get() == "gen" else 'true'))
            ...
            #        print impl
                ...
                if opt_build.get():
                    ...
                    if platform.system() == 'Windows':
                        ...
                        incspec = '/I {} /I {}'.format(os.path.join(_dir,'include'),get_support_include_dir())
                        ...
                        if opt_compiler.get() != 'g++':
                            cmd = '"{}\\VC\\vcvarsall.bat" amd64& cl /EHsc /Zi {} {}.cpp ws2_32.lib'.format(vsdir,incspec,basename)
                            if target.get() in ['gen','test']:
                                cmd = '"{}\\VC\\vcvarsall.bat" amd64& cl /MDd /EHsc /Zi {} {}.cpp ws2_32.lib libz3.lib /link {}'.format(vsdir,incspec,basename,libpspec)
                            cmd += libspec
                        else:
                            cmd = "g++ {} -I {} -I %Z3DIR%/include -L %Z3DIR%/lib -L %Z3DIR%/bin -g -o {} {}.cpp -lws2_32".format(gpp11_spec,get_support_include_dir(),basename,basename)
                            if target.get() in ['gen','test']:
                                cmd = cmd + ' -lz3'
                        if opt_outdir.get():
                            cmd = 'cd {} & '.format(opt_outdir.get()) + cmd
                    else:
                        support_path = '-I {}'.format(get_support_include_dir())
                        if target.get() in ['gen','test']:
                            paths = support_path + ' ' + ' '.join('-I {} -L {} -Xlinker -rpath -Xlinker {}'.format(os.path.join(_dir,'include'),os.path.join(_dir,'lib'),os.path.join(_dir,'lib')) for _dir in get_lib_dirs())
                        else:
                            paths = support_path
                        for lib in libs:
                            _dir = lib[1]
