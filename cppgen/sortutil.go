...
// IndentLevel tracks the current C++ indentation depth (4 spaces per level).
var IndentLevel int
...
// Indent appends the current indentation to a CodeText buffer.
...
// IndentCode appends code with indentation normalization to a CodeText.
...
// Varname converts an Ivy name to a C++ variable name using the full
// ivy_to_cpp.py rules (loc:, ext:, prm:, fml:, ret: prefixes, etc.).
...
type CppGenContext struct {
	SortToCppType map[string]CppType
...
func NewCppGenContext(mod *module.Module) *CppGenContext {
	return &CppGenContext{
		SortToCppType: make(map[string]CppType),
...
// IsLargeTypeSort is a convenience wrapper.
