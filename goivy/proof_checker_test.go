...
func TestAdmitPropositionWithComposeTactics(t *testing.T) {
	...
	prop := proofTestAstCfg.NewLabeledFormula(label, True)
...
func TestNewProofCheckerDefinitionKeyUsesDefinesName(t *testing.T) {
	...
	pc := NewProofChecker(nil, nil, nil, []*LabeledFormula{defLF}, nil)
...
func TestAdmitPropositionWithExistingSubgoals(t *testing.T) {
	...
	sg1 := proofTestAstCfg.NewLabeledFormula(label2, True)
...
func mapKeys(m map[string]*LabeledFormula) []string {
...
func TestSetLastAxiomAndSetSchema(t *testing.T) {
	...
	prop2 := proofTestAstCfg.NewLabeledFormula(label2, True)
