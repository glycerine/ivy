...
func TestParseTypeDecl(t *testing.T) {
	result, err := Parse("type node", Version{1, 7})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestParseRelation(t *testing.T) {
	result, err := Parse("relation r(X:node, Y:node)", Version{1, 7})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result.Decls) != 1 {
...
func TestParseAxiom(t *testing.T) {
	result, err := Parse("axiom [myax] forall X:t . r(X)", Version{1, 7})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result.Decls) != 1 {
...
func TestParseAction(t *testing.T) {
	result, err := Parse("action foo(x:t) = { x := x }", Version{1, 7})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result.Decls) != 1 {
