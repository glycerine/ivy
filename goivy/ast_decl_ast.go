...
func (r *DeclBase) Canon() Canonical {
...
func (d *DeclBase) Defines() []string {
	...
	for _, arg := range d.DeclArgs {
		...
		case *Atom:
			...
			if a.Rep != "" {
				if _, poly := PolymorphicSymbols[a.Rep]; !poly {
		...
		case *App:
			...
			if a.Rep != nil {
				if s, ok := a.Rep.(*Symbol); ok && s.Rep != "" {
					if _, poly := PolymorphicSymbols[s.Rep]; !poly {
...
func (r *ModuleDecl) Canon() Canonical {
	...
	return Canonical(fmt.Sprintf("(moduleDecl%v)",
