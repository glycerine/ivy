...
func TestExprListOrLambdaUnion_PopSingle(t *testing.T) {
	s := proofMkSort("S")
	c := proofMkConst("c", s)
	u := &ExprListOrLambdaUnion{Items: []Expr{c}}
...
func TestExprListOrLambdaUnion_PopMultiple(t *testing.T) {
	...
	b := proofMkConst("b", s)
	c := proofMkConst("c", s)
	u := &ExprListOrLambdaUnion{Items: []Expr{a, b, c}}
...
func TestMatchFromDefns_Single(t *testing.T) {
	...
	key, union, err := MatchFromDefns([]*LabeledFormula{defn})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if key != Key(f) {
	...
	if _, ok := union.Items[0].(*Lambda); !ok {
...
func TestMatchFromDefns_Multiple(t *testing.T) {
	...
	key, union, err := MatchFromDefns([]*LabeledFormula{defn1, defn2})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if key != Key(f) {
...
func TestMatchFromDefns_DifferentLHS(t *testing.T) {
	...
	_, _, err := MatchFromDefns([]*LabeledFormula{defn1, defn2})
	if err == nil {
...
func TestUnfoldRhsVars(t *testing.T) {
	...
	if _, ok := vars[Key(c1)]; !ok {
		t.Error("expected c1 in free vars")
	}
	if _, ok := vars[Key(c2)]; !ok {
...
func TestUnfoldFmla_SingleOccurrence(t *testing.T) {
	...
	result := UnfoldFmla(app, [][]*LabeledFormula{{defn}})

	// After unfolding f(c) with f(X) = c, we should get c
	if rc, ok := result.(*Const); ok {
		if rc.Name != "c" {
...
func TestUnfoldFmla_MultipleOccurrences_DestructivePop(t *testing.T) {
	...
	app1 := MustApply(f, a)
	app2 := MustApply(f, a)
	fmla := &Eq{T1: app1, T2: app2}

	result := UnfoldFmla(fmla, [][]*LabeledFormula{{defn1, defn2}})
	...
	lhs, lhsOk := eq.T1.(*Const)
	rhs, rhsOk := eq.T2.(*Const)
	if !lhsOk || !rhsOk {
...
func TestUnfoldFmla_NoMatch(t *testing.T) {
	...
	if rapp, ok := result.(*Apply); ok {
		if rc, ok := rapp.Func.(*Const); ok {
			if rc.Name != "g" {
...
func TestUnfoldFmla_NestedOccurrence(t *testing.T) {
	...
	inner := MustApply(f, a)
	outer := MustApply(f, inner)

	result := UnfoldFmla(outer, [][]*LabeledFormula{{defn}})
...
func TestUnfoldGoal_Simple(t *testing.T) {
	...
	result := UnfoldGoal(proofTestAstCfg, goal, [][]*LabeledFormula{{defn}})

	conc := GoalConc(result)
	if rc, ok := conc.(*Const); ok {
		if rc.Name != "a" {
...
func TestApplyUnfoldRec_UnderQuantifier(t *testing.T) {
	...
	if rc, ok := fa.Body.(*Const); !ok || rc.Name != "c" {
...
func TestMatchFromDefn_DuplicateParams(t *testing.T) {
	s := proofMkSort("S")
	x := proofMkVar("X", s)
	fs, _ := NewFunctionSort(s, s, s) // S x S -> S
	f := proofMkConst("f", fs)
	c := proofMkConst("c", s)
...
func TestMatchFromDefn_DuplicateParams_Iff(t *testing.T) {
	s := proofMkSort("S")
	x := proofMkVar("X", s)
	fs, _ := NewFunctionSort(s, s, Boolean) // S x S -> Bool
	p := proofMkConst("p", fs)
...
func TestApplyUnfoldGoal_FiltersConstantDeclPremise(t *testing.T) {
	...
	result := applyUnfoldGoal(proofTestAstCfg, Key(f), union, freeVars, goal)
	...
	for _, p := range prems {
		if cd, ok := p.(*ConstantDecl); ok {
			args := cd.Args()
			if len(args) > 0 {
				if rc, ok := args[0].(*Const); ok && rc.Name == "f" {
...
func TestApplyUnfoldGoal_KeepsNonMatchingConstantDecl(t *testing.T) {
	...
	for _, p := range prems {
		if cd, ok := p.(*ConstantDecl); ok {
			args := cd.Args()
			if len(args) > 0 {
				if rc, ok := args[0].(*Const); ok && rc.Name == "g" {
					foundG = true
				}
			}
		}
...
func TestApplyUnfoldRec_NonLambdaPopAppliesArgs(t *testing.T) {
	...
	union := &ExprListOrLambdaUnion{Items: []Expr{g}}
...
func TestApplyUnfoldRec_NonLambdaPopSortMismatch(t *testing.T) {
	...
	union := &ExprListOrLambdaUnion{Items: []Expr{c}}
