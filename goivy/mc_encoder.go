...
type Encoder struct {
	Inputs             []*Const                // original multi-bit input symbols
	Latches            []*Const                // original multi-bit latch symbols
	Outputs            []*Const                // original multi-bit output symbols
	Encoding           map[NodeKey][]string    // lg.Key(sym) -> list of sub-bit names
	Sub                *Aiger                  // underlying AIGER circuit
	Ops                map[string]ArithOp      // arithmetic operations
	IsConstructor      func(*Const) bool       // checks if symbol is a constructor
	ConstructorIndexFn func(*Const) (int, int) // returns (index, total) for constructor
...
func (e *Encoder) Lit(sym *Const) ([]int, bool) {
	enc, ok := e.Encoding[Key(sym)]
	if !ok {
		return nil, false
	}
...
func (e *Encoder) MustLit(sym *Const) []int {
	res, ok := e.Lit(sym)
	if !ok {
...
func (e *Encoder) DefineSym(sym *Const, val []int) {
	key := Key(sym)
...
func (e *Encoder) SetSym(sym *Const, val []int) {
	enc := e.Encoding[Key(sym)]
	for i, v := range val {
		if i < len(enc) {
			e.Sub.Set(enc[i], v)
		}
...
type GetDefFunc func(sym *Const) ([]int, error)
...
func (e *Encoder) Eval(expr Expr, getdef GetDefFunc) ([]int, error) {
...
func (e *Encoder) evalRec(expr Expr, getdef GetDefFunc) ([]int, error) {
	...
	case *Apply:
		...
		if IsNumeral(sym) && isInterpretedSort(sym.CSort) {
			n := getEncodingBits(sym.CSort)
...
func (e *Encoder) DefList(defs []Expr) error {
	...
	for _, df := range defs {
		...
		case *Eq:
			if c, ok := d.T1.(*Const); ok {
				dmap[Key(c)] = d.T2
			} else if app, ok := d.T1.(*Apply); ok {
	...
	getdef = func(sym *Const) ([]int, error) {
		body, ok := dmap[Key(sym)]
		if !ok {
...
func (e *Encoder) ConstructorIndex(sym *Const) (int, int) {
...
func isInterpretedSort(s Sort) bool {
	if s == nil {
		return false
	}
	switch s.(type) {
...
func bitsToBoolExprs(s string) []Expr {
...
func (e *Encoder) DecodeVal(bits []Expr, v *Const) Expr {
	...
	case *EnumeratedSort:
		...
		return NewConst(vals[num], v.CSort)
	case *RangeSort:
		num := binDecBool(bits)
		...
		return NewConst(strconv.Itoa(num), v.CSort)
	case *Theory:
		if s.Kind == BitVectorKind {
			num := binDecBool(bits)
			return NewConst(strconv.Itoa(num), v.CSort)
		}
	}
	if IsBooleanSort(v.CSort) {
		return bits[0]
	}
...
func (e *Encoder) GetSym(v *Const) Expr {
	enc, ok := e.Encoding[Key(v)]
	if !ok || len(enc) == 0 {
		return nil
	}
...
func (e *Encoder) GetNextSym(v *Const) Expr {
	enc, ok := e.Encoding[Key(v)]
	if !ok || len(enc) == 0 {
		return nil
	}
...
func (e *Encoder) GetEncoderState(post string) map[NodeKey]Expr {
	subres := e.Sub.GetState(post)
	res := make(map[NodeKey]Expr)
	for _, v := range e.Latches {
		enc := e.Encoding[Key(v)]
		bits := make([]Expr, len(enc))
		for i, s := range enc {
			b := subres[s]
			if b == '1' {
				bits[i] = &And{Terms: nil}
