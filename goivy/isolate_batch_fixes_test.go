...
func TestStripIsolateVersion16ClearsParams(t *testing.T) {
	...
	defer SetStringVersionOn(m.Cfg.IuCfg, save)
	SetStringVersionOn(m.Cfg.IuCfg, "1.6")
	sym := NewConst("p", Boolean)
	m.Params = append(m.Params, sym)

	stripMap := StripMap{"server": {"s"}}
	// Add a symbol that matches the strip map
	m.Sig.Symbols.Set("server.x", &SymbolEntry{Name: "server.x", Sort: Boolean})
...
func TestStripIsolateVersion17KeepsParams(t *testing.T) {
	...
	save := m.Cfg.IuCfg.GetStringVersion()
	defer SetStringVersionOn(m.Cfg.IuCfg, save)
	SetStringVersionOn(m.Cfg.IuCfg, "1.7")
	sym := NewConst("p", Boolean)
	m.Params = append(m.Params, sym)
...
func TestIsAssertLike_CallAction(t *testing.T) {
	a := NewCallActionOn(NewActionsConfig(), NewConst("foo", TopS))
	if IsAssertLike(a) {
...
func TestHasAssertions_WithRequiresAction(t *testing.T) {
	...
	m := mkModuleWithActions(map[string]ActionsAction{
		"foo": NewSequence(NewRequiresAction(True)),
	})
...
func TestHasAssertions_WithEnsuresAction(t *testing.T) {
	m := mkModuleWithActions(map[string]ActionsAction{
		"foo": NewSequence(NewEnsuresAction(True)),
	})
	if !HasAssertions(m, "foo") {
...
func TestHasAssertions_WithSubgoalAction(t *testing.T) {
	m := mkModuleWithActions(map[string]ActionsAction{
		"foo": NewSequence(NewSubgoalAction(True)),
	})
	if !HasAssertions(m, "foo") {
...
func TestHasAssertions_WithPlainAssertAction(t *testing.T) {
	m := mkModuleWithActions(map[string]ActionsAction{
		"foo": NewSequence(NewAssertAction(True)),
	})
	if !HasAssertions(m, "foo") {
...
func TestHasSideEffect_WithEnsuresAction(t *testing.T) {
	...
	act := NewSequence(NewEnsuresAction(True))
	m := mkModuleWithSig()
	m.Actions.Set("foo", act)
	actionMap := NewInsMap[string, ActionsAction]()
...
func TestGetStripBinding_BasicApply(t *testing.T) {
	...
	m.Sig.Symbols.Set("f", &SymbolEntry{Name: "f", Sort: sort1})
	...
	fnSort, err := NewFunctionSort(sort1, Boolean)
	if err != nil {
		t.Fatalf("NewFunctionSort error: %v", err)
	}
	fSym := NewConst("f", fnSort)
	xVar, err := NewVariable("X", sort1)
	if err != nil {
		t.Fatalf("NewVariable error: %v", err)
	}
	app, err := NewApply(fSym, xVar)
	if err != nil {
...
func TestGetStripBinding_Conflict(t *testing.T) {
	...
	fnSort, _ := NewFunctionSort(sort1, Boolean)
	xVar, _ := NewVariable("X", sort1)

	// First binding: f(X) -> X maps to "s1"
	fSym := NewConst("f", fnSort)
	app1, err := NewApply(fSym, xVar)
	if err != nil {
	...
	gSym := NewConst("g", fnSort)
	app2, err := NewApply(gSym, xVar)
	if err != nil {
...
func TestStripActionFull_BindingSubstitution(t *testing.T) {
	...
	xVar, _ := NewVariable("X", sort1)
	binding := map[NodeKey]string{
		Key(xVar): "param_s",
	}

	// Create an assign action that references X
	act := NewAssignAction(xVar, True)
...
func TestStripLabeledFormula_WithBinding(t *testing.T) {
	...
	m.Sig.Symbols.Set("f", &SymbolEntry{Name: "f", Sort: fnSort})
	...
	acfg := NewAstConfig()
	lf := acfg.NewLabeledFormula(NewConst("lbl", Boolean), app)
...
func TestStripIsolate_UsesStripActionFull(t *testing.T) {
	...
	fnSort, _ := NewFunctionSort(sortT, Boolean)
	m.Sig.Symbols.Set("server.f", &SymbolEntry{Name: "server.f", Sort: fnSort})

	// Create action with formal params [s]
	s := NewConst("s", sortT)
	act := NewSequence()
	act.SetFormalParams([]*Const{s})
...
func TestWhileHasRanking_WithRanking(t *testing.T) {
	...
	cond := True
	body := NewSequence()
	rankRel := NewConst("lt", Boolean)
	rw := &RankingWrapper{Ranking: NewRanking(rankRel)}

	wa := NewWhileAction(cond, body, rw)
...
func TestWhileHasRanking_WithoutRanking(t *testing.T) {
	...
	inv := NewConst("inv", Boolean)

	wa := NewWhileAction(cond, body, inv)
	if whileHasRanking(wa) {
...
func TestCheckInterferenceFull_TerminationCheck(t *testing.T) {
	cond := True
	body := NewSequence()
	wa := NewWhileAction(cond, body) // no ranking

	m := mkModuleWithSig()
	m.Actions.Set("bar", NewSequence(wa))

	summarized := map[string]bool{"bar": true}
	newActions := NewInsMap[string, ActionsAction]()
	newActions.Set("bar", NewSequence(wa))
...
func TestGetLocMods_FiltersOnFml(t *testing.T) {
	fmlSym := NewConst("fml:x", Boolean)
	normalSym := NewConst("y", Boolean)

	m := mkModuleWithSig()
	m.Sig.Symbols.Set("fml:x", &SymbolEntry{Name: "fml:x", Sort: Boolean})
	m.Sig.Symbols.Set("y", &SymbolEntry{Name: "y", Sort: Boolean})
	...
	act := NewAssignAction(fmlSym, True)
	...
	seqAct := NewSequence(
		NewAssignAction(fmlSym, True),
		NewAssignAction(normalSym, True),
	)
...
func TestExtAction_CreatesEnvAction(t *testing.T) {
	...
	extAct := NewEnvActionOn(NewActionsConfig(), extBranches...)
	if extAct == nil {
...
func TestCheckIsolateCompleteness_UncheckedProperty(t *testing.T) {
	m := mkModule()
	m.LabeledProps = []*LabeledFormula{
		{Label: NewConst("myprop", Boolean), Formula: True},
	}
...
func TestCheckIsolateCompleteness_CheckedProperty(t *testing.T) {
	...
	cfg := NewAstConfig()
	m.LabeledProps = []*LabeledFormula{
		cfg.NewLabeledFormula(NewConst("myprop", Boolean), True),
	}
	...
	iso := cfg.NewIsolateDef(
		[]Node{cfg.NewAtom("test_iso"), cfg.NewAtom("myprop")}, 0,
	)
...
func TestCheckIsolateCompleteness_UncheckedAssertion(t *testing.T) {
	...
	calleeAtom := NewConst("callee", TopS)
	callAction := NewCallActionOn(NewActionsConfig(), calleeAtom)
	m.Actions.Set("caller", NewSequence(callAction))
	m.Actions.Set("callee", NewSequence(NewAssertAction(True)))
...
func TestCheckIsolateCompleteness_ExportedActionUnchecked(t *testing.T) {
	m := mkModule()
	m.Actions.Set("exported_act", NewSequence(
		NewAssertAction(True),
	))
...
func TestCheckIsolateCompleteness_ScopedExportSkipped(t *testing.T) {
	m := mkModule()
	m.Actions.Set("scoped_act", NewSequence(
		NewAssertAction(True),
	))
...
func TestCheckIsolateCompleteness_RequiresUnchecked(t *testing.T) {
	...
	callAction := NewCallActionOn(NewActionsConfig(), calleeAtom)
	m.Actions.Set("caller", NewSequence(callAction))
	m.Actions.Set("callee", NewSequence(NewRequiresAction(True)))
...
func TestCheckIsolateCompleteness_NoAssertionNoError(t *testing.T) {
	...
	callAction := NewCallActionOn(NewActionsConfig(), calleeAtom)
	m.Actions.Set("caller", NewSequence(callAction))
	m.Actions.Set("callee", NewSequence()) // no assertions
...
func TestCheckIsolateCompleteness_MixinAssertionUnchecked(t *testing.T) {
	...
	callAction := NewCallActionOn(NewActionsConfig(), calleeAtom)
	m.Actions.Set("caller", NewSequence(callAction))
	m.Actions.Set("callee", NewSequence())
	m.Actions.Set("mixin_act", NewSequence(
		NewAssertAction(True),
	))

	// Add a before-mixin on callee
	m.Mixins.Set("callee", []MixinDef{
...
func TestCheckIsolateCompleteness_PropertyDedup(t *testing.T) {
	...
	m.LabeledProps = []*LabeledFormula{
		{Label: NewConst("dup", Boolean), Formula: True},
		{Label: NewConst("dup", Boolean), Formula: True},
	}
