...
func TestDivergence10UndoInReportInterpOverVar(t *testing.T) {
	...
	boolSort, err := lg.NewFunctionSort(S, lg.Boolean)
	if err != nil {
		t.Fatal(err)
	}
	p := lg.NewConst("p", boolSort)
	appFmla, err := lg.NewApply(p, xA)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDivergence11SkolemMapInReportArc(t *testing.T) {
	...
	astNode.SetLineno(ast.Location{Filename: "test.ivy", Line: 10})

	skolemFmla := &lg.ForAll{
		Variables: []*lg.Variable{eVar},
		Body:      eVar,
	}
...
func TestDivergence11NoSkolemEntry(t *testing.T) {
	...
	f := lg.NewConst("f", fSort)
	appFmla, err := lg.NewApply(f, xVar)
	if err != nil {
		t.Fatal(err)
	}
