...
func TopSortAsDefault(sig *Sig) *SortAsDefault {
	return NewSortAsDefault(sig, TopS)
}
...
func AlphaSortAsDefault(sig *Sig) *SortAsDefault {
	return NewSortAsDefault(sig, &TopSort{Name: "alpha"})
}
...
func CheckEssentiallyUninterpreted(sig *Sig, fmla Expr) error {
...
func GetSortRefinement(sig *Sig) map[NodeKey]Sort {
	result := make(map[NodeKey]Sort)
	for _, s := range sig.Sorts.All() {
		if !IsCanonicalSort(sig, s) {
			result[SortKey(s)] = CanonizeSort(sig, s)
		}
	}
	return result
}
...
func UninterpretedSorts(sig *Sig) []Sort {
	var result []Sort
	for _, s := range sig.Sorts.All() {
		if _, ok := s.(*UninterpretedSort); ok {
			name := IvySortName(s)
...
func InterpretedSorts(sig *Sig) []Sort {
	var result []Sort
	for _, s := range sig.Sorts.All() {
...
func ToStrWithVarSorts(t Expr) string {
	return PrettyFmla(t)
}
...
func FmlaToStrAmbiguous(term Expr) string {
	return PrettyFmlaAmbiguous(term)
}
...
func NewPolySymsDict() *PolySymsDict {
	return &PolySymsDict{IuCfg: NewIvyUtilsConfig(), m: make(map[string]*Const)}
}
...
func (p *PolySymsDict) Get(name string) *Const {
...
func (p *PolySymsDict) Set(name string, sym *Const) {
	p.m[name] = sym
}
