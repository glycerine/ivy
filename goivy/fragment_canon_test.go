...
func buildAllFragmentSexps(t *testing.T) map[string]string {
	...
	sortEqSym := NewConst("=", f.S)

	m := map[string]string{
		// UFNode
		"ufnode_basic":     ufNodeSexp(f.n0),
		"ufnode_nil":       ufNodeSexp(nil),
		"ufnode_set_empty": ufNodeSetSexp(map[*UFNode]bool{}),
		"ufnode_set_three": ufNodeSetSexp(map[*UFNode]bool{f.n0: true, f.n1: true, f.n2: true}),
		...
		"map_fmla_res_basic": string((&mapFmlaRes{
			node: f.n0, uvs: map[*UFNode]bool{f.n1: true, f.n2: true},
		}).Sexp()),
		"map_fmla_res_nil": string((&mapFmlaRes{
			node: nil, uvs: nil,
		}).Sexp()),
		"map_fmla_res_empty_uvs": string((&mapFmlaRes{
			node: f.n1, uvs: map[*UFNode]bool{},
		}).Sexp()),
	...
	objects := []struct {
		...
		{"mapFmlaRes", func() string {
			return string((&mapFmlaRes{
				node: f.n0,
				uvs:  map[*UFNode]bool{f.n1: true, f.n2: true},
			}).Sexp())
		}},
...
func randomSort(rng *rand.Rand) Sort {
	switch rng.Intn(3) {
	case 0:
...
func randomConst(rng *rand.Rand) *Const {
	names := []string{"f", "g", "h", "p", "q"}
	return NewConst(names[rng.Intn(len(names))], randomSort(rng))
}

// randomExpr generates a random Expr (variable, const, or eq).
func fragmentRandomExpr(rng *rand.Rand) Expr {
	switch rng.Intn(4) {
	case 0:
	...
	case 2:
		...
		v2, _ := NewVariable(v1.Name, v1.VSort) // same sort for Eq
		e, err := NewEq(v1, v2)
		if err != nil {
			return v1
		}
...
func randomUFNode(rng *rand.Rand) *UFNode {
	if rng.Intn(4) == 0 {
		return nil
	}
	return NewUFNode()
