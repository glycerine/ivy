...
func L2sGToGlobally(expr Expr) Expr {
	...
	res := ExpandNamedBindersAst(expr, g2g)
...
func extractJusticePredMap(fcs []Checker,
	...
	for _, fc := range fcs {
		...
		innerApp, ok := notExpr.Body.(*Apply)
		if !ok {
			continue
		}
...
func predLHSArgs(pred *Eq) []Expr {
	if pred == nil {
		return nil
	}
	if app, ok := pred.T1.(*Apply); ok {
		result := make([]Expr, len(app.Terms))
...
func makeSkolems(vs []Expr) []*Const {
	sks := make([]*Const, len(vs))
	for i, v := range vs {
		name, sort := exprNameSort(v)
		sks[i] = NewConst("@"+name, sort)
	}
	return sks
}
...
func applyPredToVals(rep *Const, vals []Expr) string {
	if rep == nil {
		return "<nil>"
	}
	if len(vals) == 0 {
...
func makeTermsKey(terms []Expr) termsKey {
	var k termsKey
	for i, t := range terms {
		if i >= len(k) {
			break
...
func applyFuncName(app *Apply) string {
...
func formatKeyWithRep(rep *Const, k termsKey) string {
	var parts []string
	for _, s := range k {
		if s == "" {
			break
...
func lfName(lf *LabeledFormula) string {
