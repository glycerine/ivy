...
func NewClauses(fmlas []Expr, defs []*IvyDefinition, annot interface{}) *Clauses {
	...
	idx := make(map[NodeKey]int, len(defs))
	for i, d := range defs {
...
func moduleDefinesKey(d *IvyDefinition) NodeKey {
...
func (c *Clauses) Copy() *Clauses {
	fmlas := make([]Expr, len(c.Fmlas))
	copy(fmlas, c.Fmlas)
	defs := make([]*IvyDefinition, len(c.Defs))
	copy(defs, c.Defs)
	idx := make(map[NodeKey]int, len(c.DefIdx))
...
func (c *Clauses) ToOpenFormula() Expr {
	if xtracer.Enabled {
		xtracer.Trace("ops.ToOpenFormula nFmlas=%d nDefs=%d", len(c.Fmlas), len(c.Defs))
	}
	conjuncts := make([]Expr, 0, len(c.Defs)+len(c.Fmlas))
...
func (c *Clauses) Symbols() *InsMap[NodeKey, Expr] {
...
func (c *Clauses) Apply(fn func(Expr) Expr) *Clauses {
	fmlas := make([]Expr, len(c.Fmlas))
	for i, f := range c.Fmlas {
		fmlas[i] = fn(f)
	}
	...
	for i, d := range c.Defs {
		result := fn(d)
		if nd, ok := result.(*IvyDefinition); ok {
			defs[i] = nd
		} else {
...
func FormulaToClauses(f Expr, annot interface{}) *Clauses {
	...
	f = dropUniversals(f)
	return NewClauses([]Expr{f}, nil, annot)
}
...
func ModuleIsTrue(n Expr) bool {
	return IvyIsTrue(n)
}

// IsFalse returns true if the node is logical False (empty Or).
func ModuleIsFalse(n Expr) bool {
	return IvyIsFalse(n)
}
