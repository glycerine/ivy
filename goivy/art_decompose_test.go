...
func interpState(mod *Module) *InterpState {
	sv := NewStateValue(nil, TrueClauses(nil), FalseClauses(nil))
	return NewInterpState(mod, sv, nil, "")
}
...
func interpChain(mod *Module, n int) *InterpState {
	if n <= 0 {
		return nil
	}
	cfg := NewAstConfig()
	...
	for i := 1; i < n; i++ {
		s := interpState(mod)
		s.Label = ""
		s.Expr = InterpActionApp(cfg, "step", WrapState(prev))
		s.SetPred(prev)
		prev = s
	}
	return prev
...
func TestInterpToArtStatePreservesProv(t *testing.T) {
	...
	child.Expr = InterpActionApp(cfg, "myact", WrapState(root))
	child.SetPred(root)
...
func TestInterpToArtStateActionAppRep(t *testing.T) {
	...
	root := interpState(mod)
	child := interpState(mod)
	child.Expr = InterpActionApp(cfg, "fire_missile", WrapState(root))
	child.SetPred(root)
...
func TestInterpToArtStateMemoIdentity(t *testing.T) {
	...
	child1.Expr = InterpActionApp(cfg, "a1", WrapState(shared))
	child1.SetPred(shared)
	child2 := interpState(mod)
	child2.Expr = InterpActionApp(cfg, "a2", WrapState(shared))
	child2.SetPred(shared)

	memo := make(map[*InterpState]*State)
...
func TestInterpToArtStateJoinProv(t *testing.T) {
	...
	joined.Expr = cfg.NewOr(WrapState(s1), WrapState(s2))
	joined.JoinOf = []*InterpState{s1, s2}
...
func TestArtToInterpStatePreservesExpr(t *testing.T) {
	...
	if !IsInterpActionApp(interpPost.Expr) {
