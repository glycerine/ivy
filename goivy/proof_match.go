package goivy

import (
	"fmt"

	"github.com/glycerine/ivy/goivy/xtracer"
)

// MatchProblem describes a schema-instantiation matching problem.
type MatchProblem struct {
	Schema      Expr             // the schema being instantiated (as logic node)
	SchemaLF    *LabeledFormula  // the schema as a LabeledFormula (may be nil)
	Pat         Expr             // pattern to match (conclusion of schema)
	Inst        Expr             // instance to match against (conclusion of goal)
	FreeSyms    map[NodeKey]Expr // free symbols in the schema
	Constants   map[NodeKey]Expr // constants (free variables of the goal)
	PremMatches []Expr           // premise match patterns (for Tuple matching)
	RevMap      map[NodeKey]Expr // reverse mapping for nonce symbols
	// TuplePats and TupleInsts are set by AddPremMatch when premise
	// matching creates combined premise+conclusion patterns.
	// nil means no tuple matching; non-nil activates per-element matching.
	// Python: when add_prem_match creates ia.Tuple patterns.
	TuplePats  []Expr
	TupleInsts []Expr
}

// NewMatchProblem creates a MatchProblem.
func NewMatchProblem(schema, pat, inst Expr, freesyms, constants map[NodeKey]Expr) *MatchProblem {
	fs := make(map[NodeKey]Expr, len(freesyms))
	for k, v := range freesyms {
		fs[k] = v
	}
	cs := make(map[NodeKey]Expr, len(constants))
	for k, v := range constants {
		cs[k] = v
	}
	return &MatchProblem{
		Schema:    schema,
		Pat:       pat,
		Inst:      inst,
		FreeSyms:  fs,
		Constants: cs,
		RevMap:    make(map[NodeKey]Expr),
	}
}

func (mp *MatchProblem) String() string {
	syms := make([]string, 0, len(mp.FreeSyms))
	for s := range mp.FreeSyms {
		syms = append(syms, fmt.Sprint(s))
	}
	return fmt.Sprintf("{pat:%s,inst:%s,freesyms:%v}", mp.Pat, mp.Inst, syms)
}

// --- Match functions ---

// FuncSorts returns the domain sorts followed by the range sort of a constant.
func FuncSorts(c *Const) []Sort {
	if fs, ok := c.CSort.(*FunctionSort); ok {
		dom := fs.Domain()
		result := make([]Sort, len(dom)+1)
		copy(result, dom)
		result[len(dom)] = fs.Range()
		return result
	}
	return []Sort{c.CSort}
}

// FuncsMatch checks whether two constants match structurally:
// same name, same arity, and non-free sorts agree.
func FuncsMatch(pat, inst *Const, freesyms map[NodeKey]Expr) bool {
	ps := FuncSorts(pat)
	is := FuncSorts(inst)
	if pat.Name != inst.Name || len(ps) != len(is) {
		return false
	}
	for i := range ps {
		if freesyms[Key(ps[i])] == nil && !ps[i].Equal(is[i]) {
			return false
		}
	}
	return true
}

// HeadsMatch checks if the heads of two terms match.
// Same top-level operator and same number of arguments.
// Quantifiers do not match anything.
// A function symbol matches if it has the same name and non-free sorts agree.
func HeadsMatch(pat, inst Expr, freesyms map[NodeKey]Expr) bool {
	if IsApp(pat) && IsApp(inst) {
		pc := appFunc(pat)
		ic := appFunc(inst)
		if pc != nil && ic != nil && freesyms[Key(pc)] == nil {
			return FuncsMatch(pc, ic, freesyms) && len(NodeArgs(pat)) == len(NodeArgs(inst))
		}
		return false
	}
	if IsApp(pat) || IsQuantifier(pat) {
		return false
	}
	if IsApp(inst) || IsQuantifier(inst) {
		return false
	}
	return sameNodeType(pat, inst) && len(NodeArgs(pat)) == len(NodeArgs(inst))
}

// TermSorts returns the domain+range sorts of the head function of a term,
// or the single sort for a variable.
func TermSorts(term Expr) []Sort {
	if c := appFunc(term); c != nil {
		return FuncSorts(c)
	}
	if v, ok := term.(*Variable); ok {
		return []Sort{v.VSort}
	}
	return nil
}

// LambdaSorts returns the variable sorts followed by the body sort of a lambda.
func LambdaSorts(lam *Lambda) []Sort {
	result := make([]Sort, len(lam.Variables)+1)
	for i, v := range lam.Variables {
		result[i] = v.VSort
	}
	result[len(lam.Variables)] = lam.Body.NodeSort()
	return result
}

// MatchSort matches a sort: if pat is free, map it to inst; otherwise require equality.
func MatchSort(pat, inst Sort, freesyms map[NodeKey]Expr) map[NodeKey]Expr {
	if freesyms[Key(pat)] != nil {
		return map[NodeKey]Expr{Key(pat): inst}
	}
	if pat.Equal(inst) {
		return map[NodeKey]Expr{}
	}
	return nil
}

// MergeMatches merges multiple match maps. Returns nil if any match is nil
// or if there is a conflicting assignment.
func MergeMatches(matches ...map[NodeKey]Expr) map[NodeKey]Expr {
	if len(matches) == 0 {
		return map[NodeKey]Expr{}
	}
	for _, m := range matches {
		if m == nil {
			return nil
		}
	}
	res := make(map[NodeKey]Expr)
	for k, v := range matches[0] {
		res[k] = v
	}
	for _, m := range matches[1:] {
		for k, v := range m {
			if prev, ok := res[k]; ok {
				if !EquivAlpha(prev, v) {
					return nil
				}
			} else {
				res[k] = v
			}
		}
	}
	return res
}

// EquivAlpha checks if two closed terms are equivalent modulo alpha conversion.
func EquivAlpha(x, y Expr) bool {
	if x.Equal(y) {
		return true
	}
	xLam, xOk := x.(*Lambda)
	yLam, yOk := y.(*Lambda)
	if xOk && yOk && len(xLam.Variables) == len(yLam.Variables) {
		// substitute y's variables with x's variables
		subs := make(map[NodeKey]Expr)
		for i, v := range yLam.Variables {
			subs[Key(v)] = xLam.Variables[i]
		}
		subBody, err := Substitute(yLam.Body, subs)
		if err != nil {
			return false
		}
		return xLam.Body.Equal(subBody)
	}
	return false
}

// Match matches an instance to a pattern.
// Returns an assignment sigma to freesyms such that sigma(pat) =_alpha inst.
// Returns nil on failure.
func Match(pat, inst Expr, freesyms, constants map[NodeKey]Expr) map[NodeKey]Expr {
	xtracer.Trace("proof.Match ENTER patType=%s instType=%s nfreesyms=%d", TypeName(pat), TypeName(inst), len(freesyms))
	if IsQuantifier(pat) {
		result := MatchQuants(pat, inst, freesyms, constants)
		xtracer.Trace("proof.Match EXIT viaQuants success=%v", result != nil)
		return result
	}
	if HeadsMatch(pat, inst, freesyms) {
		patArgs := NodeArgs(pat)
		instArgs := NodeArgs(inst)
		matches := make([]map[NodeKey]Expr, 0, len(patArgs)+4)
		for i := range patArgs {
			matches = append(matches, Match(patArgs[i], instArgs[i], freesyms, constants))
		}
		// match sorts
		ps := TermSorts(pat)
		is := TermSorts(inst)
		for i := range ps {
			if i < len(is) {
				matches = append(matches, MatchSort(ps[i], is[i], freesyms))
			}
		}
		if v, ok := pat.(*Variable); ok {
			matches = append(matches, map[NodeKey]Expr{Key(v): inst})
		}
		result := MergeMatches(matches...)
		xtracer.Trace("proof.Match EXIT viaHeads success=%v", result != nil)
		return result
	}

	// If pat is a free application, try to extract lambda
	if IsApp(pat) {
		c := appFunc(pat)
		if c != nil && freesyms[Key(c)] != nil {
			patArgs := NodeArgs(pat)
			B := ExtractTerms(inst, patArgs, constants)
			if B != nil {
				matches := []map[NodeKey]Expr{{Key(c): B}}
				ps := TermSorts(pat)
				ls := LambdaSorts(B)
				for i := range ps {
					if i < len(ls) {
						matches = append(matches, MatchSort(ps[i], ls[i], freesyms))
					}
				}
				result := MergeMatches(matches...)
				xtracer.Trace("proof.Match EXIT viaLambda success=%v", result != nil)
				return result
			}
		}
	}
	xtracer.Trace("proof.Match EXIT noMatch")
	return nil
}

// MatchQuants matches quantified formulas.
func MatchQuants(pat, inst Expr, freesyms, constants map[NodeKey]Expr) map[NodeKey]Expr {
	if sameNodeType(pat, inst) {
		patVars := BinderVars(pat)
		instVars := BinderVars(inst)
		if len(patVars) != len(instVars) {
			return nil
		}
		// temporarily add pat.variables to freesyms
		for _, v := range patVars {
			freesyms[Key(v)] = v
		}
		defer func() {
			for _, v := range patVars {
				delete(freesyms, Key(v))
			}
		}()

		matches := make([]map[NodeKey]Expr, 0, len(patVars)+1)
		for i := range patVars {
			matches = append(matches, Match(patVars[i], instVars[i], freesyms, constants))
		}
		mat := MergeMatches(matches...)
		if mat != nil {
			mbody := ApplyMatch(mat, BinderBody(pat))
			bodyFreesyms := ApplyMatchFreesyms(mat, freesyms)
			bodyMat := Match(mbody, BinderBody(inst), bodyFreesyms, constants)
			// Build quants map from patVars (Python: quants=pat.variables, checked with `sym not in quants`)
			patVarMap := make(map[NodeKey]Expr, len(patVars))
			for _, v := range patVars {
				patVarMap[Key(v)] = v
			}
			bodyMat = ComposeMatches(freesyms, mat, bodyMat, patVarMap)
			mat = MergeMatches(mat, bodyMat)
		}
		if mat != nil {
			for _, v := range patVars {
				delete(mat, Key(v))
			}
		}
		return mat
	}
	return nil
}

// FOMatch computes a partial first-order match.
// Matches free FO variables to ground terms, but ignores variable
// occurrences under free second-order symbols.
func FOMatch(pat, inst Expr, freesyms, constants map[NodeKey]Expr) map[NodeKey]Expr {
	xtracer.Trace("proof.FOMatch ENTER patType=%s instType=%s", TypeName(pat), TypeName(inst))
	if v, ok := pat.(*Variable); ok {
		if freesyms[Key(v)] != nil && allVariablesAreConstants(inst, constants) {
			res := map[NodeKey]Expr{Key(v): inst}
			if freesyms[Key(v.VSort)] != nil {
				res[Key(v.VSort)] = inst.NodeSort()
				return res
			}
			if v.VSort.Equal(inst.NodeSort()) {
				return res
			}
		}
	}
	if IsQuantifier(pat) && IsQuantifier(inst) && sameNodeType(pat, inst) {
		patVars := BinderVars(pat)
		// temporarily remove pat.variables from freesyms
		saved := make(map[NodeKey]Expr)
		for _, v := range patVars {
			if freesyms[Key(v)] != nil {
				saved[Key(v)] = v
				delete(freesyms, Key(v))
			}
		}
		defer func() {
			for k := range saved {
				freesyms[k] = saved[k]
			}
		}()
		return FOMatch(BinderBody(pat), BinderBody(inst), freesyms, constants)
	}
	if HeadsMatch(pat, inst, freesyms) {
		patArgs := NodeArgs(pat)
		instArgs := NodeArgs(inst)
		matches := make([]map[NodeKey]Expr, len(patArgs))
		for i := range patArgs {
			matches[i] = FOMatch(patArgs[i], instArgs[i], freesyms, constants)
		}
		return MergeMatches(matches...)
	}
	return map[NodeKey]Expr{}
}

// ComposeMatches composes two matches: for each free symbol not in quants,
// if mat1 maps it to sym1 and sym1 is in mat2, the result maps the original
// to mat2[sym1].
// quants is a set of symbol keys to exclude (not-in-quants check).
// Python: compose_matches(freesyms, mat1, mat2, quants) where quants can be
// a list of variables or a dict — both checked with `sym not in quants`.
func ComposeMatches(freesyms map[NodeKey]Expr, mat1, mat2 map[NodeKey]Expr, quants map[NodeKey]Expr) map[NodeKey]Expr {
	if mat1 == nil || mat2 == nil {
		return nil
	}
	res := make(map[NodeKey]Expr)
	for symKey, symNode := range freesyms {
		if quants[symKey] != nil {
			continue
		}
		if symNode == nil {
			continue
		}
		sym1 := ApplyMatchSym(mat1, symNode)
		if v, ok := mat2[Key(sym1)]; ok {
			res[symKey] = v
		}
	}
	return res
}

// ApplyMatch applies a match to a formula.
// Substitutes all symbols in the match with the corresponding lambda terms
// and performs beta reduction. Alpha-renames to avoid capture.
//
// Python: ivy_proof.py:1075-1086 (apply_match / apply_match_rec)
// Python first calls alpha_avoid to rename bound variables that would clash
// with free variables introduced by the substitution.
func ApplyMatch(match map[NodeKey]Expr, fmla Expr) Expr {
	xtracer.Trace("proof.ApplyMatch ENTER nmatch=%d fmlaType=%s", len(match), TypeName(fmla))
	// Python's apply_match has no early return for empty match — always processes.
	// Alpha-rename bound vars to avoid capture by match RHS free vars.
	// Python: freevars = match_rhs_vars(match); fmla = il.alpha_avoid(fmla, freevars)
	freeVars := MatchRhsVars(match)
	fmla = AlphaAvoidMap(fmla, freeVars)
	result := applyMatchRec(match, fmla)
	if result != nil {
		xtracer.Trace("proof.ApplyMatch EXIT HASH canon=%v", result.Canon())
	} else {
		xtracer.Trace("proof.ApplyMatch EXIT resultNil")
	}
	return result
}

// applyMatchRec recursively applies a match to a formula with beta reduction.
func applyMatchRec(match map[NodeKey]Expr, fmla Expr) Expr {
	args := NodeArgs(fmla)
	newArgs := make([]Expr, len(args))
	for i, a := range args {
		newArgs[i] = applyMatchRec(match, a)
	}

	// Application: check if the function is in the match
	if IsApp(fmla) {
		c := appFunc(fmla)
		if c != nil {
			if replacement, ok := match[Key(c)]; ok {
				// Beta reduction: apply the lambda to the arguments
				if lam, ok := replacement.(*Lambda); ok {
					return betaReduce(lam, newArgs)
				}
				// If replacement is a constant, build new application
				if rc, ok := replacement.(*Const); ok {
					if len(newArgs) > 0 {
						return MustApply(rc, newArgs...)
					}
					return rc
				}
				return replacement
			}
			// Apply sort mapping to the function
			newC := ApplyMatchFunc(match, c)
			if newC != c {
				if len(newArgs) > 0 {
					return MustApply(newC, newArgs...)
				}
				return newC
			}
		}
	}

	// Variable: check if in match
	if v, ok := fmla.(*Variable); ok {
		if replacement, ok := match[Key(v)]; ok {
			return replacement
		}
		// Apply sort mapping
		newSort := matchGetSort(match, v.VSort)
		if newSort != v.VSort {
			nv, _ := NewVariable(v.Name, newSort)
			return nv
		}
		return fmla
	}

	// Binder: apply match to bound variable sorts, then use CloneBinder.
	// Python: with il.BindSymbols(env, fmla.variables):
	//             fmla = fmla.clone_binder([apply_match_rec(match,v,env) for v in fmla.variables], args[0])
	if IsQuantifier(fmla) && len(newArgs) > 0 {
		vars := BinderVars(fmla)
		newVars := make([]*Variable, len(vars))
		for i, v := range vars {
			processed := applyMatchRec(match, v)
			if nv, ok := processed.(*Variable); ok {
				newVars[i] = nv
			} else {
				newVars[i] = v
			}
		}
		return CloneBinder(fmla, newVars, newArgs[0])
	}

	// Clone with new args — Python always clones even with empty args.
	return CloneNode(fmla, newArgs)
}

// betaReduce applies a lambda to arguments, performing beta reduction.
// Uses il.LambdaApply which calls lu.Substitute (capture-detecting).
// Python: lambda_apply calls lu.substitute which raises CaptureError on capture.
func betaReduce(lam *Lambda, args []Expr) Expr {
	if len(lam.Variables) != len(args) {
		// Arity mismatch — return the lambda body as-is
		return lam.Body
	}
	result, err := LambdaApply(lam, args)
	if err != nil {
		// Capture detected — return original body.
		// Python raises CaptureError which propagates.
		return lam.Body
	}
	return result
}

// ApplyMatchSym applies a match to a single symbol (constant, variable, or sort).
func ApplyMatchSym(match map[NodeKey]Expr, sym Expr) Expr {
	if v, ok := match[Key(sym)]; ok {
		return v
	}
	if v, ok := sym.(*Variable); ok {
		newSort := matchGetSort(match, v.VSort)
		if newSort != v.VSort {
			nv, _ := NewVariable(v.Name, newSort)
			return nv
		}
		return sym
	}
	if c, ok := sym.(*Const); ok {
		return ApplyMatchFunc(match, c)
	}
	return sym
}

// ApplyMatchFunc applies sort mappings to a constant's sort.
func ApplyMatchFunc(match map[NodeKey]Expr, c *Const) *Const {
	sorts := FuncSorts(c)
	newSorts := make([]Sort, len(sorts))
	for i, s := range sorts {
		if rep, ok := match[Key(s)]; ok {
			if rs, ok := rep.(Sort); ok {
				newSorts[i] = rs
				continue
			}
		}
		newSorts[i] = s
	}
	var newSort Sort
	if len(newSorts) == 1 {
		newSort = newSorts[0]
	} else {
		fs, err := NewFunctionSort(newSorts...)
		if err != nil {
			return c
		}
		newSort = fs
	}
	return NewConst(c.Name, newSort)
}

// ApplyMatchFreesyms applies a match to the free symbols set, returning a new set
// without the matched symbols.
func ApplyMatchFreesyms(match map[NodeKey]Expr, freesyms map[NodeKey]Expr) map[NodeKey]Expr {
	result := make(map[NodeKey]Expr)
	for sym := range freesyms {
		if _, matched := match[sym]; matched {
			continue
		}
		symNode := freesyms[sym]
		result[Key(ApplyMatchSym(match, symNode))] = ApplyMatchSym(match, symNode)
	}
	return result
}

// ExtractTerms returns a lambda term t such that t(terms) = inst and
// the given terms do not occur in t. Returns nil if the extraction
// would introduce non-constant variables.
func ExtractTerms(inst Expr, terms []Expr, constants map[NodeKey]Expr) *Lambda {
	if len(terms) == 0 {
		return nil
	}
	vars := make([]*Variable, len(terms))
	for i, t := range terms {
		name := fmt.Sprintf("V%d", i)
		v, _ := NewVariable(name, t.NodeSort())
		vars[i] = v
	}
	body := extractRec(inst, terms, vars)

	// Build a set of the lambda's own variables to exclude from the check
	lamVarSet := make(map[NodeKey]Expr, len(vars))
	for _, v := range vars {
		lamVarSet[Key(v)] = v
	}

	// Check that all free variables in the body (excluding lambda vars) are constants
	freeVars := FreeVariables(body)
	for v := range freeVars.All() {
		if lamVarSet[v] == nil && constants[v] == nil {
			return nil
		}
	}

	lam, err := NewLambda(vars, body)
	if err != nil {
		return nil
	}
	return lam
}

func extractRec(inst Expr, terms []Expr, vars []*Variable) Expr {
	for i, t := range terms {
		if t.Equal(inst) {
			return vars[i]
		}
	}
	args := NodeArgs(inst)
	if len(args) == 0 {
		return inst
	}
	newArgs := make([]Expr, len(args))
	for i, a := range args {
		newArgs[i] = extractRec(a, terms, vars)
	}
	return CloneNode(inst, newArgs)
}

// --- AddSymbols / RemoveSymbols ---

// AddSymbols temporarily adds symbols to a set. Use with defer Restore().
type AddSymbols struct {
	symset  map[NodeKey]Expr
	added   []Expr
	removed []Expr
}

// NewAddSymbols adds symlist to symset, tracking what was changed.
func NewAddSymbols(symset map[NodeKey]Expr, symlist []Expr) *AddSymbols {
	as := &AddSymbols{symset: symset}
	for _, sym := range symlist {
		if symset[Key(sym)] != nil {
			as.removed = append(as.removed, sym)
		}
		symset[Key(sym)] = sym
		as.added = append(as.added, sym)
	}
	return as
}

// Restore undoes the changes made by NewAddSymbols.
func (as *AddSymbols) Restore() {
	for _, sym := range as.added {
		delete(as.symset, Key(sym))
	}
	for _, sym := range as.removed {
		as.symset[Key(sym)] = sym
	}
}

// RemoveSymbols temporarily removes symbols from a set. Use with defer Restore().
type RemoveSymbols struct {
	symset map[NodeKey]Expr
	saved  []Expr
}

// NewRemoveSymbols removes symlist from symset, tracking what was changed.
func NewRemoveSymbols(symset map[NodeKey]Expr, symlist []Expr) *RemoveSymbols {
	rs := &RemoveSymbols{symset: symset}
	for _, sym := range symlist {
		if symset[Key(sym)] != nil {
			rs.saved = append(rs.saved, sym)
			delete(symset, Key(sym))
		}
	}
	return rs
}

// Restore undoes the changes made by NewRemoveSymbols.
func (rs *RemoveSymbols) Restore() {
	for _, sym := range rs.saved {
		rs.symset[Key(sym)] = sym
	}
}

// --- helpers ---

// appFunc returns the Const func of an Apply, or nil.
func appFunc(n Expr) *Const {
	switch t := n.(type) {
	case *Apply:
		if c, ok := t.Func.(*Const); ok {
			return c
		}
	case *Const:
		return t
	}
	return nil
}

// sameNodeType returns true if a and b are the same concrete type.
func sameNodeType(a, b Expr) bool {
	return fmt.Sprintf("%T", a) == fmt.Sprintf("%T", b)
}

// allVariablesAreConstants checks that all variables in a term are in the constants set.
func allVariablesAreConstants(n Expr, constants map[NodeKey]Expr) bool {
	if v, ok := n.(*Variable); ok {
		return constants[Key(v)] != nil
	}
	for _, c := range n.Children() {
		if !allVariablesAreConstants(c, constants) {
			return false
		}
	}
	return true
}

// matchGetSort looks up a sort in a match map.
func matchGetSort(match map[NodeKey]Expr, s Sort) Sort {
	if rep, ok := match[Key(s)]; ok {
		if rs, ok := rep.(Sort); ok {
			return rs
		}
	}
	return s
}
