...
func CompileAndGenerate(filename string, params map[string]string, cfg Config) (*Output, error) {
	...
	if len(batch.Outputs) != 1 {
		return nil, fmt.Errorf("ivy2golang: expected one generated output, got %d", len(batch.Outputs))
	}
...
func CompileAndGenerateAll(filename string, params map[string]string, cfg Config) (*BatchOutput, error) {
	...
	for _, isolate := range isolates {
		...
		snap := snapshotGoInterface(isoMod)
		if isolate != "" || languageVersionAtLeast(isoMod, "1.7") {
			...
			restoreGoInterface(isoMod, snap)
		}
...
func mergeParams(params map[string]string, cfg Config) (Config, map[string]string, error) {
	...
	for k, v := range params {
		...
		case "isolate":
			ivyParams[k] = v
		default:
			return cfg, nil, fmt.Errorf("ivy2golang: unknown parameter %q", k)
		}
	}
	var err error
...
func ParseArgs(args []string) (map[string]string, string, error) {
	...
	if len(rest) != 1 || !strings.HasSuffix(rest[0], ".ivy") {
		return nil, "", fmt.Errorf("usage: ivy2golang [key=value ...] file.ivy")
	}
...
func WriteOutput(out *Output, outDir string) error {
	if out == nil {
		return fmt.Errorf("ivy2golang: nil output")
	}
...
func WriteBatchOutput(batch *BatchOutput, outDir string) error {
	if batch == nil {
		return fmt.Errorf("ivy2golang: nil batch output")
	}
	for _, out := range batch.Outputs {
...
func normalizeConfig(cfg Config) (Config, string, error) {
	...
	default:
		return cfg, "", fmt.Errorf("ivy2golang: target %q is not supported", requested)
	}
...
type goInterfaceSnapshot struct {
	Actions         map[string]goActionSignature
...
type goActionSignature struct {
	Params  []*goivy.Const
	Returns []*goivy.Const
}

func snapshotGoInterface(mod *goivy.Module) goInterfaceSnapshot {
	...
	if mod.Actions != nil {
		for name, act := range mod.Actions.All() {
			...
			snap.Actions[name] = goActionSignature{
...
func restoreGoInterface(mod *goivy.Module, snap goInterfaceSnapshot) {
	if mod == nil {
		return
	}
	if mod.Actions != nil && len(snap.CallbackActions) > 0 {
	...
	for _, sig := range snap.Actions {
		for _, p := range sig.Params {
			restoreSortForGoInterface(mod, snap, p.CSort, map[string]bool{})
		}
		for _, r := range sig.Returns {
			restoreSortForGoInterface(mod, snap, r.CSort, map[string]bool{})
		}
	}
}

func restoreActionSignature(mod *goivy.Module, name string, sig goActionSignature) {
...
func restoreSortForGoInterface(mod *goivy.Module, snap goInterfaceSnapshot, sort goivy.Sort, seen map[string]bool) {
	if mod == nil || sort == nil || mod.Sig == nil {
		return
	}
	name := sortName(sort)
	if name != "" && !seen[name] {
		...
		if _, ok := mod.Sig.Sorts.Get2(name); !ok {
			if s, found := snap.Sorts[name]; found {
				_ = mod.Sig.AddSort(s)
	...
	if fs, ok := sort.(*goivy.LogicFunctionSort); ok {
		for _, d := range fs.Domain() {
			restoreSortForGoInterface(mod, snap, d, seen)
		}
		restoreSortForGoInterface(mod, snap, fs.Range(), seen)
	}
	if name != "" {
...
func ensureSortOrderName(mod *goivy.Module, snap goInterfaceSnapshot, name string) {
...
func collectCallbackActionNames(mod *goivy.Module) []string {
	if mod == nil || mod.Natives == nil {
		return nil
	}
	seen := map[string]bool{}
	var names []string
	var visit func(goivy.Node)
	visit = func(n goivy.Node) {
		if n == nil {
			return
		}
		if name, ok := callbackActionName(mod, n); ok {
			if !seen[name] {
				seen[name] = true
				names = append(names, name)
			}
...
func callbackActionName(mod *goivy.Module, arg goivy.Node) (string, bool) {
	if mod == nil || mod.Actions == nil || arg == nil {
		return "", false
	}
	name := ""
	switch n := arg.(type) {
