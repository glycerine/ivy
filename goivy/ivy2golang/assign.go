...
func (g *Generator) emitAssignLarge(w *goWriter, a *goivy.LogicAssignAction, lhsVs []*goivy.LogicVariable) {
	...
	if sort == nil {
		g.unsupported(w, "emit_assign_large unknown storage symbol %s", name)
		return
	}
	fs, ok := sort.(*goivy.LogicFunctionSort)
	if !ok || !g.goFunctionStorageFor(fs.Domain(), fs.Range()).Large {
		g.unsupported(w, "emit_assign_large requires large function storage: %s", name)
		return
	}
	base := goName(name)
	if owner != "" {
	...
	if err != nil {
		g.unsupported(w, "unsupported thunk assignment body: %s", err.Error())
		g.popScope()
		w.close("")
		return
	}
