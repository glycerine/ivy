...
// Pure ivylogic helpers ported from ivy2cpp's mechanical port of
// ivy_to_cpp.py. These are used by the action-generator planner before any Go
// source emission happens.
...
func minimalFieldReferences(
	...
	var fieldRef func(goivy.Expr) (*goivy.Const, bool)
...
func filterMinimaRefs(refs []goivy.Expr) []goivy.Expr {
	var ltChain func(goivy.Expr, goivy.Expr) bool
	...
	for _, in := range inputs {
		...
		for _, f := range refs {
			...
			if isApp && len(ap.Terms) == 1 {
				...
				if !isConst || !isFn || len(fs.Domain()) == 0 {
					add(in)
					continue
				}
				destrs, ok := sortDestructors.Get2(sortName(fs.Domain()[0]))
				if !ok {
					add(in)
					continue
				}
...
func extractInputFields(
	...
	for _, in := range inputs {
		...
		for _, y := range mrefs[keyOfConst(in)] {
	...
	var recur func(goivy.Expr) goivy.Expr
	recur = func(f goivy.Expr) goivy.Expr {
		...
		if ap, ok := f.(*goivy.Apply); ok {
			...
			if c, ok := ap.Func.(*goivy.Const); ok {
				_, matchRoot = mrefs[keyOfConst(c)]
			}
	...
	for i, d := range pre.Defs {
		newDefs[i] = &goivy.LogicDefinition{Lhs: recur(d.Lhs), Rhs: recur(d.Rhs)}
	}
