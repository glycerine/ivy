...
func GetTheorySchemata(name string, sort Sort, version string) string {
...
func GetSortTheory(sort Sort, interp map[string]interface{}) interface{} {
	name := SortName(sort)
...
// (int, nat, or range).
func HasIntegerInterp(sort Sort, interp map[string]interface{}) bool {
	name := SortName(sort)
	v, ok := interp[name]
	if !ok {
		return false
