...
func CloneNormal(clauses *Clauses, iuCfg *IvyUtilsConfig) *Clauses {
	if clauses == nil {
		return nil
	}
	newFmlas := make([]Expr, 0, len(clauses.Fmlas))
	...
	newDefs := make([]*IvyDefinition, len(clauses.Defs))
	copy(newDefs, clauses.Defs)
	return NewClauses(newFmlas, newDefs, clauses.Annot)
}
...
func normalize(expr Expr, iuCfg *IvyUtilsConfig) Expr {
	if IsMacro(expr, iuCfg) {
		return normalize(ExpandMacro(expr), iuCfg)
	}
	args := NodeArgs(expr)
...
// Corresponds to Python's clone_normal (ivy_mc.py lines 839-849).
func cloneNormal(expr Expr, args []Expr) Expr {
...
func UncomposeAnnot(annot Annotation) []Annotation {
	ca, ok := annot.(*ComposeAnnotation)
	if !ok {
		return nil
	}
	if len(ca.Args) < 2 {
...
func MCUniteAnnot(annot Annotation) []AnnotPair {
	switch a := annot.(type) {
...
type AnnotPair struct {
	Cond  Expr // condition, keyed by lg.NodeKey for structural equality
	Annot Annotation
}
...
func (h *MCMatchHandler) Eval(cond Expr) bool {
	if cond == nil {
		return false
	}
...
func (h *MCMatchHandler) Handle(action ActionsAction, env map[NodeKey]Expr) {
...
func (h *MCMatchHandler) DoReturn(action ActionsAction, env map[NodeKey]Expr) {
...
func MatchAnnotationMC(action ActionsAction, annot Annotation, handler *MCMatchHandler, mod *Module) {
...
func Checked(action ActionsAction, mod *Module) bool {
...
func NewIvyMCTrace(stvals []Expr, mod *Module) *IvyMCTrace {
	ag := NewAnalysisGraph(mod)
	// Set up initial state with the given state values as clauses.
	initClauses := &Clauses{Fmlas: stvals}
	initState := NewState(mod, initClauses)
...
func (t *IvyMCTrace) AddState(stvals []Expr, action ActionsAction) {
	cls := &Clauses{Fmlas: stvals}
	newState := NewState(t.Domain, cls)
	newState.Label = "ext"
...
func AigerWitnessToIvyTrace(witnessFile string, mod *Module) (*IvyMCTrace, error) {
