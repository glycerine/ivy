...
func TestQuantConstraints_RangeSort(t *testing.T) {
	rs := &RangeSort{Name: "myrange", Lb: NumeralBound{Value: "2"}, Ub: NumeralBound{Value: "5"}}
	sig := NewSig()
	sig.Interp["myrange"] = rs
	s := NewSolverFromSig(sig, nil)

	rangeSort := &UninterpretedSort{Name: "myrange"}
	x, _ := NewVariable("X", rangeSort)
...
func TestGebin_OverflowIsFalse(t *testing.T) {
	...
	a := ctx.Const("a", ctx.BoolSort())
	b := ctx.Const("b", ctx.BoolSort())
	bits := []smt.Z3Expr{a, b} // 2 bits can represent 0-3
...
func TestNumeralToZ3_RangeClamping(t *testing.T) {
	rs := &RangeSort{Name: "bounded", Lb: NumeralBound{Value: "0"}, Ub: NumeralBound{Value: "10"}}
	sig := NewSig()
	...
	numNeg := NewConst("-5", &UninterpretedSort{Name: "bounded"})
...
func TestSortCard_RangeSort(t *testing.T) {
	rs := &RangeSort{Name: "bounded", Lb: NumeralBound{Value: "0"}, Ub: NumeralBound{Value: "9"}}
...
func TestSortCard_BV(t *testing.T) {
	...
	sort := &UninterpretedSort{Name: "mybv"}
...
func TestRangeSortBounds_ParsesCorrectly(t *testing.T) {
	rs := &RangeSort{Name: "r", Lb: NumeralBound{Value: "3"}, Ub: NumeralBound{Value: "17"}}
...
func TestCheckSequenceWithReporter(t *testing.T) {
	...
	seq := []AssumeAssert{
		NewAssume(FormulaToClauses(p, nil), "assume_p"),
		NewAssert(FormulaToClauses(p, nil), "assert_p"),
	}
...
func TestCheckSequenceWithReporter_Abort(t *testing.T) {
	...
	seq := []AssumeAssert{
		...
		NewAssert(FormulaToClauses(p, nil), "assert_p"),
		NewAssert(FormulaToClauses(p, nil), "assert_p_again"),
	}
