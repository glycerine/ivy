...
func constCard(n int) CardFunc {
	return func(s lg.Sort) int { return n }
}
...
func TestUnrollLoops_WhileInSequence(t *testing.T) {
	assume1 := NewAssumeAction(lg.True)
	wa := mkWhileActionForTest("T")
	assume2 := NewAssumeAction(lg.True)
...
func TestUnrollLoops_WhileInIf(t *testing.T) {
	wa := mkWhileActionForTest("T")
	assume := NewAssumeAction(lg.True)
	ifAct := NewIfAction(lg.True, wa, assume)
...
func TestUnrollLoops_AndCondition(t *testing.T) {
	...
	xSym := lg.NewConst("x", sortT)
	boundSym := lg.NewConst("bound", sortT)
	ltCond, _ := lg.NewApply(ltSym, xSym, boundSym)
	flag := lg.NewConst("flag", lg.Boolean)
	andCond := &lg.And{Terms: []lg.Expr{ltCond, flag}}
...
func TestUnrollLoops_UnknownCondition(t *testing.T) {
	...
	var receivedSort lg.Sort
	card := CardFunc(func(s lg.Sort) int {
		receivedSort = s
		return 1
	})
...
func TestUnrollLoops_VerifyNesting(t *testing.T) {
	...
	if _, ok := assumeAct.Formula.(*lg.Or); !ok {
