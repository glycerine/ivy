...
func DualFormula(fmla Expr, skolemizer func(*LogicVariable) Expr, instantiator func([]Expr) *Clauses) Expr {
	...
	vars := UsedVariablesOrdered(&Clauses{Fmlas: []Expr{fmla}})
	if len(vars) > 0 {
...
func SkolemizeFormula(fmla Expr, skolemizer func(*LogicVariable) Expr, instantiator func([]Expr) *Clauses) Expr {
	...
	if len(vs) > 0 {
		subs := make(map[string]Expr, len(vs))
		for _, v := range vs {
			subs[v.Name] = skolemizer(v)
		}
		fmla = SubstituteAstByName(fmla, subs)
...
func SkolemizeAst(pos bool, vs []*LogicVariable, usedNames map[string]bool,
	...
	if IsQuantifier(fmla) {
		isExists := IsExists(fmla)
		isForall := IsForall(fmla)
	...
	if _, ok := fmla.(*LogicImplies); ok {
		...
		return CloneNode(fmla, []Expr{newLhs, newRhs})
	}
...
func WitnessAst(pos bool, vs []*LogicVariable, witnesses map[NodeKey]Expr, fmla Node) (Node, error) {
	...
	if expr, ok := fmla.(Expr); ok {
		if IsQuantifier(expr) {
			isExists := IsExists(expr)
			isForall := IsForall(expr)
			...
			if (isExists && !pos) || (isForall && pos) {
				...
				for idx, v := range vars {
					...
					if found {
						...
						termVars := UsedVariables(term)
						for _, w := range vars[idx+1:] {
							if _, used := termVars[Key(w)]; used {
	...
	args := fmla.Args()
	xtracer.Trace("ilu.witnessAst branch type=generic pos=%v exprType=%s nargs=%d", pos, TypeName(fmla), len(args))
...
// net.
func witnessExpr(n Node, fallback Expr) Expr {
	if e, ok := n.(Expr); ok {
		return e
	}
	return fallback
...
func ReskolemizeClauses(clauses *Clauses, skolemizer func(*LogicVariable) Expr) *Clauses {
	...
	for _, c := range consts {
		if strings.Contains(c.Name, "__") {
			v, _ := NewVariable(c.Name, c.CSort)
			subs[Key(c)] = skolemizer(v)
		}
	}
	if len(subs) == 0 {
		return clauses
...
func UnusedConstant(sig *Sig, usedConstants []*Const, sort Sort) *Const {
...
func DefinitionInstances(fmla Expr, instantiator func([]Expr) *Clauses) *Clauses {
	if instantiator == nil {
...
func UnfoldDefinitionsClauses(clauses *Clauses, instantiator func([]Expr) *Clauses) *Clauses {
	if instantiator == nil {
		return clauses
	}
...
func ApplyGenToClauses(gen func(Expr) Expr) func(interface{}) interface{} {
...
func ApplyFuncToClauses(fn func(Expr) Expr) func(interface{}) interface{} {
	return func(cls interface{}) interface{} {
		switch v := cls.(type) {
		case *Clauses:
			return v.Apply(fn)
