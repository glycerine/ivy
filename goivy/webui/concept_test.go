...
func mkSort(name string) *logic.UninterpretedSort {
...
func testDomainSetup() (*CDConceptDomain, *logic.UninterpretedSort) {
	S := mkSort("S")
	X := mkVar("X", S)
	Y := mkVar("Y", S)

	unaryRel := mkFuncSort(S, logic.Boolean)
	binaryRel := mkFuncSort(S, S, logic.Boolean)
