...
describe('static details styling', () => {
  ...
  it('reserves a bottom minibuffer row for CodeMirror search and replace prompts', () => {
    ...
    const horizontalScrollbarRule = ivyCss.match(/#editor-panel \.CodeMirror-hscrollbar,\s*#editor-panel \.CodeMirror-scrollbar-filler,\s*#editor-panel \.CodeMirror-gutter-filler\s*\{[^}]+\}/)?.[0] || '';
    ...
    expect(horizontalScrollbarRule).toContain('bottom: var(--ivy-codemirror-minibuffer-height) !important;');
