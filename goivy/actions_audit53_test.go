...
func TestCopyFieldAction_FourArgs(t *testing.T) {
	...
	dstField := NewConst("fld", fldSort)
	srcField := NewConst("sfld", fldSort)
...
func TestCopyFieldAction_DifferentSourceField(t *testing.T) {
	...
	sortS := actionsMkSort("S")
	fldSort := mkBinaryRelSort(sortT, sortS)
	dstField := NewConst("fld_a", fldSort)
	srcField := NewConst("fld_b", fldSort)
	dst := NewConst("dst", sortT)
	src := NewConst("src", sortT)
...
func TestWhileAction_Unroll_RefusesLargeCard(t *testing.T) {
	...
	card := func(s Sort) int { return 200 }
...
func TestWhileAction_IntUpdate_UsesUnrollContext(t *testing.T) {
	...
	uc := NewUnrollContext(func(s Sort) int { return 2 }, nil, cfg)
	uc.Enter()
	defer uc.Exit()
...
func TestAssignAction_PartialApplication(t *testing.T) {
	...
	lhs := &Apply{Func: fSym, Terms: []Expr{aSym}}
	rhs := &Apply{Func: gSym, Terms: []Expr{aSym}}
...
func TestAssignAction_VariableCheck(t *testing.T) {
	...
	yVar, err := NewVariable("Y", sortS)
	if err != nil {
		t.Fatalf("NewVariable Y: %v", err)
	}

	lhs, _ := NewApply(fSym, xVar)
