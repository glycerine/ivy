...
func (s *Solver) CheckNativeCompatSym(sym *Const) (retErr error) {
	...
	xtracer.Trace("TranslateSort_call callsite=check_native_compat_rng HASH canon=%s", fs.Range().Sexp())
...
func CheckNativeCompatSymStatic(sig *Sig, sym *Const) error {
	xtracer.Trace("ivy_solver.py:350 check_native_compat_sym() ENTER sym=%s", sym)
	if !IsFunctionSort(sym.CSort) {
		return nil
	}
...
func checkSortCompat(sig *Sig, s Sort) error {
...
// Python iterates sig.interp (not sig.symbols), so we do the same.
func CheckCompatStatic(sig *Sig) []error {
	xtracer.Trace("ivy_solver.py:374 check_compat() ENTER")
	var errs []error
...
func TermsMatch(tl1, tl2 []Expr) bool {
...
func (s *Solver) GetArgRange(model *HerbrandModel, x *Const) []Expr {
	xtracer.Trace("ivy_solver.py:846 get_arg_range() ENTER")
	sort := SortRange(x.CSort)
	universe := model.SortUniverse(sort)
	result := make([]Expr, len(universe))
...
func (s *Solver) ModelIfNone(clauses *Clauses, implied *Clauses, model *HerbrandModel) *HerbrandModel {
...
func (s *Solver) CollectModelValues(sort Sort, model *HerbrandModel, sym *Const) []*Const {
...
func NumeralAssignWithClauses(model *HerbrandModel, clauses *Clauses) map[string]string {
	result := make(map[string]string)

	// Collect existing numerals from clauses, grouped by sort
	numBySort := make(map[string][]*Const)
	if clauses != nil {
		usedConsts := ConstantsClauses(clauses)
		for _, c := range usedConsts {
			if IsNumeral(c) {
				sortName := IvySortName(SortRange(c.CSort))
...
func (s *Solver) MineInterpretedConstants(vocab []*Const) map[string][]*Const {
	xtracer.Trace("ivy_solver.py:891 mine_interpreted_constants() ENTER")
	result := make(map[string][]*Const)
	for _, c := range vocab {
		sortName := IvySortName(SortRange(c.CSort))
		if !IsInterpretedSort(s.sig, c.CSort) {
			continue
		}
...
func GetPolymacs(op string) func([]Expr) Expr {
