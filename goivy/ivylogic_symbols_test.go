...
func testConst(name string) *Const {
...
func testApply(fn Expr, terms ...Expr) *Apply {
...
func collectSyms(node Expr) []Expr {
	var result []Expr
...
func symNames(syms []Expr) []string {
...
func TestSymbolsIluAst_EarlyStop(t *testing.T) {
	...
	if c, ok := first.(*Const); !ok || c.Name != "f" {
...
func TestUsedSymbolsAsts_MultipleNodes(t *testing.T) {
	...
	a := testConst("a")
	b := testConst("b")
	nodes := []Expr{testApply(f, a), testApply(g, b)}

	syms := UsedSymbolsAsts(nodes)
	if syms.Len() != 4 {
		t.Errorf("expected 4 unique symbols, got %d", syms.Len())
	}
	for _, c := range []*Const{f, g, a, b} {
		if _, ok := syms.Get2(Key(c)); !ok {
...
func TestUsedSymbolsAsts_OverlapDedup(t *testing.T) {
	...
	g := testConst("g")
	a := testConst("a")
	nodes := []Expr{testApply(f, a), testApply(g, a)}
