...
func CheckIsolate(mod *Module, traceHook func(interface{}) interface{}) error {
	...
	if mod.IsolateProof != nil {
		...
		pfNode, _ := mod.IsolateProof.(Node)
		var err error
	...
	if err := CheckFragment(mod, false); err != nil {
	...
	axioms := make([]*LabeledFormula, 0)
	schemaInstances := make([]*LabeledFormula, 0)
	...
	if (len(mod.LabeledProps) > 0 || len(schemaInstances) > 0) &&
		...
		if check {
			...
			pre := NewState(mod, TrueClauses(EmptyAnnotation{}))
			ag.Add(pre, nil)

			nonTemporal := make([]*LabeledFormula, 0)
	...
	var checkedInvariants []*LabeledFormula
	...
	if mod.IsolateInfo != nil && len(mod.IsolateInfo.Implementations) > 0 {
		...
		impls := make([]MixinTriple, len(mod.IsolateInfo.Implementations))
	...
	if mod.IsolateInfo != nil && len(mod.IsolateInfo.Monitors) > 0 {
		...
		mons := make([]MixinTriple, len(mod.IsolateInfo.Monitors))
	...
	if len(checkedInvariants) > 0 && mod.Cfg.CheckedAction == "" && !mod.Cfg.OnlyCheckUnprovable {
		...
		if check {
			ag := NewAnalysisGraph(mod)
			ag.Initialize(AbstractorFunc(func(s *State) {})) // no-op abstractor, matching Python
			if len(ag.States) > 0 {
	...
	if len(mod.Initializers) > 0 {
		...
		if len(guarantees) > 0 && !mod.Cfg.OnlyCheckUnprovable {
			...
			if check {
				ag := NewAnalysisGraph(mod)
				ag.Initialize(AbstractorFunc(func(s *State) {})) // no-op abstractor
				if len(ag.States) > 0 {
					initState := ag.States[0]

					failState := NewState(mod, TrueClauses(EmptyAnnotation{}))

					var innerAction ActionsAction
					if aa, ok := initState.Prov.(*ActionApp); ok {
						switch rep := aa.Rep.(type) {
						case string:
							failState.Prov = NewActionApp("fail_"+rep, aa.Args...)
							if v, ok := mod.Actions.Get2(rep); ok {
								if a, ok := v.(ActionsAction); ok {
									innerAction = a
								}
							}
						case ActionsAction:
							failState.Prov = NewActionApp("fail_"+rep.Name(), aa.Args...)
							innerAction = rep
						}
					}
					if innerAction == nil {
					...
					if innerAction != nil {
						failState.Action = NewFailAction(innerAction)
						if initState.Pred != nil {
							failState.Pred = initState.Pred
						} else if aa, ok := initState.Prov.(*ActionApp); ok && len(aa.Args) > 0 {
	...
	if len(checkedActions) > 0 && len(checkedInvariants) > 0 {
		...
		for _, actname := range actionOrder {
			...
			action := BuildEnvAction(mod.Cfg.ActCfg, mod.PublicActions, mod.Actions, actname, "")
	...
	if AssertLocEnabled {
		for actname, action := range mod.Actions.All() {
			if a, ok := action.(ActionsAction); ok {
				AssertEveryActionHasLoc(a, "check.isolate_check.print actname="+actname)
			}
		}
	}
			...
			for _, sub := range guarantees {
				...
				if anyUntried && check {
					...
					for _, root := range actionOrder {
						...
						envAction := BuildEnvAction(mod.Cfg.ActCfg, mod.PublicActions, mod.Actions, root, "")
						...
						if post != nil {
							...
							failState.Action = NewFailAction(envAction)
							if post.Pred != nil {
								failState.Pred = post.Pred
							} else if aa, ok := post.Prov.(*ActionApp); ok && len(aa.Args) > 0 {
								failState.Pred = aa.Args[0]
							}
...
func CheckSubgoals(goals []*LabeledFormula, method func(*Module) error, mod *Module) error {
	...
	for _, goal := range goals {
		_ = GoalConc(goal) // used for non-temporal branch via goal itself
		...
		if tm != nil {
			...
			axiomsCopy := make([]*LabeledFormula, len(withLocalMod.LabeledAxioms))
			...
			for _, premNode := range goalPrems {
				...
				premLF, isLF := premNode.(*LabeledFormula)
				if isLF && GoalIsProperty(premLF) {
					if premLF.IsDefinition {
						// Python: df = lg.drop_universals(prem.formula)
						//         mod.updates.append(act.DerivedUpdate(df))
						if fmla, ok := premLF.Formula.(Expr); ok {
							df := IvyDropUniversals(fmla)
				...
				} else if GoalIsDefn(premNode) {
					dfnd := GoalDefines(premNode)
					if dfnd != nil && IsConstant(dfnd) {
						if sym, ok := dfnd.(*Const); ok {
			...
			ws := NewWithSymbols(withLocalMod.Sig, vocab.Symbols)
			ws.Enter()
			wsorts := NewWithSorts(withLocalMod.Sig, vocab.Sorts)
			wsorts.Enter()
			if method != nil {
...
func CheckModule(mod *Module) error {
	if xtracer.Enabled {
	...
	if mod.Cfg.Isolate != "" {
		...
		if mod.Cfg.Coverage {
			if missing := CheckIsolateCompleteness(mod); len(missing) > 0 {
	...
	if mod.Cfg.OptUncheckedProps != "" {
		var err error
		aclCfg, err = RegisterFromFile(mod.Cfg.OptUncheckedProps)
		if err != nil {
			return err
		}
	}
	...
	for _, isolate := range isolates {
		...
		isoMod := mod.Copy()
		if err := CreateIsolate(isolate, isoMod); err != nil {
...
func MCIsolate(isolate string, mod *Module, method func(*Module) error) error {
	for _, p := range mod.LabeledProps {
		if !p.IsTemporal() {
...
func GetIsolateMethod(isolate string, mod *Module) string {
	if mod.Cfg.OptMC {
		return "mc"
	}
...
func GetIsolateAttr(isolate, attrName, defaultVal string, mod *Module) string {
	if isolate == "" {
		return defaultVal
	}
...
func CheckSeparately(isolate string, mod *Module) bool {
...
func AllAssertLinenos(mod *Module) ([]Location, error) {
	seen := make(map[string]bool)
	var result []Location

	for _, action := range mod.Actions.All() {
		if act, ok := action.(interface {
			IterSubactions() []ActionsAction
		}); ok {
			for _, sub := range act.IterSubactions() {
				isAssert := IsAssertLike(sub)
				_, isRanking := sub.(*LogicRanking)
...
func CheckConjsInStateWithAG(mod *Module, ag *AnalysisGraph, post *State, indent int, pcs []*LabeledFormula) bool {
...
func filterExplicitSubgoals(props []*LabeledFormula, subgoalMap map[int64]bool) []*LabeledFormula {
