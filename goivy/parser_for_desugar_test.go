...
func TestMethcall(t *testing.T) {
	...
	if _, ok := result.(*MethodCall); ok {
		t.Error("expected composed node for App with no args, got MethodCall")
	}
	if NodeRep(result) != "x.begin" {
		t.Errorf("expected rep 'x.begin', got %q", NodeRep(result))
	}
...
func TestForLoopDesugaring(t *testing.T) {
	...
	if NodeRep(methcall(cfg, fmla, appBegin)) != "rng.begin" {
		t.Errorf("expected 'rng.begin', got %q", NodeRep(methcall(cfg, fmla, appBegin)))
	}
...
func TestForLoopMethcallCompose(t *testing.T) {
	...
	if NodeRep(result) != "rng.begin" {
		t.Errorf("expected 'rng.begin', got %q", NodeRep(result))
	}
	...
	if _, ok := resultValue.(*MethodCall); ok {
...
func TestLocalActionDesugaring(t *testing.T) {
	...
	action := SubstPrefixAtomsAst(body, subst, nil, nil, nil)
