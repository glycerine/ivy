...
func DefaultConfig(mod *Module, nSteps int) *BMCConfig {
	return &BMCConfig{
		NSteps: nSteps,
		Module: mod,
	}
...
func BMCCheckIsolate(cfg *BMCConfig) *BMCResult {
	...
	var oldActions *InsMap[string, Action]
	if cfg.NUnroll != nil {
	...
	for n := 0; n <= nSteps; n++ {
		...
		result := CheckFinalCond(ag, post, dualConj, nil, true)
		if result != nil {
...
func BuildConjecture(mod *Module) *Clauses {
	...
	for _, lc := range mod.LabeledConjs {
		if lc.Formula != nil {
			fmlas = append(fmlas, lc.Formula.(Expr))
		}
	}
	if len(fmlas) == 0 {
		return TrueClauses(nil)
...
func UnrollAction(act ActionsAction, n int) ActionsAction {
	if act == nil {
		return nil
	}
