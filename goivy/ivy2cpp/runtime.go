...
func (g *Generator) emitRuntimeImplPreamble(w *cppWriter) {
	...
	if g.Config.Target == "gen" {
		...
		w.open("if (!truth) {")
		w.line(`std::cerr << msg << ": error: assertion failed" << std::endl;`)
		w.line("__ivy_exit(1);")
		w.close("")
		w.close("")
		w.open("void ivy_assume(bool truth, const char *msg) {")
		w.open("if (!truth) {")
		w.line(`std::cerr << msg << ": error: assumption failed" << std::endl;`)
		w.line("__ivy_exit(1);")
		w.close("")
		w.close("")
		w.open("void ivy_check_progress(int, int) {")
		w.close("")
		w.open("int choose(int, int) {")
		w.line("return 0;")
		w.close("")
		w.line("#endif")
	}
