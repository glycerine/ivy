...
func (g *Generator) emitXBVClassImpl(w *cppWriter, s goivy.Sort, it cppInterpType) {
	...
	w.linef("value = %s::bv_to_x(__chacha8c_rng.Rand() %% (1<<%d));", typ, it.Bits)
	w.close(" else {")
	...
	w.close("")
	w.close("")
	w.linef("value = %s::nonces[__chacha8c_rng.Rand() %% %s::nonces.size()];", typ, typ)
	w.close("")
	...
	if it.Kind == cppInterpStrBV {
		...
		w.linef("return %s((__chacha8c_rng.Rand()%%%d) + %d);", base, it.Hi-it.Lo+1, it.Lo)
		w.close("")
	}
	w.line("#endif")
	w.blank()
