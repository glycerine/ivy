...
func TestBinDecBool(t *testing.T) {
	trueExpr := lg.Expr(&lg.And{Terms: nil})
	falseExpr := lg.Expr(&lg.Or{Terms: nil})
	...
	tests := []struct {
		...
		{[]lg.Expr{falseExpr}, 0},
		{[]lg.Expr{trueExpr}, 1},
		{[]lg.Expr{falseExpr, trueExpr}, 2},
		{[]lg.Expr{trueExpr, trueExpr}, 3},
		{[]lg.Expr{trueExpr, falseExpr, trueExpr}, 5},
	}
...
func TestDecodeValBoolean(t *testing.T) {
	...
	val := enc.DecodeVal([]lg.Expr{trueExpr}, sym)
	if !isTrueNode(val) {
		t.Error("expected true for boolean bit '1'")
	}
	val = enc.DecodeVal([]lg.Expr{falseExpr}, sym)
	if !isFalseNode(val) {
...
func TestDecodeValEnumerated(t *testing.T) {
	...
	enc := &Encoder{Interp: make(map[string]interface{})}
	sym := lg.NewConst("c", es)

	trueExpr := lg.Expr(&lg.And{Terms: nil})
	falseExpr := lg.Expr(&lg.Or{Terms: nil})

	val := enc.DecodeVal([]lg.Expr{falseExpr, falseExpr}, sym)
	if c, ok := val.(*lg.Const); !ok || c.Name != "red" {
		t.Errorf("expected red, got %v", val)
	}

	val = enc.DecodeVal([]lg.Expr{trueExpr, falseExpr}, sym)
	if c, ok := val.(*lg.Const); !ok || c.Name != "green" {
		t.Errorf("expected green, got %v", val)
	}

	val = enc.DecodeVal([]lg.Expr{falseExpr, trueExpr}, sym)
	if c, ok := val.(*lg.Const); !ok || c.Name != "blue" {
		t.Errorf("expected blue, got %v", val)
	}

	// Clamp: index 3 should clamp to last element
	val = enc.DecodeVal([]lg.Expr{trueExpr, trueExpr}, sym)
	if c, ok := val.(*lg.Const); !ok || c.Name != "blue" {
...
func TestDecodeValRange(t *testing.T) {
	rs := &lg.RangeSort{
		...
		Ub:   lg.NumeralBound{Value: "3"},
	}
	enc := &Encoder{Interp: map[string]interface{}{"idx": rs}}
	sym := lg.NewConst("i", rs)

	trueExpr := lg.Expr(&lg.And{Terms: nil})
	falseExpr := lg.Expr(&lg.Or{Terms: nil})

	val := enc.DecodeVal([]lg.Expr{trueExpr, trueExpr}, sym)
	if c, ok := val.(*lg.Const); !ok || c.Name != "3" {
		t.Errorf("expected 3 (clamped), got %v", val)
	}

	val = enc.DecodeVal([]lg.Expr{trueExpr, falseExpr}, sym)
	if c, ok := val.(*lg.Const); !ok || c.Name != "1" {
		t.Errorf("expected 1, got %v", val)
	}
}

func TestGetSymReturnsDecodedValue(t *testing.T) {
	boolSym := lg.NewConst("x", lg.Boolean)
	enc := NewEncoder([]*lg.Const{boolSym}, nil, nil)
...
func TestGetNextSymReturnsNextState(t *testing.T) {
	boolSym := lg.NewConst("s", lg.Boolean)
	enc := NewEncoder(nil, []*lg.Const{boolSym}, nil)
	enc.Interp = make(map[string]interface{})

	// Set next-state of latch to true (literal 1)
	subBits := enc.Encoding[lg.Key(boolSym)]
...
func TestGetEncoderState(t *testing.T) {
	s0 := lg.NewConst("s0", lg.Boolean)
	s1 := lg.NewConst("s1", lg.Boolean)
	enc := NewEncoder(nil, []*lg.Const{s0, s1}, nil)
	enc.Interp = make(map[string]interface{})

	stmap := enc.GetEncoderState("10")
	v0, ok0 := stmap[lg.Key(s0)]
	v1, ok1 := stmap[lg.Key(s1)]
	if !ok0 || !ok1 {
...
func TestAigerMatchHandler2Clone(t *testing.T) {
	decoder := map[string]lg.Expr{"x": lg.NewConst("x", lg.Boolean)}
	h := NewAigerMatchHandler2(nil, decoder, nil, nil, nil)
	h.States = append(h.States, []lg.Expr{lg.NewConst("a", lg.Boolean)})
...
func TestAigerMatchHandler2AddState(t *testing.T) {
	h := NewAigerMatchHandler2(nil, nil, nil, nil, nil)
	eqns := []lg.Expr{
		&lg.Eq{T1: lg.NewConst("x", lg.Boolean), T2: &lg.And{Terms: nil}},
	}
	h.AddState(eqns)
	if len(h.States) != 1 {
...
func TestAigerMatchHandler2EndCallsFinalState(t *testing.T) {
	...
	decoder := map[string]lg.Expr{boolSym.Name: boolSym}
...
func TestAigerWitnessToIvyTrace2CallsMatchAnnotation(t *testing.T) {
	...
	input := lg.NewConst("inp", lg.Boolean)
	latch := lg.NewConst("st", lg.Boolean)
	output := lg.NewConst("out", lg.Boolean)

	enc := NewEncoder([]*lg.Const{input}, []*lg.Const{latch}, []*lg.Const{output})
	enc.Interp = make(map[string]interface{})

	// Set latch next-state = input (simple pass-through)
	inpBits := enc.Encoding[lg.Key(input)]
	latchBits := enc.Encoding[lg.Key(latch)]
	inpLit, _ := enc.Sub.Lit(inpBits[0])
	enc.Sub.Set(latchBits[0], inpLit)

	// Set output = latch
	outBits := enc.Encoding[lg.Key(output)]
...
func TestAigerMatchHandler2HandleDelegation(t *testing.T) {
	...
	callAction := actions.NewSequence()
	callAction.SetLineno(ast.Location{Line: 1, Filename: "test.ivy"})

	// First call: should set LastAction
	emptyEnv := make(map[lg.NodeKey]lg.Expr)
