...
func TestThunkActionFields(t *testing.T) {
	...
	ta := cfg.NewThunkAction(label, action, sort, body)
	if NodeRep(ta.Label) != "lab" {
		t.Errorf("expected label 'lab', got %q", NodeRep(ta.Label))
	}
	if NodeRep(ta.Action) != "act" {
		t.Errorf("expected action 'act', got %q", NodeRep(ta.Action))
	}
	if NodeRep(ta.Sort) != "s" {
		t.Errorf("expected sort 's', got %q", NodeRep(ta.Sort))
	}
}
