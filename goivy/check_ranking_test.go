...
var rankingTestAstCfg = NewAstConfig()

// --- helpers ---

func checkBoolVar(name string) *LogicVariable {
	v, _ := NewVariable(name, Boolean)
	return v
}

func checkBoolConst(name string) *Const {
	return NewConst(name, Boolean)
}

func checkUnintSort(name string) Sort {
...
func TestCheckForAllWithVars(t *testing.T) {
	v := checkBoolVar("X")
	body := True
	result := CheckForAll([]*LogicVariable{v}, body)
	fa, ok := result.(*ForAll)
	if !ok {
...
func TestCheckExistsWithVars(t *testing.T) {
	v := checkBoolVar("X")
	body := True
	result := CheckExists([]*LogicVariable{v}, body)
...
func TestCheckOldOfImplies(t *testing.T) {
	...
	ri, ok := result.(*LogicImplies)
	if !ok {
		t.Fatalf("expected Implies, got %T", result)
	}
	if ri.T1.(*Const).Name != "old_A" {
...
func TestCheckOldOfForAll(t *testing.T) {
	...
	rfa, ok := result.(*ForAll)
	if !ok {
		t.Fatalf("expected CheckForAll, got %T", result)
	}
	if rfa.Body.(*Const).Name != "old_P" {
...
func TestCheckOldOfExists(t *testing.T) {
	...
	rex, ok := result.(*LogicExists)
	if !ok {
		t.Fatalf("expected CheckExists, got %T", result)
	}
	if rex.Body.(*Const).Name != "old_P" {
		t.Error("body not renamed")
	}
}
...
func TestCheckTrigGlobAnd(t *testing.T) {
	body := checkBoolConst("P")
	g, _ := NewGlobally(nil, body)
	and := &LogicAnd{Terms: []Expr{g, checkBoolConst("Q")}}
...
func TestCheckConvertToInitAnd(t *testing.T) {
	...
	for _, term := range ra.Terms {
		if _, ok := term.(*LogicNamedBinder); !ok {
...
func TestCheckDesugarNonBinder(t *testing.T) {
	c := checkBoolConst("P")
	result := RankingDesugar(c, "label", True)
...
func TestCheckDesugarNil(t *testing.T) {
	result := RankingDesugar(nil, "label", True)
	if result != nil {
...
func TestCheckL2STacticWithLets(t *testing.T) {
	...
	cfg := &L2STacticConfig{
		Goals: []*LabeledFormula{rankingTestAstCfg.NewLabeledFormula(nil, rankingTestAstCfg.NewAtom("true"))},
		Proof: &LogicProofDecl{TacticLets: []Node{rankingTestAstCfg.NewAtom("x")}},
	}
	_, err := RankingL2STactic(cfg)
	if err == nil {
...
func TestCheckModelPassNil(t *testing.T) {
	// Should not panic
	ModelPass(nil, func(n Node) Node { return n })
}
...
func TestCheckModelPassTransform(t *testing.T) {
	...
	model := &NormalProgram{
		Invars: []*LabeledFormula{
			cfg.NewLabeledFormula(nil, True),
		},
		Asms: []*LabeledFormula{
			cfg.NewLabeledFormula(nil, True),
		},
	...
	called := 0
	ModelPass(model, func(n Node) Node {
		called++
		return n
	})
	if called != 2 {
...
func TestCheckRegisterAndGetTactic(t *testing.T) {
	called := false
	RegisterTactic("test_tactic", func(cfg *L2STacticConfig) ([]*LabeledFormula, error) {
		called = true
		return nil, nil
	})
...
func TestCheckRankingModPass_PropertyPremsTransformed(t *testing.T) {
	...
	prems := []Node{propPrem, schemaPrem}

	// Define a transform that wraps the formula in Not (handles LabeledFormula).
	negate := func(n Node) Node {
		if lf, ok := n.(*LabeledFormula); ok {
			return lf.Clone([]Node{lf.Label, &LogicNot{Body: lf.Formula.(Expr)}})
		}
		return &LogicNot{Body: n.(Expr)}
	}
	...
	for i, p := range prems {
		...
		prems[i] = negate(lf).(*LabeledFormula)
	}
	// Also transform model invars.
	for i, inv := range model.Invars {
		model.Invars[i] = negate(inv).(*LabeledFormula)
	}

	// Property prem should be transformed (Not wrapped).
	transformedPrem, ok := prems[0].(*LabeledFormula)
	if !ok {
		t.Fatal("expected LabeledFormula")
	}
	if _, ok := transformedPrem.Formula.(*LogicNot); !ok {
	...
	schemaPremResult, ok := prems[1].(*LabeledFormula)
	if !ok {
		t.Fatal("expected LabeledFormula")
	}
	if _, ok := schemaPremResult.Formula.(*SchemaBody); !ok {
		t.Errorf("schema prem should NOT be transformed, got %T", schemaPremResult.Formula)
	}

	// Model invar should also be transformed.
	if _, ok := model.Invars[0].Formula.(*LogicNot); !ok {
...
func TestCheckRankingModPass_PropertyPremClonePreserve(t *testing.T) {
	...
	cloned := origLF.Clone([]Node{origLF.Label, origLF.Formula.(Expr)}).(*LabeledFormula)
...
func TestCheckRankingInvarsMerge(t *testing.T) {
	...
	modelInvar := cfg.NewLabeledFormula(checkBoolConst("model_inv"), True)
	model := &NormalProgram{
		Invars: []*LabeledFormula{modelInvar},
	}

	// Ranking-generated invars (separate list).
	rankInvar1 := cfg.NewLabeledFormula(checkBoolConst("rank_inv1"), True)
	rankInvar2 := cfg.NewLabeledFormula(checkBoolConst("rank_inv2"), True)
	invars := []*LabeledFormula{rankInvar1, rankInvar2}
...
func TestCheckRankingInvarsMerge_SharedStep3Visible(t *testing.T) {
	...
	// Merge.
	invars := []*LabeledFormula{rankInvar}
	model.Invars = append(model.Invars, invars...)
...
// TestWhenOperatorInvarGeneration verifies that LogicWhenOperator{Name:"first"}
...
func TestCheckWhenOperatorInvarGeneration(t *testing.T) {
	...
	wo := &LogicWhenOperator{Name: "first", T1: checkBoolConst("cond"), T2: checkBoolConst("val")}

	// Collect temporals — should find the WhenOperator
	temporals := TemporalsAst(wo)
	if len(temporals) == 0 {
		t.Fatal("TemporalsAst should find WhenOperator")
	}
	found := false
	for _, tmp := range temporals {
		if w, ok := tmp.(*LogicWhenOperator); ok && w.Name == "first" {
			found = true
		}
	}
	if !found {
		t.Error("Bug 14 regression: should find LogicWhenOperator{first} in temporals")
	}
}

// TestWhenOperatorInvarGeneration_NoFirst verifies that LogicWhenOperator{Name:"next"}
// does NOT generate invariants.
func TestCheckWhenOperatorInvarGeneration_NoFirst(t *testing.T) {
	wo := &LogicWhenOperator{Name: "next", T1: checkBoolConst("cond"), T2: checkBoolConst("val")}
	temporals := TemporalsAst(wo)
	for _, tmp := range temporals {
		if w, ok := tmp.(*LogicWhenOperator); ok && w.Name == "first" {
			t.Error("should NOT find LogicWhenOperator{first} when name is 'next'")
			_ = w
		}
	}
}
...
func TestCheckAddParamsToD_AcceptsNonUninterpSort(t *testing.T) {
	...
	us := &UninterpretedSort{Name: "node"}
	fs, _ := NewFunctionSort(us, Boolean)
