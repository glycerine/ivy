...
func makeLabelOnlyProof(label string, proof Node) ProofEntry {
	cfg := NewAstConfig()
	lf := cfg.NewLabeledFormula(cfg.NewAtom(label), nil)
	return ProofEntry{Formula: lf, Proof: proof}
}

// helper: make a direct proof entry (formula is non-nil).
func makeDirectProof(label string, formula Node, proof Node) ProofEntry {
	cfg := NewAstConfig()
...
func makePropFormula(label string) *LabeledFormula {
	cfg := NewAstConfig()
...
func proofNode(name string) Node {
	cfg := NewAstConfig()
...
func TestAttachProofs_EmptyLabelErrors(t *testing.T) {
	...
	mod.Proofs = []ProofEntry{{Formula: lf, Proof: proofNode("proof")}}
...
func FuzzAttachProofs(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, nProps, nConjs, nIsos, nProofs uint8, seed uint64) {
		...
		for i := uint8(0); i < nIsos; i++ {
			...
			mod.Isolates[name] = acfg.NewIsolateDef(nil, 0)
			allLabels = append(allLabels, name)
		}
