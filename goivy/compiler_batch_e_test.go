...
func TestCompileIfAction_SomeCondition(t *testing.T) {
	...
	someCond := cfg.NewSome([]Node{xParam}, fmla)
...
func TestCompileIfAction_SomeMinMaxCondition(t *testing.T) {
	...
	tSort := &UninterpretedSort{Name: "t"}
	c.Sig.Sorts.Set("t", tSort)
	idxSort := &UninterpretedSort{Name: "nat"}
	...
	c.Sig.Symbols.Set("idx", &SymbolEntry{Name: "idx", Sort: idxSort})
	...
	someCond := cfg.NewSomeMin([]Node{xParam}, fmla, idxExpr)
...
func TestCompileIfAction_SomeMaxCondition(t *testing.T) {
	...
	c.Sig.Symbols.Set("idx", &SymbolEntry{Name: "idx", Sort: tSort})
	...
	someCond := cfg.NewSomeMax([]Node{xParam}, fmla, idxExpr)
...
func TestCompileIfAction_SomeMultipleParams(t *testing.T) {
	...
	someCond := cfg.NewSome([]Node{xParam, yParam}, fmla)
...
func TestCompileWhile_SomeCondition(t *testing.T) {
	...
	result, err := c.CompileWhile(someCond, bodyNode, []Node{invNode})
	if err != nil {
	...
	_, isSome := whileAct.Cond.(*SomeCondition)
	if !isSome {
...
func TestCompileWhile_SomeMinCondition(t *testing.T) {
	...
	xParam.ASort = cfg.NewAtom("t")
	fmla := cfg.NewAtom("=", cfg.NewAtom("x"), cfg.NewAtom("x"))
	someCond := cfg.NewSomeMin([]Node{xParam}, fmla, cfg.NewAtom("idx"))
	...
	someCd, ok := whileAct.Cond.(*SomeCondition)
	if !ok {
...
func newThunkWith5Args(label, action, sort, body, cont Node) *thunkWith5Args {
	cfg := NewAstConfig()
...
func (t *thunkWith5Args) Args() []Node {
	return []Node{t.inner.Label, t.inner.Action, t.inner.Sort, t.inner.Body, t.continuation}
}

func (t *thunkWith5Args) Clone(args []Node) Node {
	cfg := t.Cfg
	return &thunkWith5Args{
		Base:         t.Base,
...
func TestCompileThunkAction_RegistersRunAction(t *testing.T) {
	...
	// Declare thunk sort
	thunkSort := &UninterpretedSort{Name: "handler"}
...
func TestCompileThunkAction_CreatesDestructors(t *testing.T) {
	...
	c.Sig.Symbols.Set("fml:x", &SymbolEntry{Name: "fml:x", Sort: xSort})
...
func TestCompileThunkAction_SelfParam(t *testing.T) {
	...
	act, ok := runAction.(ActionsAction)
	if !ok {
...
func TestCompileThunkAction_PreservesLineno(t *testing.T) {
	...
	node.SetLineno(Location{Line: 42})
...
func insMapKeys(m *InsMap[string, Action]) []string {
