...
func mkTestUpdate(modNames []string, tr Expr, pre Expr) *Update {
	...
	if modNames == nil {
		return &Update{
			...
			Pre:         FormulaToClauses(pre, nil),
		}
	}
	mods := make([]*Const, 0, len(modNames))
...
func mkEq(a, b string) Expr {
	eq, _ := NewEq(actionsMkConst(a), actionsMkConst(b))
	return eq
}
...
func TestActionsExistQuantRenames(t *testing.T) {
	fmla := mkEq("x", "y")
	xSym := NewConst("x", TopS)
	result := ExistQuant([]*Const{xSym}, fmla)
...
func TestActionsExistQuantMapReturnsMap(t *testing.T) {
	fmla := mkEq("x", "y")
	xSym := NewConst("x", TopS)
	m, result := ExistQuantMap([]*Const{xSym}, fmla)
	if m == nil {
		t.Fatal("ExistQuantMap should return non-nil map")
	}
	if _, ok := m[Key(xSym)]; !ok {
...
func TestActionsComposeUpdatesBasic(t *testing.T) {
	...
	u1 := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
	u2 := mkTestUpdate([]string{"y"}, mkEq("new_y", "y"), False)
...
func TestActionsComposeUpdatesOverlapping(t *testing.T) {
	u1 := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
	u2 := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
	result := ComposeUpdates(u1, TrueClauses(nil), u2)
	if result == nil {
		t.Fatal("ComposeUpdates returned nil")
	}
...
func TestActionsComposeUpdatesWithAxioms(t *testing.T) {
	u1 := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
	u2 := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
	axiom := FormulaToClauses(mkEq("x", "x"), nil)
...
func TestActionsJoinActionPreservesPreFalse(t *testing.T) {
	u1 := mkTestUpdate(nil, True, False)
	u2 := mkTestUpdate(nil, True, False)
	result := JoinAction(u1, u2, TrueClauses(nil))
...
func TestActionsIteActionBasic(t *testing.T) {
	...
	u1 := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
	u2 := mkTestUpdate([]string{"y"}, mkEq("new_y", "y"), False)
	result := IteAction(cond, u1, u2, TrueClauses(nil))
	if result == nil {
...
func TestActionsIteActionSameModified(t *testing.T) {
	...
	u2 := mkTestUpdate([]string{"x"}, True, False)
	result := IteAction(cond, u1, u2, TrueClauses(nil))
	if len(result.Modified) != 1 {
		t.Errorf("Modified len = %d, want 1", len(result.Modified))
	}
...
func TestActionsHideQuantifiesSymbols(t *testing.T) {
	...
	result := Hide([]*Const{actionsMkConst("y")}, u)
	if result == nil {
...
func TestActionsHideNilModifiedQuantifies(t *testing.T) {
	...
	u := mkTestUpdate(nil, mkEq("x", "y"), False)
	result := Hide([]*Const{actionsMkConst("x")}, u)
...
func TestActionsStateToActionRenames(t *testing.T) {
	...
	u := mkTestUpdate([]string{"x"}, mkEq("x", "old_x"), False)
...
func TestActionsStateToActionEmptyModified(t *testing.T) {
	u := mkTestUpdate(nil, mkEq("a", "b"), False)
	result := StateToAction(u)
...
func TestActionsActionToStateRenames(t *testing.T) {
	...
	u := mkTestUpdate([]string{"x"}, mkEq("new_x", "x"), False)
...
func TestActionsForwardImageWithUpdate(t *testing.T) {
	...
	u := mkTestUpdate([]string{"x"}, mkEq("new_x", "const_a"), False)
	pre := TrueClauses(nil)
	result := ForwardImage(pre, TrueClauses(nil), u)
	if result == nil {
		t.Fatal("ForwardImage returned nil")
	}
...
func TestActionsConstrainStatePrePreserved(t *testing.T) {
	u := mkTestUpdate([]string{"x"}, True, mkEq("err", "x"))
...
func TestActionsConditionUpdateOnFmla(t *testing.T) {
	u := mkTestUpdate([]string{"x"}, mkEq("new_x", "val"), False)
...
func TestActionsFrameUpdate(t *testing.T) {
	u := mkTestUpdate([]string{"x"}, mkEq("new_x", "val"), False)
	result := FrameUpdate(u, []*Const{actionsMkConst("x"), actionsMkConst("y"), actionsMkConst("z")})
	if result == nil {
...
func TestActionsFrameUpdateNoNewSymbols(t *testing.T) {
	u := mkTestUpdate([]string{"x", "y"}, True, False)
	result := FrameUpdate(u, []*Const{actionsMkConst("x"), actionsMkConst("y")})
...
func TestActionsAddPostAxioms(t *testing.T) {
	u := mkTestUpdate([]string{"x"}, mkEq("new_x", "val"), False)
	axiom := mkEq("x", "x")
	result := AddPostAxioms(u, FormulaToClauses(axiom, nil))
	if result == nil {
...
func TestActionsHideState(t *testing.T) {
	u := mkTestUpdate([]string{"x", "y"}, mkEq("x", "old_x"), False)
	result := HideState([]*Const{actionsMkConst("y")}, u)
	// y should be removed from Modified
	for _, s := range result.Modified {
		if s.Name == "y" {
...
func TestActionsHideStateNilModified(t *testing.T) {
	u := mkTestUpdate(nil, mkEq("x", "y"), False)
	result := HideState([]*Const{actionsMkConst("x")}, u)
	if !result.ModifiedAll {
...
func TestActionsReverseImageBasic(t *testing.T) {
	u := mkTestUpdate([]string{"x"}, mkEq("new_x", "const_a"), False)
	post := TrueClauses(nil)
	result := ReverseImage(post, TrueClauses(nil), u)
	if result == nil {
...
func TestActionsHistoryForwardStepMultiple(t *testing.T) {
	cfg := NewIvyUtilsConfig()
	h := NewHistory(cfg, PureStateClauses(TrueClauses(nil)))
	u1 := mkTestUpdate([]string{"x"}, mkEq("new_x", "val1"), False)
	u2 := mkTestUpdate([]string{"y"}, mkEq("new_y", "val2"), False)
	h2 := h.ForwardStep(TrueClauses(nil), u1, True)
	h3 := h2.ForwardStep(TrueClauses(nil), u2, True)
	if len(h3.Maps) != 2 {
...
func TestActionsHistoryAssumeRenamesSkolems(t *testing.T) {
	...
	h := NewHistory(cfg, PureStateClauses(FormulaToClauses(mkEq("a__b", "x"), nil)))
	// Assume with a formula that also has the same skolem
	assumption := FormulaToClauses(mkEq("a__b", "y"), nil)
...
func TestActionsHistorySatisfySatReturnsModel(t *testing.T) {
	...
	result := h.Satisfy(TrueClauses(nil))
	if result == nil {
...
func TestActionsJoinState(t *testing.T) {
	u1 := mkTestUpdate([]string{"x"}, mkEq("x", "old_x"), False)
	u2 := mkTestUpdate([]string{"y"}, mkEq("y", "old_y"), False)
	result := JoinState(u1, u2, TrueClauses(nil))
	if result == nil {
...
func TestActionsIteState(t *testing.T) {
	...
	u1 := mkTestUpdate([]string{"x"}, True, False)
	u2 := mkTestUpdate([]string{"x"}, True, False)
	result := IteState(cond, u1, u2, TrueClauses(nil))
	if result == nil {
...
func TestActionsComposeUpdatesEmptyModifiedPreservesNonNil(t *testing.T) {
	u1 := mkTestUpdate([]string{}, True, False)
	u2 := mkTestUpdate([]string{}, True, False)
	result := ComposeUpdates(u1, TrueClauses(nil), u2)
...
func TestActionsComposeUpdatesSequenceDoesNotPoisonModified(t *testing.T) {
	...
	assume := mkTestUpdate([]string{}, True, False)
	...
	assign := mkTestUpdate([]string{"x"}, mkEq("new_x", "y"), False)
...
func TestActionsHideEmptySyms(t *testing.T) {
	u := mkTestUpdate([]string{"x"}, mkEq("x", "y"), False)
	result := Hide([]*Const{}, u)
	// Nothing to hide
	if len(result.Modified) != 1 {
...
func TestActionsFilterAxiomsBySymsNoMatch(t *testing.T) {
	axiom, _ := NewAnd(mkEq("a", "b"), mkEq("c", "d"))
...
func TestActionsFilterAxiomsBySymsMatch(t *testing.T) {
	axiom, _ := NewAnd(mkEq("x", "b"), mkEq("c", "d"))
	result := filterAxiomsBySyms([]string{"x"}, axiom)
	if isFormulaTrue(result) {
