...
type ProofChecker struct {
	...
	Definitions map[string]*LabeledFormula
	...
	Schemata *InsMap[string, *LabeledFormula]
...
func NewProofChecker(cfg *ProofConfig, mod *Module, axioms, definitions []*LabeledFormula, schemata *InsMap[string, *LabeledFormula], astCfgs ...*AstConfig) *ProofChecker {
	if cfg == nil {
		cfg = TacticNewConfig()
	}
	...
	pc := &ProofChecker{
		...
		AstCfg:      acfg,
		Mod:         mod,
		Definitions: make(map[string]*LabeledFormula),
		Schemata:    NewInsMap[string, *LabeledFormula](),
...
func (pc *ProofChecker) astCfg() *AstConfig {
...
func (pc *ProofChecker) AdmitAxiom(ax *LabeledFormula) {
...
func (pc *ProofChecker) ApplyProof(goals []*LabeledFormula, proof Node) ([]*LabeledFormula, error) {
	if len(goals) == 0 {
		return nil, nil
	}
	if proof == nil {
	...
	if len(goals) > 0 && goals[0] != nil {
		xtracer.Trace("proof.ApplyProof ENTER proofType=%s goal[0].Formula type=%s", TypeName(proof), TypeName(goals[0].Formula))
	}
	...
	xtracer.Trace("proof.ApplyProof EXIT proofType=%s err=unknown", TypeName(proof))
...
func InstSchema(checker *ProofChecker, schema, goal *LabeledFormula, match map[string]string) ([]*LabeledFormula, error) {
...
func CheckSchema(checker *ProofChecker, goal, schema *LabeledFormula) ([]*LabeledFormula, error) {
	schemaName := schema.LabelName()
	if schemaName == "" {
		return nil, &ProofError{Msg: "schema has no label"}
	}
...
func (pc *ProofChecker) AdmitDefinition(defn *LabeledFormula, proof Node) ([]*LabeledFormula, error) {
	...
	deps := SymbolsAST(def.Rhs)
	for _, d := range deps {
		pc.Stale[ExprName(d)] = true
	}
	// Check if recursive (sym in deps)
	recursive := false
	for _, d := range deps {
		if ExprName(d) == symSym.Name {
			recursive = true
			break
		}
	}
	...
	if recursive {
		...
		var err error
		subgoals, err = pc.ApplyProof([]*LabeledFormula{defn}, proof)
		if err != nil {
...
func (pc *ProofChecker) GetSubgoals(prop *LabeledFormula, proof Node) ([]*LabeledFormula, error) {
	...
	prop = NormalizeGoal(pc.astCfg(), prop)
	subgoals, err := pc.ApplyProof([]*LabeledFormula{prop}, proof)
	if err != nil {
...
func (pc *ProofChecker) SetLastAxiom(prop *LabeledFormula) {
	if len(pc.Axioms) > 0 {
...
func (pc *ProofChecker) composeProofs(decls []*LabeledFormula, proofs []Node) ([]*LabeledFormula, error) {
	xtracer.Trace("proof.composeProofs ENTER nproofs=%d ndecls=%d", len(proofs), len(decls))
	var err error
	for i, proof := range proofs {
		if len(decls) > 0 && decls[0] != nil {
			xtracer.Trace("proof.composeProofs step=%d/%d proofType=%s goal[0].Formula type=%s", i, len(proofs), TypeName(proof), TypeName(decls[0].Formula))
		}
...
func (pc *ProofChecker) deferGoalTactic(decls []*LabeledFormula, proof *DeferGoalTactic) []*LabeledFormula {
...
func (pc *ProofChecker) showGoalsTactic(decls []*LabeledFormula, proof *ShowGoalsTactic) []*LabeledFormula {
...
func (pc *ProofChecker) forgetTactic(decls []*LabeledFormula, proof *ForgetTactic) ([]*LabeledFormula, error) {
...
func (pc *ProofChecker) proofTactic(decls []*LabeledFormula, proof *ProofTactic) ([]*LabeledFormula, error) {
	...
	for idx, decl := range decls {
		if nodeToString(decl.Label) == labelStr {
			subgoals, err := pc.ApplyProof([]*LabeledFormula{decl}, proof.Proof)
			if err != nil {
				xtracer.Trace("proof.proofTactic EXIT err=%v", err)
				return nil, err
			}
			// Remove the matched goal and append subgoals at the end.
			rest := make([]*LabeledFormula, 0, len(decls)-1+len(subgoals))
...
func (pc *ProofChecker) tacticTactic(decls []*LabeledFormula, proof *TacticTactic) ([]*LabeledFormula, error) {
	tn := nodeToString(proof.TName)
	if len(decls) > 0 && decls[0] != nil {
		xtracer.Trace("proof.tacticTactic name='%s' goal[0].Formula type=%s", tn, TypeName(decls[0].Formula))
	}
...
func ApplyMatchGoal(cfg *AstConfig, match map[string]string, goal *LabeledFormula) *LabeledFormula {
...
func collectStaleSymbols(n Node, stale map[string]bool) {
	if n == nil {
		return
	}
	...
	if expr, ok := n.(Expr); ok {
		for _, c := range UsedSymbolsAST(expr).All() {
			stale[ExprName(c)] = true
		}
		return
	}
...
func PrettyLineno(n Node) string {
	if n == nil {
		return "(internal) "
	}
	loc := n.GetLineno()
