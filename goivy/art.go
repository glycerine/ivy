...
type State struct {
	...
	Value      *Update     // assigned during BMC
	Universe   interface{} // assigned during BMC
	Action     ActionsAction
	ActionName string // name of the action (matches interp.InterpState.ActionName)
...
func (s *State) StateValue() *Update {
	if s.Update != nil {
		return s.Update
	}
	return &Update{
...
func (s *State) SetStateValue(u *Update) {
	s.Update = u
	if u != nil {
		s.Clauses = u.TR
	}
...
// Named to avoid confusion with lg.Expr, ast.Node, and interp.InterpState.Expr.
...
func NewAC(ag *AnalysisGraph, noAdd bool) *AC {
	return &AC{
		Assertions: make(map[string]Expr),
		Actions:    ag.Actions,
		Domain:     ag.Domain,
		AddFn:      ag.Add,
		NoAdd:      noAdd,
...
func (ac *AC) NewState(clauses *Clauses, exact bool, prov Provenance) *State {
...
func (ag *AnalysisGraph) Execute(checkPrecond bool, op ActionsAction, prestate *State, abstractor Abstractor, label string) (*State, error) {
...
func (ag *AnalysisGraph) JoinStates(state1, state2 *State, abstractor Abstractor) *State {
	...
	interpJoined, err := ConcreteJoin(interp1, interp2)
	if err != nil {
		log.Printf("art.JoinStates: %v", err)
		// Fallback: simple OR (defensive, matches old behavior)
		var joinedClauses *Clauses
		if state1.Clauses != nil && state2.Clauses != nil {
			joinedClauses = OrClausesTyped(state1.Clauses, state2.Clauses)
...
func (ag *AnalysisGraph) Cover(covered, covering *State) bool {
	...
	ok, _ := ModuleOrder(coveredInterp, coveringInterp)
	if ok {
...
func (ag *AnalysisGraph) ShowCore(clauseStr string, state *State) {
	clause, err := ToFormula(clauseStr)
	if err != nil {
	...
	clauseExpr, ok := clause.(Expr)
	if !ok {
		fmt.Printf("ShowCore: formula is not an lg.Expr: %T\n", clause)
		return
	}
	core := GetCore(interpState, clauseExpr)
	fmt.Println(core)
}
...
func (ag *AnalysisGraph) GetHistory(state *State, bound *int) *History {
	...
	if state.Pred == nil || (bound != nil && *bound <= 0) {
		...
		u := PureStateClauses(clauses)
		return NewHistory(ag.Domain.Cfg.IuCfg, u)
	}
	...
	if state.Update != nil {
		var axioms *Clauses = TrueClauses(nil)
		...
		if state.Prov != nil {
			if aa, ok := state.Prov.(*ActionApp); ok {
				...
				case Expr:
					actionNode = rep
				case string:
					actionNode = NewConst(rep, ActionS)
				}
			}
		}
...
func (ag *AnalysisGraph) BMC(state *State, errorCond Expr, otherArt *AnalysisGraph, bound *int) *AnalysisGraph {
	h := ag.GetHistory(state, bound)
	h = h.Assume(FormulaToClauses(errorCond, nil))

	// Use HistorySatisfy to check and extract path + universe.
	interpState := ArtToInterpState(state)
	bmcRes := HistorySatisfy(h, interpState)
	if bmcRes == nil {
		return nil
	}
...
func (ag *AnalysisGraph) CheckSafety(checkPrecond bool, state *State) *SafetyResult {
	...
	for _, asn := range ag.Assertions {
		cex := CheckStateAssertion(checkPrecond, interpState, asn)
		if !cex {
	...
	if state.Prov != nil {
		if aa, ok := state.Prov.(*ActionApp); ok {
			...
			if actionName != "" && len(aa.Args) > 0 {
				...
				exprNode := InterpActionApp(ag.Domain.Cfg.AstCfg, actionName, WrapState(interpPre))
				_, err := EvalState(checkPrecond, exprNode, ag.Domain)
				if err != nil {
					if afe, ok := err.(*IvyActionFailedError); ok {
...
func (ag *AnalysisGraph) DecomposeState(state *State) *AnalysisGraph {
	...
	var resultState *InterpState
	var err error
	if action != nil {
		resultState, err = DecomposeAction(true, ag.Domain.Cfg.IuCfg, interpState, interpPre, action)
	} else {
		exprNode := InterpActionApp(ag.Domain.Cfg.AstCfg, actionName, WrapState(interpPre))
		resultState, err = DecomposeActionApp(true, ag.Domain.Cfg.IuCfg, interpState, exprNode)
	}
	...
	// converting each interp.InterpState to an art.State and adding to the sub-graph.
	var interpStates []*InterpState
...
func (ag *AnalysisGraph) StateActions(state *State) []*Definition {
	if state.Label != "" {
		...
		for post, e := range ag.Predicates {
			...
			exprs := EvalStateActions(e, interpState)
	...
	for actionName := range ag.Actions.All() {
		...
		interpState := ArtToInterpState(state)
		acfg := ag.Domain.Cfg.AstCfg
		app := InterpActionApp(acfg, actionName, WrapState(interpState))
...
func (ag *AnalysisGraph) DoStateAction(checkPrecond bool, equation *Definition, abstractor Abstractor) *State {
	...
	is, err := EvalState(checkPrecond, rhs, ag.Domain)
	if err != nil {
...
func (ag *AnalysisGraph) RecalculateState(checkPrecond bool, state *State, abstractor Abstractor) {
	if state.Pred != nil && state.Update != nil {
		...
		interpPost, err := ConcretePost(checkPrecond, state.Update, interpPre, nil)
		if err != nil {
...
func (ag *AnalysisGraph) StateExtensions(state *State, joinFn func(*State, *State) *State) []*Definition {
	...
	for _, equation := range sas {
		...
		rhs := equation.Rhs
		ok, _ := EvalStateOrder(artCheckPrecondTrue, rhs, WrapState(interpFpc), ag.Domain)
		if !ok {
...
func (ag *AnalysisGraph) FixedpointCandidateBottomDefault(fpc map[string]*State, label string) *State {
	...
	return NewState(ag.Domain, FalseClauses(nil))
}
...
func (ag *AnalysisGraph) ConceptGraph(state *State, standardGraph func(*State) ConceptGraphView, clauses *Clauses) ConceptGraphView {
	if clauses == nil {
		clauses = state.Clauses
	}
	bg := ag.Domain.BackgroundTheory(state.InScope)
	sg := standardGraph(state)
	sg.SetGraphState(AndClausesTyped(clauses, bg))
...
func LabelFromAction(action ActionsAction) string {
...
func (ag *AnalysisGraph) AddInitialState(ic *Clauses, abstractor Abstractor) *State {
	mod := ag.Domain
	...
	if len(mod.Initializers) > 0 {
		...
		if len(seqChildren) > 0 {
			...
			env := BuildEnvActionFromAction(mod.Cfg.ActCfg, seq, "init")
			...
			upd := GetUpdate(env, ctx)
			if upd == nil {
				upd = NullUpdate()
			}
			actionAppNode := InterpActionApp(interpState.AstCfg(), "init", WrapState(interpState))
			s2interp, err := ConcretePost(artCheckPrecondFalse, upd, interpState, actionAppNode)
...
func (ag *AnalysisGraph) Initialize(abstractor Abstractor) {
	...
	if len(ag.Predicates) > 0 {
		...
		for name, p := range ag.Predicates {
			...
			is, err := EvalStateFacts(artCheckPrecondTrue, p, ag.Domain)
			if err != nil || is == nil {
				continue
			}
...
// -----------------------------------------------------------------------
// State type adapters: art.State <-> interp.InterpState
// -----------------------------------------------------------------------

// ArtToInterpState converts an art.State to an interp.InterpState for calling
// interp functions. Uses memoization to preserve pointer identity and
// converts art.State.Prov (art.Provenance) to interp.InterpState.Expr (ast.Node).
func ArtToInterpState(s *State) *InterpState {
	return artToInterpMemo(s, make(map[*State]*InterpState))
}

func artToInterpMemo(s *State, memo map[*State]*InterpState) *InterpState {
	if s == nil {
		return nil
	}
	if is, ok := memo[s]; ok {
	...
	sv := NewStateValue(nil, s.Clauses, FalseClauses(nil))
	is := NewInterpState(s.Domain, sv, nil, s.Label)
...
func provenanceToInterpExpr(prov Provenance, domain *Module, memo map[*State]*InterpState) Node {
	...
	case *StateJoin:
		...
		for _, s := range p.Args {
			terms = append(terms, WrapState(artToInterpMemo(s, memo)))
		}
		acfg := NewAstConfig()
		if domain != nil && domain.Cfg != nil && domain.Cfg.AstCfg != nil {
...
// interp.InterpState.Expr (ast.Node) to art.State.Prov (art.Provenance).
func InterpToArtState(is *InterpState) *State {
	return interpToArtMemo(is, make(map[*InterpState]*State))
}

func interpToArtMemo(is *InterpState, memo map[*InterpState]*State) *State {
	if is == nil {
		return nil
	}
	if s, ok := memo[is]; ok {
...
func interpExprToProvenance(expr Node, memo map[*InterpState]*State) Provenance {
	if expr == nil {
		return nil
	}
