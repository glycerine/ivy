...
func TestExpr6_CompileActionDef_PrmPrefixSubstitution(t *testing.T) {
	...
	actionDef := cfg.NewActionDef(
		...
		body,
		[]Node{paramX}, // FormalParams — gets fml: prefix in NewActionDef
		nil,            // FormalReturns
	)
...
func TestExpr6_CompileActionDef_FreeVarCheckInCalls(t *testing.T) {
	...
	c.TopCtx = &TopContext{
		Actions: map[string]*ActionInfo{
			"foo": {
				Params:  []*Const{NewConst("a", natSort)},
				Returns: nil,
			},
		},
	}
...
func TestExpr6_CompileLocal_AssignmentSortInference(t *testing.T) {
	...
	localSym, ok := localAct.Locals[0].(*Const)
	if !ok {
...
func TestExpr6_CompileLocal_FunctionLikeLHS(t *testing.T) {
	...
	procSort := &UninterpretedSort{Name: "proc"}
...
func TestExpr6_CompileLocal_LowerVarRoundTrip(t *testing.T) {
	...
	lowered := LowerVarStatements([]Node{varAction, trueStmt})
	if len(lowered) != 1 {
...
func TestExpr6_CompileCall_ParamCountValidation(t *testing.T) {
	...
	fooSort, _ := NewFunctionSort(natSort, natSort, natSort)
	...
	c.TopCtx = &TopContext{
		Actions: map[string]*ActionInfo{
			"foo": {
				Params:  []*Const{NewConst("a", natSort), NewConst("b", natSort)},
				Returns: []*Const{NewConst("r", natSort)},
			},
		},
	}
	...
	retTargets := []Node{cfg.NewAtom("r1"), cfg.NewAtom("r2")}
...
func TestExpr6_ExprContextExtract_LocalActionWrapping(t *testing.T) {
	...
	natSort := &UninterpretedSort{Name: "nat"}
	localSym := NewConst("tmp", natSort)

	code1 := NewAssumeAction(True)
	code2 := NewAssumeAction(True)

	ec := &ExprContext{
		Code:      []Expr{code1, code2},
		LocalSyms: []*Const{localSym},
	...
	sym, ok := localAct.Locals[0].(*Const)
	if !ok {
		t.Fatalf("expected local to be *lg.Const, got %T", localAct.Locals[0])
	}
	if sym.Name != "tmp" {
	...
	bodyAct, _ := localAct.Body.(ActionsAction)
	if bodyAct == nil {
