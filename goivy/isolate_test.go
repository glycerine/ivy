...
func isolateMkConst(name string) *Const {
	return NewConst(name, Boolean)
}

func isolateMkSort(name string) Sort {
...
func mkModuleWithActions(acts map[string]ActionsAction) *Module {
	m := mkModule()
...
func TestLookupAction(t *testing.T) {
	seq := NewSequence()
	m := mkModuleWithActions(map[string]ActionsAction{"foo": seq})
...
func TestSummarizeActionBasic(t *testing.T) {
	...
	body := NewSequence(NewAssumeAction(isolateMkConst("p")))
	body.SetFormalParams([]*Const{isolateMkConst("x")})
	body.SetFormalReturns([]*Const{isolateMkConst("r")})
...
func TestSummarizeActionWithInOutParams(t *testing.T) {
	body := NewSequence()
	p := isolateMkConst("x")
	body.SetFormalParams([]*Const{p})
	body.SetFormalReturns([]*Const{p}) // same param is both in and out

	isoCfg := NewIsolateConfig()
...
func TestAddMixinsExtRejectsMissingMixinAction(t *testing.T) {
	...
	AddMixinsExt(m, "a", NewSequence(), nil, nil, nil)
}
...
func TestCreateIsolateRejectsMissingMixinAction(t *testing.T) {
	...
	m.Actions.Set("a", NewSequence())
	m.Mixins.Set("a", []MixinDef{
		cfg.NewMixinBeforeDef(cfg.NewAtom("missing_monitor"), cfg.NewAtom("a")),
	})
	m.Isolates["iso"] = cfg.NewIsolateDef(
		[]Node{cfg.NewAtom("iso"), cfg.NewAtom("a")},
		0,
	)
...
func TestGetCallsMods(t *testing.T) {
	...
	call := NewCallActionOn(NewActionsConfig(), isolateMkConst("bar"))
	assign := NewAssignAction(isolateMkConst("x"), isolateMkConst("val"))
	seq := NewSequence(call, assign)
...
func TestGetCallsModsHavoc(t *testing.T) {
	havoc := NewHavocAction(isolateMkConst("y"))
	seq := NewSequence(havoc)
...
func TestHasSideEffectNoEffect(t *testing.T) {
	...
	m.Sig = NewSig()
	seq := NewSequence()
	actionMap := actionsInsMap(map[string]ActionsAction{"foo": seq})
...
func TestHasSideEffectWithAssert(t *testing.T) {
	m := mkModule()
	m.Sig = NewSig()
	assertAct := NewAssertAction(isolateMkConst("p"))
...
func TestHasSideEffectWithSigModification(t *testing.T) {
	m := mkModule()
	m.Sig = NewSig()
	m.Sig.Symbols.Set("x", &SymbolEntry{Name: "x", Sort: Boolean})

	assign := NewAssignAction(isolateMkConst("x"), isolateMkConst("val"))
	seq := NewSequence(assign)
	actionMap := actionsInsMap(map[string]ActionsAction{"foo": seq})
...
func TestHasSideEffectThroughCall(t *testing.T) {
	...
	assign := NewAssignAction(isolateMkConst("x"), isolateMkConst("val"))
	barSeq := NewSequence(assign)

	call := NewCallActionOn(NewActionsConfig(), isolateMkConst("bar"))
	fooSeq := NewSequence(call)

	actionMap := actionsInsMap(map[string]ActionsAction{
		"foo": fooSeq,
		"bar": barSeq,
	})
...
func actionsInsMap(m map[string]ActionsAction) *InsMap[string, ActionsAction] {
	im := NewInsMap[string, ActionsAction]()
	for k, v := range m {
		im.Set(k, v)
	}
	return im
...
func TestActionCallGraph(t *testing.T) {
	m := mkModule()
	call1 := NewCallActionOn(NewActionsConfig(), isolateMkConst("b"))
	call2 := NewCallActionOn(NewActionsConfig(), isolateMkConst("c"))
	m.Actions.Set("a", NewSequence(call1, call2))
	m.Actions.Set("b", NewSequence())
	m.Actions.Set("c", NewSequence())
...
func TestCollectSortDestructorsWithVariants(t *testing.T) {
	m := mkModule()
	m.Variants["Base"] = []Sort{isolateMkSort("Variant1")}
...
func TestStripSort(t *testing.T) {
	...
	fs, err := NewFunctionSort(sortA, sortB, sortC)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStripSortAllDomain(t *testing.T) {
	...
	fs, err := NewFunctionSort(isolateMkSort("A"), isolateMkSort("B"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, ok := result.(*UninterpretedSort); !ok {
		t.Errorf("expected UninterpretedSort, got %T", result)
	}
}

func TestStripSortZero(t *testing.T) {
	fs, _ := NewFunctionSort(isolateMkSort("A"), isolateMkSort("B"))
...
func TestStripIsolateStripsFormalParams(t *testing.T) {
	...
	act := NewSequence()
	act.SetFormalParams([]*Const{isolateMkConst("s"), isolateMkConst("x")})
	act.SetFormalReturns([]*Const{isolateMkConst("r")})
	...
	stripped, ok := strippedIface.(ActionsAction)
	if !ok {
...
func FuzzSummarizeAction(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, nParams, nReturns int, checkMode bool) {
		...
		body := NewSequence()
		params := make([]*Const, nParams)
		for i := range params {
			params[i] = NewConst("p"+string(rune('a'+i)), Boolean)
		}
		returns := make([]*Const, nReturns)
		for i := range returns {
			returns[i] = NewConst("r"+string(rune('a'+i)), Boolean)
		}
