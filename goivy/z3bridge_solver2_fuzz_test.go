...
func FuzzSortCard(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, sortKind byte, param int, extra string) {
		...
		case 1:
			...
			sort = &RangeSort{Name: "R", Lb: NumeralBound{Value: "0"}, Ub: NumeralBound{Value: string(rune('0' + hi%10))}}
		case 2:
			width := param % 32
			if width == 0 {
				width = 1
...
func FuzzRangeSortBounds(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, lb, ub string) {
		...
		rs := &RangeSort{Name: "R", Lb: NumeralBound{Value: lb}, Ub: NumeralBound{Value: ub}}
...
func FuzzNumeralToZ3Clamping(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, numStr, lb, ub string) {
		...
		runOnZ3Thread2(t, func(t *testing.T) {
			rs := &RangeSort{Name: "bounded", Lb: NumeralBound{Value: lb}, Ub: NumeralBound{Value: ub}}
			sig := NewSig()
			sig.Interp["bounded"] = rs
			mod := NewWithSig(sig)
			s := NewSolver(mod, nil)

			num := NewConst(numStr, &UninterpretedSort{Name: "bounded"})
...
func FuzzEncodeEqualityZ3(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, nElems, idx1, idx2 int) {
		...
		runOnZ3Thread2(t, func(t *testing.T) {
			...
			t1 := NewConst(ext[i1], es)
			t2 := NewConst(ext[i2], es)
...
func FuzzQuantConstraintsNatRange(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, interpKind byte, isForall bool) {
		runOnZ3Thread2(t, func(t *testing.T) {
			...
			case 1:
				sig.Interp["mysort"] = &RangeSort{Name: "mysort", Lb: NumeralBound{Value: "0"}, Ub: NumeralBound{Value: "10"}}
			case 2:
				// No interpretation
			}
			mod := NewWithSig(sig)
			...
			sort := &UninterpretedSort{Name: "mysort"}
			x, err := NewVariable("X", sort)
			if err != nil {
				return
			}
			p := relConst("P", sort)
