...
func findActionDef(decls []Node) *ActionDef {
	for _, d := range decls {
		if ad, ok := d.(*ActionDecl); ok {
			if len(ad.DeclArgs) > 0 {
				if adef, ok := ad.DeclArgs[0].(*ActionDef); ok {
					return adef
				}
			}
		}
...
method foo(x:t) = {
	...
	for i, fp := range adef.FormalParams {
		t.Logf("  FormalParams[%d]: %T rep=%q canon=%s", i, fp, NodeRep(fp), fp.Canon())
	}
}
...
action bar(x:t) = {
	...
	for i, fp := range adef.FormalParams {
		t.Logf("  FormalParams[%d]: %T rep=%q", i, fp, NodeRep(fp))
	}
}
...
method foo(x:t, y:t) = {
	...
	for i, fp := range adef.FormalParams {
		t.Logf("  FormalParams[%d]: rep=%q", i, NodeRep(fp))
	}
}
...
func TestSelfAppConstruction(t *testing.T) {
	...
	prefixed := PrefixNode(selfArg, "fml:")
	prefixedRep := NodeRep(prefixed)
