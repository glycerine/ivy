// Minimal WASI preview1 host for Go Ivy's wasip1 browser artifact.
// Keep edge-case behavior aligned with wazero's wasi_snapshot_preview1 host:
// goldweb uses wazero as the command-line oracle for this browser shim.
...
export class GoIvyWasiProcExit extends Error {
  constructor(code) {
...
export function createGoIvyWasiP1(options) {
  ...
  const heapProfile = options.heapProfile || (() => {});
  ...
  const fds = [
    ...
    new OpenFd('stdio', { write: heapProfile }),
  ];
  ...
  const wasiImport = {
    ...
    proc_exit(code) {
      throw new GoIvyWasiProcExit(code);
    },
  ...
  return {
    wasiImport: trapSafeWasiImports(wasiImport, debug),
    ...
    start(wasmInstance) {
      ...
      try {
        ...
        if (error instanceof GoIvyWasiProcExit) {
          return error.code;
        }
        throw error;
      }
