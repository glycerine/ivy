...
const wasmFile = process.env.SMT_SOLVER_ROUND_TRIP_WASM;
...
test.skip(!wasmFile, 'SMT_SOLVER_ROUND_TRIP_WASM is generated by the smt Go web test');
...
test('Big Go WASI smt package solves a tiny query through Z3 wasm in a worker', async ({ page }) => {
    ...
    const workerSource = `
      ...
      self.onmessage = async () => {
        ...
        try {
          ...
          const uninterpretedSortNameOK = instance.exports.smt_solver_uninterpreted_sort_name_ok;
          if (
            typeof satTrue !== 'function'
            || typeof unsatTrueAndNotTrue !== 'function'
            || typeof uninterpretedSortNameOK !== 'function'
          ) {
          ...
          self.postMessage({
            ...
            uninterpretedSortNameOK: uninterpretedSortNameOK() | 0,
          });
        } catch (error) {
          self.postMessage({
            ok: false,
  ...
  expect(result.uninterpretedSortNameOK).toBe(1);
});
