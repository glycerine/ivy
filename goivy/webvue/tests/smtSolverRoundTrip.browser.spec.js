...
const solverJSWasmFile = process.env.SMT_SOLVER_ROUND_TRIP_JS_WASM;
...
async function runSmtJSFixture(page, wasmFile) {
  await page.goto(baseURL);

  return await page.evaluate(async ({ wasmFile }) => {
    const assetBaseURL = window.location.origin;
    const workerSource = `
      const assetBaseURL = ${JSON.stringify(assetBaseURL)};
      const wasmFile = ${JSON.stringify(wasmFile)};
      ...
      self.onmessage = async () => {
        ...
        try {
          ...
          self.smtSolverRoundTripJSReady = false;
          self.smtSolverRoundTripJSRun = undefined;
          importScripts(assetBaseURL + '/wasm_exec-go1.25.6.js');
          ...
          let wasmMemory;
          const go = new Go();
          go.argv = ['smt_solver_round_trip_js'];
          ...
          if (!wasmMemory) {
            throw new Error('SMT js/wasm fixture does not export mem');
          }
          ...
          for (let i = 0; i < 2000 && !self.smtSolverRoundTripJSReady; i += 1) {
            if (goRunError) {
              throw goRunError;
            }
            await sleep(5);
          ...
          if (typeof self.smtSolverRoundTripJSRun !== 'function' || !self.smtSolverRoundTripJSReady) {
            throw new Error('SMT js/wasm fixture did not publish smtSolverRoundTripJSRun');
          }

          self.postMessage({
            ok: true,
            results: self.smtSolverRoundTripJSRun(),
          });
        } catch (error) {
          self.postMessage({
            ok: false,
    ...
    try {
      return await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('smt JS/Z3 worker test timed out'));
        }, 30000);
...
test('Big Go WASI smt package solves a tiny query through Z3 wasm in a worker', async ({ page }) => {
  ...
  const result = await runSmtWasip1Fixture(page, solverWasmFile, [
    ...
    'smt_solver_canon_preserves_enum_function_copy',
    'smt_solver_canon_preserves_multi_bound_enum_premise',
  ]);
  ...
  expect(result.results.smt_solver_canon_preserves_enum_function_copy).toBe(1);
  expect(result.results.smt_solver_canon_preserves_multi_bound_enum_premise).toBe(1);
});

test('Big Go JS smt package solves a tiny query through Z3 wasm in a worker', async ({ page }) => {
  test.skip(!solverJSWasmFile, 'SMT_SOLVER_ROUND_TRIP_JS_WASM is generated by the smt Go web test');

  const result = await runSmtJSFixture(page, solverJSWasmFile);

  expect(result.results.panic).toBe('');
  expect(result.results.satTrue).toBe(1);
  expect(result.results.unsatTrueAndNotTrue).toBe(-1);
  expect(result.results.uninterpretedSortNameOK).toBe(1);
  expect(result.results.canonPreservesEnumQuantifierFalse).toBe(1);
  expect(result.results.canonPreservesEnumFunctionCopy).toBe(1);
  expect(result.results.canonPreservesMultiBoundPremise).toBe(1);
});
