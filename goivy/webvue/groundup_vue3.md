...
- Auth, account, project, and access-grant state live in the shared MariaDB
  control-plane database `ivyvue`.
- Project-owned Ivy data is isolated in a generated MariaDB database per
  project.
...
Use MariaDB through Go's `database/sql` package. `ivyvue` is the shared
control-plane database for authentication, sessions, accounts, account
memberships, projects, access grants, and project database metadata.
...
Each project gets a separate MariaDB database for project-owned Ivy data. The
isolation boundary is the project, not the user or account. A single-user trial
is modeled as a personal account with one project, so it still gets isolated
storage without blocking future team projects.

Project database names are generated by the server, stored in the control
plane, and validated before use. They are never derived directly from user
input.
