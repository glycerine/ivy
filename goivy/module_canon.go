...
func (m *Module) Canon() Canonical {
	...
	return Canonical(fmt.Sprintf("(module %s)", strings.Join(parts, " ")))
}
...
func canonActionMap(actions *InsMap[string, Action]) string {
...
func canonLFSlice(lfs []*LabeledFormula) string {
	if len(lfs) == 0 {
		return "[]"
	}
	var parts []string
...
func canonSortMap(sorts *InsMap[string, Sort]) string {
	if sorts.Len() == 0 {
		return "(hash)"
	}
...
func canonPlainSortMap(sorts map[string]Sort) string {
	if len(sorts) == 0 {
		return "(hash)"
	}
...
func canonSchemaMap(schemata map[string]Node) string {
	if len(schemata) == 0 {
		return "(hash)"
	}
...
func canonInsMapSchemata(schemata *InsMap[string, Node]) string {
...
func canonExprSlice(exprs []Expr) string {
	if len(exprs) == 0 {
		return "[]"
	}
	var parts []string
...
func canonConstSlice(cs []*Const) string {
	if len(cs) == 0 {
		return "[]"
	}
	var parts []string
...
func canonNodeSlice(nodes []Node) string {
	if len(nodes) == 0 {
		return "[]"
	}
	var parts []string
...
func canonInterfaceSlice(items []interface{}) string {
	...
	for _, item := range items {
		...
		} else if c, ok := item.(Canonizer); ok {
...
func canonInsMapSort(m *InsMap[string, Sort]) string {
	if m == nil || m.Len() == 0 {
		return "(insMap)"
	}
	var b strings.Builder
...
func canonInsMapBool(m *InsMap[string, bool]) string {
	if m == nil || m.Len() == 0 {
		return "(insMap)"
	}
	var b strings.Builder
...
func canonInsMapMixins(m *InsMap[string, []MixinDef]) string {
	if m == nil || m.Len() == 0 {
		return "(insMap)"
	}
	var b strings.Builder
...
func canonInsMapHierarchy(m *InsMap[string, *InsMap[string, bool]]) string {
	if m == nil || m.Len() == 0 {
		return "(insMap)"
	}
	var b strings.Builder
...
func canonPostcondsMap(m map[string][]*LabeledFormula) string {
	if len(m) == 0 {
		return "(hash)"
	}
	keys := make([]string, 0, len(m))
...
func canonSortSliceMap(m map[string][]Sort) string {
	if len(m) == 0 {
		return "(hash)"
	}
	keys := make([]string, 0, len(m))
...
func canonNodeMapSlice(m map[string][]Node) string {
	if len(m) == 0 {
		return "(hash)"
	}
	keys := make([]string, 0, len(m))
...
func canonExprMap(m map[string]Expr) string {
	if len(m) == 0 {
		return "(hash)"
	}
	keys := make([]string, 0, len(m))
...
func canonIsolateMap(m map[string]*IsolateDef) string {
	if len(m) == 0 {
		return "(hash)"
	}
	keys := make([]string, 0, len(m))
...
func canonNativeTypeMap(m map[string]*NativeType) string {
	if len(m) == 0 {
		return "(hash)"
	}
	keys := make([]string, 0, len(m))
