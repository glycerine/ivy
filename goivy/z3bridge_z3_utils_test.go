...
func z3MustApply(t *testing.T, fn Expr, terms ...Expr) *Apply {
	t.Helper()
	app, err := NewApply(fn, terms...)
	if err != nil {
		t.Fatal(err)
	}
	return app
...
func z3MustEq(t *testing.T, t1, t2 Expr) *Eq {
	t.Helper()
	eq, err := NewEq(t1, t2)
	if err != nil {
		t.Fatal(err)
...
func TestZ3BridgeToZ3ConstHigherOrder(t *testing.T) {
	...
	s := &UninterpretedSort{Name: "S"}
	fs := z3MustFS(t, s, s, Boolean) // S * S -> Boolean
	c := NewConst("leq", fs)
	result, err := u.ToZ3(c)
	if err != nil {
		t.Fatal(err)
...
func TestZ3BridgeToZ3ApplyWithArgs(t *testing.T) {
	...
	s := &UninterpretedSort{Name: "S"}
	fs := z3MustFS(t, s, s, Boolean) // S * S -> Bool
	leq := NewConst("leq", fs)
	x := NewConst("x", s)
	y := NewConst("y", s)
...
func TestZ3BridgeZ3UtilsBatchInvalid(t *testing.T) {
	...
	results, err := u.Z3ImpliesBatch(p, []Expr{q}, false)
	if err != nil {
		t.Fatal(err)
	}
	if results[0] {
...
func TestZ3BridgeZ3UtilsBatchCacheHit(t *testing.T) {
	...
	// Second call should hit cache
	r2, err := u.Z3ImpliesBatch(pAndQ, []Expr{p}, false)
	if err != nil {
		t.Fatal(err)
	}
	if !r2[0] {
...
func buildTransitivityFixtures(t *testing.T) (
	...
	Y = z3MustVar(t, "Y", S)
	Z = z3MustVar(t, "Z", S)
	binRel := z3MustFS(t, S, S, Boolean) // S * S -> Boolean
