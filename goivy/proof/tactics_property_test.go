...
func mkTestMod(pArity int) *module.Module {
	sig := il.NewSig()
	sSort := &lg.UninterpretedSort{Name: "S"}
	sig.Sorts.Set("S", sSort)

	if pArity > 0 {
		sorts := make([]lg.Sort, pArity+1)
		for i := 0; i < pArity; i++ {
			sorts[i] = sSort
		}
		sorts[pArity] = lg.Boolean
		pSort, _ := lg.NewFunctionSort(sorts...)
		sig.Symbols.Set("P", &il.SymbolEntry{Name: "P", Sort: pSort})
	}

	return module.NewWithSig(sig)
}

func mkPCWithModule(mod *module.Module) *ProofChecker {
	return NewProofChecker(nil, mod, nil, nil, nil)
}

// mkPropertyTactic creates a PropertyTactic from the given AST nodes.
func mkPropertyTacticNode(cfg *ast.AstConfig, prop, pname, proof ast.Node) *ast.PropertyTactic {
	if pname == nil {
...
func TestPropertyTactic_BasicSkolemWitness(t *testing.T) {
	...
	goal := mkLF(cfg.NewAtom("g"), lg.True)
	...
	exBody := cfg.NewExists([]ast.Node{yv}, pAtom)
	forallBody := cfg.NewForall([]ast.Node{xv}, exBody)
	propLF := cfg.NewLabeledFormula(cfg.NewAtom("prop"), forallBody)

	// Skolem LHS: named f(X)
	skolemLhs := cfg.NewAtom("f")
	skolemLhs.Terms = []ast.Node{cfg.NewVariable("X", "S")}

	pt := mkPropertyTacticNode(cfg, propLF, skolemLhs, nil)
	pc := mkPCWithModule(mod)

	result, err := pc.propertyTactic([]*ast.LabeledFormula{goal}, pt)
	if err != nil {
	...
	for _, p := range prems {
		if cd, ok := p.(*ast.ConstantDecl); ok {
			args := cd.Args()
			if len(args) > 0 {
				if c, ok := args[0].(*lg.Const); ok && c.Name == "f" {
					foundSkolem = true
...
func TestPropertyTactic_NullarySkolemConstant(t *testing.T) {
	...
	for _, p := range prems {
		if cd, ok := p.(*ast.ConstantDecl); ok {
			args := cd.Args()
			if len(args) > 0 {
				if c, ok := args[0].(*lg.Const); ok && c.Name == "c" {
					foundSkolem = true
					// Nullary: sort should be just "S", not a FunctionSort
					if _, ok := c.CSort.(*lg.FunctionSort); ok {
...
func TestPropertyTactic_NonExistentialError(t *testing.T) {
	...
	forallBody := cfg.NewForall([]ast.Node{xv}, pAtom)
	propLF := cfg.NewLabeledFormula(cfg.NewAtom("prop"), forallBody)
	...
	_, err := pc.propertyTactic([]*ast.LabeledFormula{goal}, pt)
	if err == nil {
...
func TestPropertyTactic_MissingParameterError(t *testing.T) {
	...
	sig := il.NewSig()
	sig.Sorts.Set("S", sSort)
	pSort, _ := lg.NewFunctionSort(sSort, sSort, sSort, lg.Boolean)
	sig.Symbols.Set("P", &il.SymbolEntry{Name: "P", Sort: pSort})
	...
	exBody := cfg.NewExists([]ast.Node{yv}, pAtom)
	fa2 := cfg.NewForall([]ast.Node{zv}, exBody)
	fa1 := cfg.NewForall([]ast.Node{xv}, fa2)
...
func TestPropertyTactic_CannotInferSortError(t *testing.T) {
	...
	skolemLhs := cfg.NewAtom("f")
	skolemLhs.Terms = []ast.Node{cfg.NewVariable("W", "S")}
...
func TestPropertyTactic_ExtraParamExplicitSort(t *testing.T) {
	...
	pSort, _ := lg.NewFunctionSort(sSort, lg.Boolean)
	sig.Symbols.Set("P", &il.SymbolEntry{Name: "P", Sort: pSort})
	mod := module.NewWithSig(sig)
...
func TestPropertyTactic_NoneAST_Passthrough(t *testing.T) {
	...
	sig := il.NewSig()
	sig.Sorts.Set("S", sSort)
	sig.Symbols.Set("Q", &il.SymbolEntry{Name: "Q", Sort: lg.Boolean})
	mod := module.NewWithSig(sig)
