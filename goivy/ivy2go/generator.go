...
func (g *Generator) emitTestMain() {
	...
	g.main.line("\t// Recover from ivyAssumeFailed so a bad-assumption")
	g.main.line("\t// iteration is silently dropped (mirrors ivy2cpp's")
	g.main.line("\t// Z3-driven feasibility check). Any other panic")
	g.main.line("\t// — including ivyAssert failures — re-raises.")
	g.main.line("\tfunc() {")
	g.main.line("\t\tdefer func() {")
	g.main.line("\t\t\tif r := recover(); r != nil {")
	g.main.line("\t\t\t\tif _, ok := r.(ivyAssumeFailed); ok { return }")
	g.main.line("\t\t\t\tpanic(r)")
	g.main.line("\t\t\t}")
	g.main.line("\t\t}()")
