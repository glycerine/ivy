...
func OrClauses(args ...interface{}) interface{} {
	...
	rn := NewUniqueRenamer("__ts0", used)
...
func OrClausesTyped(args ...*Clauses) *Clauses {
	...
	fixedVs := make([]Expr, 0, len(origArgs))
	fixedArgs := make([]*Clauses, 0, len(origArgs))
	idx := 0
	for _, a := range origArgs {
		if a.IsFalse() {
			fixedVs = append(fixedVs, False) // Python: Or()
...
func orClausesInt(rn *UniqueRenamer, args []*Clauses) *Clauses {
...
func orClausesIntWithVs(rn *UniqueRenamer, args []*Clauses) (*Clauses, []Expr, []*Clauses) {
	...
	vs := make([]Expr, len(args))
	vsNodes := make([]Expr, len(args))
	for i := range args {
		name := rn.Rename("")
	...
	defIdx := NewInsMap[NodeKey, *IvyDefinition]()
	for i, cls := range args {
...
func IteClauses(cond Expr, thenCls, elseCls *Clauses) *Clauses {
...
func iteClausesInt(rn *UniqueRenamer, cond Expr, args []*Clauses) *Clauses {
	args = elimDeadDefinitions(rn, args)
...
func DualClauses(clauses *Clauses, skolemizer Skolemizer, instantiator func([]Expr) *Clauses) *Clauses {
...
func dualClauses(clauses *Clauses, skolemizer Skolemizer, instantiator func([]Expr) *Clauses) *Clauses {
	...
	for _, v := range vars {
		if skolemizer != nil {
			subs[Key(v)] = skolemizer(v)
		} else {
			// Default: var_to_skolem('__', v)
			sk := NewConst("__"+v.Name, v.VSort)
			subs[Key(v)] = sk
		}
	}
...
func ConditionClauses(clauses *Clauses, fmla Expr) *Clauses {
	negFmla := Negate(fmla)
	var newFmlas []Expr
...
func usesSymbolNameAST(names map[string]bool, node Expr) bool {
...
func UsedSymbolsClauses(clauses *Clauses) *InsMap[NodeKey, Expr] {
	if clauses == nil {
		return NewInsMap[NodeKey, Expr]()
	}
...
func RenameClauses(clauses *Clauses, subs map[NodeKey]*Const) *Clauses {
	fn := func(n Expr) Expr {
...
// The map keys are lg.NodeKey (via lg.Key(sym)) for structural equality.
func SubstituteConstantsClauses(clauses *Clauses, subs map[NodeKey]Expr) *Clauses {
	fn := func(n Expr) Expr {
...
func SubstituteNodesClauses(clauses *Clauses, subs map[NodeKey]Expr) *Clauses {
	if len(subs) == 0 {
		return clauses
	}
	var fmlas []Expr
...
func substituteNodesRec(n Expr, subs map[NodeKey]Expr) (Expr, error) {
	if d, ok := n.(*IvyDefinition); ok {
		...
		return NewIvyDefinition(lhs, rhs), nil
	}
	...
	return CloneNode(n, newChildren), nil
}
...
func filterBoundVarSubs(subs map[NodeKey]Expr, vars []*LogicVariable) map[NodeKey]Expr {
	...
	boundSet := make(map[NodeKey]struct{}, len(vars))
	for _, v := range vars {
		boundSet[Key(v)] = struct{}{}
	}
	filtered := make(map[NodeKey]Expr, len(subs))
	for k, v := range subs {
...
func collectUsedNames(args []*Clauses, extra Expr) []string {
	seen := make(map[string]struct{})
	for _, cls := range args {
		for _, s := range cls.Symbols().All() {
			seen[ExprName(s)] = struct{}{}
		}
	}
	if extra != nil {
		for _, s := range UsedSymbolsAst(extra).All() {
			seen[ExprName(s)] = struct{}{}
		}
	}
...
func elimDeadDefinitions(rn *UniqueRenamer, args []*Clauses) []*Clauses {
	...
	if len(toRename) > 0 {
		for i, a := range args {
			subs := make(map[NodeKey]*Const, len(toRename))
			for _, sym := range toRename {
				newName := rn.Rename(sym.Name)
				subs[Key(sym)] = NewConst(newName, sym.CSort)
			}
...
func elimDefinitions(clauses *Clauses, dead []NodeKey) *Clauses {
	if xtracer.Enabled {
		deadNames := make([]string, len(dead))
		for i, k := range dead {
			deadNames[i] = fmt.Sprintf("'%v'", k)
	...
	deadSet := make(map[NodeKey]bool, len(dead))
	for _, key := range dead {
		deadSet[key] = true
	}
	var defs []*IvyDefinition
...
// Uses the faithful port il.SymbolsIluAst for traversal.
func UsedSymbolsClausesOrdered(c *Clauses) *InsMap[NodeKey, Expr] {
	result := NewInsMap[NodeKey, Expr]()
	if c == nil {
		return result
	}
...
// depth-first AST traversal order, deduplicating by structural identity.
// Uses the faithful port il.SymbolsIluAst for traversal.
func UsedSymbolsExprOrdered(node Expr) *InsMap[NodeKey, Expr] {
	result := NewInsMap[NodeKey, Expr]()
...
func SubstituteClauses(clauses *Clauses, subs map[NodeKey]Expr) *Clauses {
...
func SubstituteClausesByName(clauses *Clauses, subs map[string]Expr) *Clauses {
	if clauses == nil || len(subs) == 0 {
		return clauses
	}
	fmlas := make([]Expr, len(clauses.Fmlas))
	...
	for i, d := range clauses.Defs {
		...
		if rd, ok := replaced.(*IvyDefinition); ok {
			defs[i] = rd
		} else {
			defs[i] = d
		}
...
func SubstBothClauses(clauses *Clauses, subs map[NodeKey]Expr) *Clauses {
	if clauses == nil || len(subs) == 0 {
		return clauses
	}
...
func resortClausesBySort(clauses *Clauses, subs map[NodeKey]Sort) *Clauses {
	if clauses == nil || len(subs) == 0 {
		return clauses
	}
	fn := func(n Expr) Expr {
		return ResortAst(n, subs)
	}
	return clauses.Apply(fn)
}
...
func ConstantsClauses(clauses *Clauses) []*Const {
	if clauses == nil {
		return nil
	}
	seen := make(map[string]bool)
	var result []*Const
	for _, f := range clauses.Fmlas {
		for _, sym := range UsedSymbolsAst(f).All() {
			if cc, ok := sym.(*Const); ok {
				if !seen[cc.Name] {
					seen[cc.Name] = true
	...
	for _, d := range clauses.Defs {
		for _, sym := range UsedSymbolsAst(d).All() {
			if cc, ok := sym.(*Const); ok {
				if !seen[cc.Name] {
					seen[cc.Name] = true
					result = append(result, cc)
...
func SymbolsClauses(clauses *Clauses) []Expr {
	if clauses == nil {
		return nil
	}
	result := clauses.Symbols()
	syms := make([]Expr, 0, result.Len())
...
func RelationsClauses(clauses *Clauses) []*Const {
	if clauses == nil {
		return nil
	}
	seen := make(map[string]bool)
...
func FunctionsClauses(clauses *Clauses) []*Const {
	if clauses == nil {
		return nil
	}
	seen := make(map[string]bool)
...
func AppsClauses(clauses *Clauses) []Expr {
	if clauses == nil {
		return nil
	}
	var result []Expr
	for _, f := range clauses.Fmlas {
		result = append(result, AppsAst(f)...)
	}
	for _, d := range clauses.Defs {
		result = append(result, AppsAst(d)...)
	}
	return result
}
...
func GroundAppsClauses(clauses *Clauses) []Expr {
	if clauses == nil {
		return nil
	}
	var result []Expr
	for _, f := range clauses.Fmlas {
		result = append(result, GroundAppsAst(f)...)
	}
	for _, d := range clauses.Defs {
		result = append(result, GroundAppsAst(d)...)
	}
	return result
}
...
func EqsClauses(clauses *Clauses) []*Eq {
	if clauses == nil {
		return nil
	}
	var result []*Eq
	for _, f := range clauses.Fmlas {
		result = append(result, EqsAst(f)...)
	}
	for _, d := range clauses.Defs {
		result = append(result, EqsAst(d)...)
	}
	return result
}
...
func newTseitinContext(used map[string]bool) *tseitinContext {
	...
	return &tseitinContext{
		fresh: NewUniqueRenamer("__ts", usedNames),
	}
}

// tseitinEncoding encodes a formula into a literal, adding clauses to tc.
func (tc *tseitinContext) tseitinEncoding(f Expr) Expr {
	f = ExpandAbbrevs(f)
	...
	case *LogicAnd:
		...
		sorts = append(sorts, &BooleanSort{})
		fs, _ := NewFunctionSort(sorts...)
		fn := NewConst(fname, fs)
...
func TseitinEncode(f Expr) *Clauses {
...
func SimplifyClauses(cls *Clauses) *Clauses {
	...
	fmlas := make([]Expr, len(cls.Fmlas))
	copy(fmlas, cls.Fmlas)

	for i := 0; i < 3; i++ {
		// Simplify each formula
		simplified := make([]Expr, 0, len(fmlas))
		for _, f := range fmlas {
			s := simplifyFormula(f)
...
func isTautologyFormula(f Expr) bool {
...
func SortsClauses(clauses *Clauses) map[NodeKey]Sort {
	if clauses == nil {
		return nil
	}
	result := make(map[NodeKey]Sort)
	for _, f := range clauses.Fmlas {
		for k, s := range SortsAst(f) {
			result[k] = s
		}
	}
	for _, d := range clauses.Defs {
		for k, s := range SortsAst(d) {
			result[k] = s
		}
	}
	return result
