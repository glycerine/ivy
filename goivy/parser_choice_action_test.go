...
func TestChoiceActionBranches(t *testing.T) {
	...
	if NodeRep(c.Branches[0]) != "branch1" {
		t.Errorf("expected first branch 'branch1', got %q", NodeRep(c.Branches[0]))
	}
	if NodeRep(c.Branches[1]) != "branch2" {
		t.Errorf("expected second branch 'branch2', got %q", NodeRep(c.Branches[1]))
	}
}
