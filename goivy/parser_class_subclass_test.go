...
func TestDeclReorderingClassOneToFront(t *testing.T) {
	...
	if NodeRep(decls[0]) != "C" {
		t.Errorf("expected first element to be C, got %s", NodeRep(decls[0]))
	}
	if NodeRep(decls[1]) != "A" {
		t.Errorf("expected second element to be A, got %s", NodeRep(decls[1]))
	}
	if NodeRep(decls[2]) != "B" {
		t.Errorf("expected third element to be B, got %s", NodeRep(decls[2]))
	}
}
...
func TestDeclReorderingSubclassTwoToFront(t *testing.T) {
	...
	if NodeRep(decls[1]) != "D" {
		t.Errorf("expected second element to be D, got %s", NodeRep(decls[1]))
	}
	if NodeRep(decls[2]) != "A" {
		t.Errorf("expected third element to be A, got %s", NodeRep(decls[2]))
	}
	if NodeRep(decls[3]) != "B" {
		t.Errorf("expected fourth element to be B, got %s", NodeRep(decls[3]))
	}
}
