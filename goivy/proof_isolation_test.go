...
func TestConfigIsolation(t *testing.T) {
	cfg1 := TacticNewConfig()
	cfg2 := TacticNewConfig()

	called1 := false
	cfg1.RegisterTactic("only_on_1", func(pc ProofCheckerInterface, goals []*LabeledFormula, proof Node) ([]*LabeledFormula, error) {
		called1 = true
		return goals, nil
	})
	...
	called2 := false
	cfg2.RegisterTactic("only_on_2", func(pc ProofCheckerInterface, goals []*LabeledFormula, proof Node) ([]*LabeledFormula, error) {
		called2 = true
		return goals, nil
	})
