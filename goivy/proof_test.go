...
func proofMkSort(name string) *UninterpretedSort {
...
func proofMkConst(name string, s Sort) *Const {
	return NewConst(name, s)
}

var proofTestAstCfg = NewAstConfig()

func mkLF(label, formula Node) *LabeledFormula {
...
func TestMatchProblemString(t *testing.T) {
	s := proofMkSort("S")
	x := proofMkVar("X", s)
	mp := NewMatchProblem(x, x, x, map[NodeKey]Expr{Key(x): True}, nil)
	str := mp.String()
	if len(str) == 0 {
...
func TestFuncSorts(t *testing.T) {
	...
	if !sorts2[0].Equal(s) || !sorts2[1].Equal(Boolean) {
		t.Error("wrong sorts")
	}
}
...
func TestFuncsMatch(t *testing.T) {
	...
	freesyms := map[NodeKey]Expr{Key(s): True}
...
func TestMatchSort(t *testing.T) {
	...
	free := map[NodeKey]Expr{Key(s1): True}
	m := MatchSort(s1, s2, free)
	if m == nil || len(m) != 1 {
		t.Fatal("expected match")
	}
	if !m[Key(s1)].Equal(s2) {
...
func TestMergeMatchesNil(t *testing.T) {
	m := MergeMatches(nil, map[NodeKey]Expr{})
	if m != nil {
...
func TestMergeMatchesConflict(t *testing.T) {
	...
	m1 := map[NodeKey]Expr{Key(x): y}
	m2 := map[NodeKey]Expr{Key(x): z}
...
func TestAddSymbols(t *testing.T) {
	...
	set := map[NodeKey]Expr{}
	as := NewAddSymbols(set, []Expr{x, y})
	if set[Key(x)] == nil || set[Key(y)] == nil {
...
func TestRemoveSymbols(t *testing.T) {
	...
	set := map[NodeKey]Expr{Key(x): True}
	rs := NewRemoveSymbols(set, []Expr{x})
	if set[Key(x)] != nil {
...
func TestNewProofChecker(t *testing.T) {
	...
	pc := NewProofChecker(nil, nil, []*LabeledFormula{ax}, nil, nil)
	if len(pc.Axioms) != 1 {
...
func TestRegisterTactic(t *testing.T) {
	cfg := TacticNewConfig()
	called := false
	cfg.RegisterTactic("test_tactic", func(pc ProofCheckerInterface, goals []*LabeledFormula, proof Node) ([]*LabeledFormula, error) {
		called = true
		return goals, nil
	})
...
func TestApplyMatchSubstitution(t *testing.T) {
	...
	match := map[NodeKey]Expr{Key(x): c}
...
func TestApplyMatchFunc(t *testing.T) {
	...
	match := map[NodeKey]Expr{Key(s): s2}
	result := ApplyMatchFunc(match, c)
	expected, _ := NewFunctionSort(s2, Boolean)
...
func TestComposeMatches(t *testing.T) {
	...
	free := map[NodeKey]Expr{Key(x): x}
	mat1 := map[NodeKey]Expr{Key(x): y}
	mat2 := map[NodeKey]Expr{Key(y): proofMkConst("c", s2)}
	result := ComposeMatches(free, mat1, mat2, nil)
	if result == nil {
		t.Fatal("expected non-nil result")
	}
	if !result[Key(x)].Equal(proofMkConst("c", s2)) {
		t.Error("composition should map X to c")
	}
}

func TestComposeMatchesNil(t *testing.T) {
	result := ComposeMatches(nil, nil, map[NodeKey]Expr{}, nil)
	if result != nil {
...
func TestExtractTerms(t *testing.T) {
	...
	constants := map[NodeKey]Expr{Key(x): True}
	lam := ExtractTerms(x, []Expr{x}, constants)
	if lam == nil {
...
func FuzzMergeMatches(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, a, b, c, d int, conflict bool) {
		...
		m1 := map[NodeKey]Expr{
			Key(vars[a%4]): consts[b%4],
		}
		m2 := map[NodeKey]Expr{
			Key(vars[c%4]): consts[d%4],
		}
		...
		if result != nil {
			if v, ok := result[Key(vars[a%4])]; !ok || !v.Equal(consts[b%4]) {
				t.Error("first match entry missing")
			}
			if v, ok := result[Key(vars[c%4])]; !ok || !v.Equal(consts[d%4]) {
