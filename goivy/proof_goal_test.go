...
func TestGoalSubst_OK(t *testing.T) {
	...
	result, err := GoalSubst(proofTestAstCfg, g1, g2, Location{})
	if err != nil {
...
func TestGoalSubst_NameClash(t *testing.T) {
	...
	_, err := GoalSubst(proofTestAstCfg, g1, g2, Location{})
	if err == nil {
...
func TestGoalAddPrem_Empty(t *testing.T) {
	...
	result := GoalAddPrem(proofTestAstCfg, goal, prem, Location{})
	prems := GoalPrems(result)
	if len(prems) != 1 {
...
func TestGoalAddPrem_Existing(t *testing.T) {
	...
	result := GoalAddPrem(proofTestAstCfg, goal, prem2, Location{})
	prems := GoalPrems(result)
	if len(prems) != 2 {
...
func TestGoalAddPrem_PreservesTemporalModelsConclusion(t *testing.T) {
	s := proofMkSort("S")
	c := proofMkConst("c", s)
	tm := proofTestAstCfg.NewTemporalModels(proofTestAstCfg.NewNoneAST(), True)
