...
func (u *Z3Utils) ToZ3(x Expr) (any, error) {
	key := x.Sexp()
...
func (u *Z3Utils) toZ3Internal(x Expr) (any, error) {
...
func (u *Z3Utils) toZ3VarOrConst(name string, sort Sort, isConst bool) (any, error) {
...
func (u *Z3Utils) Z3Implies(f1, f2 Expr, timeout bool) (bool, error) {
	key := [2]NodeKey{f1.Sexp(), f2.Sexp()}
...
func (u *Z3Utils) Z3ImpliesBatch(premise Expr, formulas []Expr, timeout bool) ([]bool, error) {
	s := u.Ctx.NewZ3Solver()
	if timeout {
		s.SetParam("timeout", "2000") // 2 seconds
	}
	...
	for i, f := range formulas {
		key := [2]NodeKey{premise.Sexp(), f.Sexp()}
		if cached, ok := u.ImpliesCache[key]; ok {
			result[i] = cached
			continue
		}
