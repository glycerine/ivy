...
func RelevantDefinitions(m *Module, syms map[string]bool) []*LabeledFormula {
	...
	for _, ldf := range m.Definitions {
		if def, ok := ldf.Formula.(*IvyDefinition); ok {
...
func moduleCollectSymbolNames(node Expr) []string {
	var names []string
	collectSymbolNamesRec(node, &names, make(map[string]bool))
	return names
}

func collectSymbolNamesRec(node Expr, names *[]string, seen map[string]bool) {
	switch t := node.(type) {
	case *Const:
		if !seen[t.Name] {
			seen[t.Name] = true
