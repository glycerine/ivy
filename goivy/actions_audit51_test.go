...
func TestDebugAction_ActionUpdate_ReturnsNullUpdate(t *testing.T) {
	dbg := NewDebugAction(NewConst("x", TopS))
	ctx := testCtx()
	u := dbg.IntUpdate(ctx)
...
func TestAssignFieldAction_ActionUpdate(t *testing.T) {
	...
	obj := NewConst("obj", sortT)
	val := NewConst("val", sortS)
...
func TestCopyFieldAction_ActionUpdate(t *testing.T) {
	...
	fldSort := mkBinaryRelSort(sortT, sortS)
	fldSym := NewConst("fld", fldSort)
	dst := NewConst("dst", sortT)
	src := NewConst("src", sortT)
...
func TestIfAction_Subactions_BoolCondition(t *testing.T) {
	...
	thenBody := NewAssumeAction(NewConst("p", Boolean))
	elseBody := NewAssumeAction(NewConst("q", Boolean))
...
func TestSomeCondition_NodeSort(t *testing.T) {
	p := NewConst("x", TopS)
	fmla := NewConst("f", Boolean)
	sc := &SomeCondition{
		Params: []*Const{p},
		Fmla:   fmla,
		Kind:   "some",
	}
	if sc.NodeSort() != Boolean {
...
func TestPrefixCalls_StringPrefix(t *testing.T) {
	...
	if sym, ok := callResult.Callee.(*Const); ok {
...
func TestPrefixCallsFunc_Nested(t *testing.T) {
	...
	if sym, ok := innerCall.Callee.(*Const); ok {
...
func TestPrefixCalls_Nil(t *testing.T) {
	...
	call := NewCallActionOn(NewActionsConfig(), NewConst("a", TopS))
...
func TestIterInternalDefines_NoThunks(t *testing.T) {
	seq := NewSequence(NewAssumeAction(NewConst("p", Boolean)))
...
func TestIterInternalDefines_ThunkAction(t *testing.T) {
	thunk := NewThunkAction(NewConst("handler", TopS), NewConst("body", TopS))
	thunk.SetLineno(Location{Filename: "test.ivy", Line: 42})
...
func TestIterInternalDefines_NestedThunk(t *testing.T) {
	thunk := NewThunkAction(NewConst("h", TopS))
...
func TestSchema_GetInstance(t *testing.T) {
	...
	aSym := NewConst("a", TopS)
	fSym := NewConst("f", TopS)
	lhs := MustApply(fSym, aSym)
	rhs := aSym // body is just "a"
	defn := NewDefinition(lhs, rhs)
	s := NewSchema(defn)

	// Instantiate with param "val"
	valSym := NewConst("val", TopS)
	result, err := s.GetInstance([]Expr{valSym}, false)
	if err != nil {
		t.Fatalf("GetInstance failed: %v", err)
	}

	// Result should be "val" (the body with "a" substituted for "val")
	if sym, ok := result.(*Const); ok {
		if sym.Name != "val" {
...
func TestSchema_GetInstance_ParamCountMismatch(t *testing.T) {
	...
	fSym := NewConst("f", TopS)
	lhs := MustApply(fSym, aSym)
	defn := NewDefinition(lhs, aSym)
	s := NewSchema(defn)

	// Wrong number of params
	_, err := s.GetInstance([]Expr{}, false)
	if err == nil {
...
func TestTypeCheckContext_Get_ReturnsEmptyWithFormals(t *testing.T) {
	...
	act := NewSequence()
	act.SetFormalParams([]*Const{NewConst("x", TopS)})
	act.SetFormalReturns([]*Const{NewConst("r", TopS)})
...
func TestAssertAction_ActionUpdate_CheckedAssert_Selected(t *testing.T) {
	...
	ctx := testCtx()
	ctx.CheckedAssert = fmt.Sprintf("%s:%d", a.GetLineno().Filename, a.GetLineno().Line) // matches this assertion
...
func TestActionContext_NestedEnterExit(t *testing.T) {
	...
	ctx1 := NewActionContextOn(&Module{Name: "domain1"}, cfg)
	ctx2 := NewActionContextOn(&Module{Name: "domain2"}, cfg)
...
func TestSymExContext_EnterExit(t *testing.T) {
	...
	params := []Expr{NewConst("a", TopS), NewConst("b", TopS)}
...
func TestSymExContext_NestedEnterExit(t *testing.T) {
	...
	params1 := []Expr{NewConst("x", TopS)}
	params2 := []Expr{NewConst("y", TopS), NewConst("z", TopS)}
...
func TestRunWithSymExContext(t *testing.T) {
	...
	params := []Expr{NewConst("p", TopS)}
	var insideParams []Expr
