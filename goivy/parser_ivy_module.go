...
func (m *ivyAccum) GetAstConfig() *AstConfig { return nil }

func (m *ivyAccum) String() string {
	return fmt.Sprintf("<ivyAccum decls=%d>", len(m.decls))
}

func (m *ivyAccum) Canon() Canonical {
	...
	return Canonical(fmt.Sprintf("(sequence stmts:%s)", SliceCanon(m.decls)))
}
...
func (m *ivyAccum) declare(decl Node) {
	...
	if db := GetDeclBase(decl); db != nil && len(m.attributes) > 0 {
		attrNodes := make([]Node, len(m.attributes))
	...
	if ad, ok := decl.(*ActionDecl); ok {
		for _, arg := range ad.Args() {
			if d, ok := arg.(*ActionDef); ok {
...
func nodeAttrsToStrings(attrs []Node) []string {
	if len(attrs) == 0 {
		return nil
	}
...
func hasAttributeNode(attrs []Node, name string) bool {
	for _, a := range attrs {
		if atom, ok := a.(*Atom); ok && atom.Rep == name {
			return true
		}
		if sym, ok := a.(*Symbol); ok && sym.Rep == name {
			return true
		}
	}
	return false
...
func declHasCommonAttribute(decl Node) bool {
	if db := GetDeclBase(decl); db != nil {
