...
func ConcretePost(checkPrecond bool, update *Update, state *InterpState, expr Node) (*InterpState, error) {
	if state.Domain == nil {
...
func ConcreteJoin(s1, s2 *InterpState) (*InterpState, error) {
	...
	joinedUpdate := JoinState(u1, u2, axioms)
	...
	res := NewInterpState(s1.Domain, &StateValue{
		Moded:   ModifiedNames(joinedUpdate),
...
func EvalAction(expr interface{}, mod *Module) (ActionsAction, error) {
...
func ApplyAction(checkPrecond bool, astNode Node, actionName string, action ActionsAction, state *InterpState) (*InterpState, error) {
	xtracer.Trace("interp.ApplyAction ENTER actionName=%s", ActionTypeName(action))
	...
	xtracer.Trace("interp.ApplyAction calling GetUpdate actionName=%s type=%s", ActionTypeName(action), ActionTypeName(action))
...
func EvalStateAtom(expr Node, mod *Module) (*InterpState, error) {
...
func InterpBottomState(domain *Module) *InterpState {
	var acfg *AstConfig
...
func NewStateFromClauses(mod *Module, clauses *Clauses) *InterpState {
