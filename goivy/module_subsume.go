...
func TermSubsume(term1, term2 Expr, env map[string]Expr) bool {
	if c, ok := term1.(*Const); ok {
		c2, ok2 := term2.(*Const)
		if !ok2 || c.Name != c2.Name {
			return false
		}
		args1 := NodeArgs(term1)
		args2 := NodeArgs(term2)
	...
	if app1, ok := term1.(*Apply); ok {
		app2, ok2 := term2.(*Apply)
		if !ok2 {
			return false
		}
		rep1 := GetAppRep(app1)
...
func LitSubsume(lit1, lit2 *Literal, env map[string]Expr) bool {
	...
	rep1 := GetAppRep(lit1.Atom)
	rep2 := GetAppRep(lit2.Atom)
	if rep1 == nil || rep2 == nil || rep1.Name != rep2.Name {
		return false
	}
	args1 := NodeArgs(lit1.Atom)
	args2 := NodeArgs(lit2.Atom)
	if len(args1) != len(args2) {
		return false
	}
	for i := range args1 {
...
func AtomSubsume(at1, at2 Expr) bool {
...
func CommuteLit(lit *Literal) *Literal {
	...
	args := NodeArgs(lit.Atom)
	if len(args) == 2 {
		rep := GetAppRep(lit.Atom)
		if rep != nil {
			return NewLiteral(lit.Polarity, IvyAtom(rep, []Expr{args[1], args[0]}))
		}
	}
	return lit
}
...
func moduleCopyEnv(env map[string]Expr) map[string]Expr {
	c := make(map[string]Expr, len(env))
	for k, v := range env {
		c[k] = v
	}
	return c
...
func restoreEnv(env, saved map[string]Expr) {
	for k := range env {
		delete(env, k)
	}
	for k, v := range saved {
...
func IsIndividualAst(ast Expr) bool {
	return IsIndividual(ast)
}
...
func ElimDefinitions(clauses *Clauses, dead []*Const) *Clauses {
	fmlas := make([]Expr, len(clauses.Fmlas))
	copy(fmlas, clauses.Fmlas)

	deadSet := make(map[NodeKey]bool, len(dead))
	for _, sym := range dead {
		deadSet[Key(sym)] = true
		if idx, ok := clauses.DefIdx[Key(sym)]; ok && idx < len(clauses.Defs) {
...
func RenameSymbols(rn *UniqueRenamer, clauses *Clauses, toRename []*Const) *Clauses {
	nameMap := make(map[NodeKey]*Const, len(toRename))
	for _, s := range toRename {
		newName := rn.Rename(s.Name)
		nameMap[Key(s)] = NewConst(newName, s.CSort)
	}
...
func FindTrueDisjunct(clauses *Clauses, evalFun func(Expr) bool) int {
...
func EqcmUpd(lhs, rhs Expr, symset map[NodeKey]bool, map2 map[NodeKey][]Expr) bool {
	lhsKey := Key(lhs)
	if !symset[lhsKey] {
		return false
	}
...
func ExistsQuantClausesMap(syms []*Const, clauses *Clauses) (map[NodeKey]*Const, *Clauses) {
	used := collectAllUsedNames(clauses)
	symset := make(map[NodeKey]bool, len(syms))
	for _, s := range syms {
		symset[Key(s)] = true
	}
	map1 := make(map[NodeKey]*Const)
	map2 := make(map[NodeKey][]Expr)
	...
	keyToConst := make(map[NodeKey]*Const)
	for _, df := range clauses.Defs {
		if c, ok := df.Lhs.(*Const); ok {
			keyToConst[Key(c)] = c
		}
		if c, ok := df.Rhs.(*Const); ok {
			keyToConst[Key(c)] = c
		}
	}
	for _, s := range syms {
	...
	for _, s := range syms {
		newName := rn.Rename(s.Name)
		map1[Key(s)] = NewConst(newName, s.CSort)
	}
...
func HasEnumeratedSort(sig *Sig, sym *Const) bool {
	sort := sym.CSort
