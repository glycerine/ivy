...
func GoalConc(g *LabeledFormula) Node {
...
func GoalConcExpr(g *LabeledFormula) Expr {
	if e, ok := GoalConc(g).(Expr); ok {
		return e
	}
	return nil
}
...
func GoalConcUnwrap(g *LabeledFormula) Expr {
...
func ApplyToConc(conc Node, fn func(Expr) Expr) Node {
	if conc == nil {
	...
	xtracer.Trace("proof.ApplyToConc EXIT type=%s passthrough", TypeName(conc))
	return conc
}
...
func normalizeOpsConc(conc Node) Node {
	if conc == nil {
		return conc
	}
	if tm, ok := conc.(*TemporalModels); ok {
...
func GoalPrems(g *LabeledFormula) []Node {
	if sb, ok := g.Formula.(*SchemaBody); ok {
		return sb.Prems()
	}
	return nil
...
func GoalPremGoals(goal *LabeledFormula) []*LabeledFormula {
...
func CloneGoal(cfg *AstConfig, goal *LabeledFormula, prems []Node, conc Node) *LabeledFormula {
	xtracer.Trace("proof.CloneGoal ENTER label=%s nprems=%d concType=%s", goal.LabelForTrace(), len(prems), TypeName(conc))
	var formula Node
	if len(prems) > 0 {
	...
	result := goal.CloneWithFreshID([]Node{goal.Label, formula})
...
func CloneGoalPreserveID(cfg *AstConfig, goal *LabeledFormula, prems []Node, conc Node) *LabeledFormula {
	xtracer.Trace("proof.CloneGoalPreserveID ENTER label=%s nprems=%d concType=%s id=%d", goal.LabelForTrace(), len(prems), TypeName(conc), goal.ID)
	var formula Node
	if len(prems) > 0 {
		elems := make([]Node, len(prems)+1)
		copy(elems, prems)
	...
	result := goal.Clone([]Node{goal.Label, formula}).(*LabeledFormula)
...
func MakeGoal(cfg *AstConfig, loc Location, label Node, prems []Node, conc Node) *LabeledFormula {
	xtracer.Trace("proof.MakeGoal ENTER nprems=%d concType=%s", len(prems), TypeName(conc))
	var formula Node
	if len(prems) > 0 {
		elems := make([]Node, len(prems)+1)
		copy(elems, prems)
...
func NormalizeGoal(cfg *AstConfig, g *LabeledFormula) *LabeledFormula {
	return normalizeGoalAny(cfg, g).(*LabeledFormula)
}
...
func normalizeGoalAny(cfg *AstConfig, x Node) Node {
...
func normalizeGoalLabel(x Node) string {
...
func GoalIsDefn(x Node) bool {
	...
	if _, ok := x.(*UninterpretedSort); ok {
		return true
	}
	return false
}
...
func unwrapCompiledNode(x Node) Node {
	if cn, ok := x.(*CompiledNode); ok {
		if inner, ok := cn.Node.(Node); ok {
			return inner
		}
	}
	return x
...
func GoalDefns(goal *LabeledFormula) map[NodeKey]Expr {
	...
	for _, p := range GoalPrems(goal) {
		...
		if cd, ok := p.(*ConstantDecl); ok {
			args := cd.Args()
			if len(args) > 0 {
				if c, ok := unwrapCompiledNode(args[0]).(*Const); ok {
					res[Key(c)] = c
				}
			}
		}
		// Python: elif isinstance(x, il.UninterpretedSort): res.add(x)
		if us, ok := p.(*UninterpretedSort); ok {
			res[Key(us)] = us
		}
	}
	return res
...
func GoalVocab(goal *LabeledFormula) *Vocab {
...
func goalVocabRaw(goal *LabeledFormula) *Vocab {
	prems := GoalPrems(goal)
	conc := GoalConc(goal)
	...
	for _, p := range prems {
		...
		if cd, ok := p.(*ConstantDecl); ok {
			args := cd.Args()
			if len(args) > 0 {
				if cc, ok := unwrapCompiledNode(args[0]).(*Const); ok {
	...
	variables := UsedVariablesAsts(fmlas)
...
func GoalVocabBound(goal *LabeledFormula) *Vocab {
	...
	existing := make(map[NodeKey]bool, len(v.Variables))
	for _, vr := range v.Variables {
		existing[Key(vr)] = true
	}
...
func GoalFree(goal *LabeledFormula) map[NodeKey]Expr {
	xtracer.Trace("proof.GoalFree ENTER label=%s", goal.LabelForTrace())
	bound := make(map[NodeKey]Expr)
	res := make(map[NodeKey]Expr)

	// Python fmla_vocab: collects used_sorts_ast, used_symbols_ast, used_variables_ast
	var recFmla func(Expr)
	recFmla = func(fmla Expr) {
		if fmla == nil {
			return
		}
		// Python: lu.used_sorts_ast(fmla)
		for sKey, s := range SortsAst(fmla) {
			if sExpr, ok := s.(Expr); ok {
				if bound[sKey] == nil {
					res[sKey] = sExpr
				}
			}
		...
		for cKey, cNode := range UsedSymbolsAst(fmla).All() {
			if bound[cKey] == nil {
				res[cKey] = cNode
			}
		}
		// Python: lu.used_variables_ast(fmla)
		for vKey, vNode := range FreeVariables(fmla).All() {
			if bound[vKey] == nil {
				res[vKey] = vNode
			}
		}
	...
	var rec func(*LabeledFormula)
	rec = func(g *LabeledFormula) {
		defns := GoalDefns(g)
		// Add defns to bound
		...
		for _, pg := range GoalPremGoals(g) {
			if _, ok := pg.Formula.(*SchemaBody); ok {
				rec(pg)
			} else {
...
func GoalSubst(cfg *AstConfig, g1, g2 *LabeledFormula, loc Location) (*LabeledFormula, error) {
...
func GoalAddPrem(cfg *AstConfig, goal *LabeledFormula, prem Node, loc Location) *LabeledFormula {
...
func GoalRemovePrem(cfg *AstConfig, goal *LabeledFormula, premName string) *LabeledFormula {
...
func TrivialGoal(goal *LabeledFormula) bool {
	...
	for _, prem := range GoalPremGoals(goal) {
		if len(GoalPrems(prem)) == 0 {
			...
			if pc != nil && EqualModAlpha(pc, conc) {
				return true
			}
		}
	}
...
func CheckConcsMatch(g1, g2 *LabeledFormula) error {
	c1 := GoalConcExpr(g1)
	c2 := GoalConcExpr(g2)
	if c1 == nil || c2 == nil {
		return &ProofError{Msg: "nil conclusion in goal"}
	}
	if !EqualModAlpha(c1, c2) {
...
func CompileWithGoalVocab(expr Node, goal *LabeledFormula, mod *Module) Expr {
...
func CompileDefinitionGoalVocab(cfg *AstConfig, df Node, goal *LabeledFormula, mod *Module) (*LabeledFormula, error) {
	// Python: vocab = goal_vocab(goal); free = goal_free(goal)
	vocab := GoalVocab(goal)
	free := GoalFree(goal)

	// Unwrap DerivedDecl: Python lf = df.args[0]
	var innerLF *LabeledFormula
	if lf, ok := df.(*LabeledFormula); ok {
		innerLF = lf
	} else if dd, ok := df.(*DerivedDecl); ok {
		if len(dd.DeclArgs) > 0 {
			if lf, ok := dd.DeclArgs[0].(*LabeledFormula); ok {
				innerLF = lf
			}
		}
	}
	...
	lhsAtom, ok := defFormula.Lhs.(*Atom)
	if !ok {
		return goal, nil
	}
	...
	ts := TopFunctionSort(len(vars))
	// Python: newsym = il.Symbol(lhs.rep, ts)
	newsym := NewConst(lhsAtom.Rep, ts)

	// Python: with il.WithSymbols([newsym]):
	sig := getSigFrom(mod)
	ws := NewWithSymbols(sig, []*Const{newsym})
	ws.Enter()
	defer ws.Exit()
	...
	elf := innerLF.Clone([]Node{innerLF.Label, fmla}).(*LabeledFormula)
	...
	if hasVars {
		...
		newFormula = compiledFormula.(Node).Clone([]Node{thing})
	} else {
		newFormula = thing
	}
	lf := compiledLF.Clone([]Node{compiledLF.Label, newFormula}).(*LabeledFormula)
	...
	symKey := Key(sym)
	if isEq {
		for dep := range SymbolsIluAst(eq.T2) {
			if c, ok := dep.(*Const); ok && Key(c) == symKey {
...
func compileExprVocabLF(lf *LabeledFormula, vocab *Vocab, mod *Module) (*LabeledFormula, error) {
	// Python's compile_expr_vocab emits this ENTER trace (ivy_proof.py:914).
	// The LF-specialized Go helper must mirror it.
	xtracer.Trace("proof.CompileExprVocab ENTER exprType=%s", TypeName(lf))
	...
	if formula, ok := compiled.Formula.(Expr); ok {
		...
		if inferErr == nil && len(inferred) > 0 {
			...
			compiled = compiled.Clone(cloneArgs).(*LabeledFormula)
		} else {
			...
			compiled = compiled.Clone(compiled.Args()).(*LabeledFormula)
		}
	}
...
func isInVocabOrFree(sym *Const, vocab *Vocab, free map[NodeKey]Expr) bool {
	symKey := Key(sym)
