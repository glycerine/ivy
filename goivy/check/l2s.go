...
func l2sTacticInt(pc module.ProofCheckerInterface, goals []*ast.LabeledFormula, pf ast.Node, tacticName string) ([]*ast.LabeledFormula, error) {
	...
	goal := goals[0]
	xstracer.Trace("l2s.l2sTacticInt ENTER tactic='%s' ngoals=%d goal.Formula type=%s", tacticName, len(goals), iu.TypeName(goal.Formula))
	if goal.Formula != nil {
		// Also check if it's a SchemaBody
		if sb, ok := goal.Formula.(*ast.SchemaBody); ok {
			conc := sb.Conc()
			xstracer.Trace("l2s.l2sTacticInt goal.Formula is SchemaBody, conc type=%s", iu.TypeName(conc))
		}
	}
	...
	xstracer.Trace("l2s.l2sTacticInt goalConc result type=%s (isTemporalModels=%s)", iu.TypeName(conc), pyBool(isTM))
	if !isTM {
	...
	xstracer.Trace("l2s.l2sTacticInt axiomDump nAxioms=%d", len(pc.GetAxioms()))
	for idx, ax := range pc.GetAxioms() {
		xstracer.Trace("l2s.l2sTacticInt axiomDump[%d] HASH canon=%v", idx, ax.Canon())
	}

	// Diagnostic: dump metadata for each axiom to diagnose assumed_gprops filtering
	for idx, ax := range pc.GetAxioms() {
		_, isGlobally := ax.Formula.(*lg.Globally)
		xstracer.Trace("l2s.l2sTacticInt axiomMeta[%d] explicit=%s temporal=%s isGlobally=%s formulaType=%s",
			...
			if !ax.Explicit && ax.IsTemporal() {
				if f, ok := ax.Formula.(lg.Expr); ok {
					if g, ok := f.(*lg.Globally); ok {
						...
						xstracer.Trace("l2s.l2sTacticInt assumedGprop HASH canon=%v", cloned.Canon())
	...
	xstracer.Trace("l2s.l2sTacticInt assumedGprops count=%d", len(assumedGprops))

	// If there are temporal premises, wrap: prems -> fmla
	xstracer.Trace("l2s.l2sTacticInt temporalPrems count=%d", len(temporalPrems))
	...
	if tt, ok := pf.(*ast.TacticTactic); ok {
		...
		xstracer.Trace("l2s.l2sTacticInt tacticDecls nInvars=%d nDefns=%d", len(tacticInvars), len(tacticDefns))

		// C7: compile definitions into goal premises (Python lines 152-153).
		for idx, defn := range tacticDefns {
			xstracer.Trace("l2s.l2sTacticInt compileDefn[%d] type=%s", idx, iu.TypeName(defn))
			var err error
		...
		for idx, inv := range tacticInvars {
			xstracer.Trace("l2s.l2sTacticInt compileInvar[%d] pre-compile HASH canon=%v", idx, inv.Canon())
			vocab := proof.GoalVocab(goal)
			compiledLF := proof.CompileExprVocabExtLF(inv, vocab, m)
			if compiledLF == nil {
				xstracer.Trace("l2s.l2sTacticInt compileInvar[%d] compiled=nil", idx)
				continue
			}
			labeled := proof.LabelTemporalNode(compiledLF, proofLabel).(*ast.LabeledFormula)
			xstracer.Trace("l2s.l2sTacticInt compileInvar[%d] post-compile HASH canon=%v", idx, labeled.Canon())
			invars = append(invars, labeled)
		}
	} else {
		xstracer.Trace("l2s.l2sTacticInt tacticDecls NONE (pf is not TacticTactic, type=%s)", iu.TypeName(pf))
		defnDeps = BuildDefnDeps(m, proof.GoalPrems(goal)...)
	}
	...
	modPass := func(transformName string, transform func(ast.Node) ast.Node) {
		if xtracer.Enabled {
			...
			xstracer.Trace("l2s.modPass ENTER transform=%s nInvars=%d nAsms=%d nBindings=%d nPrems=%d nPropPrems=%d",
				transformName, len(model.Invars), len(model.Asms), len(model.Bindings), len(prems), nPropPrems)
		}
		for i, inv := range model.Invars {
			lu.ResetRtrDepth()
			xstracer.Trace("l2s.modPass clone invar[%d] ENTER HASH canon=%v", i, inv.Canon())
			model.Invars[i] = transform(inv).(*ast.LabeledFormula)
			xstracer.Trace("l2s.modPass clone invar[%d] EXIT HASH canon=%v", i, model.Invars[i].Canon())
		}
		for i, asm := range model.Asms {
			lu.ResetRtrDepth()
			xstracer.Trace("l2s.modPass clone asm[%d] ENTER HASH canon=%v", i, asm.Canon())
			model.Asms[i] = transform(asm).(*ast.LabeledFormula)
			xstracer.Trace("l2s.modPass clone asm[%d] EXIT HASH canon=%v", i, model.Asms[i].Canon())
		}
		for i, b := range model.Bindings {
			lu.ResetRtrDepth()
			xstracer.Trace("l2s.modPass clone binding[%d] ENTER name=%s", i, b.Name)
			...
			xstracer.Trace("l2s.modPass clone binding[%d] EXIT name=%s", i, b.Name)
		}
		if model.Init != nil {
			lu.ResetRtrDepth()
			xstracer.Trace("l2s.modPass clone init ENTER")
			// Python: model.init = transform(model.init)
			model.Init = transform(model.Init).(actions.Action)
			xstracer.Trace("l2s.modPass clone init EXIT")
		}
		...
		for i, p := range prems {
			...
			lu.ResetRtrDepth()
			xstracer.Trace("l2s.modPass clone prem[%d] ENTER HASH canon=%v", i, lf.Canon())
			prems[i] = transform(lf).(*ast.LabeledFormula)
			xstracer.Trace("l2s.modPass clone prem[%d] EXIT HASH canon=%v", i, prems[i].(*ast.LabeledFormula).Canon())
		}
		if xtracer.Enabled {
			nPropPrems := 0
			for _, p := range prems {
			...
			xstracer.Trace("l2s.modPass EXIT transform=%s nInvars=%d nAsms=%d nBindings=%d nPrems=%d nPropPrems=%d",
				transformName, len(model.Invars), len(model.Asms), len(model.Bindings), len(prems), nPropPrems)
		}
	}
	...
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep1 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	SharedStep1_ConvertTemporals(cfg, model, modPass)
	xstracer.Trace("l2s.SharedStep1 EXIT nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	...
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep3 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	SharedStep3_CollectNamedBinders(cfg, model, full)
	xstracer.Trace("l2s.SharedStep3 EXIT nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))

	// Build save/wait/reset_w from collected binders
	xstracer.Trace("l2s.SharedBuildSaveAndWait ENTER")
	...
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep6 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	...
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep7 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	SharedStep7_InstrumentActions(cfg, model)
	xstracer.Trace("l2s.SharedStep7 EXIT nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))

	// ---------------------------------------------------------------
	// Step 8: Patch exported actions (shared, l2s mode: no postconds)
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep8 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	SharedStep8_PatchExports(cfg, model)
	xstracer.Trace("l2s.SharedStep8 EXIT nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	...
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep11 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	SharedStep11_ReplaceNamedBinders(cfg, model, modPass)
	xstracer.Trace("l2s.SharedStep11 EXIT nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))

	// ---------------------------------------------------------------
	// Step 12: Build new goal (shared)
	// ---------------------------------------------------------------
	xstracer.Trace("l2s.SharedStep12 ENTER nInvars=%d nAsms=%d nBindings=%d nPrems=%d",
		len(model.Invars), len(model.Asms), len(model.Bindings), len(prems))
	...
	xstracer.Trace("l2s.SharedStep12 EXIT nResults=%d err=%s", len(result), errStr)
	if err != nil {
		return nil, err
	}
...
func transformAction(act actions.Action, transform func(ast.Node) ast.Node) actions.Action {
	...
	xstracer.Trace("transformAction ENTER type=%s", iu.ShortTypeName(act))
	...
	xstracer.Trace("transformAction EXIT type=%s", iu.ShortTypeName(result))
	return result
}
