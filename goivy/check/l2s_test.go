...
var l2sTestCfg = ast.NewAstConfig()

func l2sVar(name string, s lg.Sort) *lg.Variable {
...
func TestL2sW(t *testing.T) {
	s := &lg.UninterpretedSort{Name: "S"}
	v := l2sVar("X", s)
	nb := l2sW([]*lg.Variable{v}, lg.True, "lbl")
	if nb.Name != "l2s_w" {
...
func TestL2sWhen(t *testing.T) {
	nb := l2sWhen("foo", nil, lg.True, "lbl")
...
func TestL2sForall_WithVars(t *testing.T) {
	s := &lg.UninterpretedSort{Name: "S"}
	v := l2sVar("X", s)
	result := forall([]*lg.Variable{v}, lg.True)
...
func TestL2sExists_WithVars(t *testing.T) {
	s := &lg.UninterpretedSort{Name: "S"}
	v := l2sVar("X", s)
	result := exists([]*lg.Variable{v}, lg.True)
...
func TestDedupeVarBodyPairs_NoDupes(t *testing.T) {
	...
	v2 := l2sVar("Y", s)
	pairs := []varBodyPair{
		{Vars: []*lg.Variable{v1}, Body: lg.True},
		{Vars: []*lg.Variable{v2}, Body: lg.True},
	}
...
func TestDedupeVarBodyPairs_WithDupes(t *testing.T) {
	...
	v := l2sVar("X", s)
	pairs := []varBodyPair{
		{Vars: []*lg.Variable{v}, Body: lg.True},
		{Vars: []*lg.Variable{v}, Body: lg.True},
		{Vars: nil, Body: lg.True},
	}
	result := dedupeVarBodyPairs(pairs)
	if len(result) != 2 {
...
func TestDesugar_Was_OK(t *testing.T) {
	...
	nb := &lg.NamedBinder{Name: "was", Variables: nil, Body: body}
	...
	if c, ok := and.Terms[0].(*lg.Const); !ok || c.Name != "l2s_saved" {
...
func TestDesugar_Was_Error(t *testing.T) {
	...
	s := &lg.UninterpretedSort{Name: "S"}
	v := l2sVar("X", s)
	nb := &lg.NamedBinder{Name: "was", Variables: []*lg.Variable{v}, Body: lg.True}
...
func TestDesugar_Happened_OK(t *testing.T) {
	body := lg.NewConst("p", lg.Boolean)
	nb := &lg.NamedBinder{Name: "happened", Variables: nil, Body: body}
	result, err := Desugar(nb, "test")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestDesugar_Happened_Error(t *testing.T) {
	...
	s := &lg.UninterpretedSort{Name: "S"}
	v := l2sVar("X", s)
	nb := &lg.NamedBinder{Name: "happened", Variables: []*lg.Variable{v}, Body: lg.True}
	_, err := Desugar(nb, "test")
	if err == nil {
...
func TestDesugar_NestedWas(t *testing.T) {
	body := lg.NewConst("p", lg.Boolean)
	wasNB := &lg.NamedBinder{Name: "was", Variables: nil, Body: body}
	outer := &lg.And{Terms: []lg.Expr{wasNB, lg.True}}
...
func TestApplyWasRec_And(t *testing.T) {
	a := lg.NewConst("a", lg.Boolean)
	b := lg.NewConst("b", lg.Boolean)
	result := applyWasRec(&lg.And{Terms: []lg.Expr{a, b}}, "lbl")
	and, ok := result.(*lg.And)
	if !ok {
		t.Fatalf("expected *lg.And, got %T", result)
	}
...
func TestApplyWasRec_Or(t *testing.T) {
	a := lg.NewConst("a", lg.Boolean)
	result := applyWasRec(&lg.Or{Terms: []lg.Expr{a}}, "lbl")
	if _, ok := result.(*lg.Or); !ok {
...
func TestApplyWasRec_Not(t *testing.T) {
	a := lg.NewConst("a", lg.Boolean)
	result := applyWasRec(&lg.Not{Body: a}, "lbl")
	...
	if _, ok := not.Body.(*lg.NamedBinder); !ok {
...
func TestApplyWasRec_Not_WithFreeVars(t *testing.T) {
	...
	s := &lg.UninterpretedSort{Name: "S"}
	v := l2sVar("X", s)
	result := applyWasRec(&lg.Not{Body: v}, "lbl")
	not, ok := result.(*lg.Not)
	if !ok {
		t.Fatalf("expected *lg.Not, got %T", result)
	}
	if _, ok := not.Body.(*lg.Apply); !ok {
...
func TestApplyWasRec_Implies(t *testing.T) {
	a := lg.NewConst("a", lg.Boolean)
	b := lg.NewConst("b", lg.Boolean)
	result := applyWasRec(&lg.Implies{T1: a, T2: b}, "lbl")
	if _, ok := result.(*lg.Implies); !ok {
...
func TestApplyWasRec_Leaf(t *testing.T) {
	...
	if _, ok := result.(*lg.NamedBinder); !ok {
...
func TestTransformAction_Simple(t *testing.T) {
	...
	assume, ok := result.(*actions.AssumeAction)
	if !ok {
...
func TestApplyL2sInit_NotWrapped(t *testing.T) {
	c := lg.NewConst("c", lg.Boolean)
	result := applyL2sInit(nil, &lg.Not{Body: c}, "lbl")
	not, ok := result.(*lg.Not)
	if !ok {
		t.Fatalf("expected *lg.Not, got %T", result)
	}
...
func FuzzDesugar(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, name string, numVars int) {
		...
		for i := 0; i < numVars; i++ {
			v, _ := lg.NewVariable("V"+string(rune('0'+i)), s)
			vars = append(vars, v)
		}
		nb := &lg.NamedBinder{Name: name, Variables: vars, Body: lg.True}
