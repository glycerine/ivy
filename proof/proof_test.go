...
func TestMatchProblemString(t *testing.T) {
	s := mkSort("S")
	x := mkVar("X", s)
	mp := NewMatchProblem(x, x, x, map[lg.Node]bool{x: true}, nil)
	str := mp.String()
	if len(str) == 0 {
...
func TestFuncsMatch(t *testing.T) {
	...
	c4 := mkConst("p", fs2)
	freesyms := map[lg.Node]bool{s: true}
...
func TestMergeMatchesNil(t *testing.T) {
	m := MergeMatches(nil, map[lg.Node]lg.Node{})
	if m != nil {
...
func TestMergeMatchesConflict(t *testing.T) {
	...
	m1 := map[lg.Node]lg.Node{x: y}
	m2 := map[lg.Node]lg.Node{x: z}
...
func TestApplyMatchFunc(t *testing.T) {
	...
	match := map[lg.Node]lg.Node{s: s2}
...
func TestComposeMatches(t *testing.T) {
	...
	free := map[lg.Node]bool{x: true}
	mat1 := map[lg.Node]lg.Node{x: y}
	mat2 := map[lg.Node]lg.Node{y: mkConst("c", s2)}
...
func TestComposeMatchesNil(t *testing.T) {
	result := ComposeMatches(nil, nil, map[lg.Node]lg.Node{}, nil)
	if result != nil {
...
func TestApplyMatchFreesyms(t *testing.T) {
	...
	free := map[lg.Node]bool{s: true, s2: true}
	match := map[lg.Node]lg.Node{s: s2}
...
func FuzzMergeMatches(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, a, b, c, d int, conflict bool) {
		...
		m1 := map[lg.Node]lg.Node{
			vars[a%4]: consts[b%4],
		}
		m2 := map[lg.Node]lg.Node{
			vars[c%4]: consts[d%4],
		}
