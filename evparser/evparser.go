...
func (l *lexerState) expect(typ tokenType) (token, error) {
	t := l.next()
	if t.typ != typ {
...
// Parse parses an event trace string into Events.
func Parse(s string) (Events, error) {
	l := newLexer(s)
	return parseEvents(l)
}

// parseEvents: events = /* empty */ | events event
func parseEvents(l *lexerState) (Events, error) {
	var result Events
	for {
		t := l.peek()
		// An event starts with optdir (empty, '>', '<') then SYMBOL.
		// So we check for GT, LT, or SYMBOL.
		if t.typ == tokGT || t.typ == tokLT || t.typ == tokSYMBOL {
			ev, err := parseEvent(l)
			if err != nil {
				return nil, err
			}
...
// parseEvent: event = optdir SYMBOL optargs optsubs
func parseEvent(l *lexerState) (*Event, error) {
	dir := DirNone
	t := l.peek()
	if t.typ == tokGT {
		l.next()
	...
	sym, err := l.expect(tokSYMBOL)
	if err != nil {
		return nil, fmt.Errorf("expected event name: %w", err)
	}

	args, err := parseOptArgs(l)
	if err != nil {
		return nil, err
	}

	children, err := parseOptSubs(l)
	if err != nil {
		return nil, err
	}

	return &Event{Dir: dir, Rep: sym.val, Args: args, Children: children}, nil
}

// parseOptArgs: optargs = /* empty */ | '(' list ')'
func parseOptArgs(l *lexerState) ([]Value, error) {
	if l.peek().typ != tokLPAREN {
		return nil, nil
	}
	l.next() // consume '('
	vals, err := parseList(l)
	if err != nil {
		return nil, err
	}
...
// parseOptSubs: optsubs = /* empty */ | ';' | '{' events '}'
func parseOptSubs(l *lexerState) (Events, error) {
	t := l.peek()
	if t.typ == tokSEMI {
		l.next()
		return nil, nil
	...
	if t.typ == tokLCB {
		l.next() // consume '{'
		evs, err := parseEvents(l)
		if err != nil {
			return nil, err
		}
		_, err = l.expect(tokRCB)
		if err != nil {
			return nil, fmt.Errorf("expected '}': %w", err)
		}
		return evs, nil
...
// parseValue: value = SYMBOL | SYMBOL '(' list ')' | '[' ']' | '[' list ']' | '{' '}' | '{' dict '}'
func parseValue(l *lexerState) (Value, error) {
	t := l.peek()
	switch t.typ {
	case tokSYMBOL:
		l.next()
		if l.peek().typ == tokLPAREN {
			...
			return &App{Rep: t.val, Args: vals}, nil
		}
		return &Symbol{Name: t.val}, nil
	case tokLBR:
		l.next() // consume '['
		if l.peek().typ == tokRBR {
			l.next() // consume ']'
		...
		_, err = l.expect(tokRBR)
		if err != nil {
			return nil, fmt.Errorf("expected ']': %w", err)
		}
		return &ListValue{Items: vals}, nil
	case tokLCB:
		l.next() // consume '{'
		if l.peek().typ == tokRCB {
			l.next() // consume '}'
		...
		return d, nil
	default:
		return nil, fmt.Errorf("unexpected token %q at position %d", t.val, l.cur)
	}
}

// parseList: list = value | list ',' value
func parseList(l *lexerState) ([]Value, error) {
	v, err := parseValue(l)
	if err != nil {
		return nil, err
	}
	result := []Value{v}
	for l.peek().typ == tokCOMMA {
		l.next() // consume ','
		v, err := parseValue(l)
		if err != nil {
			return nil, err
...
// parseDict: dict = SYMBOL ':' value | dict ',' SYMBOL ':' value
func parseDict(l *lexerState) (*DictValue, error) {
	d := NewDictValue()
	// First entry
	key, err := l.expect(tokSYMBOL)
	if err != nil {
		return nil, fmt.Errorf("expected dict key: %w", err)
	}
	_, err = l.expect(tokCOLON)
	if err != nil {
		return nil, fmt.Errorf("expected ':': %w", err)
	}
...
// keep the compiler happy about unused imports
